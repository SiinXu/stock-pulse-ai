...
def get_history_share_image(
    ...
    try:
        ...
        log_safe_exception(
            logger,
            "Share image report generation failed",
            exc,
    ...
    if image_bytes is None:
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "message": (
                    f"分享图片生成失败，请检查 {engine} 转图工具是否已安装并可用。"
                    " playwright 引擎需要: cd apps/dsa-web && npm ci && "
                    "npx playwright install chromium"
                ),
            },
        )

    filename = f"stockpulse-report-{result.get('id') or record_id}.png"
    return Response(
        content=image_bytes,
