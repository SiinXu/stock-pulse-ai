...
Every `DecisionSignalItem` response also carries a read-only canonical `presentation` with `action/label/confidence/summary/risk/timestamp` for direction, localized label, confidence, reason summary, risk, and creation time. `action` is the sole direction authority, so a conflicting compatibility `action_label` cannot override it; notifications localize that action for the report language and Web localizes it for the current UI language. Cards, details, portfolio summaries, timelines, and API export clients read `presentation` first, while the flat fields remain only for compatibility and rolling-upgrade fallback. The field is not added to primary report, history, StockBar, or backtest schemas.
