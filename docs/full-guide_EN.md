...
| `TUSHARE_HTTP_URL` | Tushare Pro API endpoint for self-hosted nodes, proxies, or internal mirrors. Empty keeps the official default and behavior unchanged; private/internal hosts must also be added to `OUTBOUND_HTTP_ALLOWLIST`. | `http://api.tushare.pro` | Optional |
...
| `TENCENT_PRIORITY` | Tencent direct A-share daily K-line provider priority; lower values are tried earlier. Defaults to `5` as the final fallback after the other built-in A-share daily providers. Does not affect realtime quotes. | `5` | Optional |
...
> - CN sector Top tables now include an additive sector-index analysis. The average move of available major indices is used as the session benchmark, and industry/concept leaders and laggards receive session direction, relative-strength, and risk labels. The same contract is available at `market_review_payload.sector_analysis`. When ranking providers return no rows, both the structured result and deterministic English/Chinese reports retain an `unavailable` state and the data boundary. Current public providers expose session rankings but not namespace-aware sector index codes/levels, collision-free canonical IDs, ETF mappings, historical series, or sector fund flow, so those fields remain explicitly unavailable and are never inferred by the LLM.
...
Whole-source fallback semantics are fixed: `source` keeps the actual successful provider token, while `fallback_from` records the highest-priority whole source that failed in the current attempt; if the primary source succeeds and later providers only supplement missing fields, `fallback_from` is not set. `AnalysisContextBuilder` only maps these upstream artifacts, performs no extra fetches, and does no quality scoring; quote block status collapses as `STALE > FALLBACK > PARTIAL > AVAILABLE`, where `partial` carries a non-high `field_trust.analysis_input` (conflict, stale, missing/legacy metadata) so the existing core-degraded rule forbids `confidence_level=High`. Analysis reports render a small source, confidence, and gap summary from that bounded quote metadata / public overview `quote_trust_*` warnings; they do not re-fetch `/trust` or expose full `field_trust`. When realtime price overlays `today`, the pipeline marks `is_partial_bar`, `is_estimated`, `estimated_fields`, `realtime_source`, and quote metadata. The `daily_bars` block still represents the complete daily-bar window in storage; partial/estimated markers only enter the technical block. Freshness scoring, intraday cache TTL tiers, Agent tool-level reuse, and API/Web display remain follow-ups.
...
`AgentResult.runtime_facts` is an optional internal field for facts collected during the current Orchestrator run: base Agent opinions, degradation events, Pipeline termination, and the actual risk application. A degradation event uses `DURING_STAGE` for a stage failure and `BEFORE_STAGE` when that stage did not start because of the Pipeline deadline or budget guard. A completed stage is not recorded as a timeout degradation when the deadline check fires afterward. `pipeline_termination.last_completed_stage` is taken from the latest actual `COMPLETED` result in `AgentRunStats.stage_results` and may be empty. Cancellation still takes precedence over timeout and degradation: a cancelled result may carry low-sensitivity facts collected before cancellation, but it does not create a degradation or termination fact.

A structured Orchestrator dashboard is processed as input preparation, one risk application, and post-risk finalization. Post-risk finalization updates the top-level decision and operation advice, core signal and position advice, battle-plan position strategy, risk warning, and the DecisionAgent signal/canonical payload; normal non-overridden hold rendering keeps its existing semantics. The shared parsing boundary removes model-authored `agent_disagreement_explanation`, which is reserved for deterministic post-processing, and `runtime_facts` is never inserted into the dashboard, report schema, or public API. This stage does not rewrite directional wording in other free-text dashboard fields and does not generate a public explanation field.
...
Verification trail: the runtime behavior is implemented in `src/analyzer.py`, `src/core/pipeline.py`, `src/core/backtest_engine.py`, `src/report_language.py`, and `src/agent` decision-path modules (with corresponding tests in `tests/core/test_backtest_engine.py`, `tests/analyzer/test_analyzer_news_prompt.py`, `tests/analyzer/test_decision_stability.py`, and `tests/test_agent_pipeline.py`); it does not add/remove runtime config fields or config-cleanup logic in `src/config.py` or persistence code paths.
...
Every `DecisionSignalItem` response also carries a read-only canonical `presentation` with `action/label/confidence/summary/risk/timestamp` for an action mirror, localized label, confidence, reason summary, risk, and creation time. Top-level `action` is the sole direction authority; `presentation.action` is only its derived same-value mirror, so neither a conflicting compatibility `action_label` nor a conflicting nested action can override it. Alert notifications read `REPORT_LANGUAGE` from the current worker run and fully support `zh/en/ko`, while Web localizes that same top-level action for the current UI language. An explicitly persisted creation `report_language` is formal presentation provenance: object or null metadata replacement during a status PATCH retains that key, while a row without formal provenance cannot promote it from replacement metadata, so status changes cannot silently alter the same signal's canonical label. Cards, details, portfolio summaries, timelines, and API JSON consumers derive direction from top-level `action` and read the other five display fields from `presentation` first; flat fields remain only for compatibility and rolling-upgrade fallback. There is currently no dedicated DecisionSignal export endpoint or export UI; API JSON is the existing export surface. The field is not added to primary report, history, StockBar, or backtest schemas.
...
- `PUT /api/v1/decision-signals/{signal_id}/feedback`: upsert the selected signal's latest `useful|not_useful` feedback. Feedback is opinion-only: mixed payloads that include PredictionOutcome / market-actuals fields are rejected and do not rewrite `decision_signal_outcomes` or `agent_predictions.outcome_json` (#1124 DAG-1). Soul-boundary markers, notes longer than 1000 characters, reason codes longer than 64, and illegal C0 controls are rejected at the write path (#1124 DAG-2). Transport `source` (`web`/`api`) is not provenance; the server stamps `provenance_source=user_feedback` and optional `actor_id=local_admin`, and client provenance keys are rejected (#1124 DAG-3).
- `GET /api/v1/decision-signals/{signal_id}`: fetch one signal; missing IDs return 404.
- `PATCH /api/v1/decision-signals/{signal_id}/status`: update a valid status and optional `metadata`; omitting metadata preserves the stored value, explicit `null` clears caller metadata, and an object replaces caller metadata. A non-null formal `decision_profile` overwrites a conflicting metadata profile; for a legacy formal `NULL`, the profile key is removed from the request object and the formal field is not promoted. When the row contains a creation-time `report_language`, object and null replacements preserve that formal presentation provenance (so a null replacement retains that one metadata key instead of writing SQL `NULL`); without formal language provenance, the same key is removed from a replacement object instead of being promoted. Status updates therefore cannot change the canonical label language. `expired/invalidated/closed/archived` terminal states cannot be patched directly back to `active`; expired renewal still requires re-posting active data with a future `expires_at`.
...
#1390 P4 wires the existing `DecisionSignal` API into the Web UI. The single Signal Center is now `/signals`, with four shared Signal feed, Rules, Push history, and Reassessment & stats tabs plus a non-tab pressed-button group for the signal-list and rule `scope=all|holdings|watchlist` context. Legacy `/decision-signals` and `/alerts` URLs retain parameter-mapping redirects. Current-stock and timeline views use their own stock context, with the `stock` URL parameter authoritative on direct entry and Back/Forward navigation. Push history and outcome statistics keep the existing APIs' global scope. Those views hide the scope control while preserving a legal scope in the URL so it applies again when the user returns to the list or rules. The feed defaults to `status=active`, supports filtering by market, stock code, action, market phase, source, source report ID, and status, and the timeline adds a profile filter backed by the server-side list `decision_profile` parameter. `unknown` is used only to filter and display legacy `NULL` rows, and the regular advanced list does not add a profile filter. The page also includes a latest-active lookup by stock code. Cards, details, and timeline displays read the first-class `decision_profile` field first and fall back to legacy metadata only when that field is missing; explicit `null`, missing history, or invalid profile values display as unknown. Signal details show action, profile, confidence/score, horizon, plan_quality, market_phase, price plan, risk, watch conditions, source report, and data quality. The Web UI only allows marking a signal as `closed`, `invalidated`, or `archived`; it does not restore terminal states to active.

#1390 P5 adds signal-level feedback, forward outcome evaluation, and stats sidecars. It does not extend the `decision_signals` main table and does not reuse `BacktestResult`, which is tied to `analysis_history_id`. `decision_signal_feedback` stores the latest `useful|not_useful` feedback per `signal_id` with optional reason/note/source. `decision_signal_outcomes` stores idempotent rows by `(signal_id, horizon, engine_version)`, currently `engine_version=decision-signal-v1`. #1124 DAG-1 locks fact versus opinion write keys: feedback cannot mutate outcome actuals, and mixed PredictionOutcome fields on feedback payloads are rejected. #1124 DAG-2 rejects Soul-boundary markers and oversized free-text notes on that write path. #1124 DAG-3 stamps server-owned `provenance_source` / optional `actor_id`; transport `source` (`web`/`api`) is not provenance. Each outcome freezes `action/market/market_phase/source_type/source_agent/plan_quality/data_quality_level/holding_state` at evaluation time so historical stats are not rewritten by later live-join changes. Deleting history first finds `source_type=analysis` signals bound to the deleted history IDs, then removes their feedback/outcome sidecars.
...
#1390 P6 reuses `DecisionSignal` across alerts, notifications, and portfolio risk without adding tables, migrations, or configuration. Real stock-level alert triggers first link the latest active signal for the same symbol and write a low-sensitive `decision_signal_summary` into `alert_triggers.diagnostics`; when no active signal exists, the worker creates only a minimal `source_type=alert`, `action=alert` signal. Its `trace_id=alert-rule-<hash>` is for best-effort retry de-duplication, not active-signal overwrites, and the payload intentionally omits `market_phase` to avoid cross-phase duplicates. The analysis flow attaches the canonical `presentation` at runtime while aggregate analysis reports intentionally omit the duplicate DecisionSignal excerpt; the existing structured `action/action_label` fields in primary report and history schemas still use `action` as the direction authority. The Web report action card and history badges share the same taxonomy-label helper, with raw `operation_advice` retained only as a legacy fallback when structured action is absent. Only a supported string `metadata.report_language` participates in presentation language selection; non-string or unsupported caller metadata remains round-trippable but is ignored by presentation serialization. Alert notifications reference `action/label/confidence/summary/risk/timestamp` plus public fields such as `horizon/watch_conditions/source_report_id`, and derive labels from the canonical action in the report language. Notification failure does not block trigger or signal writes. `GET /api/v1/portfolio/risk` now includes a `decision_signal_risk` block that counts active `sell/reduce/alert` signals for current holdings, explicitly excluding `avoid/buy/add/hold/watch`; if signal lookup fails, the risk endpoint fails open and the Web risk card shows a degraded state.
...
Regular stock history report details no longer embed the extracted `source_type=analysis` signals and no longer issue a `source_report_id=<recordId>` query when the report details open. To inspect structured AI recommendations, use `/signals` and filter by source report ID, open the `/signals?sourceReportId=<recordId>` deep link, or search by stock. When source report ID is filled or provided through that URL parameter, the Web UI sends an exact `source_type=analysis + source_report_id=<recordId>` query without adding default `status=active` or other list filters, preserving the best-effort lazy backfill semantics for older reports.
...
The backtest start date first uses `market_phase_summary.effective_daily_bar_date` from the historical snapshot; older snapshots use the trading calendar only when their market and phase are trustworthy. When `market_phase_summary` is absent and cannot be rebuilt (legacy pre-phase rows), the engine falls back to the analysis-date expected start (pre-phase semantic) and persists `resolution_notes=legacy_analysis_date` instead of marking the row `insufficient_data`. The start bar and forward window must come from one market identity and one stored code shape, so prices are never spliced across aliases. If no bar exists on the exact expected date (suspended/halted stocks or local gaps), the engine uses the nearest previous bar within a bounded lookback (up to 10 trading sessions, or ~21 calendar days when the market calendar is unavailable) and persists `resolution_notes=prior_session_start`. Genuine no-data-at-all remains `insufficient_data`. Non-`force` runs skip only `eval_status=completed` rows for the same `(analysis_history_id, eval_window_days, engine_version)` key, so previously poisoned `insufficient_data` / `error` rows are re-attempted on later runs (higher re-eval cost until they complete). `GET /api/v1/backtest/performance/{code}` returns the canonical bare stock code in the summary payload (for example `AAPL`, not `AAPL.US`) when a summary exists.
...
- **Home attention hub** - Home renders exactly Today's Focus / To-dos / Signal summary by default. Morning report / Market review and today's versioned scheduled tasks live in a configurable area that is collapsed by default and persists its expanded preference in browser `localStorage`. A separate customizable dashboard board (watchlist, portfolio health, alerts, recent reports) supports show/hide/reorder with revisioned browser-profile preferences; see [home-dashboard-layout_EN.md](./home-dashboard-layout_EN.md).
- **Research overview** - `/research` is the dedicated hub for Market Review, Discover, Analysis Workbench, and Backtest; Market Review remains at `/research/market`. In expanded desktop navigation and the mobile drawer, the Research parent row opens `/research`, while its separate trailing control expands or collapses the four tool links without a duplicate overview child. In the compact rail, Research remains a menu trigger and Research overview is the first flyout item.
- **Legacy Home analysis-link compatibility** - `/?recordId=...`, `/?runFlow=...`, `stock`, and legacy analysis `workspace` parameters replace-redirect to `/research/analysis` and normalize to the corresponding `segment=launch|history|tasks` (`launch` is the default and may be omitted from the URL), while safe unrelated query and hash state survive. Bare `/` always remains on Home and does not restore an old report context.
- **Analysis Workbench** - `/research/analysis` under Research keeps Launch & batch / Running tasks / History & comparison as three segments on one page. `segment`, `recordId`, and Run Flow state are URL-restorable. Single-stock, watchlist, and chart/document import submissions switch directly to Tasks, while completion actions open the corresponding historical report. History also owns trend comparison, the full Markdown report, reanalysis with the current strategy, an Ask AI handoff with unconsumed report context, the centered fullscreen Run Flow Modal, and confirmed multi-select deletion. Regular stock history skips API pages containing only Market Review records until it finds a stock report or confirms that history is exhausted.
- **Global notifications and quick access** - The Shell mounts one notification Bell in either the desktop Sidebar or mobile header, aggregates recent signals and alerts, and deep-links to the corresponding `/signals` item by signal ID or alert trigger ID. Signals and alerts keep separate browser-local unread boundaries; a single-channel failure shows a retryable partial-availability notice without prematurely marking recovered items read, and clearing site data resets both boundaries. Sidebar Search and `Cmd/Ctrl+K` open the same command palette for pages, Signal Center actions, one-shot Market Review, stock lookup, and the canonical `/research/analysis` analysis destination.
...
- **Market Review visibility** - After triggering Market Review under Research, the API returns a `task_id` and the UI polls `GET /api/v1/analysis/status/{task_id}` to show progress; completed/failure states are rendered explicitly and failure messages are shown directly in the UI error area.
...
| `/api/v1/analysis/tasks/{task_id}/cancel` | POST | Request cancel for a `stock_analysis` task; unknown or other-kind ids return 404 without cancelling |
...
> Note: Web Analysis Workbench exposes an explicit strategy selector. When users do not pick one, `skills` is not sent and legacy behavior is preserved; when selected, it is passed through to this endpoint and persisted in task status/history snapshots.
...
> Audit note: priority and fallback are defined by `Config._load_from_env()` in `src/config.py` (`LITELLM_CONFIG` > `LLM_CHANNELS` > legacy). Regression coverage is in `tests/config/test_llm_channel_config.py` (configuration source parsing) and `tests/market/test_market_review_runtime.py` (shared runtime assembly). The endpoint lock is process/host-level only; multi-instance deployments still need external distributed idempotency controls.
...
> Note: `POST /api/v1/backtest/run` is a **synchronous** request. The response has no job, task, run, or idempotency identifier, and there is no status or cancel route. A Web client wait timeout is not proof that the server handler stopped. The Web backtest page shows an unknown-outcome state and refuses a second POST until the user dismisses it.
...
> - Verifiable tests:
>   - `tests/config/test_llm_channel_config.py` (configuration priority and provider/base URL mapping)
>   - `tests/market/test_market_review_runtime.py` (`build_market_review_runtime` shared assembly path)
...
> Note: `POST /api/v1/analysis/tasks/{task_id}/cancel` is a kind-scoped adapter over the process-local queue cancel port. Only `kind=stock_analysis` tasks are accepted; discovery, market review, and other kinds return `404` without calling cancel. The call is authenticated with the same `/api/v1/*` session as list/status when `ADMIN_AUTH_ENABLED=true`; there is no per-user ACL. Repeating cancel is idempotent and returns the current snapshot, including a competing `completed`/`failed` terminal state. Pending cancel stops the runner before it starts. Processing cancel is cooperative: the snapshot becomes `cancel_requested` immediately and `cancelled` when the runner returns. Reports or notifications that already persisted are not rolled back, and the UI must not claim they were undone. Do not reuse `POST /api/v1/discover/screen/tasks/{task_id}/cancel` for analysis tasks. Chat "Stop generating" remains a separate SSE abort contract.
...
## Opt-in Technical ∥ Intel stage wave

`AGENT_STAGE_PARALLEL_ENABLED=false` (the default) preserves today's serial contract: Intel starts only after Technical completes, and Intel can see Technical's non-sealed `ctx.data` writes and opinions. When set to `true`, only Native Multi `standard` / `full` / `specialist` runs execute Technical and Intel concurrently against **the same sealed** `AgentContext` snapshot (isolated deepcopies, `max_workers=2`) when the adjacent pair is exactly Technical then Intel and neither stage was restored from a checkpoint. Merge order is always technical then intel regardless of completion order; overlapping non-sealed keys are first-wins (Technical) when the flag is on. `quick` never opens the wave, even when the flag is on. Risk still runs after Intel and can read the merged `intel_opinion`. SkillRouter, Critic, red-team, and debate stay on their existing serial Decision insertion points.

Failure contract: Intel failure with isolate on a non-critical stage records degraded, keeps Technical, and continues to Decision. Technical failure cancels Intel, does not commit an Intel-only context, and fail-fasts the pipeline. Intel timeout isolates and keeps Technical. Hard budget breaches (`budget_tools` / `budget_turns` / `budget_cost` / `budget_tokens`) fail-fast and never isolate Intel. User cancel is probed after the wave join and before commit or checkpoint: if `cancelled_check` is true and Technical did not fail, the wave does not commit context, does not merge Intel, and does not write a `technical_intel` or `technical` checkpoint; it still emits declaration-order `stage_done`, returns cancelled, and does not enter Decision. A checkpoint named `technical_intel` is written only when both stages `COMPLETED` and the run was not cancelled; after a partial Intel isolate only `technical` is saved. On resume, a completed `technical_intel` checkpoint expands to technical and intel and skips both; a `technical`-only checkpoint skips Technical and runs Intel serially. SSE: both `stage_start` events are emitted first in declaration order; `stage_done` is also emitted in declaration order; in-wave `thinking` / `tool_*` events may interleave but must carry `stage`. Unknown types remain ignorable; `done` / `error` are unchanged.

Cost boundary: the flag is default-off, so default installs are unchanged. Rollback is keeping or setting `AGENT_STAGE_PARALLEL_ENABLED=false`, or reverting the change; no data migration. This is issue #1290 slice 1 and does not close a full dependency DAG, a max-concurrent knob, or a default-on flag.

## Bounded Multi-Agent Critic

`AGENT_CRITIC_ENABLED=false` (the default) preserves existing behavior. When set to `true`, only non-Chat Native Multi analysis adds one tool-free Critic LLM call after the technical/intel/risk/specialist opinions and before `DecisionAgent`. Single, Multi Chat, and daily or ordinary analysis with the setting disabled do not enter this stage.
...
Whenever Native Multi actually enters Critic — including pass-without-revision, budget skip, and stage failure — the run writes an additive `dashboard.critic` appendix (`enabled`, `ran`, `verdict`, `convergence_status`, `retry_status`, `revision_occurred`, `iteration_consumed`, `iteration_max`, and a deterministic sanitized English summary of at most 300 characters) without changing existing report fields or Decision authority. The same stage emits existing `AgentEventType.PHASE_START` / `PHASE_END` events named `critic` and `critic_retry` through `emit_agent_event` → `diagnostics.agent_events` → `/flow`, so the current process timeline can display them. evidence-chain-v1 projects one `pipeline_stage` item with `source_id=critic`, one reasoning step with `stage`/`role` critic, and `dashboard.critic` coverage through the existing builder only. Standard and research Markdown, notification, and WeChat paths render the appendix; brief mode omits it. Chat behavior and the default-off Critic setting are unchanged.
...
### Adversarial red-team second opinion

`AGENT_RED_TEAM_ENABLED=false` (default) preserves existing behavior. When `true`, only non-Chat Native Multi `full` / `specialist` analysis adds one tool-free LLM adversarial review *after* `DecisionAgent` completes. `quick` / `standard` do not run it by default; Chat never runs it. The stage writes an independent `dashboard.red_team` record (`challenges` / `missing_evidence` / `suggested_confidence_pressure`) and deterministically appends those findings to `phase_decision.data_limitations` and `risk_warning`. The `data_limitations` merge preserves unique existing primary-decision items in original order up to the 12-item product cap, deduplicates red-team lines against that list, and appends only into remaining slots. Overflow stays on `dashboard.red_team` (with `data_limitations_merge`) and must not evict, reorder, or replace the primary decision object. It must not rewrite `decision_type`, `confidence_level`, or `operation_advice`, and it does not feed red-team text back into DecisionAgent. Standard/research Markdown, history, and notification (daily dashboard / single-stock / WeChat renderer) show the same additive section, including missing evidence and typed unavailable/skipped status; brief omits it. There is no public HTTP or CLI `enable_red_team` field in this slice. Hard cap: one LLM turn, gated by remaining `AGENT_ORCHESTRATOR_TIMEOUT_S` and per-mode hard budgets. Provider/parse/timeout/budget failure records `budget_skip` or `data_unavailable`, keeps the primary decision successful, and invents no challenges. This is not Bull-Bear debate (#117) and not a Risk veto. Rollback: disable or remove `AGENT_RED_TEAM_ENABLED`.
...
Each run mode (`quick` / `standard` / `full` / `specialist` / chat) has hard caps for LLM turns, tool calls, and estimated USD cost (optional token ceiling via `AGENT_MODE_BUDGET_MAX_TOKENS`). Consumption is tracked on a shared `mode_budget` account and exposed in diagnostics (`ctx.meta["mode_budget"]` / `result.budget_snapshot`). Multi `AgentOrchestrator.run()` / `chat()` copy `budget_snapshot` and `failure_reason` onto the public `AgentResult` (results without budget metadata keep those fields unset; successful result ordering is unchanged). Optional end-of-run reflection and in-process post-mortem LLM calls consume the same LLM-turn cap; skipped optional work is `budget_skipped`, and the returned `budget_snapshot` includes those charges. On breach the run terminates with `success=false` and an explicit reason (`budget_turns` / `budget_tools` / `budget_cost` / `budget_tokens`). Existing residual wall-clock skips remain `budget_skip` / `timeout` and record into the same snapshot — there is a single budget concept, not a parallel system. Registered keys are `AGENT_MODE_BUDGET_ENABLED` and the global `AGENT_MODE_BUDGET_MAX_*` tighteners. Optional per-mode caps are Config attributes (`agent_mode_budget_<mode>_max_llm_turns` / `max_tool_calls` / `max_cost_usd` for `quick|standard|full|specialist|chat`); the loader maps optional unregistered `AGENT_MODE_BUDGET_<MODE>_MAX_*` env vars onto those attributes. They are not Web Settings fields. Prefer the global tighteners unless you need mode-specific caps; `0` keeps the built-in mode default. See `.env.example`. There is no `AGENT_MAX_RUN_LLM_CALLS` key; the run LLM-turn cap is `AGENT_MODE_BUDGET_MAX_LLM_TURNS` (and optional per-mode Config overrides).

Production Deep Research (`POST /api/v1/agent/research`, bot `/research`, and Native `ExecutionMode.RESEARCH`) shares that same `mode_budget` account for one research run: decompose, every sub-question loop, and synthesis charge one specialist-profile account. `AGENT_DEEP_RESEARCH_BUDGET` is the research token ceiling on that account (the specialist built-in `max_tokens=0` does not turn the ceiling off). A global positive `AGENT_MODE_BUDGET_MAX_TOKENS` tightens it with `min()`. When mode budgets are enabled, specialist turn/tool/cost defaults apply (12 LLM turns / 64 tool calls / $1.50, then global and optional specialist overrides). Each sub-question loop is clamped to remaining LLM turns on that shared account, so the specialist turn cap cannot bill one extra call. Exceeding a hard cap terminates with typed `budget_*` and `success=false`; leftover markdown is not a finished report. `AGENT_MODE_BUDGET_ENABLED=false` restores the previous research token-only stop, but exceeding `AGENT_DEEP_RESEARCH_BUDGET` still fail-closes as `budget_tokens` rather than warning and synthesising a successful report. Wall-clock `AGENT_DEEP_RESEARCH_TIMEOUT` and residual `budget_skip` / `timeout` mapping are unchanged. Cancel still outranks budget. When `AGENT_PLANNING_ENABLED` is true, opt-in RUN and single-agent Chat planning gather charge that same chat-profile `mode_budget` account; a `budget_*` breach fail-closes before synthesis.


## Agent Runtime Guards

Agent timeouts have three levels. `AGENT_TOOL_TIMEOUT_S` limits one tool call to 120 seconds by default. Optional `AGENT_DATA_TOOL_TIMEOUT_S`, `AGENT_SEARCH_TOOL_TIMEOUT_S`, `AGENT_ANALYSIS_TOOL_TIMEOUT_S`, and `AGENT_ACTION_TOOL_TIMEOUT_S` default to 0, meaning no category cap and the global one-call budget still applies; a positive cap applies only to the matching `ToolDefinition.category` (market-style network data tools use the data category). Accepted range is 0-3600 seconds; values above 3600 warn and clamp to 3600. `AGENT_TECHNICAL_AGENT_TIMEOUT_S`, `AGENT_INTEL_AGENT_TIMEOUT_S`, `AGENT_RISK_AGENT_TIMEOUT_S`, `AGENT_DECISION_AGENT_TIMEOUT_S`, `AGENT_PORTFOLIO_AGENT_TIMEOUT_S`, and `AGENT_SKILL_AGENT_TIMEOUT_S` can set independent stage limits. `AGENT_ORCHESTRATOR_TIMEOUT_S` defaults to a 600-second limit for the complete single-agent loop or multi-agent pipeline. When limits overlap, the shortest remaining budget wins (positive category cap, global one-call cap, and remaining orchestrator budget). Tool results that arrive after a timeout are fenced off and cannot be accepted as successful results. Invalid, non-finite, or negative category timeouts warn and degrade to no category cap without failing startup. Each multi-agent stage runs against an isolated context copy and commits state only when it returns `COMPLETED` before the limit; late state and progress cannot enter later stages. Python cannot forcibly terminate a native thread that is already running, so a custom stage or tool handler may still finish in the background and produce its own external side effects, but it cannot write back into the accepted Agent context or tool-session cache. Progress callbacks are synchronous hooks and must return promptly; the supported SSE and runtime adapters only enqueue or publish without waiting. The stage fence orders an accepted callback before closure but does not attempt to preempt arbitrary caller code already executing inside a callback.

`AGENT_MAX_IDENTICAL_TOOL_CALLS=3` permits an exact tool name and normalized argument set at most three times in one run; the fourth attempt stops the current Agent before dispatch. `AGENT_MAX_STAGE_ENTRIES=1` permits a stage name to enter once in the ordinary pipeline and hard-stops a duplicate before execution. The bounded Critic's sole whitelist retry is an explicit, separately budgeted path; it does not modify the re-entry guard applied to ordinary pipeline entries. Set a numeric guard to `0` to disable that guard. Loop logs contain only the tool name and a short hash of the argument signature, never the raw arguments.

`AGENT_STAGE_FAILURE_POLICY=isolate` is the default. Failures or uncaught exceptions from existing non-critical `intel`, `risk`, and specialist/skill stages are recorded and continue through the degraded path; `technical` and `decision` remain critical boundaries. With `fail_fast`, any failed ordinary stage stops the pipeline. The bounded Critic and its dedicated one-shot retry are explicit exceptions: their failures always preserve prior evidence, record a `fail_soft` limitation, and continue to Decision so the optional verifier cannot replace final authority or turn opt-in into a new hard-failure boundary. Guards write low-sensitivity `agent_runtime_guard {JSON}` log records for `tool_timeout`, `run_timeout`, `tool_loop_detected`, `stage_loop_detected`, `stage_exception_captured`, `stage_timeout`, `stage_failure_isolated`, and `stage_failure_fail_fast`. These internal logs do not change the public SSE, API, report, or Web/Desktop contracts.
