...
#1390 P4 wires the existing `DecisionSignal` API into the Web UI. The single Signal Center is now `/signals`, with Signal feed, Rules, Push history, and Reassessment & stats tabs plus URL-owned `scope=all|holdings|watchlist` context for the signal list and rules. Legacy `/decision-signals` and `/alerts` URLs retain parameter-mapping redirects. Current-stock and timeline views use their own stock context; Push history and outcome statistics keep the existing APIs' global scope. Those views hide the scope control while preserving a legal scope in the URL so it applies again when the user returns to the list or rules. The feed defaults to `status=active`, supports filtering by market, stock code, action, market phase, source, source report ID, and status, and the timeline adds a profile filter backed by the server-side list `decision_profile` parameter. `unknown` is used only to filter and display legacy `NULL` rows, and the regular advanced list does not add a profile filter. The page also includes a latest-active lookup by stock code. Cards, details, and timeline displays read the first-class `decision_profile` field first and fall back to legacy metadata only when that field is missing; explicit `null`, missing history, or invalid profile values display as unknown. Signal details show action, profile, confidence/score, horizon, plan_quality, market_phase, price plan, risk, watch conditions, source report, and data quality. The Web UI only allows marking a signal as `closed`, `invalidated`, or `archived`; it does not restore terminal states to active.
