...
- **Home attention hub** - Home renders exactly Today's Focus / To-dos / Signal summary by default. Morning report / Market review and recent analyses live in a configurable area that is collapsed by default and persists its expanded preference in browser `localStorage`.
...
The global Critic revision-round budget is controlled by `AGENT_CRITIC_MAX_ITERS` (default 1, hard cap 2); the same target is not re-entered. Each revision records a before/after evidence diff under `critic_trace.revision_rounds`. When convergence fails, Critic reasons and missing_evidence are projected into dashboard `phase_decision.data_limitations` instead of being dropped silently. When convergence succeeds, only a short revision note and revision-round summary are projected — original gap opinions are not restated. Optional revisions remain gated by residual `AGENT_ORCHESTRATOR_TIMEOUT_S` budget and, when present, per-mode hard budgets (#1213). The whitelist retry uses a dedicated one-shot budget path; it does not relax `AGENT_MAX_STAGE_ENTRIES` or create a general stage re-entry mechanism. The retry removes the target's prior opinion/data in an isolated context and replaces shared context atomically only when it returns `COMPLETED` with new target evidence. A failure, timeout, or evidence-free completion preserves the first stage context, result, and telemetry and passes the limitation to Decision.
