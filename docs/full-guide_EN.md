...
- **Global notifications and quick access** - The Shell mounts one notification Bell in either the desktop Sidebar or mobile header, aggregates recent signals and alerts, and deep-links to `/signals`; the unread boundary is local to the current browser and resets when site data is cleared. Sidebar Search and `Cmd/Ctrl+K` open the same command palette for pages, Signal Center actions, stock lookup, and the canonical `/research/analysis` analysis destination.
