...
Every `DecisionSignalItem` response also carries a read-only canonical `presentation` with `action/label/confidence/summary/risk/timestamp` for direction, localized label, confidence, reason summary, risk, and creation time. `action` is the sole direction authority, so a conflicting compatibility `action_label` cannot override it; alert notifications read `REPORT_LANGUAGE` from the current worker run and fully support `zh/en/ko`, while Web localizes the same action for the current UI language. An explicitly persisted creation `report_language` is formal presentation provenance: object or null metadata replacement during a status PATCH retains that key, while a row without formal provenance cannot promote it from replacement metadata, so status changes cannot silently alter the same signal's canonical label. Cards, details, portfolio summaries, timelines, and API JSON consumers read `presentation` first, while the flat fields remain only for compatibility and rolling-upgrade fallback. There is currently no dedicated DecisionSignal export endpoint or export UI; API JSON is the existing export surface. The field is not added to primary report, history, StockBar, or backtest schemas.
