...
| [ADR-008](ADR-008-persisted-schedule-process-local-execution-boundary.md) | Accepted | Durable schedule definitions and occurrences separated from process-local execution authority | [Issue #441](https://github.com/SiinXu/stock-pulse-ai/issues/441) |
| [ADR-009](ADR-009-pinned-unicode-casefold-contract.md) | Accepted | Unicode 15.0.0 default full case folding is the cross-runtime uniqueness authority for investment-framework dimension names | [PR #687](https://github.com/SiinXu/stock-pulse-ai/pull/687) |
| [ADR-010](ADR-010-import-cycle-ratchet.md) | Proposed | Shrink-only CI ratchet for bidirectional package import pairs (module-level edges) | Implementation PR introducing `scripts/check_import_layers.py` |
| [ADR-011](ADR-011-config-access-ratchet.md) | Proposed | Shrink-only CI ratchet for bare `get_config()` call sites; prefer injection / `ApplicationServices.config` | Issue #625; implementation PR introducing `scripts/check_config_access.py` |
| [ADR-012](ADR-012-installable-package-layout.md) | Accepted | Four existing import roots become installable packages via `pip install --build-constraint build-constraints.txt -e . --no-deps`; no import rewrite | Issue #167; implementation PR introducing `[tool.setuptools.packages.find]` |
