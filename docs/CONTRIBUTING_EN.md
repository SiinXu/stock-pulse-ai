...
| `pr-review` | Opt-in advisory review via `workflow_dispatch` with a required `pr_number`. The `security-check` job lists changed files through the GitHub Pulls API and does not checkout or `git diff` the PR head. Downstream jobs may still run static syntax/flake8, a PR-size advisory (~1000 lines excluding lockfiles/generated), and same-repo AI review (requires `GEMINI_API_KEY` and/or `OPENAI_API_KEY` repository secrets). Fork PRs stay read-only (no secrets, no write jobs). AI review is non-blocking. Dispatch from the Actions tab, select the PR branch as the workflow ref so downstream static checks still see that tree, and pass the pull request number; a dispatch without a valid `pr_number` fails closed. | ❌ (advisory) |
