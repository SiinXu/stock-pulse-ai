...
| `web-gate` | Always concludes; for Web changes runs `npm run lint` + `npm run test:i18n` + `npm run test:coverage` + `npm run build`, then runs the runtime performance soft gate and blocks on the bundle size budget; otherwise records a no-frontend summary; fails closed when change detection is unavailable | ✅ |
