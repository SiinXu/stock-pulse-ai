...
Web 普通设置不再提供全局「保存配置」按钮。字段修改会先进入本地草稿，并在 800ms 无继续输入后按原子字段组自动保存；模型 Connection、Task Routing 与 Reliability 属于同一个模型图分组，按 FIFO 串行提交，避免用旧 `config_version` 并发覆盖。

- `等待自动保存`、`自动保存中`、`已自动保存` 分别表示 scheduled、saving 和 saved。
- 保存失败保留本地输入，并提供「重试此分组」与「恢复服务器值」。失败、保存中或冲突状态会触发离开保护。
- HTTP 409 不会静默覆盖本地或服务器配置；页面展示三方冲突处理，解决后才会继续提交。
- 被遮罩的 API Key 只有在用户实际修改时才提交；mask token 不会覆盖已保存密钥。
- 「测试连接」与运行时能力检测仍是显式操作。未测试或测试失败不会阻止配置保存，也不会由自动保存隐式触发真实模型请求。

首次向导完成时的明确「保存并应用」是向导提交动作，不是普通设置页的全局 Save。
...
> **新版编辑体验补充**：模型接入按“模型服务商 / 模型连接 / 可用模型 / 任务模型”组织。Provider Catalog 决定字段要求和是否显示“获取模型”，新建连接不会预填容易过期的示例模型。DeepSeek、阿里百炼（DashScope）等 OpenAI-compatible 连接通过 `{base_url}/models` 获取模型，Ollama 使用 `/api/tags`；用户可从发现结果中多选，但系统不会自动全选。不支持发现、鉴权失败或服务暂时不可达时仍可逐个手动添加。每个模型是独立可删除的 token，底层继续保存为 `LLM_{CONNECTION}_MODELS=model1,model2`，旧逗号配置会自动解析为 token；Provider 身份另存为 `LLM_{CONNECTION}_PROVIDER=<provider_id>`，不会再从可重命名的 Connection 名称猜测。任务路由中的报告 / Agent / Vision / 备用模型只能从可用模型目录选择，不支持任意输入；历史失效值会保留并标记“当前配置不可用”，保存不会静默清理。
