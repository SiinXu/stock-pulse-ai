...
日常下载和管理统一在 **设置 -> AI 与模型 -> 本地模型** 完成。通用模型与金融模型卡片均来自仓库权威 catalog，展示体积、内存档、能力、许可证、安装状态和下载进度；按本机可检测内存高亮推荐档，检测不到时不猜测。纯 Web 环境由后端使用服务端 `LLM_OLLAMA_BASE_URL` 代理 Ollama，客户端不能提交目标 URL；Desktop 复用同一面板，通过主窗口 preload 控制 Ollama 的发现、启停、下载和删除。只有浏览器与后端的原生安装平台一致，且 Web 页面、API 请求入口及后端配置的 Ollama 端点都确认指向 loopback 时，macOS 和 Windows 才会直接下载对应的 Ollama 官方安装包；安装并启动后，返回页面会自动重新检测运行时。Linux、移动端、未知平台、远程 API/Ollama 和无法确认的代理或容器场景继续展示官方安装方式，远程场景必须在 StockPulse 后端所连接的机器上安装 Ollama。模型下载失败时才会提供对应的 `ollama pull <tag>` 命令作为恢复方式，不暴露底层错误。
