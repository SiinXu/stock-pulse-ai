...
日常下载和管理统一在 **设置 -> AI 与模型 -> 本地模型** 完成。通用模型与金融模型卡片均来自仓库权威 catalog，展示体积、内存档、能力、许可证、安装状态和下载进度；按本机可检测内存高亮推荐档，检测不到时不猜测。纯 Web 环境由后端使用服务端 `LLM_OLLAMA_BASE_URL` 代理 Ollama，客户端不能提交目标 URL；Desktop 复用同一面板，通过主窗口 preload 控制 Ollama 的发现、启停、下载和删除。Ollama 不可达时，面板会提供对应的 `ollama pull <tag>` 命令与安装指引，不暴露底层错误。
