...
> - 兼容验证入口：`tests/services/system_config/test_system_config_service.py`、`tests/test_system_config_api.py` 以及现有前端模型配置页回归用例。
...
- `AGENT_GENERATION_BACKEND=auto` 会继承 `GENERATION_BACKEND`；也可在 Web 设置页显式选择 `litellm|codex_cli|claude_code_cli|opencode_cli`。选择本地 CLI 后，问股 Agent 通过严格 JSON 工具桥接运行，不再要求另配 API 模型。
- 编辑已有模型连接时，“测试连接”和“获取模型”会在 Provider、协议与 Base URL 未变化的前提下复用已保存且已掩码的凭据；已保存的 API Key 不会回传浏览器，输入框会显示为空并只接受替换值，因此也不会提供只能显示掩码的“查看”按钮。修改连接身份或端点后必须重新输入 API Key，避免凭据被发送到不同服务。
- 选择本地 CLI 作为 Agent 生成后端后，历史 `AGENT_LITELLM_MODEL` 值不会参与当前 Agent 的模型引用和可用性校验；报告仍使用 LiteLLM 时，`LITELLM_MODEL` 及其来源约束保持有效。
- **CLI Agent 能力边界**：本地 CLI 进程仍禁用其内建工具、MCP 和写入能力，只能返回严格 JSON 的应用工具请求。工具名称、参数、权限、股票范围、超时与审计继续由 StockPulse 的 `BoundToolSession` 校验和执行；工具结果在下一次 CLI 回合中作为不可信数据传回。该桥接为 experimental，CLI 每个 Agent 回合都会启动受限进程，通常比原生 API tool calling 慢，且不支持 token streaming。
...
- Phase 6a 建立的 DSA Tool Surface 继续作为 deny-by-default 执行边界：共享 schema、结构化错误、审计/脱敏、execution-owned grant、stock scope、递归 URL outbound policy 与有界参数检查同样覆盖本地 CLI 桥接；模型输出不能自行增加 capability。stock-scoped 工具调用仍必须满足既有 scope 契约。本地 CLI 桥只把严格 JSON 转换到同一原生 Agent 工具循环，不会向 CLI 进程授予直接工具权限。
...
日常下载和管理统一在 **设置 -> AI 与模型 -> 本地模型** 完成。通用模型与金融模型卡片均来自仓库权威 catalog，展示体积、内存档、能力、许可证、安装状态和下载进度；按本机可检测内存高亮推荐档，检测不到时不猜测。纯 Web 环境由后端使用服务端 `LLM_OLLAMA_BASE_URL` 代理 Ollama，客户端不能提交目标 URL；Desktop 复用同一面板，通过主窗口 preload 控制 Ollama 的发现、启停、下载和删除。只有浏览器与后端的原生安装平台一致，且 Web 页面、API 请求入口及后端配置的 Ollama 端点都确认指向 loopback 时，macOS 和 Windows 才会直接下载对应的 Ollama 官方安装包；安装并启动后，返回页面会自动重新检测运行时。Linux、移动端、未知平台、可检测的远程 API/Ollama 和无法确认的代理或容器场景继续展示官方安装方式，远程场景必须在 StockPulse 后端所连接的机器上安装 Ollama。HTTP 无法区分真正同机 loopback 与 `ssh -L` 等保留 loopback 的隧道；通过此类隧道访问时，安装包会下载到浏览器所在机器，运维人员仍须在后端所连接的机器上按官方指引安装 Ollama。模型下载失败时才会提供对应的 `ollama pull <tag>` 命令作为恢复方式，不暴露底层错误。

下载成功后，后端通过 `SystemConfigService` 把模型追加到 `LLM_OLLAMA_MODELS`、确保 `LLM_CHANNELS` 包含 `ollama`，并热加载配置。只有当前确实没有主模型时才自动选择 `ollama/<id>`；已有主模型保持不变，面板会询问是否显式切换。Agent 模型通过独立操作设置，不会隐式替换主模型。Desktop 的长时间下载和删除绑定操作开始时看到的配置版本与规范化运行时的 SHA-256 身份摘要；请求不携带目标 URL，后端只从服务端配置重算摘要和访问目标，任一快照值变化都会拒绝完成操作。Desktop 流式下载另有 30 分钟绝对截止时间。首次启动向导的“本地模型”分支嵌入同一个面板，要求用户显式选中一个已就绪模型后再继续；下载的新模型保持选中，不会被 catalog 中更靠前的旧模型覆盖。完成后直接进入分析工作台，无需编辑 `.env` 或打开终端。

备用顺序继续由现有多渠道配置和 **设置 -> AI 与模型 -> 可靠性** 中的模型 fallback 编辑器管理，本地模型中心不引入第二套 fallback 配置。Catalog 中标记为 guided-only 的金融模型只显示转换/许可证指引，不提供直接下载。被主模型、Agent、Vision、fallback 或当前隐式主 channel 引用的权重，必须先切换相应任务模型后才能删除。Desktop 在删除已注册模型或仅存在于 Ollama 的未注册模型前，都会收到一个短时、一次性的恢复 token；token 待处理期间，同一模型不能通过本地模型服务并发下载或重新注册，后端也会暂时拒绝所有 `SystemConfigService` 配置写入（包括任务模型分配），避免取消注册后又在权重删除前产生新引用。确认删除、完成恢复或短 TTL 到期后会释放这项配置写入租约。恢复同时绑定模型、删除预留后的精确配置版本和原运行时；已注册模型只有在原运行时仍报告对应权重时才会写回注册，运行时停止、不可达或权重缺失都会拒绝恢复。未注册模型的恢复只释放预留，不会创建注册。若 Desktop 保守判断权重仍在、但后端随后的原运行时探测确认权重已缺失，面板会按删除完成收敛并保持取消注册状态。权重删除成功后会重试一次幂等 token 撤销；若最终撤销因网络或状态响应仍无法确认，删除依旧视为成功并返回非破坏性的 `local_model_delete_finalize_unconfirmed` 警告。token 不能恢复已缺失的权重，并会在短 TTL 后过期；后端未收到最终确认时，配置写入可能在 TTL 剩余时间内继续返回版本冲突，刷新后重试即可。
...
- “获取模型”是否可用由 Provider Catalog 的 `supports_discovery` 决定；OpenAI-compatible / DeepSeek 使用模型列表接口，Ollama 使用 `{base_url}/api/tags` 并允许空 Key。“测试连接”默认只对模型列表首项发起一次最小聊天请求，并在结果中展示后端规范化后的 `resolved_model`。若返回 `details.reason=model_access_denied`，请优先确认该模型是否已在当前账号/key 下开通；未覆盖或语义不同的 provider 文案会继续走兜底诊断。可选的“运行时能力检测”必须由用户显式选择后触发，会额外发起 JSON / tools / stream / vision smoke 请求；诊断不会写回 `.env`，也不会阻止保存。首次配置向导的可选“测试连接”复用同一后端诊断：除展示 `resolved_model` 外，还会给出阶段与错误码可操作提示，并附带 JSON 与 Vision 能力检查结果；能力结果仅供参考，不会改变基础连通判定，也不会阻止保存。
...
- Web 凭据字段使用统一的浏览器身份边界：管理员当前密码、新密码和确认密码具有不同的稳定 `name` 与标准密码 `autocomplete`；Provider API Key 和其它 Schema 敏感配置使用 `autocomplete=off` 及独立于管理员密码的稳定名称，通用敏感配置仍保留显式聚焦后才可编辑的门禁。首次向导和模型连接弹窗中的填充式变更只更新各自的本地草稿，不会自动触发模型发现、连接测试或配置保存。首次创建管理员密码并跳转到配置向导后，Provider API Key 保持为空，只有用户明确输入后才进入向导草稿。
...
- mask token 或省略的 `LLM_{CONNECTION}_API_KEY(S)` / `LLM_{CONNECTION}_EXTRA_HEADERS` 只有在同一 Connection 名称下，且 Provider、协议、Base URL 均未改变时，才表示“保留已持久化 secret”。修改任一身份字段或重命名 Connection 时，必须提交新的明文密钥/请求头 JSON 或明确清空；不能把旧端点的凭据静默带到新端点。只存在于运行时环境、未写入持久配置的 secret 也不能通过 mask/省略复用，必须由用户显式重新输入。预览、冒烟测试和正式保存执行同一作用域校验。
...
- 当前仓库对此链路的依赖约束是 `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0`（见 `requirements.txt`）；回归覆盖包括 `tests/services/system_config/test_system_config_service.py`、`tests/test_system_config_api.py` 和 `apps/dsa-web/src/components/settings/__tests__/LLMChannelEditor.test.tsx`。

> **外部 provider 示例模型说明**：`cohere/*`、`google/*`、`xai/*` 等 provider 前缀值仅用于说明当前保存保留语义，**不代表该依赖约束内的逐型号可用性保证**。文档或测试中的具体模型名都是配置保留行为样例，不是生产推荐；实际可用性请以对应官方模型文档为准，并结合仓库依赖约束 `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0` 复核。

### 回退与兼容性证据

- 依赖约束与运行时引用保护：在 `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0` 下，使 runtime 模型引用（`LITELLM_MODEL`、`AGENT_LITELLM_MODEL`、`VISION_MODEL`、`LITELLM_FALLBACK_MODELS`）失效的保存会被后端校验拒绝而不是静默清理，`cohere/*`、`google/*`、`xai/*` 等非渠道直连模型会被保留。
...
- 回退回归证据：`tests/services/system_config/test_system_config_service.py::test_import_desktop_env_restores_runtime_models_after_cleanup` 覆盖“清理后用桌面导出备份恢复 runtime 引用”。
- 直连 provider 回归证据：`tests/services/system_config/test_system_config_service.py::SystemConfigServiceTestCase::test_validate_accepts_minimax_model_as_direct_env_provider`、`test_validate_accepts_cohere_model_as_direct_env_provider`、`test_validate_accepts_google_model_as_direct_env_provider`、`test_validate_accepts_xai_model_as_direct_env_provider` 覆盖直连 provider 保留语义。
...
- 本仓库兼容性回归覆盖见：`tests/config/test_llm_channel_config.py`、`tests/analyzer/test_market_analyzer_generate_text.py`、`tests/test_agent_pipeline.py`、`tests/services/system_config/test_system_config_service.py`。
...
- 回归验证入口：
  ...
  - 运行时源清理与恢复（含桌面导出备份链路）：`tests/services/system_config/test_system_config_service.py`
...
- 回归与证据：`tests/llm/test_llm_param_recovery.py`、`tests/services/system_config/test_system_config_service.py`、`tests/config/test_llm_channel_config.py`、`tests/test_system_config_api.py`、`tests/analyzer/test_market_analyzer_generate_text.py`、`tests/test_agent_pipeline.py`；桌面导入与运行时清理回退另有 `test_import_desktop_env_restores_runtime_models_after_cleanup` 直接覆盖。
