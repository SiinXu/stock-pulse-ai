...
The Web page is `/approvals`, reached from the Home To-dos card without adding a primary navigation domain. It shows pending and terminal states, countdowns, original and conservative signals, approve/reject controls, and minimal rule settings. Approve and reject open a second confirmation that names the action and target in localized labels; Escape, backdrop, and Cancel leave the proposal pending. Local controls suppress duplicate submits. When decide returns `409`, the confirmation overlay stays busy and holds the document isolation lock (`inert` / `aria-hidden` on the app root, `overflow: hidden` on `body`) until the server refresh finishes, then closes the overlay and restores page interaction. Unknown status/signal/risk codes stay visible as sanitized diagnostics instead of raw unlabeled machine values.
