...
开启且样本达到适配器阈值后，`BaseAgent` 把展示/决策置信度交给 `calibrate_confidence`，后者使用 `AgentMemory.get_calibration` 已存的 `CalibrationResult.calibration_factor`（且仅在 `calibrated` 为真时生效）。门控调用使用既有适配器签名（`agent_name`、`stock_code`），不传 `skill_id`；非门控路径仍传入 `extract_skill_id(self.agent_name)`。AgentMemory 已将 `historical_accuracy / avg_confidence` 钳制到 `0.5..1.5`，其中 `historical_accuracy=0.0` 是真实的 0% 准确率；适配器不得再用 `accuracy or 0.5` 这类真值回退重算该比值。随后将置信度限制在 `[0,1]`。样本来源仍是既有 `AGENT_MEMORY_ENABLED` / `AgentMemory`，本切片不新增存储。工具有效性仍是恒等桩，不会解锁已拒绝的 ToolSurface 工具。路由偏好按阈值生效：适配器开启、`N >= AGENT_ONLINE_ADAPTERS_MIN_SAMPLES`、且已评分预测结果 miss-rate `>= 0.5`（`1 - OUTCOME_NUMERIC_SCORE` 均值）时，`prefer_route` 上调一档（`quick→standard`，`standard→full`），并追加一条 `adapter.route_preference` EvolutionEvent。恒等路径与写入失败不改变返回的 mode。`prefer_route` **不会**写入 `AGENT_ORCHESTRATOR_MODE`，AgentRouter **不会**调用它。影响只记录在运行期 `AgentContext.meta["adapter_influence"]`（不写入 episode）。本切片 **不** 实现真正的 `rank_tools` 打分（#1123）、自动晋升、HTTP EvolutionEvent 列表（#1113）、episode schema 持久化或晋升。
