...
- 记忆 provenance、事实/意见隔离与防投毒基线：[#1124](https://github.com/SiinXu/stock-pulse-ai/issues/1124)。DAG-0 威胁注释、DAG-1 事实/意见锁定、DAG-2 Soul/超限写路径拒绝（`src/schemas/memory_write_guard.py`）、DAG-3 服务端盖章 provenance（`src/schemas/memory_provenance.py`）和 DAG-4 默认关闭的 AgentMemory 注入隔离（`src/agent/agents/base_agent.py` / `src/agent/memory.py`）已落地。不要并入 #1118 存储/UI、#1119 遗忘策略或 #1105 产品反馈 API。在维护者确认全部验收项之前保持 issue 开放。
