...
Settings → **Agent 行为 → 投资框架** 横向 Tab 直接提供页内结构化编辑器和框架状态：
...
**历史版本** 会在同一页面打开只读抽屉。列表和只读检查器都会展示版本 `created_at`；历史版本不会原地恢复或修改。“复制到当前草稿”只复制内容，后续保存仍携带当前 aggregate revision 并创建新版本。修订冲突（HTTP 409）时界面会重新加载服务端状态；若刷新失败，页面隐藏旧草稿并显示可重试的加载错误，避免继续编辑过期内容。后端 422 校验仍是最终权威；公开且已脱敏的 `details.issues[].loc` 会映射到对应节点、维度或按行规则，并同时保留全局错误提示。编辑已知字段时使用对象合并，并在传输边界保留未知的未来字段，避免滚动升级期间静默丢失服务器所有内容。
