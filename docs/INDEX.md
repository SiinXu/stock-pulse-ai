...
| [Agent 深度路由规则库](agent-router.md) | 纯规则 AgentRouter 与结构化事实投影；仪表盘 `AgentOrchestrator.run()` 每次 run 应用路由器并恢复构造时 mode（#1120 第一至三切片；Chat/factory 仍未接线；[英文](agent-router_EN.md)） |
