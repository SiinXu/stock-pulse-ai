...
| [Agent 深度路由规则库](agent-router.md) | 纯规则 AgentRouter 与结构化事实投影；仪表盘 `run()` 应用路由器并记录 run-local 元数据；Chat 在 `incremental_tool` 时跳过 `_execute_pipeline`（#1120 第一至五切片；factory 仍未接线；[英文](agent-router_EN.md)） |
