...
| [本地专用模式](local-only-mode.md)（[EN](local-only-mode_EN.md)） | 失败即关闭的策略管辖 HTTP 闸门、威胁模型、限制与核验面（#218） |
...
| 参与开发或提交 PR | [贡献指南](CONTRIBUTING.md) | [工程效率操作手册](engineering-efficiency-playbook.md)、[共享运行时 session 契约所有权](runtime-session-contract-owners_CN.md)（[EN](runtime-session-contract-owners.md)）、[业务架构](business-architecture.md)、[技术架构总览](architecture-overview.md)、[ADR 注册表](adr/README.md)、[API 规格](architecture/api_spec.json) |
...
| [Agent 深度路由规则库](agent-router.md) | 纯规则 AgentRouter 与结构化事实投影；仪表盘 `AgentOrchestrator.run()` 每次 run 应用路由器，无显式覆盖时尊重构造时/Settings 深度，恢复构造时 mode，并把 secret-free 决策写入 run-local 元数据（#1120 第一至四切片；Chat/factory 仍未接线；[英文](agent-router_EN.md)） |
...
| [预测核验安全放量](prediction-verification-rollout.md) | Issue #1115 运营放量顺序、示例名到落地键映射、当前延期边界（[英文](prediction-verification-rollout_EN.md)） |
...
| [EvolutionEvent 审计日志](evolution-events_CN.md) | 仅追加的 EvolutionEvent 存储与 UTC 时间范围/精确类型查询（#1113 第一切片，未接线生产者；[EN](evolution-events.md)） |
...
| [Service-split template（英文）](service-split-template.md) | 行为保持的 facade / re-export 拆分清单、棘轮验证与兼容 shim 删除条件；含 merged PR #1352 偏差审计，以及独立验收的 end-to-end 证据 PR #1547（#1088） |
...
| [Web UI 基础控件契约](web-ui-foundation.md) | Button、IconButton、Input、Field、Textarea 的语义、尺寸、命中区、守卫和迁移边界；表面角色与密度 D5 棘轮（#877）；原生 button 采用棘轮（#878）；Theme token freeze Phase 0（#1300） |
