...
- `quote` 会透传 #1386 P3 的 `fetched_at`、`provider_timestamp`、`is_stale`、`stale_seconds`、`fallback_from`，并投影既有 `field_trust.analysis_input` 为有界 metadata `{confidence, conflict_count, gap_codes, failed_provider_count}`。完整 `field_trust` 不得作为 quote item value。状态优先级固定为 `STALE > FALLBACK > PARTIAL > AVAILABLE`：`is_stale=True`、`price_stale`、`quote_stale`、`quote_stale_seconds` 等显式 marker 标为 `stale`；`stale_seconds` 且 `is_stale=False` 只是元数据，不单独推断 stale；`analysis_input.confidence != high`（含冲突、过期、未归因、provider 失败、跳过的冲突检查、缺失/legacy `metadata_absent`）在尚未 stale/fallback 时标为 `partial`，以触发既有 core-degraded High 禁令。builder 只映射上游 artifact，不做质量评分。
...
**兼容性证据与核验边界：**本轮仅复用既有 LLM 配置链路读取配置，不新增 `.env` 写入分支，不新增配置迁移/清理/回写入口。官方依据沿用：`LiteLLM OpenAI-compatible` <https://docs.litellm.ai/docs/providers/openai_compatible>、`OpenAI Chat Completion` <https://platform.openai.com/docs/api-reference/chat/create>；版本约束见 `requirements.txt`（`litellm`、`openai`）当前窗口。可回溯代码路径：`main.py::_bootstrap_environment`、`src/analyzer.py::_init_litellm`、`src/agent/agents/base_agent.py::_get_analyzer_config`（仅读取）、`src/agent/executor.py`、`src/agent/orchestrator.py`、`src/core/pipeline.py`、`src/services/daily_market_context.py`、`src/daily_market_context_guardrail.py`。回归核验点为 `tests/test_config_env_compat.py`、`tests/test_config_registry.py`、`tests/services/system_config/test_system_config_service.py`、`tests/test_system_config_api.py`、`tests/config/test_llm_channel_config.py`、`tests/market/test_market_review_runtime.py`。
...
P4 Web 展示只在报告详情页渲染 `AnalysisContextSummary`，位置在策略点位和资讯之后、运行诊断之前；该区域通过共享 `Collapsible` 默认折叠（`aria-expanded` / `aria-controls`），折叠头部展示可用数、缺失数、非零的其他状态计数和触发来源。展开后的每个数据块只消费已有低敏摘要：状态、来源、告警和说明；未记录来源时显式标记。Quote block 上的 `quote_trust_*` 告警会再渲染为低敏行情可信度行（来源 / 置信度 / 缺口），不公开完整 `field_trust`、provider attempts 或 circuit blob。Jinja 分析报告从同一 overview quote 摘要渲染对应的报告段（`templates/report_markdown.j2` 及同一 renderer 的 wechat/brief）。说明将已知 `missing_reasons` 翻译为原因、对本次分析的影响与可执行建议，并保留安全的诊断码；未知码按 block 状态提供通用建议，非安全码不原样渲染。`fallback`、`stale`、`estimated`、`partial` 等没有缺失原因的降级状态也在同一说明行解释，数据限制和告警使用语义 `InlineAlert`，数据块以无额外卡片的分隔行呈现。报告页相关资讯由独立接口加载，其有无不能由 overview 的 `metadata.news_result_count` 推断；资讯区只说明自身是报告页补充资讯，输入数据块只说明新闻是否进入本次 LLM 分析。展开区仍展示状态计数和输入阶段新闻结果数。P5 后折叠头部还会展示质量分/等级，展开后展示 `limitations` 和 `fetch_failed` 状态。无 overview 时不渲染占位。在 #1386 P4b 中，Web 会在同一报告详情页展示 `report.meta.market_phase_summary` 阶段标签，并继续复用该低敏数据质量摘要；不扩大完整 pack、Prompt summary、raw payload 或 snapshot 内部字段的公开面。P4/P5 不覆盖 pending/processing TaskPanel 的 AnalysisContextPack 数据质量摘要或 SSE 进行中 overview 可见性，不改通知摘要、Bot/Desktop 专属展示或 `market_review` overview。
