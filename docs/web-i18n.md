...
`i18n:high-risk` 会从真实源字典和十语言 bundle 提取交易动作、风险、认证、Credential、错误和免责声明文案，核对已纳入审计的 TypeScript 内部 code / 稳定显示 key 清单，锁定逐类、逐语言快照，用数量与 SHA-256 摘要分别保护稳定 key decision 与逐 locale 修订层，并校验高风险 key 清单（`high-risk-i18n-keys.json`）与母语评审管线证据（`nativeReviewPipeline`、`reviewPasses`、`languageReview`）。`--verify-baseline` 还会用 TypeScript AST 解析记录的 main bundle，逐项确认 `before` 值来自当前分支 merge-base，并要求实际修订集合与 decision 完全相等。`--print-keys` 打印实时清单；`--write-key-inventory` 在评审 key 集合变更后重写已提交清单。审计文档中 “Deferred to UIUX” 的组件内原始 code fallback 不在该保证内。快照与自动化通过不代表母语金融签核。`PENDING_NATIVE_REVIEW` 本身不会阻塞 CI；升级到 `NATIVE_REVIEWED` 需要 reviewer 身份、日期与 `reviewedSnapshotSha256`。高风险评审工作不得扩大 identical-to-English baseline。
