...
`i18n:high-risk` 会从真实源字典和十语言 bundle 提取交易动作、风险、认证、Credential、错误和免责声明文案，核对已纳入审计的 TypeScript 内部 code / 稳定显示 key 清单，锁定逐类、逐语言快照，并用数量与 SHA-256 摘要保护 decision 清单。`--verify-baseline` 还会用 TypeScript AST 解析记录的 main bundle，逐项确认 `before` 值来自当前分支 merge-base。审计文档中 “Deferred to UIUX” 的组件内原始 code fallback 不在该保证内。快照与自动化通过不代表母语金融签核。
