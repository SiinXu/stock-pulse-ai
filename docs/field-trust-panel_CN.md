...
| 分析输入 | 与 provider 无关的 `{ confidence, gaps[] }`；仅当每个覆盖字段都新鲜、已归因且无冲突时才为 `high` |
...
- Quote `to_dict()` 在存在时包含 `field_trust`，以便分析 `_safe_to_dict(realtime_quote)` 收到 gaps/confidence。
- 记录辅助函数对数据失败开放（从不打断行情路径），对信任失败关闭（缺失载荷视为未知）。
