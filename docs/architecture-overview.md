...
| `src/market/` | Canonical market-analysis, market-context, Market Light metrics (`src/market/metrics.py`), market-review prompt builders (`src/market/prompts.py`), no-LLM template degradation (`src/market/degradation.py`), pure report formatters (`src/market/formatters.py`), markdown block builders (`src/market/blocks.py`), market-overview fetch orchestration (`src/market/market_data.py`), news retrieval (`src/market/news.py`), generation-diagnostic redaction (`src/market/diagnostics.py`), report section helpers (`src/market/report_sections.py`), phase-prompt, phase-summary, regime-prompt, and structure-prompt implementations. Root-level `src/market_*.py` shims for those modules have been removed. `src/market_sector_analysis.py` remains real implementation imported by `src/market/analyzer.py`. Production callers import `src.market.analyzer` (the public facade), not `src.market.prompts`, `src.market.degradation`, `src.market.formatters`, `src.market.blocks`, `src.market.market_data`, `src.market.news`, `src.market.diagnostics`, or `src.market.report_sections`. **New production code must import the canonical package**; see [legacy facade import policy](legacy-facade-import-policy.md). |
...
| `src/services/` | Application use cases, task queue adapter, scheduling, analysis, history, portfolio, alerts, intelligence, and rendering services. Gravity-module splits follow the [service-split template](service-split-template.md) (ADR-006). | HTTP transport schemas or provider-specific normalization. |
...
| `src/core/pipeline.py`, `src/core/stages/`, and `src/core/contracts/` | Analysis orchestration facade, stage implementations, typed stage outcomes, and formal stage IO contracts (`RunContext`, fetch/analyze/render IO, stage errors). `pipeline.py` is orchestration-only; business rules stay in stages/services. Optional enhancer construction (MarketHotspot / Search / SocialSentiment fail-open) lives in `src/core/stages/optional_services.py`; production import remains `src.core.pipeline`. | Transport lifecycle or persistent query APIs. |
