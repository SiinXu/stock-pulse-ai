...
| `src/market/` | Canonical market-analysis, market-context, Market Light metrics (`src/market/metrics.py`), market-review prompt builders (`src/market/prompts.py`), no-LLM template degradation (`src/market/degradation.py`), pure report formatters (`src/market/formatters.py`), phase-prompt, phase-summary, regime-prompt, and structure-prompt implementations. Root-level `src/market_*.py` shims for those modules have been removed. `src/market_sector_analysis.py` remains real implementation imported by `src/market/analyzer.py`. Production callers import `src.market.analyzer` (the public facade), not `src.market.prompts`, `src.market.degradation`, or `src.market.formatters`. **New production code must import the canonical package**; see [legacy facade import policy](legacy-facade-import-policy.md). |
