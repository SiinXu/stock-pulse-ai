...
当前生产 registry 的目标版本是 `202607190003_portfolio_idempotency_scope_schema`。`202607160001_migration_runner_registry` 建立有序 registry 所需的 additive metadata；`202607190001`、`202607190002`、`202607190003` 分别把原先由 startup `_ensure_*` 兼容步骤补写的 `llm_usage` 遥测列，`decision_signals` 的 `decision_profile` 列/索引/回填，以及 `portfolio_idempotency_records` 的 scope 列、唯一索引、legacy 数据规范化和 legacy 冲突 guard trigger 改为正式 migration，对缺失的旧库幂等补齐，对 fresh 库为 no-op。
...
升级不删除现有业务表、字段或数据。`llm_usage` 遥测列、`decision_signals` 的 `decision_profile` 列/索引/回填，以及 `portfolio_idempotency_records` 的 scope 列/唯一索引/回填/guard trigger 的 startup 兼容步骤已分别转为 `202607190001`、`202607190002`、`202607190003` 正式 migration；`create_all` 与其余 `_ensure_*`（Intelligence scope value/unique index）仍作为兼容债务，后续继续通过独立切片逐项转换为正式 migration。
