...
当前生产 registry 的目标版本是 `202608260001_layered_memory_observation_schema`。该切片新增 `layered_memory_observations` / `layered_memory_consent` / `layered_memory_access_audit`（#1118 存储契约；观测表允许 DELETE 以满足清空/过期；审计表 trigger 拒绝 UPDATE/DELETE；`downgrade` 只 DROP 这三张表及其索引/trigger，不触碰 `agent_episodes` / `agent_evolution_events`）。审批门禁之后的 additive migration 还包括 skill-opinion 结果、task-queue inflight 检查点、自选股分组、组合健康快照、通知收件箱已读状态、chat turn identity、analysis-history FTS、`llm_usage` 成本归因列、`agent_predictions` 预测核验表（到期扫描索引与 resolved 不可变 trigger）、记忆写入 provenance 空列（`decision_signal_feedback` / `decision_signal_outcomes` / `agent_predictions` / `agent_episodes`），可选的 `agent_run_feedback` / `agent_prediction_feedback` sidecar（#1105，不改写预测 actuals 或 append-only episode），可选的 `agent_episode_forward_returns` sidecar（#1096，CLI 研究用前向收益分桶，不 `UPDATE` append-only episode），可选的 `agent_episode_curator_grades` sidecar（#1096，CLI 评测 fixture 策展等级，不 `UPDATE` append-only episode），以及仅追加的 `agent_evolution_events` 表（#1113，UPDATE/DELETE 由 trigger 拒绝，不复用 security audit / episode / curator grade）。`202607160001_migration_runner_registry` 建立有序 registry 所需的 additive metadata；`202607190001`~`202607190005` 依次把原先由 startup `_ensure_*` 兼容步骤补写的 `llm_usage` 遥测列，`decision_signals` 的 `decision_profile` 列/索引/回填，`portfolio_idempotency_records` 的 scope 列/唯一索引/规范化/guard trigger，`intelligence_items` 的 legacy scope 值规范化，以及 `intelligence_items` 从 legacy url 唯一到 scoped 复合唯一键的重建改为正式 migration；`202607240001_security_audit_events` 新增 append-oriented `security_audit_events` 表及查询索引；`202607240002_scheduled_task_schema` 新增版本化 scheduled-task 定义、definition/generation/dispatch 栅栏、准入失败计数、通知结果与运行记录表；`202607240003_investment_framework_schema` 新增个人投资框架 aggregate 与不可变版本历史表；`202607250001_approval_gate_schema` 新增 owner-scoped 审批规则和一次性审批提案表。历史库会幂等补齐，fresh 库由 metadata 建表后 migration 验证 shape。启动路径不执行临时业务 schema DDL 兼容步骤。审批表完整业务与回滚合同见[人工审批安全门禁](human-approvals.md#数据迁移与回滚)。
...
- **消费者遇到不认识的版本必须优雅降级**：跳过该区块 / 返回空 / 回落默认值，绝不因历史 payload 版本不符而硬失败。既有实现：`src/market/structure_prompt.py` 与 `src/utils/data_processing.py` 在 `schema_version` 不匹配时跳过；`AnalysisContextPack.pack_version` 用 `Literal` 拒绝未知值。
