...
| `upgrade` | 接收 SQLAlchemy `Connection`、同步执行并返回 `None` 的升级 callable |
...
Production migration 是经过审查并随仓库发布的受信代码，不是用户脚本、插件或远程载荷。Migration 注册会递归检查 `inspect.unwrap` wrapper、`functools.partial` 目标和 callable object，拒绝 coroutine、generator、async-generator function 以及循环 wrapper；因此 `contextmanager`/`asynccontextmanager` 也不能隐藏 lazy upgrade。Runner 会对 source-bound production migration 的 Python AST 做静态检查，拒绝直接访问底层 DBAPI 连接以及已知的事务控制调用；运行时 guard 还会阻断公开 SQLAlchemy 事务 API 和 SQLite transaction/savepoint opcode。这些检查是防止误用和回归的纵深防御，不是 Python 安全沙箱，也不能把不受信代码变成可安全执行的 migration。新增 migration 仍必须经过源码审查和完整测试，禁止从配置、数据库或网络动态加载 upgrade 代码。
...
Migration 在 `DatabaseManager` 初始化调用内同步完成，不会在后台继续升级。SQLite 的 `create_all + _ensure_* + baseline` 兼容阶段也在一个数据库级写锁和事务内串行化，避免两个 fresh 进程在进入 runner 前竞争建表。首次需要 `DatabaseManager` 的后端路径只有在迁移完成后才会返回。正常 migration 失败时，`DatabaseManager` 保持未初始化，runner 持有的当前 transaction 回滚，且不记录 applied row。受信 migration 若绕过 source guard 并直接提交底层 DBAPI transaction，Runner 仍会拒绝 applied row，但无法撤销已在其控制范围外提交的 DDL/DML；该契约违规必须按备份恢复或显式向前修复处理。
...
第 4 步适用于 runner 仍持有 transaction 的正常失败。若错误是 transaction ownership 违规，底层 DBAPI 可能已经提交了部分状态；不要盲目重试同一 DDL，也不要手工补 applied row。应先与升级前备份比对，再恢复备份或发布一个能显式证明并修复该状态的向前 migration。
...
- 备份恢复用于数据库损坏、transaction ownership 违规或其他灾难场景。runner 仍持有 transaction 的普通 migration 失败使用事务回滚和向前重试。
...
3. `upgrade` 使用同步、非 generator callable 并显式或隐式返回 `None`；只执行当前 migration 的 DDL/DML，不自行 begin、commit、rollback、close 或访问底层 DBAPI transaction；确保 source-bound 源码 guard 通过。不要把 guard 当作运行不受信 migration 的沙箱。
