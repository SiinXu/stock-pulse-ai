...
3. `upgrade` 只执行当前 migration 的 DDL/DML，不自行 begin、commit、rollback、close 或访问底层 DBAPI transaction；确保 source-bound 源码 guard 通过。不要把 guard 当作运行不受信 migration 的沙箱。
