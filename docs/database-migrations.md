...
当前生产 registry 的目标版本是 `202607240002_scheduled_task_schema`。`202607160001_migration_runner_registry` 建立有序 registry 所需的 additive metadata；`202607190001`~`202607190005` 依次把原先由 startup `_ensure_*` 兼容步骤补写的 `llm_usage` 遥测列，`decision_signals` 的 `decision_profile` 列/索引/回填，`portfolio_idempotency_records` 的 scope 列/唯一索引/规范化/guard trigger，`intelligence_items` 的 legacy scope 值规范化，以及 `intelligence_items` 从 legacy url 唯一到 scoped 复合唯一键的重建改为正式 migration；`202607240001` 新增 append-oriented `security_audit_events` 表及查询索引；`202607240002` 新增版本化 scheduled-task 定义与运行记录表。历史库会幂等补齐，fresh 库由 metadata 建表后 migration 为 no-op。启动路径不执行临时业务 schema DDL 兼容步骤。
