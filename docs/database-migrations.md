...
当前生产 registry 的目标版本是 `202607190002_decision_signal_profile_schema`。`202607160001_migration_runner_registry` 建立有序 registry 所需的 additive metadata；`202607190001_llm_usage_telemetry_columns` 与 `202607190002_decision_signal_profile_schema` 分别把原先由 startup `_ensure_*` 兼容步骤补写的 `llm_usage` 遥测列，以及 `decision_signals` 的 `decision_profile` 列、profile 感知索引和从 `metadata_json` 的 legacy 回填改为正式 migration，对缺失的旧库幂等补齐，对 fresh 库为 no-op。
...
Migration 在 `DatabaseManager` 初始化调用内同步完成，不会在后台继续升级。SQLite 的 `create_all + _ensure_* + baseline stamp + pending migrations + baseline 证明` 全部在同一个数据库级写锁和事务内串行化，避免两个 fresh 进程竞争建表，并让整笔初始化对 registry、Schema 与 baseline 证明保持原子性。startup 在这把写锁内直接应用有序 migration，因此 baseline 证明看到的是补列后的完整 Schema。首次需要 `DatabaseManager` 的后端路径只有在迁移完成后才会返回。初始化任一步失败时，`DatabaseManager` 保持未初始化；create_all、兼容修复、applied row 与该事务内的所有 DDL/DML 一起回滚，不留半迁移状态。
...
升级不删除现有业务表、字段或数据。`llm_usage` 遥测列与 `decision_signals` 的 `decision_profile` 列/索引/回填的 startup 兼容步骤已分别转为 `202607190001_llm_usage_telemetry_columns` 和 `202607190002_decision_signal_profile_schema` 正式 migration；`create_all` 与其余 `_ensure_*`（Portfolio idempotency scope、Intelligence scope value/unique index）仍作为兼容债务，后续继续通过独立切片逐项转换为正式 migration。
