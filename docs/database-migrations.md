...
当前生产 registry 的目标版本是 `202607190001_llm_usage_telemetry_columns`。`202607160001_migration_runner_registry` 建立有序 registry 所需的 additive metadata；`202607190001_llm_usage_telemetry_columns` 把原先由 startup `_ensure_*` 兼容步骤补写的 `llm_usage` 遥测列改为正式 migration，对缺列的旧库幂等补列，对已含这些列的 fresh 库为 no-op。
...
升级不删除现有业务表、字段或数据。`llm_usage` 遥测列的 startup 兼容步骤已转为 `202607190001_llm_usage_telemetry_columns` 正式 migration；`create_all` 与其余 `_ensure_*`（DecisionSignal profile、Portfolio idempotency scope、Intelligence scope value/unique index）仍作为兼容债务，后续继续通过独立切片逐项转换为正式 migration。
