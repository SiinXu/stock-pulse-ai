...
- 轨迹层 / 可选即时层 LLM 在运行带有 `ctx.meta["mode_budget_account"]` 时计入同一账户。生产 Chat/单 Agent 循环会把该账户挂到 executor 上，供运行结束反思读取。循环内可选步骤批评还会预留一轮 Decision；运行结束反思不预留。它们不经过 `run_agent_loop`，因此只计一次。跨运行 meta-review 为离线任务，没有 run 账户。
