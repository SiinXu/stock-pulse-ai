...
### 第一步：启用认证并修改 .env 中的监听地址

用编辑器打开 `.env`（在项目根目录，即包含 `main.py` 的目录），先启用管理员认证：
...
> `127.0.0.1` 表示只有本机能访问，`0.0.0.0` 表示允许任何来源访问。云服务器需要把 `.env` 中的 `WEBUI_HOST` 改成 `0.0.0.0`，或在启动命令里显式传入 `--host 0.0.0.0`，才能从外网打开界面。若 `ADMIN_AUTH_ENABLED=false`，服务会在监听前拒绝启动。
...
由于 Compose 会发布该端口，启动前必须在 `.env` 中设置：

```env
ADMIN_AUTH_ENABLED=true
```

认证关闭时，`server` 容器会拒绝非本机监听并退出，而不是暴露未认证的 API。
...
1. **Docker 镜像发布版本**：先在当前仓库的 Packages 页面或对应 registry 确认 tag 已存在，再查看部署时使用的镜像 tag，例如 `ghcr.io/siinxu/stock-pulse-ai:<published-tag>`。仓库的 Docker 发布由 `.github/workflows/docker-publish.yml` 按 `v*.*.*` Git tag 触发，所以 Docker 版本应以已发布的镜像 tag / GitHub Releases 为准。若没有可用 tag，请从当前源码构建，不要执行下面的镜像拉取示例。
...
```bash
# 方式 2：回看你的拉取命令
docker pull ghcr.io/siinxu/stock-pulse-ai:<published-tag>
```

请先把 `<published-tag>` 替换为在 Packages 页面或 registry 中确认存在的固定 tag。如果你一直使用已发布的 `latest`，建议改成显式版本 tag；否则很难仅凭容器内页面信息判断自己是否已经重复更新到同一版本。
...
改法：先确认 `.env` 中设置了 `ADMIN_AUTH_ENABLED=true`，再把 `WEBUI_HOST=127.0.0.1` 改成 `WEBUI_HOST=0.0.0.0` 并重启服务；也可以在启动命令里显式添加 `--host 0.0.0.0`。认证关闭时，非本机监听会按安全策略拒绝启动。
...
把 Web 界面暴露到公网之前必须开启登录密码保护；认证关闭时，StockPulse 会拒绝 `0.0.0.0`、局域网地址和其他非本机 HTTP 监听：
...
`ALLOW_INSECURE_PUBLIC_BIND=true` 是高风险紧急逃生口，会在认证关闭时允许对外监听并写入持久安全警告。它不能替代认证、HTTPS 或网络访问控制，也不应作为常规部署配置。

---

遇到其他问题？欢迎 [提交 Issue](https://github.com/SiinXu/stock-pulse-ai/issues)。
