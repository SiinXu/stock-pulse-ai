...
- `NOTIFICATION_DELTA_FIRST` 是默认关闭的通知展示开关：设为 `true` 时，在单股与聚合出站通知顶部展示有界的“较上次分析变化”摘要，并明确区分首次分析、无实质变化和暂时无法对比。它只读取已持久化历史，不额外调用模型、不修改本地保存报告；对比或格式化失败不会阻断原通知。回退时删除该变量或设为 `false`，无需迁移数据。
- 单股投递会在通知渠道可用性早退之前把普通本地 Markdown 报告写入 `reports/report_YYYYMMDD_<code>.md`。未配置渠道时跳过发送，已配置渠道的发送、去重、降噪和 delta-first 行为保持不变；本地写入失败 fail-open，不会阻断分析或已配置渠道的推送。
...
兼容性排除说明：
- 本轮未改动 `src/notification_parts/senders/wechat_sender.py`、`src/notification_parts/senders/slack_sender.py`、`src/notification_parts/senders/telegram_sender.py` 的发送路径；`src/notification_parts/senders/feishu_sender.py` 新增 `send_feishu_file()` 文件发送路径，Webhook 模式回退为发送文件内容文本，App Bot 文字发送路径（`send_to_feishu` → `_send_via_app_bot`）保持不变。
...
| `EMAIL_SENDER_NAME` | advanced | email | Variable or Secret | `StockPulse` |
...
高分歧告警（#134）在分析历史落库后按 `HIGH_DISAGREEMENT_ALERTS_ENABLED` / `HIGH_DISAGREEMENT_THRESHOLD` 判定；**仅消费** `schema_version=disagreement-handling-v1` 的既有 `disagreement_handling` 记录，**不**重新计算分歧。存在 `disagreement_score` 时仅当 `score >=` 阈值才告警（`high_disagreement` 不能旁路阈值）；分数缺失时回退到 `high_disagreement=true`。告警与报告推送共用出站意图：`--no-notify` / `send_notification=false` / dry-run 时不发送。告警正文包含分歧要点与历史入口（相对路径 `/research/analysis?segment=history&recordId=...`；当 `WEBUI_HOST` 为可用主机时附绝对 URL）。发送失败或单渠道失败不会中断分析。渠道配置仍归通知渠道 Settings（#931）；应用内收件箱投影仍归 #953。
...
- Web 设置页的路由下拉只列出已配置成功的内置渠道（存量已保存但对应渠道未配置的值仍会显示并可取消勾选）。已启用插件注册的 `notification_channel` 会在 Notifications 枢纽单独列出，并与 Extensions 面板双向深链；未加载或未激活时保持诚实空态，不会伪装为已连接。插件 ID 仍可通过现有环境配置写入路由。未配置任何内置渠道时展示引导入口，可跳转到「通知渠道」页完成配置。
...
- `--check-notify` 使用当前插件生命周期快照并直接显示 canonical ID、display name 与状态：未知、disabled、failed 或 unloaded 插件 ID 报 error；enabled 但 unavailable 的插件目标报 warning；enabled 且 available 的插件 ID 计入已配置渠道。内置与插件 ID 混用时不会把插件 ID 强制转换为内置枚举。
...
每次发送先取得 root-owned dispatch lease，冻结 enabled adapter tuple，再分别判断 availability，并用“内置 routable ID + 本次 enabled 插件 ID”冻结 route allowlist。aggregate 路径直接用同一 retained snapshot 同时决定本路由是否存在可投递目标和实际目标，不会先通过独立的 `is_available()` 预检释放一次 lease。空路由仍发送到全部 available 渠道；非空路由只保留 configured、enabled、available 的交集，交集为空不会 fallback 到 broadcast。并发 disable、replace、reset 或 close 会等待所有已进入的发送快照结束后才执行 `onunload()`。adapter 在本次发送线程中请求 disable 时，`PluginOperationResult` 返回 `success=False, deferred=True` 与当前 state；同线程 root 替换请求返回时只表示已入队，尚未切换 root。deferred 请求会在所有已进入的 lease（包括同线程嵌套发送）退出后执行，并在此前阻止任何新快照进入。当同时存在其他已进入的发送时，发起请求的发送可以先返回，所以 `deferred=True` 不表示卸载已完成。因此已冻结目标不漏发、不重复，下一个快照不再包含已移除 adapter。

route、上下文/静态渠道策略、降噪、Markdown 图片准备、逐渠道错误隔离、插件结果映射和 attempt 聚合由 `NotificationService` 的私有实现模块 `src/notification_parts/dispatch.py` 唯一负责。单发（`send` / `send_with_results`）与聚合报告路径共用同一结构化分发管线：事件 → 路由/降噪 → 渠道适配 → 隔离发送 → 结果留痕；`send()` 仍返回历史 bool，`send_with_results()` 返回 `NotificationDispatchResult`（`status` 含 `sent` / `partial_failed` / `all_failed` / `no_channel` / `noise_suppressed`），并通过 `channel_summaries()` 提供 `[{channel, ok, error}, ...]` 可查询摘要；`get_last_dispatch_result()` 可在调用后读取最近一次结构化结果（同实例并发发送时 last-writer-wins，权威结果以返回值为准）。结果类型定义在 `src/notification_parts/contracts.py`（经 `src.notification` / `src.notification_parts.contracts` 再导出）。Pipeline 单股与聚合 DISPATCH stage 的 `output_summary.channels` 会挂载同一摘要，便于诊断与 API 侧查询。`POST /api/v1/agent/chat/send` 使用同一摘要：`success` 在任一渠道成功时仍为 true（兼容历史 bool），`status=partial_failed` 与 `channels` 标出混合失败；全部失败返回 `error=send_failed` 而不是误报 `no_channels`。运行诊断 `components.notification.details.results`、大盘复盘 notification_runs、CLI 摘要、Agent EventMonitor 告警和 bot 休市通知也消费该形状。分析成功与通知部分失败保持分离：单渠道失败不会中断后续渠道或分析主流程，除非调用方已有显式 fail-fast。未提供 `send_with_results` 的旧 bool 调用方由 `invoke_notifier_dispatch()` 包装，不会发明第二套分发管线；`send_with_results()` 只有返回规范 `NotificationDispatchResult` 或历史 bool 时才可产生成功，仅带字符串 `status` 的 duck-typed 对象会 fail-closed 为 `all_failed`。Pipeline 只保留聚合主报告的 render -> dispatch 顺序、逐渠道 stage fence 和运行诊断落库。adapter 只接收这些决策之后构造的 immutable `NotificationRequest`；返回的 `NotificationAdapterResult` 会映射到既有 `ChannelAttemptResult`，aggregate 路径也会保留 error code、retryable 与脱敏 diagnostics，并对成功或不可重试的 attempt 做物理发送防重。availability 异常或非法返回只记录脱敏日志并从本次 target snapshot 排除，不生成发送 attempt；send 异常、非法结果或失败返回才生成脱敏 attempt，并继续后续渠道，不改变分析成功标准。canonical ID 与内置或其他插件重名会在 factory 执行前关闭式拒绝；v1 factory 可以是接收一个 `Config` 的普通函数或 adapter class，额外参数可以是 optional/variadic，但 core 的既定调用不得要求更多参数。返回 adapter 的 `channel_id` 必须与 registration ID 相同，display name 及 `is_available()`/`send(request)` 签名会在 publication 前校验。native adapter 还必须以 canonical ID 和每次 native publication 独有的不透明 owner token，与 PluginManager 中 lifecycle-stable 的 enabled registration 精确配对后才会进入发送快照；因此即使替代注册复用同一 ID 和同一个 factory/class，旧 owner 也不能误接入尚在 `onload()` 或最终加载失败的新 adapter。`--check-notify` 在 manager 提交前同样不会把 pending adapter 报告为 enabled；disable/unload 会通过既有 owner handle 清理 adapter。

`NotificationService` 的 Config 与 notification registry 来自同一个已安装 `ApplicationServices` 快照。仅用于报告渲染的 service 构造保持惰性，不会为了检查插件而安装或启动默认 root；首次 availability、route 或 send 操作才绑定投递 runtime，并关闭式拒绝与 service Config 不同的并发安装 root。adapter 在每次 enable 时取得该 root 当时的 Config。默认 root 的 `Config.reset_instance()` 会影响之后构造的 service，但不会暗中替换已经发布的 adapter；先 disable 再 enable 会用重载后的 Config 重建 adapter。显式注入 Config 的 root 始终保留该对象，进程全局 Config reset 不会改写它。
...
P5 强化聚合报告通知路径的失败边界：Pipeline 将已渲染的聚合报告交给统一 dispatch policy；该 policy 在 report 路由过滤后对每个静态或插件通知渠道单独发送。某个渠道抛异常会记录日志并视为该渠道失败，但不会跳过后续渠道，也不会中断分析主流程。无来源上下文且 report 路由仅包含 `dingtalk` 时，聚合报告会直接调用钉钉 sender，并按正常渠道 attempt 写入 DISPATCH stage。
...
#1390 P6 的决策信号摘要沿用同一失败隔离边界：分析流程只在运行时挂载低敏 `decision_signal_summary`，聚合分析报告仍不重复渲染 DecisionSignal excerpt；告警通知会追加顶层 canonical `action`、`presentation` 的标签/置信度/摘要/风险/时间戳，以及周期、观察条件和来源报告。两条路径都不会输出 signal `metadata`、`evidence`、raw diagnostics 或 webhook/token。告警 worker 从本次运行配置读取 `REPORT_LANGUAGE`，完整支持 `zh/en/ko`，并由顶层 action 生成对应语言标签，不使用可能冲突的兼容 `action_label` 或嵌套 `presentation.action`。告警通知发送失败只记录通知尝试或 dispatch fallback，不回滚已经写入的 trigger 或 DecisionSignal。
