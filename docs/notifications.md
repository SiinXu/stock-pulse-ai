...
每次发送先取得 root-owned dispatch lease，冻结 enabled adapter tuple，再分别判断 availability，并用“内置 routable ID + 本次 enabled 插件 ID”冻结 route allowlist。空路由仍发送到全部 available 渠道；非空路由只保留 configured、enabled、available 的交集，交集为空不会 fallback 到 broadcast。并发 disable、replace、reset 或 close 会等待所有已进入的发送快照结束后才执行 `onunload()`。adapter 在本次发送线程中请求 disable 时，`PluginOperationResult` 返回 `success=False, deferred=True` 与当前 state；同线程 root 替换请求返回时只表示已入队，尚未切换 root。deferred 请求会在所有已进入的 lease（包括同线程嵌套发送）退出后执行，并在此前阻止任何新快照进入。当同时存在其他已进入的发送时，发起请求的发送可以先返回，所以 `deferred=True` 不表示卸载已完成。因此已冻结目标不漏发、不重复，下一个快照不再包含已移除 adapter。

route、降噪、Markdown 图片准备、计时、错误映射和 attempt 聚合仍由 core 负责。adapter 只接收这些决策之后构造的 immutable `NotificationRequest`；返回的 `NotificationAdapterResult` 会映射到既有 `ChannelAttemptResult`。availability 异常或非法返回只记录脱敏日志并从本次 target snapshot 排除，不生成发送 attempt；send 异常、非法结果或失败返回才生成脱敏 attempt，并继续后续渠道，不改变分析成功标准。canonical ID 与内置或其他插件重名会关闭式拒绝该插件；factory/adapter 的 ID、display name 和可调用签名在 publication 前必须完全匹配；disable/unload 会通过既有 owner handle 清理 adapter。
