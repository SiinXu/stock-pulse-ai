...
Agent prompts include the applicable market rules, quote currency, timezone, and market-specific fields. `get_chip_distribution` and `get_capital_flow` are A-share-only, so HK and US Chat turns do not expose or require them. A mixed-market Single-Agent turn conservatively removes both tools; a Multi-Agent comparison isolates capability per symbol, exposing them only to an A-share pipeline and never to a non-A-share pipeline. Multi-Agent then uses a tool-free synthesis step, and all symbol pipelines plus synthesis share one overall timeout budget. Other missing provider or tool coverage is reported explicitly instead of being filled with A-share defaults.
