...
Issue #1707 的首版能力聚焦“合规资讯源采集、本地沉淀、可查询证据”，不把本地资讯池与按需搜索语义混为一谈，也不默认新增独立舆情页。

> 说明：按需新闻搜索链路另有可选的 `RSS_NEWS_FEED_URLS`（issue #271），作为 SearXNG/付费搜索的**补充**源，走 `SearchService` 的过滤与相关度路径；它与本页的 `NEWS_INTEL_*` 本地资讯池相互独立。留空 `RSS_NEWS_FEED_URLS` 时搜索侧 RSS 完全惰性。
...
- 默认禁止 `localhost`、`.local`、回环地址、内网地址、链路本地地址、保留地址、共享地址段和组播地址；可信私有源仅可通过精确 `OUTBOUND_HTTP_ALLOWLIST` 条目放行，metadata/link-local 等硬拒绝目标不可覆盖，详见 [出站 HTTP 安全策略](security-outbound-policy.md)；
...
- 结构化检测提示中的模型/API 兼容风险在本次改动中不成立：`news_context` 注入链路仅复用现有 LLM 分析输入构造流程（`src/core/pipeline.py`、`src/market/analyzer.py`、`src/analyzer.py`），且不新增 `.env` 写入、保存前清理、清空/回填逻辑。
