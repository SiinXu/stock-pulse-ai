...
First open does **not** consume cloud LLM tokens. Completing the local-model wizard path starts a real brief analysis only after the selected local model is the configured primary route; if an existing primary was preserved, the user must click **Set as primary** first. Cloud LLM and market data APIs may still be required depending on the path chosen in the wizard; this is not the offline `LOCAL_ONLY_MODE` path.
...
- **Auto-update**: production `electron-updater@6.8.9`; `build.win.publish` remains GitHub Releases (`SiinXu/stock-pulse-ai`). After the UI loads, the shell queries `GET /api/v1/security/local-only` on the launched backend. GitHub metadata and Windows NSIS `electron-updater` run only when that endpoint confirms Local Only is **off**. If the mode is on, or the status cannot be confirmed, the check is skipped (fail-closed); Settings About shows the skip reason and `logs/desktop.log` records `[update] skipped ...`. `main.js` keeps `autoDownload = true` and Windows install-directory backup/restore around updates when a check is allowed. The product `0.x` line still cannot auto-update from installed `3.x` clients (semver); see the Chinese doc version-restart section.
