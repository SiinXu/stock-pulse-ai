...
Testing Library `findBy` / `waitFor` does **not** inherit Vitest `testTimeout`. `vitest.config.ts` detects `--coverage` in the main CLI process (where argv is authoritative) and injects `WEB_VITEST_COVERAGE=1` into workers via `test.env`. `src/setupTests.ts` reads that flag — not worker `process.argv`, which never includes `--coverage` under Vitest forks — and sets `asyncUtilTimeout` to 10s. The fast local loop stays at the 1s default. React.lazy report panels (for example `ReportDiagnostics` in `ReportSummary`) stay absent until the chunk resolves; tests must wait for those nodes as part of ready, not after a news-only settle. Timeouts live in `src/test-utils/coverageTimeouts.ts`.
