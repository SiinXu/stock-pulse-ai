...
- 缺失依赖边界：如果运行环境缺少 `alphasift.dsa_adapter`，`status` 返回 `available=false + diagnostics.reason=missing_module`；`strategies` 和 `screen` 返回 `424` 并显示下文完整可复现安装流程或建议重建 Docker/桌面后端产物，不会在业务请求中自动 `pip install`。该流程包含受 `constraints.txt` 约束的 pip bootstrap、`build-constraints.txt` 和 `pip check`。
...
- 升级路径：
  - 部署侧按部署方式执行 `git pull`，再运行 `python -m pip install --upgrade --constraint constraints.txt pip`、`python -m pip install --build-constraint build-constraints.txt -r requirements.txt` 和 `python -m pip check`，最后重启服务；
...
- 热点刷新容错：`/api/v1/alphasift/hotspots` 未显式传入 provider 且未配置 `INDUSTRY_PROVIDER` 时，默认使用 DSA EastMoney 兜底 provider（响应中 provider 为 `akshare`），避免落到 AlphaSift 的空 provider 路径；东方财富热点直连源遇到连接中断、超时或 `Connection aborted` 会做短 backoff 重试；手动刷新失败且没有可用热点缓存时返回稳定空态 payload、`source_errors=["eastmoney_hotspot_unavailable"]` 和用户可读 `message`，原始异常仅保留在服务端日志或诊断链路中。Web 热点面板区分真正空结果、数据源降级和上次成功缓存：空结果保留刷新说明并提供数据源入口，降级不会再复用空态文案或只展示原始 `source_errors`，失败刷新会保留并标注上次成功结果；服务端 200 last-good 回退（`cache_used` / `fallback_used` 且 `source_errors` 非空）同样标注上次成功卡片并保留热点区 Retry 与数据源入口。桌面端更新会保留 `data/alphasift/hotspots.json`、`data/alphasift/hotspot.history.jsonl`、`data/alphasift/hotspot_details` 与 `data/alphasift/snapshot.last_good.json`，避免更新后丢失 last-good 缓存。
...
- Hotspot 契约兼容核验：`docs/alphasift-integration.md` 与 `src/api/v1/endpoints/alphasift.py`、`src/services/alphasift_service.py` 保持 `hotspots`/`hotspots/{topic}` 字段与 `tests/api/alphasift/test_alphasift_api.py` 一致，调用前后默认使用 `snapshot.last_good` 缓存兜底。
...
- 设置页提供 AlphaSift 开关，开启后写入 `ALPHASIFT_ENABLED=true` 并检查适配层是否可用；若缺失，会回滚开关并显示本文的完整可复现安装流程或建议重建 Docker/桌面后端产物。
- `ALPHASIFT_ENABLED` 是“开启选股”按钮背后的持久化状态，不作为普通数据源配置项重复展示。
- 选股页未开启时展示开启按钮；开启后读取 AlphaSift 策略列表。
- 选股页空态与降级态分型（#871）：
  - **从未运行**：结果区引导说明本页用途，主 CTA 为「运行选股」；
  - **筛选无命中**：已完成运行但候选为 0，提供「调整条件 / 重试」，与「无数据」区分；
  - **数据源、LLM、StockPulse 增强及通用降级**：分别归类展示，不把 LLM 或增强失败误称为行情数据源失败；仍有候选时结果照常展示；
  - **数据源全不可用 / 适配层不可用**：错误态 + CTA 链到现有 Settings → Data sources → providers（`/settings?section=data_sources&view=providers`），不依赖尚未落地的 Data Hub IA。
  - **状态检查失败**：`401/403`、网络错误、`5xx` 或响应校验失败不会被误判为未开启；页面显示解析后的原因、状态重试操作，并在鉴权失败时提供管理员登录入口。
  ...
  - **热点题材 empty / degraded / last-good**：空结果、无缓存或刷新失败时，热点区块内的刷新重试（含折叠态）与「打开数据源设置」挂在同一恢复控件组；失败刷新或带 `cache_used`/`fallback_used` 且 `source_errors` 非空的 200 缓存回退保留并标注上次成功卡片；降级详情仍映射 `source_errors`，Data Sources 位于热点 section 恢复表面，而不是页面孤立按钮。健康热点卡片不展示该 CTA。默认打开页面时的成功缓存读取（`cache_used` 但无 `source_errors`）仍视为健康，不套用 last-good 标题。
...
- `python -m py_compile src/api/v1/endpoints/alphasift.py src/services/alphasift_service.py tests/api/alphasift/test_alphasift_api.py src/config.py src/core/config_registry.py`
