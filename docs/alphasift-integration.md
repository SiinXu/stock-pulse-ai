...
- 热点刷新容错：`/api/v1/alphasift/hotspots` 未显式传入 provider 且未配置 `INDUSTRY_PROVIDER` 时，默认使用 DSA EastMoney 兜底 provider（响应中 provider 为 `akshare`），避免落到 AlphaSift 的空 provider 路径；东方财富热点直连源遇到连接中断、超时或 `Connection aborted` 会做短 backoff 重试；手动刷新失败且没有可用热点缓存时返回稳定空态 payload、`source_errors=["eastmoney_hotspot_unavailable"]` 和用户可读 `message`，原始异常仅保留在服务端日志或诊断链路中。Web 热点面板区分真正空结果、数据源降级和上次成功缓存：空结果保留刷新说明并提供数据源入口，降级不会再复用空态文案或只展示原始 `source_errors`，失败刷新会保留并标注上次成功结果；服务端 200 last-good 回退仅识别失败后回吐缓存的形状（`cache_used` 且 `fallback_used`，卡片仍在），使用 last-good 正文而不是空列表文案，并保留热点区 Retry 与数据源入口。成功的实时 fallback（`fallback_used` 但 `cache_used` 为 false，即使带 `source_errors`）不当作 last-good 或降级。桌面端更新会保留 `data/alphasift/hotspots.json`、`data/alphasift/hotspot.history.jsonl`、`data/alphasift/hotspot_details` 与 `data/alphasift/snapshot.last_good.json`，避免更新后丢失 last-good 缓存。
