...
信号中心是一级工作区，侧栏或移动抽屉中始终提供入口。也可以从业务上下文直接进入：
...
| `/signals?tab=history` | 历史 |
| `/signals?tab=review` | 复盘 |
...
| Tab 名称 | 职责 | 什么时候去 |
| --- | --- | --- |
| **信号** | 浏览、筛选、关闭、反馈建议 | 每天扫一眼 |
| **规则** | 设置「到价/涨跌/指标」等提醒 | 想盯盘时 |
| **历史** | 查「规则有没有触发、通知有没有发出」 | 手机没响时 |
| **复盘** | 重新评估并查看事后统计 | 周末复盘时 |

---

一级 Tab 固定在页头下方。进入「信号」后，再用紧凑的分段控件切换「全部信号 / 当前股票 / 股票信号时间线」；进入「历史」后用同样的分段控件切换触发历史与通知尝试。全部 / 持仓 / 自选范围只出现在它实际作用的信号或规则内容区，不会在切换一级 Tab 时推动主导航上下移动。

选中一条信号时，唯一身份是 `?signal=<id>`。全部信号 / 当前股票 / 时间线只是入口，相同来源的两条信号不会互相顶替。选中后，页头下方的固定上下文条会显示代码、来源和状态。浏览器前进后退会恢复同一个 id。如果当前页没有这条记录，深链仍按 id 拉取。

关闭详情抽屉不会清掉 `?signal=` 或上下文条。上下文条保持可见且可操作，再次点击会打开同一 id，不会改身份。全部信号 / 当前股票 / 时间线刷新按 id 查找，也不会覆盖来自其他入口的更新选择。
...
详情里的 **决策记忆** 有两个互不排斥的开关：

- **重点记忆**：让这条记录在日后的历史决策复盘中更优先。
- **忽略**：不让它进入历史决策记忆检索。
- 两个都开时以 **忽略** 为准；界面会明确提示。保存结果来自服务器，失败时可重试，重新打开详情仍会读取已保存状态。
...
- **预警冷却** 有三种选择：使用后端默认值（当前 24 小时）、关闭冷却（明确发送 `0` 秒）、自定义 1–31,536,000 整数秒（最多 365 天）。默认和 `0` 不是一回事。
- 不建议随意关闭冷却：条件一直满足时可能反复提醒。编辑旧规则时，界面只改已知冷却秒数，不会丢掉服务端保留的其他策略字段。
...
触发历史的状态列和阶段/质量列使用已有用户文案（已触发 / 已跳过 / 降级 / 失败，以及良好 / 可用 / 受限 / 较差），而不是原始机器码。未知码显示为带清洗码的本地化诊断，而不是裸 snake_case。自由文本触发原因和测试消息会标为诊断。规则列表中的组合指标（最高权重、最大回撤等）使用客户端文案。API 仍返回机器码。
...
- 样本很少时，胜率数字 **几乎没有意义**。  
- 过新的信号可能还在冷却窗口，暂时算不了。  
- 统计常常是 **全局已复盘** 口径：你在信号里筛了「持仓」，统计数字也不一定跟着变——页面若写了「全局」，请相信它，不是 bug。
...
| 浏览全局后验结果 | 按周期、结果、评估状态、引擎版本或信号 ID 筛选并分页；点一行可追溯到对应信号详情 |
| 决策风格重评估 | 对有来源报告的信号显式进入/退出重评估会话；会话中锁定当前股票和信号，列表/最新/时间线点击不会换上下文；可能被风控阻断或因缺报告而不可用 |
...
「我分析完 600519 了，然后呢？」  
→ 打开信号，状态选 active，找到这只票，只读动作、风险、失效条件。若完全不符合你的判断，点「无用」或关闭即可，**不必强迫自己执行**。

### 案例 B：我想在 1800 元附近被提醒

→ 规则 → 价格条件 → 试运行 → 启用。触发后先看历史是否成功，再决定要不要打开最新报告。

### 案例 C：我明明开了 Telegram，怎么没消息？

→ 历史看渠道错误 → 设置里测试推送 → 修好后再等下一次触发。不要先把规则删了重建十遍。
