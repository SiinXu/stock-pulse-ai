...
| **信号摘要** | 近期 AI 建议 / 告警概览 | 点进去通常进入 **信号中心**。数据源或请求失败会显示可重试的错误，而不是破折号或伪装成成功的空 0。成功的 0 仍显示 0。重试时只有被标为部分/陈旧的上次成功计数才会保留。 |
...
「今日焦点」只根据已有本地记录生成最多 5 条（接口上限 10 条）的优先摘要，不会为了填满列表而制造推荐。

### 可执行的「今天」时间契约

“今天”按**标的所属市场本地自然日**判定，不是单一服务器时区：

| 市场 | 时区 | 日界 |
| --- | --- | --- |
| A 股 (`cn`) | `Asia/Shanghai` | 本地日历日 00:00 → 本次构建时刻 |
| 港股 (`hk`) | `Asia/Hong_Kong` | 本地日历日 00:00 → 本次构建时刻 |
| 美股 (`us`) | `America/New_York` | 本地日历日 00:00 → 本次构建时刻 |
| 无法识别的代码 | `daily_brief_timezone` 回退（默认 `Asia/Shanghai`） | 同上 |

**跨市场规则：** 每条证据只对照**该标的自身市场**的本地日窗口做新鲜度判断。同一 UTC 时刻可能对美股算“今天”、对 A 股不算（或相反）。响应中的 `temporal_policy.markets[]` 会列出本次构建用到的全部窗口。

未带时区的历史时间按 UTC 解释（`naive_timestamp_policy=assume_utc`）；缺少时间的记录不参与筛选。开盘前、休市日和周末仍使用同一本地自然日窗口，**不会**把前一交易日的旧证据顺延为今天（`non_trading_day_policy=same_local_day_only`）。交易日标志在日历可用时仅作信息展示。

### 可入选依据

- 在该标的市场本地“今天”内、状态为 `triggered` 的告警（目标化全量查询，禁止只读 `list_triggers` 第一页）；
- 同一市场本地窗口内的重大公司事件（仅在运行时接入可信事件读取器时）；
- 最新分析发生在市场本地今天、上一条在最近 90 天内，且 `buy` / `sell` / `hold` 方向发生变化的分析结论。

### 候选范围、金额有限性与降级

自选代码与**完整**持仓缓存只用于确定候选范围，港股前缀/后缀等别名会先规范化。持仓金额 / 权重 / 涨跌幅字段若存在则必须有限（`isfinite`）；非有限行会被排除，并在 `universe_contract.excluded_non_finite_positions` 与 data notes 中显式说明，**不会**用 0 伪装。持仓的历史未实现盈亏不是“当日涨跌”，不会作为入选理由。读取今日焦点不会刷新行情、运行分析、重放持仓账本或写入快照；某个本地来源失败时，接口会返回 `degraded` 和具体来源。若只剩非有限持仓数据，空态原因为 `insufficient_finite_data`。
