...
第一次打開時，首頁往往只有幾塊卡片和提示，沒有完整報告列表——它先幫你判斷**今天該看什麼**，再進入分析、信號或持倉。

> 💡 **開始前建議先完成**  
> 1. 至少配置好一個能測通的 **AI 模型**，並點過保存。  
> 2. 自選股裏至少有 **1～3 個** 你認識的代碼（例如 `600519`）。  
> 如果這兩項沒做好，首頁會一直提醒「基礎配置未完成」。配置不齊時跑分析，通常會失敗並白白消耗額度。
...
- 標題/說明類似：只讀查看今天已運行和即將運行的任務。  
- 任務類型可能包括：**股票分析**、**研究簡報**、**風險檢查**；不認識的新類型會顯示爲「新版任務」一類文案。  
- 狀態可能包括：進行中、等待重試、已跳過等。  
- 這裡**不能**新建或修改定時規則。要改每天幾點跑、是否開啓，去 **設置 → 系統與安全 → 調度**。  
- 顯示「今日暫無定時任務」時，表示今天沒有相關任務，一般不是故障。

調度語義與 API 細節見 `docs/scheduled-tasks.md`（偏實現）。
...
| 待辦 | 需要你再看一眼的研究提醒（常與信號臨期有關） |
| 有效信號 active | 還沒被關掉、作廢、歸檔的結構化建議 |
| 引導配置 | 從首頁跳進設置就緒檢查 |
| 晨報 | 首頁上最新大盤復盤的入口，不是個股買賣建議 |
| 今日定時任務 | 首頁只讀的當日調度任務列表 |

---

## 相關閱讀

- [01 導航與頂欄](01-shell.md)（導航、命令面板、通知鈴）  
- [03 分析工作台](03-analysis-workbench.md)  
- [06 信號中心](06-signals.md)  
- [10 設置](10-settings.md)  
- [客戶端安裝](../../../beginner-client-setup.md)  

上一篇：[01 導航與頂欄](01-shell.md) · 下一篇：[03 分析工作台](03-analysis-workbench.md)
