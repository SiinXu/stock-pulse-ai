# 10 设置：把「能跑起来」的小事一次做对

你好。设置页看起来选项很多，但你真正需要先做对的，通常只有三件事：
...
> 💡 这一章只讲 **界面怎么点**。申请云厂商账号、服务器端口、Docker 映射，请看 [完整指南](../full-guide.md) 与 [小白安装](../beginner-client-setup.md)。
...
以下能力在**未合并 PR / 功能分支**里已有实现或进行中；**以你当前安装的版本界面是否出现为准**。合并后手册会继续对齐。

| 能力 | 大概入口 | 说明 | 关联方向 |
| --- | --- | --- | --- |
| **本地模型包导入** | 设置 → AI 与模型 → 本地模型 | 可导入版本化模型包归档，绑定私有快照后激活使用 | PR 模型包 / Local Models |
| **首页今日定时任务** | 首页只读列表 | 展示今天已运行/即将运行的定时任务（分析、研究简报、风险检查等），不能在此改调度 | PR scheduled research tasks |
| **个人投资框架** | 设置或研究相关入口（以实现为准） | 版本化投资框架上下文，供分析引用；研究用途 | PR investment framework |
| **通知渠道插件** | 设置 → 通知 / 插件 | 除内置渠道外，可挂载通知渠道插件 | PR notification channel plugin |

> 💡 若你的界面还没有这些入口，说明当前运行版本尚未包含对应合并。升级或等待 PR 合入后再用。
