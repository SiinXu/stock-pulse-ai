...
| 高级 · 运行能力 | 只读查看当前进程的能力来源、可执行状态与 Agent 模型部署 |
| 高级 · 诊断 | 排障时给自己或给协助者看 |
...
**高级 -> 运行能力** 展示当前服务进程发现的能力、来源状态以及 Agent deployment/model 运行配置。这里用于确认“服务当前看到了什么”，不会修改注册表，也不会改变问股所用模型。登记、更新、下线、依赖解析和任务路由等控制面操作仍只面向操作员/API。
...
通知渠道测试推送 → 修 Token → 保存 → 再看信号中心「历史」与冷却。
