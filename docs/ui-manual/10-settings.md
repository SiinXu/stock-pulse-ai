...
页面级的加载、保存和跨分区校验错误统一显示在顶部 Toast；可用右上角关闭按钮暂时收起，校验摘要也可直接跳到对应字段。字段旁的具体修正提示仍会保留；错误内容发生变化时，摘要会重新出现。
...
出于安全考虑，服务端不会把已经保存的 API Key、Token、Webhook Secret 或密码明文回传给页面。重新进入设置时，这类字段显示为空，并提示“密钥已保存；输入新密钥可替换”；此时没有眼睛按钮，因为页面手里没有可显示的明文。直接保存会保留原密钥，不会把空白覆盖进去。

输入新密钥后，眼睛按钮会出现，只用于显示或隐藏当前尚未保存的草稿。多密钥字段同样从一行空白替换框开始，不会把 `******` 当成一把真实 Key。需要更换时输入完整的新值并保存；保存成功后页面会再次回到不可回显的保存态。
...
自定义 OpenAI 兼容服务通过“获取模型”发现模型后，会自动按原样使用 `/models` 返回的模型 ID，包括 `openai/...`、组织名或多级路径；不要为了通过检测手工重复添加供应商前缀。旧配置和手动填写默认保持 LiteLLM 路由语义；仅当远端把前缀视为模型 ID 的一部分时，才设置 `LLM_<CONNECTION>_MODEL_ID_MODE=literal`。
...
开启后，需要 **长运行** 的 Web/API/Desktop 进程才能按点执行。状态卡会明确显示本 API 进程的模式、是否挂载以及服务端调度时区；不会用浏览器时区猜测。只有本进程已挂载、遗留批处理已启用且没有分析运行时，「立即执行一次」才可用。被接受的运行会持续跟踪到成功或失败；旧版服务端无法提供关联结果时会显示“结果无法确认”，不会把空闲误报为成功。版本化定义下方的「运行历史」按需读取多次运行，展示尝试次数、执行/结果引用、错误和通知失败；「加载更多」扩大真实查询上限。
