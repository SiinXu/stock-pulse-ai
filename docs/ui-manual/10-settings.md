...
| Agent 行为 · 投资框架 | 维护研究时只读注入的个人框架；编辑器直接显示在该横向标签页，历史版本从页面内抽屉查看 |
...
| **个人投资框架** | **已合入最小 Web 编辑器** | 设置 → **Agent 行为 → 投资框架**；编辑器直接显示在独立横向标签页，历史版本使用页面内只读抽屉 | 支持创建、版本化保存、历史查看、停用和删除；字段边界见 `docs/personal-investment-framework.md` |
