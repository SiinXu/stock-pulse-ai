...
预设会启用 Agent 并取消「暂不使用 Agent」确认，但不会改凭据、技能列表、全局深度研究预算、Risk Agent 否决或 HITL 审批策略。设置面使用三层渐进披露：**基础配置**（默认展开）、**行为**、**治理 / 专家**（后两层默认折叠）。分层只改呈现与默认折叠，不改变任何配置的实际语义或默认值；所有注册表字段仍可访问。专家级 JSON（例如 **告警 → 事件** 下的告警规则）放在专家层，不会出现在默认表面。预设可用后可直接点 **开始提问** 进入对话，无需再打开行为或治理深层设置。
