...
- **回滚到上一次良好配置**：携带页面当前配置版本，经危险操作确认后原子替换并重载。若服务器版本已变化，页面不会自动重试；先显式载入最新配置。服务器实际恢复的字段会同步为回滚值，其他未保存草稿和登录状态不会被回滚动作清除。
桌面端升级或重装前，先导出一次配置备份。
...
| **个人投资框架** | **已合入结构化 Web 编辑器与历史** | 设置 → **Agent 行为 → 投资框架**；结构化编辑器直接显示在独立横向标签页，历史版本使用页面内只读抽屉 | 支持决策树、评估维度、不可变历史只读查看/复制到草稿、版本化保存、停用和删除；字段边界见 `docs/personal-investment-framework.md` |
