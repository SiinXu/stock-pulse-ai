...
这些区块只消费结构化字段，不从中文、英文或韩文叙事中猜测结论。旧报告、单策略报告或缺少对应字段的部分报告可以不显示某些区块；原始 JSON、Markdown 和证据分层仍保留用于追溯。字段损坏时，界面会忽略损坏区块而不是编造占位结论。支持策略、委员会立场和冲突列表使用各自独立的可折叠区块，并在展开后各自独立使用「展开全部 / 收起」，互不影响。
