...
> **服务说明（与上一历史版本比较）**
> 后端历史比较服务可对两个已保存版本执行确定性的字段级差异计算（`compare_analyses` / `get_latest_delta`）。版本使用唯一的历史记录主键；可重复的查询 ID 仅作为关联元数据。最新版本比较限定在明确的报告类型内，且没有隐藏的历史天数截断。详见 [分析差异比较服务契约（英文）](../analysis-delta-comparison.md)。**生成的个股报告与 Web/历史 Markdown 顶部**会渲染“较上次分析的变化”摘要（首次分析 / 无实质变化 / 字段级变化明确区分）；Web 端 Home/History 的独立导航发现入口仍属后续 UI 任务。只有一条匹配历史记录时，服务返回“无基线/首次分析”，而不是“无变化”。
...
| 策略综合 | 综合信号、加权分、置信度、共识、冲突等级、支持与反方策略、冲突参与者及未采纳意见数。支持 / 反方策略与冲突列表默认收在可折叠区块内，展开后默认只显示前 3 条，可用「展开全部 / 收起」查看其余项；阶段决策与信号归因始终完整展开 |

这些区块只消费结构化字段，不从中文、英文或韩文叙事中猜测结论。旧报告、单策略报告或缺少对应字段的部分报告可以不显示某些区块；原始 JSON、Markdown 和证据分层仍保留用于追溯。字段损坏时，界面会忽略损坏区块而不是编造占位结论。支持策略、委员会立场和冲突列表使用各自独立的可折叠区块，并在展开后各自独立使用「展开全部 / 收起」，互不影响。输入数据块、运行诊断和报告过程时间线同样使用共享 Collapsible，默认保持折叠。
...
| 你在界面上 | 怎么理解 |
| --- | --- |
| 默认首屏 | 决策卡（结论）加上分层里的**风险**与**免责声明**。事实、缺口、推断、框架对齐默认收起，需点开证据详情 |
| 看到分层区块 | 按块阅读；**不要**把「模型推断」当成「已核实事实」 |
| 切换报告 | 证据详情会重新收起；不会把上一份报告的展开状态带到下一份 |
