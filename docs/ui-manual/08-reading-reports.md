...
| 策略综合 | 综合信号、加权分、置信度、共识、冲突等级、支持与反方策略、冲突参与者及未采纳意见数。支持 / 反方策略与冲突列表默认只展开前 3 条，可用「展开全部 / 收起」查看其余项；阶段决策与信号归因始终完整展开 |

这些区块只消费结构化字段，不从中文、英文或韩文叙事中猜测结论。旧报告、单策略报告或缺少对应字段的部分报告可以不显示某些区块；原始 JSON、Markdown 和证据分层仍保留用于追溯。字段损坏时，界面会忽略损坏区块而不是编造占位结论。支持策略、委员会立场和冲突列表各自独立折叠，互不影响。
