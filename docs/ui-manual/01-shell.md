# 01 壳层与全局操作

打开 StockPulse 之后，中间的内容会换，但**外面那一圈**通常还在：左侧（或顶部）导航、通知铃、搜索/命令入口、语言和主题。

这一圈就叫 **壳层**。先把它认熟，后面换到分析、持仓、设置时，你就不会「找不到回去的路」。

不涉及配置文件和部署，只讲界面导航和全局入口。

> ⚠️ 产品输出仅供研究学习，**不构成投资建议**。

---

## 你什么时候会用到壳层

| 场景 | 用壳层的哪一块 |
| --- | --- |
| 第一次打开，不知道功能在哪 | 主导航 |
| 想从任意页立刻去分析 | 命令面板 `Cmd/Ctrl + K` |
| 想知道有没有新提醒 | 通知铃 |
| 菜单想英文、报告想中文 | 界面语言（壳层）vs 报告语言（设置） |
| 晚上看屏幕刺眼 | 主题（浅色 / 深色） |
| 开启了登录保护 | `/login` 登录页 |

---

## 整体长什么样

宽屏时，常见是左侧导航 + 中间主内容 + 顶栏工具；窄屏时，导航可能收成汉堡菜单或顶栏抽屉——**功能还在，只是位置挤了**。

```mermaid
flowchart TB
  subgraph shell [壳层]
    N[主导航]
    B[通知铃]
    K[命令面板]
    L[语言与主题]
  end
  subgraph main [主内容区]
    P[当前页面]
  end
  N --> P
  K --> P
  B --> S[信号中心等深链]
```

| 区域 | 常见位置 | 含义 |
| --- | --- | --- |
| 主导航 | 左侧 / 顶栏 | 目录：首页、研究、Agent、信号中心、组合、设置 |
| 主内容区 | 中间 | 你正在用的功能页 |
| 通知铃 | 桌面侧栏底部、用户入口旁 | 有没有新信号/告警 |
| 命令面板 | 快捷键或搜索框 | 万能跳转 |

桌面客户端和 Web 的信息架构一致；个别桌面浮动窗以实际客户端为准。

---

## 主导航：六个一级域

当前产品把核心工作区收成六个一级域（文案以你屏幕为准）：

| 导航名 | 路由 | 里面有什么 | 手册 |
| --- | --- | --- | --- |
| **首页** | `/` | 今日焦点、待办、配置缺口 | [02](02-home.md) |
| **研究** | `/research` | 概览与下表中的研究工具 | [03](03-analysis-workbench.md) / [04](04-market-review.md) / [09](09-backtest.md) |
| **Agent** | `/chat` | 问股多轮对话（页面标题常写「问股」） | [05](05-agent-chat.md) |
| **信号中心** | `/signals` | AI 信号、规则、触发与通知历史、复盘 | [06](06-signals.md) |
| **组合** | `/portfolio` | 持仓记账（页面标题常写「持仓」） | [07](07-portfolio.md) |
| **设置** | `/settings` | 模型、数据源、通知、安全 | [10](10-settings.md) |

在桌面展开侧栏和移动抽屉中，点击 **研究** 主标签会直接打开
`/research`；行尾箭头只负责展开或收起研究工具，不再重复显示一条
「研究概览」。在紧凑侧栏中，**研究** 仍会打开弹出菜单，第一项是
**研究概览**。

### 「研究」页面

| 页面 | 路由 | 手册 |
| --- | --- | --- |
| 研究概览 | `/research` | 四个研究工具的入口卡片 |
| 大盘复盘 | `/research/market` | [04](04-market-review.md) |
| 发现 | `/research/discover` | 暂无分册；侧栏 **研究 → 发现**（旧 `/screening` 会重定向） |
| 分析工作台 | `/research/analysis` | [03](03-analysis-workbench.md) |
| 回测 | `/research/backtest` | [09](09-backtest.md) |

### 侧栏里没有、但很重要的入口

| 去哪 | 怎么进 |
| --- | --- |
| **人工审批** `/approvals` | **不在一级侧栏**；首页 **查看人工审批** / **Review human approvals** 按钮进入（需已开启管理员登录；文案以界面为准），或在命令面板搜索；工程说明见 `docs/human-approvals.md` |
| **通知中心** `/notifications` | 通知铃 → 查看全部，或在命令面板搜索 |
| **事件告警** `/event-alerts` | 事件日历 / 信号中心入口，或在命令面板搜索 |
| **报告对比** `/research/report-compare` | 个股工作区对比入口，或在命令面板搜索 |
| **个股工作区** | 命令面板输入代码，或打开 `/stocks/<code>`（例如 `/stocks/600519`）；页面会显示字段级来源、时滞、过期与跨源冲突，冲突不会被悄悄选定为一个真值 |
| **登录** `/login` | 开启管理员认证后；受保护页会带 `?redirect=` |

旧路径如 `/decision-signals`、`/alerts`、`/backtest`、`/screening` 一般会重定向到新地址。

---

## 命令面板

| 系统 | 快捷键 |
| --- | --- |
| macOS | `Cmd + K` |
| Windows / Linux | `Ctrl + K` |

| 你想做的事 | 可以试着输入 |
| --- | --- |
| 跳转页面 | `分析`、`持仓`、`信号`、`设置`、`首页`、`通知中心`、`事件驱动告警`、`报告版本对比` |
| 策略 / 管线 | `缠论`、`多头`、`管线`、`标准研究` |
| 设置分节 | `模型来源`、`数据源`、`认证` |
| 找股票 | `600519`、`AAPL` |
| 找历史报告 | 股票名称、代码、报告类型、日期或报告结论中的关键词（至少 3 个字符） |
| 大盘相关 | `复盘`、`大盘` |

匹配结果会按股票、报告、策略、管线、设置、页面和操作分组显示。策略可进入选股/发现并带上策略参数；管线预设与设置分节会跳到对应设置深链。输入时可继续使用 `↑` / `↓` 选择结果并按回车打开；历史报告会直接进入分析工作台中的对应记录。报告搜索暂时不可用时，股票、页面和操作搜索仍可继续使用。个人表现仍是模拟盘账户上下文入口，不会作为命令面板页面项出现。

### 小练习：三步回到分析

1. 你正在设置页改模型。  
2. 按 `Cmd/Ctrl + K`，输入「分析」。  
3. 回车进入分析工作台——不必在侧栏里一层层点。

---

## 通知铃

铃铛汇总较新的信号与告警提醒。

- 点某条：常会深链到信号详情或历史。  
- **查看全部**：进入信号中心。  
- 一直是空的：若你还没跑过分析、也没建规则，**完全正常**。

---

## 界面语言、报告语言、主题

| 开关 | 改什么 | 不改什么 |
| --- | --- | --- |
| **界面语言** | 菜单、按钮、空状态 | 报告正文语言 |
| **报告语言** | 分析报告正文（多在设置 → 报告） | 菜单语言 |
| **主题** | 浅色 / 深色 | 业务逻辑 |

所以可以：**菜单英文 + 报告中文**。两者互不影响。

个人菜单中的主题与界面语言使用同样的紧凑下拉选项；当前选择采用中性高亮。宽屏时选项从设置项右侧展开，窄屏时会自动调整方向并限制高度，避免超出可视区域。

产品界面可能支持很多语言；本手册以简中 + 英文维护。

## 全局错误提示

接口或外部服务失败时，错误会显示在页面顶部居中的紧凑 Toast 浮层中，不再插入页面内容流，因此不会挤压表格、报告或表单。错误 Toast 会保留到你主动关闭；如果该操作支持重试，重试按钮会显示在通知右侧。

---

## 登录（若开启了认证）

1. 访问受保护页面时跳到 `/login`。  
2. 输入管理员密码登录。  
3. 成功后回到原来的 `redirect` 目标或首页。  
4. 改密码：设置 → 系统与安全 → 认证与安全。

未开启认证时，本地可能直接进业务页，以你的部署为准。

---

## 使用案例

**案例 A：迷路了**  
任意页 → `Cmd/Ctrl + K` → 输入「首页」→ 重新开始。

**案例 B：想看有没有推送**  
点铃铛 → 没有就去跑分析或建规则 → 有就点进去读。

**案例 C：菜单想换成英文**  
在壳层或设置切换界面语言为 English；报告语言保持中文即可。

---

## 术语

| 术语 | 含义 |
| --- | --- |
| 壳层 | 几乎每页都在的外框导航与工具 |
| 信息架构 IA | 菜单怎么分组、功能怎么摆 |
| 命令面板 | 快捷搜索跳转 |
| 深链 | 带参数的链接，直接打开某条信号/某段设置 |

---

## 相关

- [02 首页](02-home.md)  
- [06 信号中心](06-signals.md)  
- [10 设置](10-settings.md)  

上一篇：返回 [手册目录](README.md) · 下一篇：[02 首页](02-home.md)
