...
Changing account or cost method immediately clears visible Health, Stress, and Rebalance results, and late responses cannot overwrite the new context. Assumptions, limitations, data quality, and provenance remain available in expandable evidence sections. Status, health band, insight, reason, rebalance action, basket degradation, and built-in scenario labels come from client-owned catalogs keyed by backend codes. Home health and Risk Metrics reuse the same formatters, including health-band keys already owned by the Home widget. The stress-test position table, evidence section, and workspace snapshot banner share one limitation formatter: known codes such as `realtime_quote_best_effort` become user-facing labels, while unknown codes stay visible as localized diagnostics. The stress quality cell joins status, stale, and limitation labels (for example `Partial · Stale · Realtime quotes are best-effort`) instead of raw `ok` / `partial` machine codes. Partial, unavailable, insufficient-data, and backend-refusal states stay explicit. An unavailable health result uses the known reason such as `negative_equity`, not the health disclaimer. Authoritative `statusMessage` is localized or labeled as a diagnostic. Custom YAML scenario names stay as sanitized user content. Unknown codes stay visible as localized diagnostics rather than raw snake_case or a rewritten known state. Server English sentences are not primary copy; free-form detail is labeled as a diagnostic when it is shown.

Paper **process quality** (`/portfolio/performance`) scores paper trades by reason codes such as analysis support and risk-gate compliance. The page localizes those codes and the trade side; it does not render the server disclaimer or reason `message` as primary copy.
