...
| Page prefixes | `--home-*`, `--settings-*`, `--chat-*`, `--backtest-*`, `--portfolio-*` | Frozen. Do not add names. Do not promote them to Layer 1 to green CI. `--login-*`, `--backtest-*`, `--portfolio-*`, `--chat-*`, and `--settings-*` are collapsed to zero; do not reintroduce those prefixes. |
