...
`i18n:high-risk` extracts trading-action, risk, authentication, credential, error, and disclaimer copy from the real source registries and ten locale bundles. It checks the audited TypeScript internal-code/stable-display-key inventories, locks per-category/per-locale snapshots, and protects both the stable-key decision list and locale-revision layers with counts and SHA-256 digests. `--verify-baseline` additionally parses the recorded main-branch bundles with the TypeScript AST, verifies every recorded `before` value against the current branch merge-base, and requires the actual revision set to exactly equal the decisions. Component-owned raw-code fallbacks listed under "Deferred to UIUX" in the audit remain outside this guarantee. Passing snapshots and automation is not native financial sign-off.
