...
`i18n:high-risk` extracts trading-action, risk, authentication, credential, error, and disclaimer copy from the real source registries and ten locale bundles. It checks TypeScript internal-code/stable-display-key boundaries and locks per-category, per-locale snapshots. It also rejects missing sources, fabricated review status, and high-risk copy drift without an audit update. Passing snapshots and automation is not native financial sign-off.
