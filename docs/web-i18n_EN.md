...
`i18n:high-risk` extracts trading-action, risk, authentication, credential, error, and disclaimer copy from the real source registries and ten locale bundles. It checks the audited TypeScript internal-code/stable-display-key inventories, locks per-category/per-locale snapshots, and protects the decision list with a count and SHA-256 digest. `--verify-baseline` additionally parses the recorded main-branch bundles with the TypeScript AST and verifies every recorded `before` value against the current branch merge-base. Component-owned raw-code fallbacks listed under "Deferred to UIUX" in the audit remain outside this guarantee. Passing snapshots and automation is not native financial sign-off.
