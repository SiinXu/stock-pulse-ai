...
Personal Performance, Portfolio Insights, and Alerts follow the same code-first presentation contract. Known backend enum, status, reason, quality, and limitation codes map through the existing `dataQualityFormat` helpers and locale catalogs at render time. Known English prose that has a stable machine identity (`reason.code`, insight `code`, scenario `id`, rebalance `action`) is replaced by client-owned localized copy. Free-form server detail is sanitized, labeled as a diagnostic, and shown only where it helps investigation. Unknown codes stay visible as localized diagnostics with a sanitized `{code}`; they are never injected as HTML, never matched by sentence, and never dropped silently. Empty or missing values stay honest (`—` / `--`). Language switching updates already-loaded rows immediately.
