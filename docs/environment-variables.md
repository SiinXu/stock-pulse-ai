...
| `ALPHASIFT_SOURCE_CALL_TIMEOUT_SEC` | `空` | 是 | 模板中注释; (If you want to maintain consistency with runtime automatic injection, it is recommended to leave it blank and not se... |
...
| `SNAPSHOT_SOURCE_PRIORITY` | `tushare,sina,efinance,akshare_em,em_datacenter` | 是 | 模板中注释; Compatibility note: - ALPHASIFT_INSTALL_SPEC is used to explicitly call /api/v1/alphasift/install; will not automatic... |
...
| `YFINANCE_PRIORITY` | `0` | 是 | 模板中注释; Example: prioritize Yahoo Finance in eligible non-US general daily routes |
