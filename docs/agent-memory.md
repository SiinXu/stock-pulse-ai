...
- Memory provenance, fact/opinion isolation, and anti-poisoning baseline: [#1124](https://github.com/SiinXu/stock-pulse-ai/issues/1124). DAG-0 threat notes, DAG-1 fact/opinion lock, DAG-2 Soul/oversize write reject (`src/schemas/memory_write_guard.py`), DAG-3 server-stamped provenance (`src/schemas/memory_provenance.py`), and DAG-4 default-off AgentMemory inject isolation (`src/agent/agents/base_agent.py` / `src/agent/memory.py`) have landed. Do not fold in #1118 store/UX, #1119 forgetting, or #1105 product feedback APIs. Keep the issue open until maintainers confirm every acceptance item.
