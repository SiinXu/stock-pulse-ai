...
| `src/schemas/memory_consolidate_policy.py` | Library-only per-symbol compression of old episodic rows into one `mode=consolidate` summary (#1119 Slice 3) |
...
When enabled and samples meet the adapter threshold, `BaseAgent` routes displayed/decided confidence through `calibrate_confidence`, which applies the stored `CalibrationResult.calibration_factor` from `AgentMemory.get_calibration` (and only when `calibrated` is true). The gated call uses the existing adapter signature (`agent_name`, `stock_code`) and does not pass `skill_id`; the ungated path still passes `extract_skill_id(self.agent_name)`. AgentMemory already clamps `historical_accuracy / avg_confidence` to `0.5..1.5`, including a real `historical_accuracy=0.0`; the adapter must not re-derive that ratio with truthy fallbacks such as `accuracy or 0.5`. Confidence is then clamped to `[0,1]`. Sample source is existing `AGENT_MEMORY_ENABLED` / `AgentMemory`; this slice does not add a second store. Tool-effectiveness remains an identity stub and does not unlock denied ToolSurface tools. Route preference is threshold-gated: with adapters on, `N >= AGENT_ONLINE_ADAPTERS_MIN_SAMPLES`, and miss-rate `>= 0.5` from scored forecast outcomes (`1 - OUTCOME_NUMERIC_SCORE` mean), `prefer_route` steps one richer rung (`quick→standard`, `standard→full`) and appends one `adapter.route_preference` EvolutionEvent. Identity paths and append failures do not change the returned mode. `prefer_route` does **not** write `AGENT_ORCHESTRATOR_MODE` and AgentRouter does **not** call it. Influence is recorded only on run-local `AgentContext.meta["adapter_influence"]` (not episodes). This slice does **not** implement real `rank_tools` scoring (#1123), auto-promote, HTTP EvolutionEvent list (#1113), episode schema persistence, or promotion.
...
## Episode consolidation policy (#1119 Slice 3)

Library-only per-symbol compression in `src/schemas/memory_consolidate_policy.py`, applied by `AgentEpisodeRepository.apply_consolidate` on the existing `agent_episodes` table. The source set is exactly the per-symbol rows Slice 2 would delete (strict `created_at < cutoff`, then oldest `id` capacity overflow) excluding `mode=consolidate` rows. Fewer than two source rows is a no-op so existing forgetting still drops leftovers. UPDATE remains aborted by `trg_agent_episodes_immutable`. No new table, env key, public API, Web, or Desktop CRUD.

| Rule | Contract |
| --- | --- |
| Scope | An explicit non-empty `symbol`. Missing / blank symbol is no-policy or fail-closed unscoped — never a global rewrite |
| Source set | Slice 2 forget delete set minus existing `mode=consolidate` summaries |
| Minimum batch | Fewer than two source rows writes nothing |
| Summary | Exactly one size-capped `AgentEpisodeCreate` with `mode=consolidate`, counts and compact lessons only. No Soul text, no user-note facts, no raw trajectory dump. `require_episodic_write` must admit it or the pass aborts |
| No-policy | Neither cutoff nor `max_rows` writes nothing; `remaining_count` is a live COUNT for that symbol (or the whole table if no symbol) |
| Clock | Cutoff from an injected datetime (`retention_days` requires `now`). Reuses existing `AGENT_EPISODE_RETENTION_DAYS` / `AGENT_EPISODE_MAX_ROWS` |
| Audit | Irreversible source DELETE inserts a metadata-only `episode.consolidate` EvolutionEvent (`symbol`, counts, cutoff/max_rows, SHA-256 of source row ids, new summary `episode_id`) in the **same** `session_scope` after the admitted summary INSERT and before DELETE. Summary or audit failure rolls back every write. Dry-run and no-policy write no event and store no episode body, lessons, or trajectory. |
| Transactions | Consolidate itself is one SQLite writer transaction: select source ids → admitted summary INSERT → one EvolutionEvent insert → chunked `DELETE ... id IN (...)` of those source ids only → commit. Chunks are not separate commits. `append` still commits first; insert+consolidate+forget is **not** one transaction (#1090 fail-soft). SQLite serializes writers; this pass does not use `SELECT FOR UPDATE`. |
| Recovery | Code revert cannot restore deleted source episode rows. Recovery requires backup / point-in-time restore. `AGENT_EPISODE_LOG_ENABLED=false` only stops new writes and append-triggered consolidate/forget. |
| Not in scope | Predictions, decision-memory ranking or score decay, sidecar feedback/labels, Soul edits, semantic/procedural persistence, public API / Web / Desktop |

After a successful episode append, `AgentEpisodeService` runs this pass for the stored symbol only, **before** existing forgetting. Analysis stays fail-soft: consolidate failure is logged, does not write, does not turn a successful append into `None`, and does not skip the later forget pass. Explicit `consolidate_symbol(...)` is not fail-soft. Rows of other symbols, and existing summaries, are not sources for that pass.

This slice does **not** add Decision Memory retrieval-score decay, the #1118 store, auto-promotion, or new product feedback APIs. [#1119](https://github.com/SiinXu/stock-pulse-ai/issues/1119) stays open.
...
- Write admission / consolidation / forgetting: [#1119](https://github.com/SiinXu/stock-pulse-ai/issues/1119). Slice 1 (library write admission), Slice 2 (deterministic per-symbol episode forgetting), and Slice 3 (compact per-symbol episode consolidation) are documented above. Remaining: semantic/procedural candidate promotion without Soul edits, Decision Memory retrieval-score decay, and drop of rolled-back procedural flags after [#1113](https://github.com/SiinXu/stock-pulse-ai/issues/1113). Keep #1119 open.
