...
**Version history** opens a read-only drawer on the same page. Both its newest-first list and inspector display each version's `created_at`; history is never restored or edited in place. “Copy into current draft” copies content only, and the following save still uses the current aggregate revision to create a new version. On an HTTP 409 conflict, the UI reloads server state. If that refresh fails, it hides the stale editor and shows a retryable page error so outdated content cannot be edited further. Backend 422 validation remains authoritative; public, sanitized `details.issues[].loc` entries are placed on the corresponding node, dimension, or line-based rule while a global error remains visible. Known-field edits use object merges, and the transport preserves unknown future fields so rolling upgrades do not silently drop server-owned content.
