...
- 当前股票上下文会显示已应用的代码、名称和可推导市场，并提供清空入口。`stock` URL 参数在直接打开、同页跳转和浏览器 Back/Forward 中都是权威来源；URL 恢复哪个标的，latest 与时间线就重新加载哪个标的。清空会让 latest 与时间线回到引导态，不影响高级列表筛选或列表来源详情抽屉。
...
- 告警通知只引用顶层 canonical `action`、`presentation` 的 `label/confidence/summary/risk/timestamp`，以及 `horizon`、`watch_conditions`、`source_report_id`；分析聚合报告保持字段隔离，不重复渲染 DecisionSignal excerpt。
...
- `decision_signal_feedback` 保存每个信号最新的 `useful|not_useful` 反馈、可选原因/备注和来源。反馈是意见 sidecar：`PUT /api/v1/decision-signals/{signal_id}/feedback` 不得改写 `decision_signal_outcomes` 或 `agent_predictions.outcome_json`。混入行情 actuals / PredictionOutcome 字段的载荷会被拒绝（#1124 DAG-1，`src/schemas/memory_fact_opinion.py`）。含 Soul 边界标记、超过 `note` 1000 / `reason_code` 64，或夹带非法 C0 控制字符的自由文本会被拒绝，不会截断后写入（#1124 DAG-2，`src/schemas/memory_write_guard.py`）。传输通道 `source`（`web`/`api`）不是 provenance；服务端盖章 `provenance_source=user_feedback` 与可选 `actor_id=local_admin`，客户端 provenance 键会被拒绝（#1124 DAG-3，`src/schemas/memory_provenance.py`）。GET/PUT 响应可返回这两个附加字段；文档化的 DAG-1 请求体仍然有效。
