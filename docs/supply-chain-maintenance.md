...
Python 3.10 as the minimum and a fixed global `exclude-newer` cutoff. A reviewed
`resolver.exclude_newer_packages` mapping can advance one transitive package to
an independently reviewed upload timestamp without moving the rest of the
dependency graph; the lock header records both cutoffs. It retains marker forks needed by the currently released
...
3. Advance `resolver.exclude_newer` in
   `scripts/dependency_lock_policy.json` to the review timestamp for a reviewed
   dependency cohort. For a single urgent transitive security release, add its
   canonical package name and fixed upload timestamp to
   `resolver.exclude_newer_packages` instead, so unrelated packages remain on
   the global cutoff. Do not use a future or floating cutoff.
