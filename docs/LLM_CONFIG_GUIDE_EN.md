...
- **CLI capability boundary**: local CLI backends cover report generation only; Q&A Agent needs a tool-capable API model.
...
- The Phase 6a DSA Tool Surface is now hardened by #191 as a deny-by-default execution boundary. In addition to shared schemas, structured errors, audit summaries, and redacted diagnostics, it enforces supported capabilities, execution-owned grants, stock scope, recursively nested outbound URL policy, and bounded argument inspection; model output cannot add a capability. Stock-scoped tool calls must pass an explicit `ToolAccessContext.stock_scope`, and tools with a `stock_code` parameter but no declared stock scope fail closed. It does not add external runtime adapters, an MCP server, REST APIs, Web UI, `.env` settings, or generation/Agent backend route changes. Codex / Claude / OpenCode / Hermes must not bypass this Tool Surface with ad-hoc provider-specific schemas before a wire-level tool call / tool result roundtrip probe proves support; `codex_cli` / `claude_code_cli` / `opencode_cli` remain generation-only with `supports_tools=false`.
