...
- The current dependency constraint for this flow in the repository is `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0` (see `requirements.txt`). Regression coverage for it lives in `tests/services/system_config/test_system_config_service.py`, `tests/api/system_config/test_system_config_api.py`, and `apps/dsa-web/src/components/settings/__tests__/LLMChannelEditor.test.tsx`.
...
- Regression evidence for this path is in `tests/llm/test_llm_param_recovery.py`, `tests/services/system_config/test_system_config_service.py`, `tests/config/test_llm_channel_config.py`, `tests/api/system_config/test_system_config_api.py`, `tests/market/test_market_analyzer_generate_text.py`, `tests/test_agent_pipeline.py`; desktop backup import restore is directly covered by `test_import_desktop_env_restores_runtime_models_after_cleanup`.
