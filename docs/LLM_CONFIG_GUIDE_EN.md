...
- `AGENT_GENERATION_BACKEND=auto` inherits `GENERATION_BACKEND`. Web settings can also select `litellm|codex_cli|claude_code_cli|opencode_cli` explicitly. With a local CLI selected, the Q&A Agent uses the strict JSON tool bridge and no separate API model is required.
- When editing an existing model connection, Test Connection and Discover Models reuse its saved masked credential only while the Provider, protocol, and Base URL remain unchanged. The saved API key is never returned to the browser: the field is empty and accepts only a replacement, so no reveal action is shown for masked content. Changing the connection identity or endpoint requires re-entering the API key so credentials are never forwarded to a different service.
- When Agent uses a local CLI backend, a historical `AGENT_LITELLM_MODEL` value is excluded from active Agent model-reference and availability validation. `LITELLM_MODEL` remains enforced while report generation still uses LiteLLM.
- **CLI Agent capability boundary**: the local CLI process still has its built-in tools, MCP, and write access disabled. It can only return strict JSON application-tool requests. Tool names, arguments, permissions, stock scope, timeouts, and audit remain enforced by StockPulse's `BoundToolSession`; tool results return in the next CLI turn as untrusted data. This bridge is experimental, starts a restricted process for every Agent turn, is usually slower than native API tool calling, and does not provide token streaming.
- **`AGENT_FEATURES_ACKNOWLEDGED_OFF`** (default `false`): settles `llm_agent` readiness for CLI-only users (`optional`); superseded when an Agent API model is configured. Does not make CLI Agent-capable.
- The Phase 6a DSA Tool Surface remains the deny-by-default execution boundary. Shared schemas, structured errors, audit summaries, redacted diagnostics, execution-owned grants, stock scope, nested outbound URL policy, and bounded argument inspection apply equally to the local CLI bridge; model output cannot add a capability. Stock-scoped calls still require the existing scope contract. The bridge translates strict JSON into the same native Agent tool loop and never grants a CLI process direct access to application tools.
...
> **[Anspire Open](https://open.anspire.cn/)** supports Chinese-optimized search and OpenAI-compatible model access using a shared key.
...
# Get your key at: https://open.anspire.cn/
...
>
> **Outbound security**: Admin-configured loopback Ollama (`LLM_OLLAMA_BASE_URL` on `127.0.0.0/8`, `::1`, or `localhost`) is allowed by default without `OUTBOUND_HTTP_ALLOWLIST`. Non-loopback targets still need an exact allowlist entry. See [Outbound HTTP Security Policy](security-outbound-policy.md).
>
> If Clash/Mihomo TUN Fake-IP DNS resolves public model hosts into `198.18.0.0/15` or `fdfe:dcba:9876::/48`, set `OUTBOUND_HTTP_ALLOW_PROXY_FAKE_IP=true` on that trusted workstation. The opt-in accepts only the operating system's configured loopback proxy and hostnames with a public suffix; private names, literal Fake-IP URLs, and metadata targets remain blocked.

#### Settings Local Model Center (Recommended And Sole Download Entry)

Use **Settings -> AI & Models -> Local Models** for routine downloads and management. General and finance cards come from the repository's authoritative catalog and show size, memory tier, capabilities, license, installation state, and download progress. A recommendation is highlighted only when host memory can be detected. Web deployments proxy Ollama through the backend using the server-owned `LLM_OLLAMA_BASE_URL`; clients cannot supply a target URL. Desktop reuses the same panel and controls Ollama detection, start/stop, downloads, and deletion through the main-window preload. macOS and Windows receive the matching official Ollama installer only when the browser and native backend installer platforms match and the Web page, API request entry, and server-configured Ollama endpoint are all confirmed as loopback; returning to the page after installation and startup automatically re-detects the runtime. Linux, mobile, unknown-platform, detected remote API/Ollama, and unconfirmed proxy or container deployments continue to show official installation guidance, and remote deployments require Ollama on the machine configured for the StockPulse backend. HTTP cannot distinguish true same-machine loopback from a loopback-preserving tunnel such as `ssh -L`; when using such a tunnel, the installer downloads to the browser machine, so operators must still follow the official instructions on the backend-connected machine. The matching `ollama pull <tag>` command is shown only as recovery after a model download fails, without exposing low-level errors.

After a successful pull, the backend uses `SystemConfigService` to append the model to `LLM_OLLAMA_MODELS`, ensure `LLM_CHANNELS` contains `ollama`, and hot-reload configuration. It selects `ollama/<id>` automatically only when no primary model exists. An existing primary remains unchanged and the panel offers an explicit switch; Agent assignment is a separate action and never implicitly replaces the primary. Desktop binds long pull and delete operations to the configuration version and a SHA-256 identity of the normalized runtime observed at their start. Requests never carry a target URL: the backend recomputes the identity and target from server-owned configuration and rejects completion if either snapshot value changed. Desktop pull streams also have an absolute 30-minute deadline. The first-run wizard's Local Model branch embeds this same panel and requires the user to select one ready model explicitly before continuing. A newly downloaded model remains selected instead of being replaced by an earlier catalog entry. Setup then routes directly to the Analysis Workbench with no `.env` editing or terminal step.

Fallback order remains owned by the existing multi-channel configuration and the model fallback editor under **Settings -> AI & Models -> Reliability**; the Local Model center does not add a second fallback setting. Finance entries marked guided-only show conversion and license guidance without a download action. Weights referenced by the primary, Agent, Vision, fallback, or effective implicit-primary channel must be reassigned before deletion. Before deleting either a registered model or an unregistered model found only in Ollama, Desktop receives a short-lived, single-use recovery token. The pending token reserves that model against concurrent local-model downloads or registration. While it is pending, the backend also rejects every `SystemConfigService` configuration write, including task-model assignment, so no new reference can race between unregistration and weight deletion. Finalization, recovery, or the short TTL releases this configuration-mutation lease. Recovery is bound to the model, exact post-reservation configuration version, and original runtime. A removed registration is written back only when that original runtime still reports the weights; a stopped or unavailable runtime and missing weights all reject recovery. Recovery for an unregistered model only releases the reservation and never creates a registration. If Desktop conservatively sees the weights but the backend's following original-runtime probe reports them missing, the panel converges on successful deletion and keeps the registration removed. Successful weight deletion retries idempotent capability revocation once. If a network or status response leaves final revocation unconfirmed, deletion remains successful and returns the non-destructive `local_model_delete_finalize_unconfirmed` warning. The token cannot restore missing weights and expires after its short TTL. When the backend never received finalization, configuration writes may continue to return a version conflict for the remaining TTL and can be retried after refresh.
...
> **New editor behavior**: Model access is organized as Provider / Connection / Model / Task Assignment. The dynamic `connection_fields` schema controls whether fields are required, visible, and enabled. The Provider Catalog is authoritative only for Provider identity, localized labels, default Base URL, protocol, discovery capability, local/custom attributes, and related links; its `supports_discovery` value controls whether discovery is offered. OpenAI-compatible providers use their model-list endpoint, Ollama uses `/api/tags`, and unsupported or failed discovery falls back to manual model-ID entry; discovery never auto-selects results. Models remain independent tokens stored in `LLM_{CONNECTION}_MODELS`, while Provider identity is stored separately in `LLM_{CONNECTION}_PROVIDER=<provider_id>` and is never guessed from a renameable Connection name. Task Routing also exposes the report-generation method explicitly. Codex / Claude Code / OpenCode CLI are report-generation backends rather than model connections, so they do not appear in model dropdowns. Report / Agent / Vision / fallback assignments only accept the available-model catalog. Historical unavailable values remain visibly marked until the user replaces or removes them; saving does not silently clean them.

The available-model catalog uses a connection-aware `ModelRef` as the task-model identity; uniqueness is `connection_id + runtime_route`. If two OpenAI Connections expose the same model name, they remain two selectable entries. Task Routing persists the chosen Connection's `ModelRef`, which resolves to that Connection's credentials, endpoint, and runtime route before execution. A legacy bare route remains compatible only when it has one Connection owner; multiple owners produce `ambiguous_model_route` and require an explicit choice instead of silently selecting the first. YAML aliases have no Connection identity and retain legacy-route compatibility.

### First-run Setup Status

The backend exposes a read-only status endpoint at `GET /api/v1/system/config/setup/status`. It reports whether the minimum first-run pieces are present: primary LLM Connection, Agent model inheritance/configuration, stock list, optional notification channel, and local storage. The first-run wizard and regular Model Access share the same `connection_fields` contract; the Provider Catalog authoritatively supplies Provider identity, labels, initialization defaults, discovery capability, and related links, but does not authoritatively determine field requirements. Under the current built-in schema, Ollama needs no key, official default endpoints need no manual Base URL, and Custom requires one. The wizard writes the explicit `_PROVIDER` field, preserves existing Connections, never saves a bare model name, and never auto-selects discovery results.

After Provider, credential, and model selection, the cloud wizard also configures the primary model, an ordered fallback chain, and an optional Vision route. It persists these choices as connection-aware `LITELLM_MODEL`, `LITELLM_FALLBACK_MODELS`, and `VISION_MODEL` values. A successful save keeps the completion view open with the effective execution and routing summary, never echoes the API key, and links directly to Task Assignment for further changes. Configured users can reopen the wizard from the setup-status card to add another Connection. The status endpoint itself only reads saved configuration and does not mutate or test it.

Discovery and connection-test results are bound to the Provider, credential, and transport inputs used to start them. Changing those inputs invalidates both settled and in-flight results. The connection test prioritizes the selected report primary model, but it does not probe fallback or Vision routes supplied by other Connections; test those independent routes from their respective Connections.
...
Web Settings exposes the same action under **Advanced → Configuration backup**. It displays and submits the current `config_version`, requires an explicit danger confirmation, and refreshes configuration, version, and diagnostics after success. A `409` is never auto-retried; the user can explicitly load the latest configuration. Fields named in the server's `updated_keys` response synchronize to their rolled-back values; other unsaved drafts and the current signed-in session remain intact.
...
- "Discover models" is shown only when the Provider Catalog reports `supports_discovery`. OpenAI-compatible / DeepSeek providers use their model-list endpoint; Ollama calls `{base_url}/api/tags` with an empty key allowed. The default "Test connection" action sends one minimal chat completion request against the first configured model and shows the backend-normalized `resolved_model`. Capability diagnostics never write back into `.env` and do not block saving. The first-run wizard's optional "Test connection" reuses the same backend diagnostics: beyond the resolved model it surfaces actionable stage and error-code hints and JSON and vision capability results; capability outcomes are informational and never change the base connectivity verdict or block saving.
- The Provider Catalog also supplies optional credential, console, model-list, and official-documentation URLs. The model-access dialog and first-run wizard consume only these backend values: absent URLs produce no empty action, external links open safely in a new tab, and the Web no longer keeps a provider-ID keyed business-link table. If the Catalog is temporarily unavailable, saved Connections retain their persisted Provider ID/compatibility summary and must not be relabeled as Custom; only new or Catalog-dependent editing actions are limited.
- `GET /api/v1/system/config/llm/providers` also returns the dynamic Connection `connection_fields` schema. Whenever that property is present (as `connectionFields` after Web response conversion), including an explicit empty array `[]`, the schema is the sole authority for required/visible/enabled state and the writable field set; the Web must not read legacy requirement flags from Provider entries. An isolated rolling-upgrade Catalog fallback is used only when an older backend omits the property entirely. Backend save validation and Web completeness diagnostics evaluate `connection_name/display_name/provider_id/protocol/base_url/api_key/api_keys/models/extra_headers/enabled` from the same `contract` with AND condition semantics. Any unknown operator in an AND list takes precedence over an earlier unmet condition: the field remains visible, read-only, and diagnostic, and saving is blocked. Legacy `is_required` remains a deprecated compatibility projection, while `contract.requirement/required_when` is authoritative. `requires_connection_test` reports state and does not turn a connection test into a save gate. Legacy configurations that omit `LLM_<CONNECTION>_DISPLAY_NAME` entirely use the Connection ID as their compatibility display name; an explicitly submitted empty value is still rejected by the required-field contract.
- Every built-in Catalog Provider supplies `label_zh` and `label_en`, and the Web selects the visible label from the live UI language. Legacy `label` is compatibility-only. Provider IDs, protocols, capabilities, and user-defined display names are never translated; if the requested localized built-in label is absent, English falls back to the stable Provider ID rather than showing the Chinese legacy label.
- System Config 409/422 envelopes treat `details` as authoritative and expose deprecated, read-only `detail` as an equal-value compatibility alias. Old clients may keep reading `detail`; new clients should prefer `details`. Removal is reserved for a future major or versioned API.
- The configuration Schema API preserves the complete `contract`: `requirement`, `required_when`, `visible_when`, `enabled_when`, and `requires_connection_test`. The Web interprets conditions while backend validation remains authoritative. During rolling deployment or with an older cached Schema, an AI field with a missing or unknown `ui_placement` is quarantined as a read-only Advanced diagnostic instead of becoming a normal editor. Unknown conditions likewise remain visible but read-only and expose a Schema diagnostic code.
- The regular Settings path has no global Save. Changes are grouped by backend category; all Connection, Task Routing, and fallback keys share one atomic `ai_model` group and are saved serially 700ms after editing stops. Each group exposes Scheduled, Saving, Saved, Failed, or Conflicted state. Failures retain the draft with Retry and Restore-server-value actions; HTTP 409 opens the `base/server/local` conflict view, and Reset affects only the current group. Saving, failed, or conflicted drafts trigger leave protection, and a masked secret is submitted only after the user actually changes it. Connection tests and model discovery remain explicit diagnostics: autosave does not trigger them and they are not save gates.
- Web credential fields use one browser-identity boundary. The current administrator password, new password, and confirmation have distinct stable `name` values and standard password `autocomplete` semantics. Provider API keys and other Schema-sensitive configuration use `autocomplete=off` with stable names isolated from administrator passwords; generic sensitive settings retain their existing explicit-focus edit gate. Autofill-like changes inside the first-run wizard or model-connection dialog update only that local draft and never trigger model discovery, a connection test, or configuration persistence. After the first administrator password is created and the setup wizard opens, the Provider API Key remains empty until the user explicitly enters one.
- Configuration GET masks every field whose Schema declares `is_sensitive`; Provider keys, notification tokens/passwords/webhooks, and Connection authentication headers are never sent to the browser. `raw_value_exists` only reports whether the server has a persisted value. Dynamic `LLM_{CONNECTION}_EXTRA_HEADERS` is a sensitive JSON object; malformed JSON and non-object values are rejected by the backend instead of being silently ignored at runtime.
- A mask token or omitted `LLM_{CONNECTION}_API_KEY(S)` / `LLM_{CONNECTION}_EXTRA_HEADERS` means “preserve the persisted secret” only for the same Connection name with unchanged Provider, protocol, and Base URL. Changing any identity field or renaming the Connection requires a fresh literal key/header JSON or an explicit clear; credentials bound to the old endpoint are never carried to the new endpoint silently. A runtime-injected secret that was never persisted cannot be reused through a mask or omission and must be entered explicitly. Preview, smoke-test, and persisted-update paths enforce the same scope check.
...
- The current dependency constraint for this flow in the repository is `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0` (see `requirements.txt`). Regression coverage for it lives in `tests/services/system_config/test_system_config_service.py`, `tests/test_system_config_api.py`, and `apps/dsa-web/src/components/settings/__tests__/LLMChannelEditor.test.tsx`.
...
- Runtime reference protection under `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0`: a save that would invalidate runtime references (`LITELLM_MODEL`, `AGENT_LITELLM_MODEL`, `VISION_MODEL`, `LITELLM_FALLBACK_MODELS`) is rejected by backend validation instead of being silently sanitized; non-channel direct providers such as `cohere/*`, `google/*`, and `xai/*` are preserved.
...
- Rollback evidence: `tests/services/system_config/test_system_config_service.py::test_import_desktop_env_restores_runtime_models_after_cleanup` covers restore from exported desktop backup after runtime cleanup.
- Direct-provider evidence: `tests/services/system_config/test_system_config_service.py::SystemConfigServiceTestCase::test_validate_accepts_minimax_model_as_direct_env_provider`, `test_validate_accepts_cohere_model_as_direct_env_provider`, `test_validate_accepts_google_model_as_direct_env_provider`, and `test_validate_accepts_xai_model_as_direct_env_provider` cover the preserved direct-provider behavior.
...
- Anspire Open: <https://open.anspire.cn/>
...
- Repository-side compatibility coverage lives in `tests/config/test_llm_channel_config.py`, `tests/analyzer/test_market_analyzer_generate_text.py`, `tests/test_agent_pipeline.py`, and `tests/services/system_config/test_system_config_service.py`.
...
- Regression evidence for this path is in `tests/llm/test_llm_param_recovery.py`, `tests/services/system_config/test_system_config_service.py`, `tests/config/test_llm_channel_config.py`, `tests/test_system_config_api.py`, `tests/analyzer/test_market_analyzer_generate_text.py`, `tests/test_agent_pipeline.py`; desktop backup import restore is directly covered by `test_import_desktop_env_restores_runtime_models_after_cleanup`.
