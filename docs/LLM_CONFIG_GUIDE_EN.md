...
Activation is serialized with the persisted write. If candidate construction or runtime reset fails, the service restores the exact previous `.env` content, affected process-environment values, and the previous global `Config` object, then rejects the update with `runtime_activation_failed`. A successful activation writes a local `.env.last-good-*` snapshot beside `ENV_FILE` with mode `0600`. The snapshot contains the raw previous configuration, including secrets; it is covered by the repository's `.env.*` ignore rule and must never be uploaded, attached to issues, or committed.
