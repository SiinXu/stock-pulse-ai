...
> 状态：**Approved（HITL-H1～H8 批准结论已记录，2026-07-17，见 §18.1）**
> 审计日期：2026-07-16
> 审计基线：`ed729c1b579fcf7c4f11f6e7beddc7230b71302b`
> 建议 IA：`今日 / 研究 / 组合 / 信号 / 更多 / 设置`
> 范围：只读审计与候选契约收敛；不包含 UI-02 及之后的生产代码修改。

> 2026-07-22 后续裁决：本文件保留为固定历史审计。当前目标 IA 以
> [Issue #368](https://github.com/SiinXu/stock-pulse-ai/issues/368) 为准；其中用量已裁决为
> “设置 > 用量与成本”，`/usage` 仅作为保留查询参数和 hash 的兼容重定向。
...
| --- | --- | --- | --- | --- | --- |
| `stockpulse-work-tracker.md` | §1、§5、§8；UI-01=`Next`，UI-02～UI-07=`Planned` | **任务身份已对齐**：UI-02/03/04/05/06 保持 tracker 的完整父任务定义；当前能力前置工作另记为 UI-02A、UI-03A～F、UI-04A、UI-05A1/A2/B 与 UI-06A/B/C0/C1/C2/D/E/F/G | 旧稿曾用父任务 ID 指 current-only 工作，造成同一 ID 跨文档含义漂移；tracker 也尚未登记这些候选子任务 | tracker 继续作为工作状态唯一索引；HITL 后先原子登记父子范围与状态，再允许分配子任务；UI-01 不直接修改外部 tracker | **是**：批准 UI-01、子任务登记和执行顺序；不是由审计自行改 tracker 状态 |
| `stockpulse-work-tracker.md` | §6 Agent Framework；AR-01～AR-08 | **Runtime 边界一致，Usage 导航未获批准**：Ask Stock 是 Research 用户任务，Agent 行为/运行诊断归 Settings；tracker 没有决定 Usage 是一级还是 More | 当前正式体系是 Single/Multi/Deep Research；Technical/Intel/Risk/Decision/Portfolio/Skill 是角色，不是独立 Runtime。外部 Runtime 仍为 Planned/POC | 不为 CrewAI/CAMEL/Hermes 等建产品导航或平行配置；`/usage` 页面身份保持，导航归属由 HITL-H8 与 H1/H2 联合审批 | **是**：Usage/More 必须进入 H1/H2/H8 tuple；AR-01/Runtime lifecycle 是独立架构 gate |
...
| `stockpulse-product-enhancement-prd.md` | E18 一致空状态与降级体验 | **一致**：UI-05 是增强而非重建 | 现有公共状态组件是基础，但缺跨页词汇、source/as-of/quality 的完整 API 映射 | UI-05A1 扩展 additive state primitive 并完成 Usage 状态 pilot；UI-05A2 另在 C0/C1 后只接 Usage landmark/focus；UI-05B 只维护页面接入矩阵、fixture 与总 gate；缺字段时显式列依赖，不用 broad fallback | **否**（方向已由 current code + PRD 约束）；执行仍受 UI-01 gate 阻断 |
...
- **候选，审计时仍未批准（2026-07-17 已由 §18.1 的 HITL-H1～H8 批准记录裁决）**：六项一级导航、`/research`、问股/选股/告警/回测/用量的目标归属、
...
属于不可扫描媒体，只做人工脱敏检查；两份 JSON 证据通过固定审计基线的 artifact scanner。

这些 artifact 只存在于本地 gitignored `.context`，**不能由当前 PR 独立复现或远端访问**；
因此本文件引用它们时只表示固定基线上的本地观察，不构成维护者可独立复核的 PR 证据。
未来每个 UI 实施 PR 必须重新生成、脱敏并通过 PR 附件、Actions artifact 或其它可访问链接
发布自己的证据，不能沿用本地路径或为它伪造 URL。
...
| `/usage` | 时段、KPI、模型/调用类型、最近调用 | Sidebar | 刷新；时段切换 | period 为本地状态；usage dashboard API | Mobile KPI 单列；error/retry 清楚 | `DEMOTE` 到 More 是候选，待 H1/H2/H8 联合 tuple；route identity 保持 |
...
下表区分“代码/运行中已有表达”和“仍需 UI-05 contract”。它是 UI-05B aggregate matrix 的
审计输入，实际接入仍由各页面 owner 完成；`—` 表示该状态对当前页面不适用，不是已通过验收；
“部分”表示领域自有文案存在，但尚未统一到 §9.2。
...
flowchart TD
  Shell --> Today[今日 /]
  Shell -. UI-03C 验收后由 UI-06B 暴露 .-> Research[研究 /research]
...
- 推荐用量进入“更多”，但必须通过 HITL-H1/H2/H8 联合 tuple 批准；`/usage` route/page identity 不变。运行诊断与数据源健康归设置，报告级数据质量归研究报告。
- Desktop 侧栏和 mobile Drawer 使用同一分组树、同一名称、同一 canonical link builder。

Research 一级入口使用以下唯一生命周期；上图只表示联合选择 H1 六槽、H2 Research hub、H8 More
且生命周期完成后的推荐状态：

1. `UI-03A` 只建立 parser、normalizer、`buildTo`、return URL validation 和 current/legacy-path
   compatibility helper；不注册 handler，不发布用户可访问的 `/research` route。
2. H2 批准 Research hub 时，`UI-03C` 把 `/research` route registration、handler/fallback、
   `ResearchPage` composition、compatibility tests 与 direct/refresh/invalid/auth/mobile/desktop/
   title/H1/focus smoke 作为同一个原子发布单元。
3. `UI-03C` 原子单元完整通过并发布前，不存在可访问的 `/research`，Shell 也不得显示 Research 入口。
4. 上述单元发布后，只有 H1 六槽 + H2 Research hub 组合才由 `UI-06B` 把 `/research` 加入默认
   一级导航；H1 旧平铺 + H2 Research hub 只保留 additive deep link，不加入默认一级导航。
5. `/chat`、`/screening`、`/?recordId=` 在整个阶段继续是 current canonical 和可直达路径；
   主导航指向 `/research` 不等于旧路径 redirect、失效或完成 canonical cutover。
6. Research parser、handler、fallback 和 compatibility tests 只有随 `UI-03C` 发布后才成为 route
   durable assets。UI-03A helper 在没有 consumer 时可整体回退；已有 current-path consumer 时，
   helper public API/tests 只因该 consumer 而 durable，这仍不代表 `/research` 已发布。
7. canonical 切换与旧路径退出仍由 `HITL-H6` 根据兼容证据另行决定。
8. 若 `HITL-H2` 不批准 Research hub，UI-03A 只保留通用 Context/URL helper，UI-03C 跳过；
   默认导航严格采用 §8.1 的 H1/H2/H8 联合决策矩阵，禁止 Agent 推断替代槽位。
...
| 10 | Mobile nav -> 页面 -> Overlay | Drawer focus/escape 可用；Home/Chat 又有第二个相似汉堡按钮 | 全局汉堡固定；页内历史用明确 History 图标/标签 | 320/390 无歧义；取消/Escape 回 opener，成功跨页后由 UI-06C0 在目标 ready 时聚焦 H1 |
...
- **依赖**：HITL-H1/H2/H8 联合决定；UI-06B 的共享 navigation descriptor；UI-03A 的 helper 与
  UI-03C 的 Research 原子发布决定。
...
- **实施归属**：`UI-03C` 独占 route/page 原子发布并通过完整 smoke；随后只有六槽 + hub tuple
  才由 `UI-06B` 在独占 `Shell/SidebarNav/navigation descriptors` 窗口暴露 `/research`。旧平铺 +
  hub 只保留 deep link；六槽 + 无 hub 未指定替代槽位时 blocked。两者不并行。
...
- **推荐决策**：先由 `UI-03A` 建立 additive URL helper 与 invalid-param 规范，再由领域页面 owner
  顺序接入；identity/filter/view 进入 query，轮询与规范化只用 replace。
- **依赖**：HITL-H2；不需要新 API，HITL-H6 只管理 UI-03C 发布后的 canonical cutover；
  source-context 原子性另见 `UI01-P1-07`。
- **验收证据**：每页 direct/refresh/share/Back/Forward/invalid-ID table test；canonical 与 legacy URL
  同时跑，且无关 query/hash 不被清除。
- **实施归属**：`UI-03A` 独占共享 URL helper，不编辑 `App.tsx`；`UI-04A` 在其后接
  Portfolio/Backtest，Alerts/Signals 由 `UI-03E` 接入。只有 `UI-03C` 在原子发布窗口独占
  `App.tsx` route registration，避免并行修改共享边界。
- **兼容 / 回滚**：新 query additive；旧页忽略未知参数仍能运行。领域 consumer 可 revert 自身接入，
  UI-03A helper 本身不构成已发布 URL；有 current-path consumer 时，public helper API/tests 随该
  consumer durable。只有 Research parser/handler/fallback/compatibility tests 在 UI-03C 发布后成为
  route durable assets，不得用 rollback 吞掉已发布链接。
...
- **推荐决策**：拆成 UI-06C0 route focus coordinator、UI-06C1 public landmark contract、
  UI-06C2 route adoption gate 与 UI-06E Menu/Popover；Pagination 由
...
- **实施归属**：`UI-06C0/C1/D/E` 分别独占 route focus、landmark 与其它 public primitive；page owner
  只在自身窗口注册 C0/C1，UI-06C2 维护总 gate，不与页面 composition owner 并发编辑同一文件。
...
- **当前行为**：9 个图表/时间线 raw hex 位于 `types/analysis.ts` 与
  `utils/decisionSignalTimeline.ts`。现有 guard 只加载生产 `TSX`、`App.css` 与 `index.css`，并用
  CSS path assertion 防止静默遗漏新增生产 CSS；它不加载 `TS` / `JS`。`index.css` 的可执行例外
  只是 `:root/.dark` 内 `--*` custom-property declaration 的句法例外，尚不验证 semantic ownership；
  tests/fixtures 被目录级排除，未实现具名 hex 白名单与理由校验。
- **用户影响**：颜色无法随主题/token 修正同步，状态可能只靠颜色或与中国市场涨跌语义串色。
- **根因**：颜色由数据 helper 返回 raw presentation value，semantic color ownership 穿透到类型/工具层。
- **推荐决策**：数据层返回 semantic key，渲染层解析现有 chart/sentiment token；扩展 guard 到普通
  生产 `TS/TSX/JS/CSS`（含 types、utils、renderer），只允许 `index.css` 的 `:root/.dark` semantic
  token source declaration 使用 raw color，并为固定 hex fixture 建立具名白名单与语义理由；不从
  Coinstax 增加 raw value。
- **依赖**：先确认现有 token 是否足够；缺口需单独更新 DESIGN_GUIDE + `index.css` 并获视觉批准。
- **验收证据**：扩展后的 scope self-test 证明生产 `TS/TSX/JS/CSS`（含 types/utils/renderer）均受检；
  只有 `index.css` `:root/.dark` 的 semantic-token source declaration 可含 raw color，普通 declaration
  与块外 custom property 均失败；fixture 目录排除不自动放行 raw hex，只有记录 exact entry 和
  color/guard-sentinel reason 的显式白名单可通过。另附 light/dark、涨/跌/中性/风险截图与非颜色 label。
...
- **实施归属**：UI-05 父任务后续 action primitive；不属于 UI-05A1 的 Usage state pilot、UI-05A2
  的 Usage landmark/focus adoption 或 UI-05B
  aggregate gate。实施时另取 `ConfirmDialog/Button` 独占窗口并在 tracker 明确登记。
...
| Embedded report/history | `/research?view=report&reportId=...&stock=...` | `MERGE` | 保留报告、诊断、趋势、Markdown、Run Flow | 现有 history API | candidate alias 在 UI-03C 原子发布前不可访问；`/?recordId=` 至少跨两个 minor release，发布后先双向 link | 报告主列 + contextual rail；mobile page | UI-03A helper + UI-03C～F；timeline/Evidence 仍属 UI-03 |
| Chat | `/research?view=chat&session=...` | `MERGE` | 保留会话、SSE、技能、停止、导出 | 需确认 session source-context 原子性 | candidate alias 在 UI-03C 原子发布前不可访问；`/chat` 保持 current canonical，迁移期不强制 redirect | Desktop optional session rail；mobile Drawer | UI-03A helper + UI-03C～F |
| Screening | `/research?view=screening` | `MERGE` | 保留 AlphaSift、热点、任务、候选 | 现有 API；实验状态显式 | candidate alias 在 UI-03C 原子发布前不可访问；`/screening` 保持 current canonical；真实 `ALPHASIFT_ENABLED` 只控制领域能力，不隐藏 route identity | Advanced filters 可折叠 | UI-03A helper + UI-03C～F；UI-05B aggregate gate |
...
| Backtest | `/backtest`（current canonical）；`/portfolio?view=backtest` 仅为 additive candidate | `DEMOTE` | 不删功能；作为组合验证工具 | 现有 backtest API | `/backtest` 保持直达；candidate 只作 link，不 redirect current route | Route link；filters compact；table row-detail | UI-04A；UI-05B aggregate gate |
| Decision Signals | `/decision-signals` | `REFACTOR` | 成为“信号”入口，含列表/详情/后验 | get-by-id 已有 | 现 path 永久；只增加 UI-03E 拥有的 `signal/filters/page/returnTo` query | 首屏 summary；mobile filters Drawer | UI-03E；UI-05B aggregate gate |
| Alerts | `/alerts` | `MERGE`（仅导航分组） | 保留规则、触发、通知结果与独立 page identity | 现有 alerts API | `/alerts` 保持 current canonical；不承诺未被 slice 拥有的 Signals-hosted Alerts alias | Route navigation；mobile list/row detail；不使用 tab role | HITL-H5 + UI-03E/UI-06B；UI-05B aggregate gate |
| Usage | `/usage`（推荐 More，待联合 tuple） | `DEMOTE` 候选 | 保留独立 dashboard/page identity，不并入 Settings | 现有 API | path、权限和收藏保持；More 只改变发现方式 | Desktop Sidebar / mobile Drawer disclosure | HITL-H1/H2/H8 + UI-05A1/A2/UI-05B/UI-06B/C2/G/UI-07 |
| Settings | `/settings?section=&view=` | `KEEP` | 两级 IA、autosave、409、Provider Catalog 全保留；修复已证实的 mobile 内容裁切 | 无新 API；具体 overflow descendant 待确认 | legacy category/sub replace migration 保留 | 保留 nav 模式；UI-06F 覆盖 320/390/200% | UI-06F + UI-05B/UI-07 evidence |
...
| Research hub | `/research`（候选；当前 paths 仍 canonical） | `DEFER` | 可先组合现有单资产、报告、问股、选股；时间线只作 future 归属 | 现有页面/API 足够做 shell；E06 timeline 需 ResearchEvent/游标 contract | UI-03A 只建 helper；UI-03C 原子注册 route + compose page + 完整 smoke。六槽 + hub 才由 UI-06B 加默认入口；旧平铺 + hub 仅 deep link；H6 另决 canonical cutover | Desktop workspace/mobile page | UI-03A～F + UI-06B；完整能力仍属 UI-03 |
...
以下六槽表只表示推荐联合决定：H1=六槽 IA、H2=Research hub、H8=More。其它允许结果由表后的
联合矩阵确定，不能从本表单独推导。
...
| 2 | 研究 / Research | search/chart | `/research` | 单资产、报告、历史、时间线、问股、选股 | UI-03A 只建 helper；UI-03C 原子发布 route/page 并完成 direct/refresh/invalid/auth/mobile/desktop/title/H1/focus smoke 后，UI-06B 才在六槽 + hub 组合显示本项；此前 current paths 保持 |
...
H1、H2、H8 必须作为一个联合决定记录：

| H1 | H2 | H8 / 一级 Usage 结果 | 导航结果 | 是否允许 |
| --- | --- | --- | --- | --- |
| 六槽 IA | Research hub | More 或 Usage 恰选一个 | 六槽包含 Research；UI-03C 原子发布并通过 smoke 后，UI-06B 才曝光 | 推荐 |
| 六槽 IA | 不建 Research hub | More 或 Usage 恰选一个，但不得补空 Research | 不得直接实施；维护者必须指定第五/替代槽位并先更新 IA 与 tracker | 禁止未补决策实施 |
| 保留旧平铺 | Research hub | 保持当前一级 Usage，不新增 More | UI-03C 发布 `/research` additive deep link，但不加入默认一级导航 | 允许但非推荐 |
| 保留旧平铺 | 不建 Research hub | 保持当前一级 Usage，不新增 More | 保持当前导航，只执行获批 bugfix | 允许 |

六槽方案由 H8 在一级 More 与一级 Usage 中恰选一个；旧平铺按字面保留当前一级 Usage 且没有 More。
不允许空 Research 槽位，不允许一级 More 与一级 Usage 同时出现，也不允许 Agent 自行推断替代槽位。
若维护者选择 flat + More 或其它未列组合，必须先更新本 IA 和 tracker，再开始实现。

### 8.2 canonical 与 compatibility 规则

- UI-02～UI-07 期间：所有现有路由仍是各自 current canonical；禁止先做 redirect 再补目标页面。
- 若 HITL-H2 批准 Research hub：UI-03A 只准备 helper，不注册可访问 route；UI-03C 把 route
  registration、`ResearchPage` composition、handler/fallback、compatibility tests 与完整 smoke 作为
  同一个发布单元。只有 H1 六槽 + H2 hub 时，UI-06B 才在 smoke 后把 `/research` 作为默认一级
  导航目标；H1 旧平铺 + H2 hub 时，`/research` 只作为 additive deep link。
- `/chat`、`/screening`、`/?recordId=` 在上述导航暴露后仍是 current canonical、可直达且不自动
  replace；主导航目标变化不等于 canonical cutover。切换 canonical 或退出旧路径只由 HITL-H6
  基于兼容使用证据另行批准。若 H2 不批准，UI-03C 跳过，导航使用 §8.1 已批准的联合矩阵结果；
  六槽 + 无 hub 在维护者指定替代槽位前保持 blocked。
...
只属于瞬态组件或 coordinator 内存：hover、tooltip、未确认 popover、纯视觉 collapse、toast、
未提交 Modal draft，以及 UI-06C0 内部 `RouteFocusEntry`。它们不进入业务 URL；未保存配置 draft
继续由 Settings draft/leave guard 管理。Route focus metadata 只由 UI-06C0 按 `location.key` 保存在
内存中，不写 URL、history state、`localStorage` 或 `sessionStorage`，不保存 DOM node/ref，也不由
页面建立第二套真源。
...
- `UI-06C0 RouteFocusCoordinator` 是跨路由焦点分类、内部 history-entry focus store 和执行时序的唯一真源；
  它结合 React Router navigation type 与 `location.key` 区分 PUSH、REPLACE、POP 与
  initial/no-transition 类。Direct load、hard refresh 和 new-tab 都进入后者并使用正常初始焦点；
  三类仍保留独立 journey。页面不得各自判断 navigation type 或维护第二套 focus store。
- PUSH/REPLACE 的同窗口完整页面导航由 coordinator 等待目标 `ready`，确认最终本地化 H1 与
  `document.title` 已更新后聚焦 H1。Coordinator 只捕获带稳定公共 `data-route-focus-key` 的可聚焦
  触发点；key 必须在当前页面唯一且稳定，禁止数组 index、翻译文本或动态 DOM 顺序。POP/Back/Forward
  优先恢复该 history entry 的内部 key；重复、缺失、失效或不可聚焦的 key 一律 fail closed 到 ready H1，
  REPLACE 丢弃失效的 destination entry。
- Cmd/Ctrl-click、中键和“在新标签打开”不得触发当前文档移焦；direct load、hard refresh 和新标签页
  使用页面正常初始焦点，不执行 transition focus。页面 hook 只接受 `routeId`、H1 ref 和 `ready`，
  不接受 `focusKey`、navigation type 或 history entry；`WorkspaceNavigation`、Shell/Sidebar 和页面组件
  均不直接调用跨路由 `focus()`。内部 metadata 只存内存，不写 URL/history/storage，也不保存 DOM ref。
- Drawer/Modal/Confirm 保留现有焦点陷阱、Escape、inert 与 scroll lock；取消、Escape 或无导航关闭时
  回 opener，成功跨页导航时交给 coordinator 执行上述 contract；嵌套时只关闭 topmost overlay。
...
- Usage 必须保持独立 route/page identity，不与 Provider credential 或系统诊断混在同一页；Sidebar
  归 More 还是继续一级由 §8.1 的 HITL-H1/H2/H8 联合 tuple 决定。
...
- Route focus：只扩展 `RouteFocusCoordinator/useRouteFocusTarget` 这一套 typed contract；hook 只接受
  `RouteFocusTarget { routeId, headingRef, ready }`。`RouteFocusEntry`、PUSH/REPLACE/POP/
  initial-no-transition 分类、稳定 `data-route-focus-key` 捕获和 history-entry restore 全归 coordinator；
  direct/refresh/new-tab 是分别验证的 no-transition journey。
...
- 页面向 hook 传 `focusKey`、navigation type 或 history entry，私自判断 PUSH/POP、维护第二套 focus
  store，或把 metadata/DOM ref 写入 URL、history state、`localStorage`、`sessionStorage`；
  `WorkspaceNavigation`/Shell/Sidebar/页面也不得直接调用跨路由 `focus()`。
...
| Research shell、现有报告/问股/选股归组 | history/chat/AlphaSift APIs 已有 | HITL + tracker 登记后先做 helper；默认导航仍 blocked | UI-03A 只建 helper；UI-03C 原子发布 route/page/smoke；是否由 UI-06B 曝光取决于 H1/H2/H8 联合矩阵；只是完整 UI-03 的 current-capability preparation |
...
| State vocabulary、responsive Pagination/Menu/Landmark/route focus | 无新 API | HITL + tracker 登记后可做 | UI-05A1/A2/B、UI-06C0/C1/C2/D/E/F |
...
| `UI-05` | 完整 E18 跨页面状态体系；所有 owner 采用统一 loading/empty/degraded/stale/error/retrying 语义并通过总 gate | `UI-05A1/A2/B` State primitives/Usage state pilot、Usage landmark/focus adoption 与 page adoption/aggregate gate | UI-05A1/A2 完成不代表 UI-05 Done；仍需所有领域 owner 接入并由 UI-05B 证明完整 matrix |
| `UI-06` | 收敛页面级响应式、移动导航与无障碍；320/390/desktop、键盘、焦点和读屏均通过 | `UI-06A/B/C0/C1/C2/D/E/F/G` | 任一子任务完成都不能代表 UI-06 Done；必须全部完成并由 C2/G/07 总 gate 证明 current route matrix |

> HITL 批准后，必须在开始实现前原子更新 `stockpulse-work-tracker.md`，登记 UI-02A、
> UI-03A～F、UI-04A、UI-05A1/A2/B 与 UI-06A/B/C0/C1/C2/D/E/F/G 的父子范围、状态、依赖和执行顺序。
...
3. 准备 parser/helper 不等于发布 URL。只有 route 与有效页面作为完整单元向用户发布后，其
   parser/handler/fallback/compatibility tests 才成为 durable assets；已写入持久数据及其兼容层也不随
   页面 composition 或 navigation exposure 回退。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| UI-02A Current-data Home | 否 | 5. Scoped PR/commit revert | 合入的 Home implementation、route/visual tests 与 Git history | 合入后 `/` 使用新 current-data hierarchy；无用户可选旧模式 | Web composition；API 不变；Desktop 加载同一 Web | UI-06C0/C1 后，在 UI-06C2 的 Home 窗口注册并接入 | URL parser/legacy tests 与 Home composition/1024 fix/行为测试分单元；composition 回退时 1024 与 hierarchy tests 同退。已发布 parser durable；旧 `/` 与 legacy tests 因旧页面仍支持这些路径可保留 |
| UI-03A URL/compatibility helper | 否 | 3. Helper-only，无可访问 route | H2 hub：research parser/normalizer/`buildTo`/return validation/current-legacy tests；H2 no-hub：中性 Context/URL helper/tests | 所有 current routes 行为不变；`/research` 未注册、不可深链；no-hub 不留 dormant Research view schema | Web helper；API/Desktop runtime 不变 | hub 分支把 helper 交给 C 的原子 route/page/smoke 单元；no-hub 分支只供 current-path consumer 使用并跳过 C | 无 consumer 时 helper/tests 可整体回退；有 current-path consumer 时 public API/tests 随 consumer durable，但不代表 route 已对用户开放。只有 C 对外开放 route 后，Research parser/handler/fallback/compatibility tests 才按 C 契约 durable |
| UI-03B WorkspaceNavigation | 否 | 3. 公共组件逐 consumer 接入 | public component API/ARIA tests | 未接入 consumer 行为不变 | Web component；API/Desktop wrapper 不变 | UI-03C 首个 consumer；跨路由焦点只由 UI-06C0 管理；未来 UI-04A 顺序消费 | 每个 consumer adapter 与其 navigation/mobile tests 同单元；focus contract/tests 归 UI-06C0/C2。public API/unit tests 在仍有 consumer 时 durable，最后一个 consumer 退出后才可一起删除。current-route tests 不依赖该 primitive，可保留 |
| UI-03C Research atomic publication | 否 | 2. Route + page + smoke 原子发布 | `App.tsx` route registration、handler/fallback、`ResearchPage` composition/adapters、A helper、route/compatibility tests | 单元发布前 `/research` 不可访问；发布后 planned view 仍不渲染，current routes 保持 canonical | Web route/composition；复用现 API；Desktop 同 Web | 依赖 A/B 与 UI-06C0/C1；原子单元完整 smoke 通过后，是否进入默认一级导航再由 H1/H2/H8 联合矩阵决定 | 发布前 route/page/tests 整体回退；发布后 parser/handler/fallback/compatibility tests durable，页面问题 forward-fix，不得独立撤页留下无有效页面 route。current-route smoke 在两种状态都保留 |
| UI-03D Research context | 否 | 3. ContextLink 逐 report/chat/screening consumer 接入 | context helper contract 与各 consumer tests | 未迁 consumer 保持当前入口 | Web links/session context；API 只在确认 source contract 后使用；Desktop 同 Web | 先 helper，再 report/chat，最后 screening | report/chat/screening consumer 分别连同行为测试回退；helper 仅在全部 consumer 退出后删除。已持久化 metadata/schema durable；Research parser/handler/fallback 只在 C 发布后成为 route durable assets；legacy route/session/security tests 仍通过 |
...
| UI-03F Compatibility evidence | 否 | 5. PR/test gate | current/published-candidate route journeys 与 artifact index | 无运行时行为 | 测试/证据；API/Desktop runtime 不变 | 汇总 UI-03A/B/D/E；只有 UI-03C 实际发布时才汇总 candidate route，始终保留 current-route smoke | 行为断言归对应 A～E 中实际执行的 rollback unit；F 只因证据错误修正。current/legacy smoke、C 发布后的 alias fallback/security tests、harness 与历史 artifact durable；不得删测试掩盖仍支持 contract 的失败 |
| UI-04A Current Portfolio | 否 | 2. Additive query/view，不进默认导航 | Portfolio parser、route/task tests | 无 query 的 `/portfolio` 与 `/backtest` 在本 slice 始终 current canonical | Web composition；API/payload 不变；Desktop 同 Web | 共享 URL helper 与 UI-06C0/C1 后接入；canonical cutover 留给完整父任务 UI-04 | parser/fallback 与 composition consumer 分单元；发布后 parser、canonical routes、账户/任务/写入数据 durable。current-route、safe-fallback、mutation/API tests 在旧 composition 下仍通过；view/select tests 随 consumer 回退 |
| UI-05A1 State vocabulary and Usage state pilot | 否 | 3. Additive public state primitive + 单页状态 pilot | public state API/tests/exports 与 `TokenUsagePage` 状态呈现/tests | 除 Usage 状态呈现外未迁页面保持现状；不改 landmark/PageHeader/focus；pilot 完成不代表 UI-05 Done | Web component/Usage state；API contract 不扩写；Desktop 同 Web | 冻结词汇和 primitive，在 Usage 验证状态呈现后释放 common files；不等待 C0/C1 | Usage state consumer 与 fixture tests 同单元；primitive/API tests 在仍有 consumer 时 durable。后端 error/schema/parser 与 legacy page smoke 可保留 |
| UI-05A2 Usage landmark and route-focus adoption | 否 | 3. 单 route consumer adoption | `TokenUsagePage` 与对应 landmark/focus tests | 只为 Usage 建立唯一 H1、非嵌套 main 与 `RouteFocusTarget` registration；不重新编辑 common state primitive | Web Usage page；API/Desktop runtime 不变 | UI-06C0/C1 后取得 Usage 独占窗口；由 UI-06C2 与 UI-05B aggregate gate 共同验收 | Usage landmark/focus adoption 与 DOM/registration tests 同单元回退；C0/C1 在仍有 consumer 时 durable，B/C2 gate 把 Usage 标记未完成 |
| UI-05B Page adoption and aggregate gate | 否 | 3. 页面 owner 顺序接入 + 聚合 gate | state matrix、shared fixtures、aggregate gate | 未迁页面保持现状；任一适用 owner 未完成时父任务保持未完成 | Web tests/evidence；API/Desktop runtime 不变 | Home=UI-02A；Research 仅在 UI-03C 实际发布时=UI-03C，否则按 H2 记录 N/A；Chat/Screening=UI-03D；Signals/Alerts=UI-03E；Portfolio/Backtest=UI-04A；Settings=UI-06F；Usage state=UI-05A1、landmark/focus=UI-05A2 | 每页 adoption/tests 跟随页面 owner 回退；UI-05B 只修正 matrix/fixture/gate，必须把 A1/A2 或其它退出 consumer 标记未完成，不得保留全页通过断言 |
| UI-06A Auth deep link | 否 | 4. 普通 correctness/security bugfix | auth Router contract 与 integration tests | 修复始终生效 | Web auth；现 API/Desktop 不变 | 完成后释放 `App.tsx` 给 UI-06C0 | 默认 forward-fix；仅更严重 auth 阻断允许把 owner fix 与新 deep-link tests 同单元紧急回退。独立 redirect validation durable；旧实现仍通过的 external-rejection 与 login/session smoke 可保留 |
| UI-06B Shell/Sidebar | 否 | 5. 原子 PR/commit revert；若需 audience rollout 则机制 6 并 blocked | navigation descriptor、Shell/Sidebar tests、Git history | 只实现已记录的 H1/H2/H8 联合矩阵结果；旧 paths 全保留 | Web desktop/mobile nav；API 不变；Desktop 同 Web | 六槽+hub 在 UI-03C smoke 后暴露 Research；旧平铺+hub 只 deep link；六槽+无 hub 在替代槽位获批前 blocked；旧平铺+无 hub 只做 bugfix | descriptor/labels 与四组合 tree tests 为一单元；responsive/brand layout 与 overflow/brand tests 为独立 correctness 单元。routes/parser 与 current page smoke durable；新树或新布局断言不在旧状态保留 |
| UI-06C0 Route transition focus coordinator | 否 | 3. Additive routing boundary + registration hook | public `RouteFocusTarget`、internal `RouteFocusEntry`、coordinator/hook 与 event/fail-closed tests | 未注册页面保持现状；注册页面的跨路由焦点只由 coordinator 执行 | Web Router/focus；API/Desktop wrapper 不变 | `App.tsx` 严格按 UI-06A -> C0 -> UI-03C 移交；C0 后接 C1，再由 page owner/C2 adoption | Coordinator、hook 与 event/key/fail-closed/memory-only tests 同单元；页面 registration/tests 跟随页面 owner。已有 URL/history entry 不因 coordinator 回退而改写 |
| UI-06C1 Landmark primitive | 否 | 3. Additive public API | `AppPage/PageHeader` API 与 component/ARIA tests | `AppPage` 保持现有默认 `<main>`；只有显式 opt-in consumer 改语义，未迁页面不变 | Web component；API/Desktop wrapper 不变 | UI-06C0 后、C2 前完成 | 未消费时 API 与 API tests 可整体回退；有 consumer 时 API durable，须先退出全部 C2 consumers。默认 `<main>` compatibility fixture 在旧状态仍通过；opt-in tests 随实现/consumer 回退 |
| UI-06C2 Route adoption | 否 | 3. 按 route consumer 顺序接入 | C0/C1 contract、route DOM/axe/keyboard/focus gate | 每个已迁 route 使用新 landmark 并注册 focus target；未全迁不得标记完成 | Web pages；API 不变；Desktop 同 Web | 与各 page owner 的独占窗口串行；C2 只维护 gate，C0 执行分类和恢复 | 每 route adoption 与 DOM/axe/registration tests 同单元；C0/C1 在仍有 consumer 时 durable。aggregate gate 保留但标记该 route 未完成；不得保留被回退 route 或全路由完成断言 |
...
| UI-06G Accessibility evidence | 否 | 5. PR/test gate | current/additive route a11y matrix 与 artifact index | 无运行时行为 | 测试/证据；API/Desktop runtime 不变 | 汇总 UI-06A/B/C0/C1/C2/D/E/F | A～F 与 C0 行为 tests 归各 owner 单元；G 只聚合并因证据错误修正。harness、current/published-route compatibility、安全 tests 与历史 artifact durable；不保留已回退行为断言 |
| UI-07 Visual/interaction evidence | 否 | 5. PR/test gate | Playwright journeys、snapshots、artifact scanner | 无运行时行为 | 测试/证据；API/Desktop runtime 不变 | 汇总 UI-05A1/A2/B、UI-06C0 与全部已合入 slice；不构造 flag matrix | journey/snapshot 跟随 owner rollback unit；UI-07 只保留 scanner/harness、legacy smoke 与不可改写历史 evidence。不得更新 snapshot 接受已知错误；未发布行为撤销时 snapshot 与实现同退 |
...
| 前置 / 后置 | 前置：HITL-H1/H4、tracker 登记、UI-06C0/C1；在 UI-06C2 的 Home 独占窗口注册并接入。后置：E04 薄规格/API 完成后继续父任务 UI-02；UI-03D helper 必须先稳定，再由 UI-02A 消费 Home context link。 |
...
| 字段 | 定义 |
| --- | --- |
| 用户可见结果 | 无新增可访问 route；现有 `/chat`、`/screening`、`/?recordId=` 始终可直达。后续 owner 复用获批分支的 URL/context contract，而不是各自拼 URL。 |
| 范围 | H2 建 hub 时建立 research URL parser、normalizer、`buildTo`、受控 return URL validation、current/legacy-path compatibility helper 与 tests；H2 不建 hub 时只建立供 current paths 使用的中性 Context/URL helper 与 tests。 |
| 非目标 | 不编辑 `App.tsx`，不注册 handler/fallback，不发布 `/research` deep link，不移动页面内容、不实现 timeline/compare/hypothesis、不 redirect 当前 canonical path；no-hub 分支不保留 dormant Research view schema 或 Research 专用命名。 |
| 文件所有权 | H2 hub 分支可新增 `routing/researchRouteContract.ts`（最终目录按现有 convention）；no-hub 分支使用中性 routing/context helper 命名。对应 unit tests 同属本 slice；不拥有 route table。 |
| API/数据依赖 | 无新 API；HITL-H2 决定 UI-03C 是否使用 helper 发布 Research hub。H2 不建 hub 时，本 slice 只保留通用 Context/URL helper；HITL-H6 仍只负责更晚的 canonical cutover 与 legacy exit。 |
| URL/状态 | hub 分支 identity 使用 `stock/reportId/session/signal/view/returnTo`；no-hub 分支只编码 current-path context/return identity，不创建 candidate `view` schema。用户动作 push，invalid normalization replace；query/hash 透传白名单化。 |
| rollout/兼容 | 机制 3：additive helper，无运行时 flag、无用户可访问行为。只有 UI-03C 的 route/page/smoke 原子单元才能发布 `/research`。 |
| 验收测试 | hub：parse/normalize/`buildTo`、合法/非法 return URL、view/identity 白名单、current/legacy mapping；no-hub：中性 current-path context/return helper 且无 Research view schema。两分支都证明未增加 route registration/handler，current direct/refresh/share/Back/Forward 行为不变。 |
| 回滚 | 无 consumer 时 helper/tests 可整体回退；有 current-path consumer 时 public helper API/tests 随 consumer durable，但没有 durable published `/research`。UI-03C 发布后，由 C 使用的 Research parser/handler/fallback/compatibility tests 才按 C 的发布契约 durable；legacy direct/refresh 与 returnTo security tests 始终保留。 |
| 前置 / 后置 | 前置：HITL-H2、tracker 原子登记；不占用 `App.tsx`，HITL-H6 只约束更晚 cutover。后置：hub helper 交给 UI-03C/D/E，no-hub helper 交给 D/E/current-route consumer；UI-03C 仍须等待 UI-06C0/C1 和 UI-03B。H2 不建 hub 时跳过 UI-03C。 |
...
| 范围 | `nav > ul > li > Link` 与 mobile labelled `<select>` 的公共 responsive presentation、active contract 和 tests。当前 Report/Chat/Screening 是 sibling page routes，因此禁止伪装成同页 panel。可聚焦触发点可用稳定 item identity 渲染 `data-route-focus-key`，但 primitive 不捕获、不存储也不恢复焦点。 |
...
| 键盘 | 保留原生 link 模型：Tab/Shift+Tab 逐链接，Enter 激活；Cmd/Ctrl-click、中键、context menu/new-tab 有效且不触发当前文档移焦。不得拦截方向键、Home/End/Space，不做 roving tabindex 或 manual tab activation。普通同窗口激活后的 H1 聚焦只交给 UI-06C0。 |
| URL/状态 | URL 是唯一真源；consumer 从完整 URL 解析 `current`。显式导航用 Router `push`，missing/invalid normalization 用 `replace`；H2 hub 分支的 UI-03A helper 按 Research view 白名单构造 `to`，no-hub 分支只构造 current-path context/return identity；两者都禁止盲拷贝 query。 |
| 焦点 / 标题 | 页面只向 UI-06C0 注册 `routeId`、带 `tabIndex={-1}` 的 H1 ref 与 `ready`。Coordinator 根据 navigation type/`location.key` 在 title/H1 更新后处理 PUSH/REPLACE focus、POP restore/fallback；direct/hard refresh/new-tab 不执行 transition focus。此 primitive 不判定导航类型、不保存 focus，也不调用 `focus()`。 |
...
| Component API | `WorkspaceNavItem<V> { id, label, to }`；props 至少含 `ariaLabel/current/items/onCompactNavigate/className?`。稳定且页面内唯一的 `id` 可派生 `data-route-focus-key`，禁止翻译 label、数组 index 或 DOM 顺序。组件不读 location/API、不生成 URL、不持有 local active state、不接受 panel children、不显示 planned/blocked item。 |
| Consumer 职责 | UI-03A 负责 parse/normalize/buildTo/legacy/returnTo；UI-03C 注册 route、提供 labels/order/current、只 mount 当前目标并注册 `RouteFocusTarget`。UI-06C0 独占 navigation classification、key capture/store/restore/execution；WorkspaceNavigation 不读 Router、不存 metadata 或直接操作 DOM focus。Chat adapter 修正 candidate route classification；UI-03E 不把 Signals/Alerts 塞进 Research navigation model。 |
...
| 验收测试 | unit：navigation/list/link href、唯一 `aria-current`、零 tab role、全部 links tabbable、稳定/唯一 marker、禁止 label/index/order key、Desktop 原生 link/new-tab 语义、mobile options/current/callback 与“仅同窗口”边界、长 zh/en；Router/Playwright：UI-03C 发布后的四个 view direct/refresh/share/push、invalid replace、legacy/candidate 等价；UI-06C0/C2 gate 验证 ready/title/H1、valid-key restore、fail-closed fallback、modifier/new-tab/direct/POP 分类；Chat candidate 不误亮 completion badge。 |
...
| 前置 / 后置 | 前置：UI-05A1 common primitive ownership window 结束、UI-03A URL helper、UI-06C0/C1；后置：由 UI-03C 接入，未来 UI-04A 可消费但不并行改此文件。 |

#### UI-03C Research route/page atomic publication 与二级导航

| 字段 | 定义 |
| --- | --- |
| 用户可见结果 | 原子发布后，可通过 additive Research destination 在当前已实现的资产概览、报告/历史、问股、选股页面间导航；发布前不存在可访问的 `/research`，planned view 不出现，现有 canonical routes 始终可直接使用。 |
| 范围 | 在一个发布单元内完成 `/research` route registration、handler/fallback、`ResearchPage` composition、现有页面 adapter、route navigation 配置、compatibility tests 与 loading/unsupported 边界；同一时刻只 mount 当前目标页面，不构造持续挂载 tabpanel。 |
| 非目标 | 不重写 Chat/Screening 业务逻辑，不实现 E06/E08/E09，不处理 Signal 报告/告警 loop。 |
| 文件所有权 | `App.tsx` route table、新 `pages/ResearchPage.tsx`、`components/research/*`、Research locales/route/compatibility/behavior tests；消费 UI-03A/B/C0/C1，页面只注册 `RouteFocusTarget`，不维护 focus store。 |
...
| rollout/兼容 | 机制 2：route registration + page composition + compatibility tests + 完整 smoke 作为一个原子发布单元，无“先发布 route、后补页面”的中间状态，无运行时 flag。发布后的默认导航结果由 H1/H2/H8 联合矩阵决定。 |
| 验收测试 | PR/base evidence 证明 A-only baseline 没有 `/research` route；C 的持续测试验证 route registration 与有效 page/handler/fallback 同时存在，并覆盖每个现有 view 的 direct、refresh、invalid、auth、Back/Forward；UI-06C0 验证同窗口 title/H1/focus 时序、同 URL PUSH 的新 `location.key`、stable-key restore、duplicate/missing/stale/unfocusable fallback、direct/new-tab 正常初始焦点；AlphaSift 三态；desktop、mobile（390/320）、1440/1024、light/dark、zh/en、keyboard。全部通过才可发布。 |
| 回滚 | 发布前 route/page/adapters/UI-03B consumer/tests 整体回退。发布后 parser/handler/fallback/compatibility tests durable，页面缺陷默认 forward-fix；不得独立回退 `ResearchPage` 而留下可访问但无有效页面的 route。current-route smoke 始终保留。 |
| 前置 / 后置 | 前置：HITL-H2 批准 hub、UI-03A/B、UI-06C0/C1，且从 C0 取得 `App.tsx` 独占窗口；后置：UI-03D/E。发布后只有六槽 + hub 组合交 UI-06B 暴露一级入口；旧平铺 + hub 保持 deep-link only。 |
...
| 验收测试 | 导航后 `0ms` 只断言 context identity；刷新/真实 Link 新标签/Back/Forward 保持 identity；页面更新 title，并只向 UI-06C0 注册 `routeId`、H1 ref、`ready`，由 C0 验证同窗口 focus 和新标签不移当前文档焦点；停止生成后返回；invalid/deleted report；IME 不回归。 |
| 回滚 | report/chat/screening consumer 分别连同 refresh/back/context tests 回退；helper 仅在全部 consumer 退出后删除。已持久化 source metadata/schema 始终 durable；Research parser/handler/fallback 只在 UI-03C 已发布时 durable；legacy route/session/security tests 仍通过。 |
| 前置 / 后置 | 前置：UI-03A、UI-06C0/C1；H2 批准 candidate Research 时再依赖 UI-03C，否则直接在 current `/chat`、`/screening`、`/?recordId=` 上接入。后置：UI-02A/UI-04A 只消费稳定 helper，UI-03F 汇总证据。 |
...
| URL/状态 | 覆盖 current canonical、additive candidate、invalid、Back/Forward、refresh、returnTo 安全边界，以及 UI-06C0 的 PUSH/REPLACE/POP/direct/refresh/new-tab focus 事件分类。 |
| rollout/兼容 | 机制 5：无独立 runtime rollout；始终跑 current canonical compatibility smoke，只有 UI-03C 已发布时才加 candidate route journey；旧 path gate 保留到 HITL 定义的退出条件。 |
| 验收测试 | report-chat-back、screening-context、signal-report-alert-back、任意 signal ID；C0 同窗口 ready H1 focus、同 URL 新 key PUSH、modifier/new-tab 排除、direct/refresh 正常初始焦点、POP restore/fallback；四视口、zh/en、light/dark；发布可访问的脱敏 artifact。 |
| 回滚 | A～E 行为断言跟随各 owner rollback unit；F 只修正错误证据。current/legacy smoke、UI-03C 发布后的 alias fallback/security tests、harness 与历史 artifact durable；不得删除测试掩盖仍支持 contract 的失败。 |
| 前置 / 后置 | 前置：UI-03A/B/D/E 与相应 C0/C1/C2 route gate；只有 H2 批准并由 UI-03C 实际发布 candidate Research 时才要求 candidate journey。H2 不建 hub 时仍须覆盖 current canonical journeys。后置：UI-07 汇总 visual/real-interaction evidence。 |
...
| 测试/截图 | 幂等写操作、timeout-after-commit、partial/stale、任务离开/返回、320 modal/table；UI-06C0 验证同窗口 ready H1 focus、Desktop modifier/new-tab 不移动原文档焦点与 mobile select 能力边界。 |
...
| 前置 / 后置 | 前置：HITL-H3/H5/H6/H7、tracker 登记、UI-03A URL helper、UI-06C0/C1；在 UI-06C2 的 Portfolio/Backtest 独占窗口注册并接入。后置：完整 UI-04 等领域/合规/API contract。 |

### UI-05（父任务）：状态体系统一

父任务保持 tracker 的完整 E18 含义。UI-05A1/A2/B 把公共 state primitive、Usage 状态 pilot、
Usage landmark/focus adoption、页面接入和 aggregate gate 拆成可独立登记、分配、验收和关闭的
候选子任务；A1 或 A2 完成都不得把 UI-05 标记为 Done，也不得用未登记的内部阶段重新打开已关闭任务。

#### UI-05A1 State vocabulary and Usage state pilot

| 字段 | 定义 |
| --- | --- |
| 用户行为 | 用户在 Usage pilot 中能区分 loading、empty、degraded、stale、error 和 retrying，并在可重试错误下保留稳定布局。 |
| 范围 | 冻结六个状态词汇；additive 扩展现有 `EmptyState/Loading/InlineAlert/ApiErrorAlert/DashboardStateBlock` 等公共 state primitive、tests/exports；只把 `TokenUsagePage` 接入状态呈现作为低风险 pilot。 |
| 非目标 | 不新建第二套状态组件，不跨页面批量修改，不改变领域 API fallback；不修改 landmark、`PageHeader`、H1/main 或 route-focus registration。缺少 source/as-of/quality 时只登记 dependency，不在前端伪造。 |
| 文件所有权 | 公共状态 primitive、对应 tests/exports，以及 `TokenUsagePage` 和其状态 tests 的独占 pilot 窗口；完成后释放公共文件和 Usage 页面，不继续编辑其它领域页面。 |
| API/数据依赖 | 复用现有 Usage API；只有权威响应已提供时才展示 source/as-of/quality/limitations，缺失字段进入依赖清单。 |
| URL/状态 | `/usage` identity、period query/本地状态和权限不变；retry 不污染 history；invalid identity 清理遵循 §9。 |
| rollout/兼容 | 机制 3：先提交 additive primitive contract，再提交 Usage consumer；无运行时 flag。Pilot 完成不代表父任务完成。 |
| 验收测试 | primitive contract tests；Usage loading/empty/error/retry/degraded fixtures；zh/en、light/dark、320/390/desktop；区块失败不阻断仍可用内容。 |
| 回滚 | Usage consumer 与其状态 fixture tests 同单元回退；公共 primitive/API tests 在仍有 consumer 时 durable，最后一个 consumer 退出后才可删除。后端 error/schema/parser 与 legacy Usage smoke 不依赖新呈现，可保留。 |
| 前置 / 后置 | 前置：HITL-H7 + tracker 原子登记；后置：释放 common state primitive 给 UI-06C1/D/E 和页面 owner，UI-05B 开始维护 state adoption gate。Usage landmark/focus 另由已登记的 UI-05A2 在 C0/C1 后取得新独占窗口。 |

#### UI-05A2 Usage landmark and route-focus adoption

| 字段 | 定义 |
| --- | --- |
| 用户行为 | `/usage` 始终只有一个页面级 H1 和一个非嵌套 main；同窗口 transition 进入且页面 ready 后由 UI-06C0 执行统一 focus，direct/refresh/new-tab 保持正常初始焦点、不强制移焦。 |
| 范围 | 只在 `TokenUsagePage` 接入唯一 H1、Shell 下非-main `AppPage` variant，以及 `RouteFocusTarget { routeId, headingRef, ready }`；更新对应 DOM/ARIA/registration tests。 |
| 非目标 | 不重新修改公共 state primitive、状态词汇或 Usage API/state presentation；不编辑 `AppPage/PageHeader` 公共实现，不接受或生成 `focusKey`、navigation type、history entry，不建立页面 focus store。 |
| 文件所有权 | `TokenUsagePage` 与其 landmark/focus tests 的独占窗口；公共 state primitive 属 A1，C0/C1 公共文件保持冻结。 |
| API/数据依赖 | 无新 API；只消费 UI-06C0 public target 和 UI-06C1 landmark/PageHeader contract。 |
| URL/状态 | `/usage` identity、period query、权限与 H8 导航归属不变；页面只注册 public target，跨路由 metadata 由 C0 内存管理。 |
| rollout/兼容 | 机制 3：单 route consumer adoption，无运行时 flag；A2 只分配一次、验收一次、关闭一次，不作为 A1 的隐藏第二阶段。 |
| 验收测试 | `/usage` 恰有一个 H1/main、无 nested main、H1 ref/`tabIndex={-1}`/ready registration；public hook 没有 `focusKey`/navigation/history input；C0 focus matrix、UI-05B state gate 与 UI-06C2 route gate 均接受 Usage。 |
| 回滚 | Usage landmark/focus adoption 与 DOM/ARIA/registration tests 同单元回退；C0/C1 在仍有 consumer 时 durable。UI-05B 与 C2 保留 gate 但必须把 Usage 标记未完成；A1 状态呈现不随 A2 回退。 |
| 前置 / 后置 | 前置：UI-05A1 已释放 `TokenUsagePage`、UI-06C0/C1 完成、tracker 已登记 A2；后置：UI-05B 与 UI-06C2 共同验收，随后 UI-06G/UI-07 汇总证据。 |

#### UI-05B Page adoption and aggregate gate

| 字段 | 定义 |
| --- | --- |
| 用户行为 | 各页面按同一状态词汇渐进接入；任一页面未完成时，aggregate gate 明确显示缺口，不宣称 UI-05 完成。 |
| 范围 | 维护 state matrix、共享 fixtures 和总 gate；页面接入严格由既有领域 owner 在其独占窗口完成。 |
| 非目标 | 不与领域 Agent 并发编辑页面文件，不接管页面 composition/API/URL，不用 broad fallback 或 test-only state 伪造权威字段。 |
| 页面所有权 | Home=`UI-02A`；Research 仅在 UI-03C 实际发布时=`UI-03C`，否则按 H2 决定标记 N/A；Chat/Screening=`UI-03D`；Signals/Alerts=`UI-03E`；Portfolio/Backtest=`UI-04A`；Settings=`UI-06F`；Usage state=`UI-05A1`，Usage landmark/focus=`UI-05A2`。 |
| 文件所有权 | UI-05 aggregate matrix、跨页 state fixtures 和 gate tests；UI-05B 不拥有上述领域生产文件。 |
| API/数据依赖 | 各页面沿用自身 API contract；source/as-of/quality 缺口由 owner 记录依赖，UI-05B 不补 schema。 |
| rollout/兼容 | 机制 3：页面 owner 按 §20.1 顺序消费 A1 state primitive；A2 单独完成 Usage landmark/focus。UI-05B 只汇总完成度，无运行时 flag。 |
| 验收测试 | 每个 owner 提供 loading/empty/degraded/stale/error/retrying 的适用矩阵与反例；总 gate 校验 current route、zh/en、light/dark、320/390/desktop，明确不适用项及依据；UI-03C 未发布时 Research 以 H2 决定为 N/A，不阻塞 current Chat/Screening；并确认 A1 Usage state 与 A2 Usage landmark/focus 均通过。 |
| 回滚 | 每页 consumer/tests 跟随该页面 owner 的 rollback unit；UI-05B 保留 gate 但把退出页面标记未完成，不得保留全页通过断言。Matrix/fixture 只因证据本身错误 forward-fix。 |
| 前置 / 后置 | 前置：UI-05A1 primitive/pilot 完成后建立 state gate；Usage 最终通过还依赖 A2 与 UI-06C2。后置：随各领域 owner 顺序关闭 aggregate gate，A1/A2 与所有 owner 都完成后才允许父任务 UI-05 完成。 |

### UI-06 移动导航和无障碍

该任务拆成以下独立 slice。`App.tsx` 或 RouteOutletBoundary 的共享文件按
`UI-06A -> UI-06C0 -> UI-03C` 顺序移交 `App.tsx`，UI-06C1 在 C0 后提供公共 landmark，再进入
page adoption/C2；UI-06B 独占
Shell/Sidebar，D/E/F 各自独占不同 primitive/page，C2 按领域 owner 的文件窗口顺序接入，
G 在生产文件冻结后取证。
不得让两个 Agent 同时编辑同一核心文件。A/B/C0/C1/C2/D/E/F/G 都是待 tracker 登记的 UI-06
...
| 文件所有权 | `App.tsx`、`LoginPage.tsx`、必要 `AuthContext.tsx` 与 auth/login tests；本 slice 完成后释放 `App.tsx` 给 UI-06C0。 |
...
| 前置 / 后置 | 前置：不需要额外产品或 API 决策，但仍受全局 HITL-H1～H8 批准/记录与 tracker 原子登记门禁约束；后置：把 `App.tsx`/RouteOutletBoundary 共享边界交给 UI-06C0；UI-03A 可在独立 helper 文件工作，UI-03C 在 C0 后取得 route table；UI-06G 补全四视口证据。 |
...
| 范围 | Shell breakpoint、Sidebar/Drawer route descriptors、brand/active/unavailable state、mobile opener；取消/Escape 回 opener，成功 route navigation 的目标焦点交给 UI-06C0，C2 只负责验证。可恢复触发点只渲染稳定、页面内唯一的 `data-route-focus-key`，不保存 metadata 或执行 focus。 |
...
| API/数据依赖 | HITL-H1/H2/H8 必须联合记录，另依赖 H3/H5；AlphaSift status 只影响 capability state，不删除 route descriptor。Research hub route 由 H2 控制，一级曝光由 §8.1 联合矩阵控制。 |
| URL/状态 | navigation target 由 descriptor 提供；collapsed/open 只属 UI state；当前 business query 不因跨 breakpoint 清除。 |
| rollout/兼容 | 机制 5：只消费已记录的 H1/H2/H8 tuple。六槽+hub 在 UI-03C 完整 smoke 后加入 Research；六槽+无 hub 在维护者指定替代槽位并更新 IA/tracker 前 blocked；旧平铺+hub 保持 `/research` deep-link only；旧平铺+无 hub 只做获批 bugfix。随后独立提交 responsive/brand correctness；若要求分受众 rollout，先写 ADR，本 slice 保持 blocked。 |
| 验收测试 | 四行联合矩阵、禁止空 Research 槽、禁止一级 More+Usage、缺替代槽位和未列 tuple 均 fail closed；1440/1024/768/390/320、1440@200%、zh/en、light/dark、reduced motion；Drawer 取消/Escape/inert 回 opener，稳定 marker 由 C0 捕获，选中 route 后由 C0 执行、C2 验证 ready H1 focus；Home gauge 无 occlusion。 |
...
| 前置 / 后置 | 前置：HITL-H1/H2/H8 tuple 与其它 HITL 决定、tracker 登记；如联合结果允许 Research 一级入口，还需 UI-03C 已原子发布并通过完整 smoke。后置：UI-06G compatibility evidence。 |
...
// Coordinator-internal only. Pages and page hooks cannot write this type.
...
页面唯一公开调用形状是 `useRouteFocusTarget(target: RouteFocusTarget): void`；该 hook 不接受
`focusKey`、navigation type 或 history entry。`RouteFocusEntry` 只由 coordinator 创建和维护，
不是页面 API，也不提供可让页面写入 internal metadata 的 registration wrapper。

推荐公共边界：

```text
components/routing/RouteFocusCoordinator.tsx
hooks/useRouteFocusTarget.ts
```

| 字段 | 定义 |
| --- | --- |
| 用户可见结果 | 同窗口前进导航在目标内容 ready 后稳定聚焦 H1；Back/Forward 恢复离开前的稳定触发点，无法恢复时 fallback H1；direct、refresh、new-tab 不被强制移焦。 |
| 唯一职责 | 结合 React Router navigation type 与 `location.key` 统一分类 PUSH、REPLACE、POP 与 initial/no-transition；direct、hard refresh、new-tab 均属 no-transition，但保留独立 journey。同 URL 的重复 PUSH 仍按新 key 作为独立 transition。等待 ready/title/H1 后执行 focus；只捕获带稳定、当前页面唯一 `data-route-focus-key` 的可聚焦触发点，并在内存中创建/校验 `RouteFocusEntry`。 |
| 页面职责 | 页面只调用 `useRouteFocusTarget({ routeId, headingRef, ready })`；不得提交 key、navigation type 或 history entry，不得判断 PUSH/POP、保存第二套 focus store 或调用跨路由 `focus()`。Shell、Sidebar 与 WorkspaceNavigation 也只可渲染稳定 marker，不捕获、存储或恢复。 |
| Key / fail-closed | marker 禁止使用数组 index、翻译文本或动态 DOM 顺序。重复、缺失、失效或不可聚焦的 key 全部 fail closed 到 ready H1；REPLACE 丢弃失效的 destination entry，不把旧 entry 带入替换后的目的地。 |
| 排除行为 | modifier click、中键、context-menu new-tab 不改变当前文档焦点；direct load、hard refresh、new tab 不执行 transition focus。Transient metadata 只存 coordinator 内存，不写 URL、history state、`localStorage` 或 `sessionStorage`，history/store 中不保存 DOM node/ref。 |
| 文件所有权 | 新 `components/routing/RouteFocusCoordinator.tsx`、`hooks/useRouteFocusTarget.ts`、tests/exports，以及在 `App.tsx` 或既有 RouteOutletBoundary 挂载 coordinator 的独占窗口；页面、Shell、Sidebar、`WorkspaceNavigation` 均不直接调用跨路由 `focus()`。 |
| API/数据依赖 | 无新 API；只消费 React Router location/navigation contract 和 C1 将提供的稳定 H1 ref。 |
| URL/状态 | 不改变业务 URL。`location.key` 与 coordinator 内存中的 `RouteFocusEntry` 是 transition 真源；页面业务 state、URL parser、history state 和任何 storage 不复制该状态，DOM ref 不进入 entry。 |
| rollout/兼容 | 机制 3：先建立 coordinator/hook contract，再由 C1 提供稳定 H1/ref，最后由页面 owner 顺序注册；未注册 route 保持现状，无运行时 flag。 |
| 验收测试 | PUSH、REPLACE、同 URL 新 key PUSH、POP Back/Forward、direct、hard refresh、new-tab、modifier/middle/context-menu；valid stable-key restore；duplicate/missing/stale/unfocusable -> ready H1；REPLACE 清除 invalid destination entry；hook 类型拒绝 `focusKey`/navigation/history input；URL/history state/localStorage/sessionStorage 均无 metadata，entry/store 均无 DOM ref；ready/title/H1 时序，H1 不进普通 Tab。 |
| 回滚 | Coordinator/hook 与 event-classification tests 同单元；每页 registration/tests 跟随页面 owner 回退。若仍有 consumer，public contract durable；aggregate gate 必须把退出 route 标为未完成。 |
| 前置 / 后置 | 前置：UI-06A 已完成并释放 `App.tsx`/RouteOutletBoundary；UI-03A helper 不占此文件。后置：UI-06C1，再由 UI-03C 或各页面 owner 顺序接入并由 UI-06C2 验收。共享边界禁止并行编辑。 |
...
| URL/状态 | 无 URL change；primitive 不读 Router、不判定 navigation type/ready、不更新 title，也不直接调用 `focus()`；只向页面提供可注册给 UI-06C0 的稳定 H1/ref 能力。 |
...
| 前置 / 后置 | 前置：UI-05A1 已释放 common files、UI-06C0、tracker 登记；后置：UI-06C2 与各 page owner，UI-05A2 取得 Usage 独占窗口。 |

#### UI-06C2 Route-by-route landmark adoption and gate

| 字段 | 定义 |
| --- | --- |
| 用户可见结果 | 每个 current route 恰有一个 `main` 和可访问 H1；页面注册 ready target 后由 C0 执行唯一 transition/restore contract；表单控件不只依赖 placeholder。 |
| 范围 | 按 owner 独占窗口顺序迁移 Home、Chat、Screening、Portfolio、Signals、Backtest、Alerts、Usage、Settings、Login、404；现有 `AppPage` consumer 在 Shell 下显式采用 C1 的非-main variant；页面只注册 `RouteFocusTarget`，C2 维护 title/H1/registration/focus 结果总 gate，C0 独占 event 分类、marker 捕获、entry store 和恢复。UI-03C 实际发布 `/research` 后再加入 Research candidate route。 |
| 非目标 | 不在 C2 重做页面 composition/API/业务状态，不让 C2 Agent 与页面 owner 并发编辑同一文件，不以 candidate `/research` 替代遗漏的 `/chat` 或 `/screening`；C2 不接收/捕获 `focusKey`，不复制分类或建立第二套 store。 |
| 文件所有权 | Login=`UI-06A`；Research=`UI-03C`；Chat/Screening=`UI-03D`；Home=`UI-02A`；Signals/Alerts=`UI-03E`；Portfolio/Backtest=`UI-04A`；Settings=`UI-06F`；Usage=`UI-05A2`；`NotFoundPage` 由 C2 取得隔离窗口。各 page owner 更新 title/H1，并只注册 `RouteFocusTarget`；C2 维护总 gate/fixtures，不抢占 owner 文件。 |
| API/数据依赖 | 无新 API；依赖 C0/C1 contract 与页面 owner 的稳定 composition。 |
| URL/状态 | 不改变业务 route/query、filters 或 task。C0 结合 navigation type/`location.key` 处理 PUSH/REPLACE、POP、direct/refresh/new-tab，并在内存管理 internal entry；页面和 C2 不复制分类、marker capture 或 focus store。 |
| rollout/兼容 | 机制 3：逐 route consumer 接入，无运行时 flag；任何 route 未完成时 C2 状态保持未完成。 |
| 验收测试 | 每个 route 恰有一个 `main`/H1，控件有 label，页面 hook 只接收 `RouteFocusTarget`；C0 的同窗口 ready/title/H1、同 URL 新 key PUSH、valid-key restore、duplicate/missing/stale/unfocusable fail-closed、REPLACE cleanup、modifier/new-tab 排除、direct/refresh、Back/Forward 全矩阵；C2 只验证结果，不实现 store；zh/en、320/390/desktop、原生 200% zoom、ARIA snapshot/axe + 人工键盘；Research 只在 UI-03C 实际发布时加入。 |
| 回滚 | 每 route adoption 与其 DOM/axe/registration tests 同单元回退；C0/C1 在仍有 consumer 时 durable。Aggregate gate 保留但必须把该 route 标记未完成；不得保留被回退 route 或“全路由完成”的通过断言。 |
| 前置 / 后置 | 前置：UI-06C0/C1；与 UI-06A、UI-03C/D/E、UI-02A、UI-04A、UI-05A2、UI-06F 的文件窗口串行。C2 验收 A2 的 Usage adoption；全部 route 通过后才释放 UI-06G/UI-07 最终证据。 |
...
| 前置 / 后置 | 前置：UI-05A1 common ownership 完成；后置：UI-03E/UI-04A consumer adoption，UI-06G visual evidence。 |
...
| 前置 / 后置 | 前置：UI-05A1 common ownership；后置：Provider/Settings owner 释放文件窗口后单独迁 LLM menu，UI-02A 完成后迁 Home strategy menu。 |
...
| 字段 | 定义 |
| --- | --- |
| 用户可见结果 | 无新增行为；提供 Shell、Login、Settings、C0 focus coordinator 与公共 primitive 的可访问复验证据和剩余人工风险。 |
| 范围 | Playwright/ARIA/axe/visual evidence、四视口、200% zoom、reduced motion、人工读屏 checklist、C0 public/internal API 边界、route-boundary event/key/fail-closed/memory-only 矩阵与 artifact hygiene。 |
| 非目标 | 不用 test-only CSS 掩盖 overflow，不 mock 掉 auth/router/overlay 风险，不把空 DOM 当 Login 证据。 |
| 文件所有权 | UI-06 专用 `e2e/*`、accessibility fixtures 与 artifact index；A/B/C0/C1/C2/D/E/F production files 冻结。 |
| API/数据依赖 | 隔离 backend/Vite/fake provider；脱敏 auth/setup fixture；不读取开发者 credentials。 |
| URL/状态 | 当前 routes + UI-03C 实际发布的 additive candidate；Login redirect、Back/Forward 与 invalid params。C0 metadata 不出现在 URL/history state/storage。 |
| rollout/兼容 | 机制 5：无独立 runtime rollout；CI 始终跑 current canonical，UI-03C 发布后才跑 additive candidate compatibility matrix；bugfix 路径始终跑。 |
| 验收测试 | `/login` 补 1440/1024/390/320、light/dark、zh/en、first-run/returning-admin、keyboard/reduced-motion/200%；Settings 15 views；执行 PUSH/REPLACE、同 URL 新 key PUSH、POP、direct/refresh/new-tab/modifier focus 事件矩阵；valid marker restore，duplicate/missing/stale/unfocusable fallback，REPLACE cleanup，public hook 仅接 target，metadata memory-only 且无 DOM ref；人工读屏结果单列。证据须脱敏并通过 PR 附件、Actions artifact 或其它可访问 URL 发布。 |
| 回滚 | A/B/C0/C1/C2/D/E/F 行为 tests 跟随各 owner rollback unit；G 只聚合并因证据错误修正。harness、current/published-route compatibility、安全 tests 与历史 artifact durable；不保留已回退 auth/layout/landmark 行为断言。一次性 PNG 留 PR/artifact，不入仓库。 |
| 前置 / 后置 | 前置：UI-06A/B/C0/C1/C2/D/E/F；后置：UI-07 汇总跨 slice visual gate。 |

### UI-07 视觉回归与真实交互证据

| 字段 | 定义 |
| --- | --- |
| 用户行为 | 无新增产品行为；证明已获批的 UI-02A、UI-03A～F、UI-04A、UI-05A1/A2/B 与 UI-06 slices 在真实后端/fixture 下可完成；不冒充完整 UI-02/03/04/05 已交付。 |
| 范围 | Playwright journeys、visual snapshots、contrast/occlusion、artifact hygiene；不在测试中重做业务实现。 |
| 依赖 | 稳定脱敏 fixture、四视口、fake provider、current routes、UI-03C 实际发布的 candidate、UI-05A1/A2、UI-06C0 event classification 与 §17.2 rollout contract。 |
| 主要文件 | `e2e/*`, `playwright.config.ts`, test fixtures, artifact scanner；一次性图片不入库。 |
| URL/状态 | 覆盖 canonical + legacy + invalid + Back/Forward，以及 C0 对 PUSH/REPLACE/POP/direct/refresh/new-tab 的分类、internal entry 与无 URL/history/storage metadata 契约。 |
| 测试/截图 | 12 条关键流程、全状态、light/dark zh/en、长内容、读屏人工记录；包含 A1 Usage state、A2 Usage landmark/focus、ready H1、同 URL 新 key PUSH、valid marker restore、duplicate/missing/stale/unfocusable fallback、REPLACE cleanup、modifier/new-tab 排除、direct/refresh 正常初始焦点、public/internal API 边界及无 DOM ref；每个实施 PR 发布可访问的脱敏前后对比。 |
| rollout/兼容 | 机制 5：始终跑 current canonical compatibility smoke；只有 UI-03C 实际发布后才跑 additive candidate，不构造 flag matrix。 |
...
文件所有权总门禁：UI-05A1 先独占 common state primitives 与 `TokenUsagePage` 状态 pilot，释放后
UI-05A2 只在 C0/C1 完成后重新取得 `TokenUsagePage`/tests 独占窗口完成 landmark/focus；UI-05B 只维护
matrix/fixtures/aggregate gate。`App.tsx` 或 RouteOutletBoundary 严格按
`UI-06A -> UI-06C0 -> UI-03C` 移交；UI-03A 只改 helper，不占共享 route table。UI-06C1 在 C0 后
独占 `AppPage/PageHeader`，页面 owner 再注册 C0/C1，UI-06C2 只维护总 gate；UI-06B 独占
Shell/Sidebar。UI-02A 独占 Home；UI-03C/D/E 分别独占 Research 原子 route/page publication、
Chat/Screening context、Signals/Alerts loop；UI-04A 独占
Portfolio/Backtest；UI-06F 在 Provider/Settings owner 释放相关文件后取得 Settings 独占窗口；404
由 C2 取得隔离窗口。Evidence slice 在生产文件冻结后运行，所有共享边界禁止并行编辑。
...
HITL-H1、HITL-H2、HITL-H8 必须引用 §8.1 的同一行作为一个 tuple 联合记录；不得分别批准后由
Agent 拼接结果。未列 tuple 或六槽 + 无 Research hub 且没有明确替代槽位时，UI-06B 保持 blocked。

| ID | 未决决定 | 推荐方案 | 备选方案 | 影响 | 阻塞 slice |
| --- | --- | --- | --- | --- | --- |
| `HITL-H1` | Shell 一级 IA 与 zh/en canonical terms | 六槽：`今日/Today · 研究/Research · 组合/Portfolio · 信号/Signals · [More 或 Usage，由 H8 恰选其一] · 设置/Settings` | 保持当前 8～9 个功能入口平铺 | 必须与 H2/H8 联合选择 §8.1 矩阵行。六槽 + 无 hub 未指定替代槽位时 blocked；旧平铺 + hub 也不把 Research 加入默认一级导航；禁止空槽和同时一级 More+Usage | UI-02A label/entry、UI-03C、UI-04A label、UI-06B；UI-05A1/A2/B 可设计但仍不得开工 |
| `HITL-H2` | 问股、选股与 Research hub | 建 hub：UI-03A 只建 helper，UI-03C 原子发布 route/page/smoke；当前 `/chat`、`/screening`、`/?recordId=` 保持 canonical | 不建 hub：UI-03A 只保留通用 Context/URL helper，UI-03C 跳过 | 与 H1/H8 联合决定默认导航：六槽+hub smoke 后曝光；旧平铺+hub只 deep link；六槽+无 hub 等明确替代槽；旧平铺+无 hub 只做 bugfix。触发点可提供稳定 marker，目标页只注册 `RouteFocusTarget` | UI-03A～D、UI-06B、UI-06C0/C2 |
...
| `HITL-H6` | 新旧 Web path 策略与退出条件 | UI-02～UI-07 期间 `/chat`、`/screening`、`/?recordId=` 等 current paths 继续 canonical；UI-03C 发布的 `/research` 只 additive，允许的 UI-06B 曝光也不构成 cutover；退出条件基于实际使用、兼容测试和另一次 HITL | 立即把 `/research` 等设为 canonical，并 replace redirect legacy URL | H6 是 canonical 切换和旧路径退出的唯一后续 gate；推荐风险最小、可回滚但双入口期更长，备选可能破坏书签/Desktop/通知链接 | UI-03C/F、UI-04A compatibility、UI-06B/G |
| `HITL-H7` | 执行顺序、rollout 与文件所有权 | 批准 §17 父子身份、H1/H2/H8 tuple、无虚构 flag、tracker 原子登记和独占顺序：A1 -> B gate + D/E -> UI-06A -> UI-03A helper -> C0 -> C1 -> A2/UI-03B -> [H2 hub 时 UI-03C 原子发布] -> UI-06B 矩阵结果 -> domain owner -> B/C2 gate -> evidence | 仍按 UI-02～UI-07 六个大任务逐项托管，或先建设未定义的通用 flag 系统 | A1/A2 各只分配一次；`App.tsx` 按 UI-06A -> C0 -> UI-03C 移交。未登记 tuple/替代槽位、未列组合或共享文件未释放时不得分配 UI-06B/对应 owner | UI-02A、UI-03A～F、UI-04A、UI-05A1/A2/B、UI-06A/B/C0/C1/C2/D/E/F/G、UI-07 全部 |
| `HITL-H8` | Usage / More 归属 | 六槽 tuple 中 `/usage` 保持独立 route，并放在 More disclosure；不创建空 `/more`，不并入 Settings | 六槽 tuple 中 Usage 直接占第五槽并替换 More | 与 H1/H2 联合记录，六槽恰选 More/Usage 之一；旧平铺保留当前一级 Usage、无 More。Mobile 取消/Escape 回 opener；More/Usage 触发点可暴露稳定 marker，UI-05A2 的 Usage 目标页只注册 `RouteFocusTarget`，C0 内部恢复。route/API/auth/page identity 不变 | UI-05A2、UI-06B/C0/C2 |
...
`HITL-H1`～`HITL-H8` 的批准结论已记录于 §18.1，`stockpulse-work-tracker.md` 已于 2026-07-17
原子登记新增子任务；自此 UI-02A、UI-03A～F、UI-04A 与 UI-05A1/A2/B、UI-06A/B/C0/C1/C2/D/E/F/G
可按 §17 顺序与独占文件窗口分配。UI-02～UI-07 父任务仍按各自完成条件推进，不得因子任务完成标记 Done。

### 18.1 HITL 批准记录（2026-07-17）

维护者（SiinXu）于 2026-07-17 对 HITL-H1～H8 记录如下批准结论。H1、H2、H8 按 §8.1 联合矩阵
第 1 行（六槽 IA + Research hub + More，标注「推荐」）作为一个 tuple 联合记录。同日已在外部
`stockpulse-work-tracker.md` 原子登记 UI-02A、UI-03A～F、UI-04A、UI-05A1/A2/B 与
UI-06A/B/C0/C1/C2/D/E/F/G 的父子范围、状态与执行顺序。

| ID | 批准结论 |
| --- | --- |
| `HITL-H1` | 批准推荐方案：六槽一级 IA `今日/Today · 研究/Research · 组合/Portfolio · 信号/Signals · 更多/More · 设置/Settings`；第五槽由 H8 选定为 More，不与一级 Usage 并存。与 H2/H8 引用 §8.1 联合矩阵第 1 行联合记录。 |
| `HITL-H2` | 批准推荐方案：建 Research hub。UI-03A 只建 helper，UI-03C 原子发布 route/page/smoke；`/chat`、`/screening`、`/?recordId=` 在发布并通过 smoke 前保持 canonical。tuple 引用同 H1。 |
| `HITL-H3` | 批准推荐方案：一级 canonical term 采用「组合 / Portfolio」；当前 `/portfolio`、`/backtest` 为其二级；未来总资产/配置/再平衡/情景归完整 UI-04，数字资产研究归 UI-03，达到 Beta 后再显示。 |
| `HITL-H4` | 批准推荐方案：登记前置子任务 UI-02A（current-data Home 层级、view URL、1024 修复）；完整 UI-02 继续等待 E04 Today aggregation、服务端排序原因、区块错误与已读/处理 contract。UI-02A 完成不代表 UI-02 Done。 |
| `HITL-H5` | 批准推荐方案：Signals 一级下放 Decision Signals、Alerts 与单 signal outcome；历史 AI 建议 Backtest 保留在 Portfolio 二级；所有现有 path 保留。 |
| `HITL-H6` | 批准推荐方案：UI-02～UI-07 期间 `/chat`、`/screening`、`/?recordId=` 等 current paths 继续 canonical；UI-03C 发布的 `/research` 只 additive；canonical 切换与旧路径退出须另行 HITL。 |
| `HITL-H7` | 批准推荐方案：§17 父子任务身份、H1/H2/H8 tuple、无虚构 flag、tracker 原子登记与独占执行顺序：A1 -> B gate + D/E -> UI-06A -> UI-03A helper -> C0 -> C1 -> A2/UI-03B -> UI-03C 原子发布 -> UI-06B 矩阵结果 -> domain owner -> B/C2 gate -> evidence。 |
| `HITL-H8` | 批准推荐方案：六槽 tuple 中 `/usage` 保持独立 route 并放入 More disclosure；不创建空 `/more`，不并入 Settings。route/API/auth/page identity 不变。tuple 引用同 H1。 |

以上记录不改变外部前置状态：D-01～D-09、E04/E06/E08/E10/E12/E18 薄规格与合规批准仍按 §18
外部前置表推进，不由本记录代批。

## 19. 截图、录像与 artifact 索引

临时证据位于 `.context/ui01-artifacts/`，不加入 Git。共 49 个文件（47 PNG + 2 JSON），
约 4 MB，无录像。该路径只存在于审计者本机，当前 PR 不能独立复现或访问这些文件；下表结论
均是固定基线上的 local observations，不是远端可复核证据，也不得为其伪造 artifact URL。
...
泄密处理：fixture 在 import 前清空 ambient product credentials，截图中无真实持仓/聊天/密钥；
设置概览路径图已删除。两份 JSON 在固定审计基线的 `scripts/scan_playwright_artifacts.py`
下通过（2 files scanned）；实施前必须使用当时目标分支的最新 scanner 重跑，不能把固定基线结果
表述成最新检查。PNG/JPEG/WebM 被 scanner 按策略直接拒绝，不能声称“图片扫描通过”；图片只做
人工检查，是本地临时审计材料，不构成当前 docs PR 的持久附件。后续每个 UI 实施 PR 必须重新
生成、脱敏并通过 PR 附件、Actions artifact 或其它可访问链接发布自己的证据；临时文件不得提交
仓库，本地 bundle 也不能替代未来 PR 的可访问证据。
...
1. `UI-05A1`：冻结 additive state vocabulary/public primitives，在 `TokenUsagePage` 独占窗口只完成状态呈现 pilot；不改 landmark/PageHeader/focus。完成不代表 UI-05 Done。
2. `UI-05B`：建立 matrix/fixtures/aggregate gate 后只跟踪页面 owner；`UI-06D/E` 可在 UI-05A1 释放 common files 后分别完成 Pagination、Menu/Popover primitive。
3. `UI-06A`：收敛 post-auth navigation owner 后立即释放 `App.tsx`；C0/C1 完成后，UI-06A 只重新取得 `LoginPage`/tests 窗口完成 Login registration 与 C2 验收，不再占用 shared routing boundary。
4. `UI-03A`：H2 建 hub 时建立 research parser/normalizer/`buildTo`/return validation/current-legacy helper；H2 不建 hub 时只建立中性 current-path Context/URL helper，不保留 Research view schema/专用命名。两者都不编辑 `App.tsx` 或注册 `/research`。
5. `UI-06C0`：从 UI-06A 取得 `App.tsx` 或 RouteOutletBoundary，冻结 public `RouteFocusTarget`、internal `RouteFocusEntry`、navigation type + `location.key`、stable marker、fail-closed、REPLACE cleanup 与 memory-only/no-DOM-ref contract。
6. `UI-06C1`：在 C0 后完成 landmark/PageHeader primitive；`UI-03B` 随后建立 `<nav>` + Router Link primitive。页面 adoption/C2 只能从此处开始。
7. `UI-05A2`：只取得 `TokenUsagePage`/tests 窗口，接入唯一 H1、非嵌套 main 和 `RouteFocusTarget`；不重开 A1 或修改 state primitive，由 UI-05B/C2 验收。
8. H2 批准 hub 时执行 `UI-03C`：从 C0 取得 route table，把 `/research` registration、`ResearchPage` composition、handler/fallback、compatibility tests 与完整 smoke 原子发布；H2 不建 hub 时跳过。
9. `UI-06B` 按已记录 tuple 分支：六槽+hub 仅在 C smoke 后曝光 Research；六槽+无 hub 在替代槽位获批并更新 IA/tracker 前 blocked；旧平铺+hub 保持 deep-link only；旧平铺+无 hub 只做获批 bugfix。H8 在六槽中恰选 More/Usage，禁止推断未列组合。
10. `UI-03D`：先稳定 ContextLink，再顺序接 Report/Chat/Screening、C0/C1 与 C2 gate；这必须早于 Home/Portfolio 消费 helper。
11. `UI-02A`：只做 current-data Home hierarchy、1024 修复与 C0/C1/C2 adoption；完整 E04 仍属父任务 UI-02。
12. `UI-03E`：关闭 signal/report/alert loop，并完成 C0/C1/C2 Signals/Alerts adoption；不实现 E06 timeline/Evidence。
13. `UI-04A`：只稳定当前股票 Portfolio/Backtest、URL、task tracking 与 C0/C1/C2 adoption；完整配置/再平衡仍属 UI-04。
14. `UI-06F`：取得 Provider/Settings 文件独占窗口，修复 15 个 Settings view 并完成 C0/C1/C2 Settings adoption。
15. `UI-06C2` 在隔离窗口完成 404 并验收 A2/所有 route；`UI-05B` 同时验收 A1/A2 并关闭 state aggregate gate。任一 current route 未完成时对应父任务不得 Done。
16. `UI-03F`、`UI-06G` 和 `UI-07`：汇总 current、UI-03C 实际发布的 additive route、legacy、visual 与真实 a11y/interaction 证据，并发布可访问的脱敏 artifact。
...
- 当前 UI-01 artifact 只在本地 `.context`，不能由 PR 独立复现；相关结论只是 local observations。未来 UI 实施 PR 必须重新生成并发布可访问证据，不能引用或伪造本地 URL。
...
- UI-02A、UI-03A～F、UI-04A、UI-05A1/A2/B 与 UI-06A/B/C0/C1/C2/D/E/F/G 仍未登记到外部 tracker；HITL 后必须原子登记，登记前不能分配。
...
当前 correction contract 默认 **forward-fix**。不得建议单独 revert `569e99a3`，因为那会恢复
已知错误的 rollback/focus 契约；若发现新的文档错误，新增一个 docs correction commit 并保持
PR body 与最终 head 一致。只有维护者决定完全放弃 UI-01 候选时，才 revert 整个 PR #8。

未来生产 UI slice 仍按 §17.2 把实现与只有该行为存在时才通过的测试作为同一 rollback unit；
需要受众灰度时先立 rollout ADR，不共享不可逆 migration。文档 forward-fix 不改变生产 slice 的
correctness/security 紧急回退边界。
...
- 当前审批状态（Approved，HITL-H1～H8 批准结论见 §18.1）；除 §18.1 记录外，不得把绿色辅助 job 表述为维护者或 AI 批准。
- 初次 runtime audit 与当前 docs-only checks 的验证来源；完整 main CI 未触发时必须明确。
- §19 的证据覆盖和未验证项；明确本地 audit evidence 不能由 PR 独立复现，不伪造 URL；未来 UI 实施 PR 必须发布自己的可访问脱敏证据。
- §17.2 的 rollback unit/durable assets、§18 的 HITL-H1～H8、tracker 原子登记门禁。
- 明确 `/research` 的 route/page/smoke 唯一原子 publication owner 是 UI-03C；UI-03A 没有 route
  registration，parser/handler/fallback/compatibility tests 只在 C 对外开放 route 后 durable。
- 明确 UI-05A1/A2 的独立 tracker 身份和唯一执行窗口、Route Focus public/internal API 边界，以及
  已批准的 H1/H2/H8 tuple；如为六槽 + 无 hub，必须记录维护者指定的替代槽位。
- 文档错误默认新增 correction commit；不建议单独 revert `569e99a3`，只有维护者放弃整个 UI-01 候选时才 revert PR #8 全部。
