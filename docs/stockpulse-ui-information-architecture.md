...
| --- | --- | --- | --- | --- | --- |
| `stockpulse-work-tracker.md` | §1、§5、§8；UI-01=`Next`，UI-02～UI-07=`Planned` | **任务身份已对齐**：UI-02/03/04/05/06 保持 tracker 的完整父任务定义；当前能力前置工作另记为 UI-02A、UI-03A～F、UI-04A、UI-05A/B 与 UI-06A/B/C0/C1/C2/D/E/F/G | 旧稿曾用父任务 ID 指 current-only 工作，造成同一 ID 跨文档含义漂移；tracker 也尚未登记这些候选子任务 | tracker 继续作为工作状态唯一索引；HITL 后先原子登记父子范围与状态，再允许分配子任务；UI-01 不直接修改外部 tracker | **是**：批准 UI-01、子任务登记和执行顺序；不是由审计自行改 tracker 状态 |
...
| `stockpulse-product-enhancement-prd.md` | E18 一致空状态与降级体验 | **一致**：UI-05 是增强而非重建 | 现有公共状态组件是基础，但缺跨页词汇、source/as-of/quality 的完整 API 映射 | UI-05A 先扩展 additive primitive 并以 Usage 作单页 pilot；UI-05B 只维护页面接入矩阵、fixture 与总 gate；缺字段时显式列依赖，不用 broad fallback | **否**（方向已由 current code + PRD 约束）；执行仍受 UI-01 gate 阻断 |
...
Research 一级入口使用以下唯一生命周期；上图只表示 H2 获批且生命周期完成后的目标状态：

1. `UI-03A` 只建立 parser、additive `/research` route 和 current-path 兼容层。
2. `UI-03C` 验收前，默认 Shell 不显示 Research 一级入口。
3. `UI-03C` 先通过 direct、refresh、invalid、auth、mobile、desktop、title/H1/focus smoke。
4. 只有上述 smoke 通过后，`UI-06B` 才把 Research 一级入口加入默认导航，目标固定为 `/research`。
5. `/chat`、`/screening`、`/?recordId=` 在整个阶段继续是 current canonical 和可直达路径；
   主导航指向 `/research` 不等于旧路径 redirect、失效或完成 canonical cutover。
6. canonical 切换与旧路径退出仍由 `HITL-H6` 根据兼容证据另行决定。
7. 若 `HITL-H2` 不批准 `/research`，`UI-06B` 不显示空 Research 槽位，并采用 H2 备选导航。
...
- **实施归属**：`UI-03C` 先独占 Research 页面内容并通过完整 smoke；随后 `UI-06B` 才在独占
  `Shell/SidebarNav/navigation descriptors` 窗口暴露 `/research`。两者不并行，H2 不批时不留空槽。
...
- **验收证据**：light/dark、涨/跌/中性/风险截图与非颜色 label；源码扫描只允许 semantic token，
  合法固定 hex 测试 fixture 必须有显式白名单和颜色语义说明。
...
- **实施归属**：UI-05 父任务后续 action primitive；不属于 UI-05A 的 Usage state pilot 或 UI-05B
...
| Screening | `/research?view=screening` | `MERGE` | 保留 AlphaSift、热点、任务、候选 | 现有 API；实验状态显式 | `/screening` 保留；真实 `ALPHASIFT_ENABLED` 只控制领域能力，不隐藏 route identity | Advanced filters 可折叠 | UI-03A～F；UI-05B aggregate gate |
...
| Usage | `/usage`（推荐 More，待 H8） | `DEMOTE` 候选 | 保留独立 dashboard/page identity，不并入 Settings | 现有 API | path、权限和收藏保持；More 只改变发现方式 | Desktop Sidebar / mobile Drawer disclosure | HITL-H8 + UI-05A/UI-06B/C2/G/UI-07 |
...
| Research hub | `/research`（候选；当前 paths 仍 canonical） | `DEFER` | 可先组合现有单资产、报告、问股、选股；时间线只作 future 归属 | 现有页面/API 足够做 shell；E06 timeline 需 ResearchEvent/游标 contract | UI-03A 只建 additive route；UI-03C smoke 全过后 UI-06B 才加入默认导航；H6 另决 canonical cutover；不预建空 timeline | Desktop workspace/mobile page | UI-03A～F + UI-06B；完整能力仍属 UI-03 |
...
| 2 | 研究 / Research | search/chart | `/research` | 单资产、报告、历史、时间线、问股、选股 | UI-03A 只建 additive route；UI-03C 完成 direct/refresh/invalid/auth/mobile/desktop/title/H1/focus smoke 后，UI-06B 才显示本项。此前 current paths 保持；H2 不批时用备选导航且不显示空槽位 |
...
- 若 HITL-H2 批准 `/research`：UI-03A 先只发布 parser/additive route/兼容层；UI-03C 验收前默认
  Shell 不显示 Research。UI-03C 的 direct、refresh、invalid、auth、mobile、desktop、title/H1/focus
  smoke 全部通过后，UI-06B 才把 `/research` 作为默认导航目标。
- `/chat`、`/screening`、`/?recordId=` 在上述导航暴露后仍是 current canonical、可直达且不自动
  ...
  基于兼容使用证据另行批准。若 H2 不批准，UI-06B 不显示空 Research 槽位并采用 H2 备选导航。
...
继续由 Settings draft/leave guard 管理。Route focus metadata 只由 UI-06C0 按 history entry
保存在内存中，不写 `localStorage`/sessionStorage，也不由页面建立第二套真源。
...
- `UI-06C0 RouteFocusCoordinator` 是跨路由焦点分类、history-entry focus store 和执行时序的唯一真源；
...
- PUSH/REPLACE 的同窗口完整页面导航由 coordinator 等待目标 `ready`，确认最终本地化 H1 与
  `document.title` 已更新后聚焦 H1。POP/Back/Forward 优先恢复该 history entry 离开前的稳定
  `focusKey`，找不到目标节点时才 fallback 到 ready H1；瞬态 focus state 不写 `localStorage`。
- Cmd/Ctrl-click、中键和“在新标签打开”不得触发当前文档移焦；direct load、hard refresh 和新标签页
  使用页面正常初始焦点，不执行 transition focus。页面只注册 `routeId`、H1 ref 和 `ready`；
  `WorkspaceNavigation`、Shell/Sidebar 和页面组件均不直接调用跨路由 `focus()`。
...
- Route focus：只扩展 `RouteFocusCoordinator/useRouteFocusTarget` 这一套 typed contract；页面只注册
  `routeId`、H1 ref、`ready`，PUSH/REPLACE/POP/initial-no-transition 分类与 history-entry focus restore
  归 coordinator；direct/refresh/new-tab 是分别验证的 no-transition journey。
...
- 页面私自判断 PUSH/POP、维护第二套 focus store、用 `localStorage` 持久化瞬态 route focus，或由
  `WorkspaceNavigation`/Shell 直接调用跨路由 `focus()`。
...
| Research shell、现有报告/问股/选股归组 | history/chat/AlphaSift APIs 已有 | HITL + tracker 登记后先做兼容层；默认导航仍 blocked | UI-03A 只建 route/parser；UI-03C 完整 smoke 后 UI-06B 才曝光；只是完整 UI-03 的 current-capability preparation |
...
| State vocabulary、responsive Pagination/Menu/Landmark/route focus | 无新 API | HITL + tracker 登记后可做 | UI-05A/B、UI-06C0/C1/C2/D/E/F |
...
| `UI-05` | 完整 E18 跨页面状态体系；所有 owner 采用统一 loading/empty/degraded/stale/error/retrying 语义并通过总 gate | `UI-05A/B` State primitives/Usage pilot 与 page adoption/aggregate gate | UI-05A pilot 完成不代表 UI-05 Done；仍需所有领域 owner 接入并由 UI-05B 证明完整 matrix |
...
> UI-03A～F、UI-04A、UI-05A/B 与 UI-06A/B/C0/C1/C2/D/E/F/G 的父子范围、状态、依赖和执行顺序。
...
| UI-03A Research routing | 否 | 2. Additive route，不进入默认导航 | `App.tsx` route table、共享 parser 与 route tests | `/chat`、`/screening`、`/?recordId=` 仍 current canonical；`/research` 只可深链 | Web route；API 不变；Desktop 同 Web | A 只建 parser/route/兼容层；UI-03C smoke 全过后 UI-06B 才能暴露入口 | 未发布前 route/parser/tests 可整体回退；发布后 handler/parser/fallback 与 tests durable，只回退页面/导航曝光。legacy direct 与 returnTo security tests 在旧 canonical routes 下仍通过 |
...
| UI-03C Research composition | 否 | 2. Additive deep-link route；smoke 通过前不进默认导航 | `ResearchPage` composition、UI-03A parser/tests | current routes 保持默认入口；planned view 不渲染 | Web composition；复用现 API；Desktop 同 Web | 依赖 UI-03A/B 与 UI-06C0/C1；direct/refresh/invalid/auth/mobile/desktop/title/H1/focus smoke 全过后才交 UI-06B | `ResearchPage`/adapter/B consumer 与四 view 行为测试同单元；已发布 A handler/parser/fallback durable。current-route smoke 与 alias-to-canonical fallback tests 在页面回退后仍通过 |
...
| UI-03F Compatibility evidence | 否 | 5. PR/test gate | current/additive route journeys 与 artifact index | 无运行时行为 | 测试/证据；API/Desktop runtime 不变 | 汇总 UI-03A/B/D/E；H2 批准时再汇总 C，始终保留 current-route smoke | 行为断言归对应 A～E 中实际执行的 rollback unit；F 只因证据错误修正。current/legacy smoke、已发布 alias fallback/security tests、harness 与历史 artifact durable；不得删测试掩盖仍支持 contract 的失败 |
...
| UI-05A State vocabulary, primitives and pilot | 否 | 3. Additive public primitive + 单页 pilot | public state API/tests/exports 与 `TokenUsagePage` pilot | 除 Usage 外未迁页面保持现状；pilot 完成不代表 UI-05 Done | Web component/Usage；API contract 不扩写；Desktop 同 Web | 先冻结词汇和 primitive，再在 Usage 独占窗口验证，之后释放给页面 owner | Usage consumer 与 fixture tests 同单元；primitive/API tests 在仍有 consumer 时 durable。后端 error/schema/parser 与 legacy page smoke 不依赖新呈现，可保留 |
| UI-05B Page adoption and aggregate gate | 否 | 3. 页面 owner 顺序接入 + 聚合 gate | state matrix、shared fixtures、aggregate gate | 未迁页面保持现状；任一 owner 未完成时父任务保持未完成 | Web tests/evidence；API/Desktop runtime 不变 | Home=UI-02A；Research=UI-03C；Chat/Screening=UI-03D；Signals/Alerts=UI-03E；Portfolio/Backtest=UI-04A；Settings=UI-06F；Usage=UI-05A；UI-05B 不编辑领域页 | 每页 adoption/tests 跟随页面 owner 回退；UI-05B 只修正 matrix/fixture/gate，必须把退出 consumer 标记未完成，不得保留全页通过断言 |
...
| UI-06B Shell/Sidebar | 否 | 5. 原子 PR/commit revert；若需 audience rollout 则机制 6 并 blocked | navigation descriptor、Shell/Sidebar tests、Git history | 获批 destination 已通过验收后新树随合入成为默认；旧 paths 全保留 | Web desktop/mobile nav；API 不变；Desktop 同 Web | 依赖 HITL-H1～H8；Research 仅在 UI-03C 完整 smoke 通过后暴露；H2 不批则使用无空槽的备选树 | descriptor/labels 与 tree tests 为一单元；responsive/brand layout 与 overflow/brand tests 为独立 correctness 单元。routes/parser 与 current page smoke durable；新树或新布局断言不在旧状态保留 |
| UI-06C0 Route transition focus coordinator | 否 | 3. Additive routing boundary + registration hook | `RouteFocusCoordinator`、`useRouteFocusTarget`、typed contract 与 event tests | 未注册页面保持现状；注册页面的跨路由焦点只由 coordinator 执行 | Web Router/focus；API/Desktop wrapper 不变 | 严格位于 UI-06A -> UI-03A -> C0 -> C1 -> page adoption/C2 的共享文件顺序 | Coordinator、hook 与 PUSH/REPLACE/POP/direct/refresh/new-tab tests 同单元；页面 registration/tests 跟随页面 owner。已有 URL/history entry 不因 coordinator 回退而改写 |
...
| UI-07 Visual/interaction evidence | 否 | 5. PR/test gate | Playwright journeys、snapshots、artifact scanner | 无运行时行为 | 测试/证据；API/Desktop runtime 不变 | 汇总 UI-05A/B、UI-06C0 与全部已合入 slice；不构造 flag matrix | journey/snapshot 跟随 owner rollback unit；UI-07 只保留 scanner/harness、legacy smoke 与不可改写历史 evidence。不得更新 snapshot 接受已知错误；未发布行为撤销时 snapshot 与实现同退 |
...
| 范围 | 只建立 React Router route/parser/normalizer、受控 return URL、legacy-to-candidate mapping、invalid param 反馈和 additive `/research` handler；不拥有默认导航曝光。 |
...
| API/数据依赖 | 无新 API；HITL-H2 决定是否创建 additive `/research`。HITL-H6 不阻塞创建或后续 UI-06B exposure，只负责更晚的 canonical cutover 与 legacy exit。 |
| URL/状态 | identity 使用 `stock/reportId/session/signal/view/returnTo`；用户动作 push，invalid normalization replace；query/hash 透传白名单化。 |
| rollout/兼容 | 机制 2：先提交 handler/parser/fallback 与 contract tests，再发布 additive `/research` deep link；UI-03C 完整 smoke 通过前不进入默认导航，当前 paths 仍 canonical，无运行时 flag。 |
| 验收测试 | direct/refresh/share/Back/Forward；同 URL 的重复 PUSH 仍以新 `location.key` 形成独立 history entry；合法/非法 return URL；legacy 与 additive candidate 语义等价；默认导航不暴露未验收 route。 |
...
| 前置 / 后置 | 前置：UI-06A 完成并释放 `App.tsx`；后置：先把 `App.tsx` 或 RouteOutletBoundary 独占窗口交给 UI-06C0，再执行 UI-06C1 与 UI-03C/D/E。若 HITL 不批 `/research`，本 slice 缩为共享 URL helper。 |
...
| 验收测试 | unit：navigation/list/link href、唯一 `aria-current`、零 tab role、全部 links tabbable、Desktop 原生 link/new-tab 语义、mobile options/current/callback 与“仅同窗口”边界、长 zh/en；Router/Playwright：四个 view direct/refresh/share/push、invalid replace、legacy/candidate 等价；UI-06C0/C2 gate 验证 ready/title/H1/transition focus、modifier/new-tab/direct/POP 分类；Chat candidate 不误亮 completion badge。 |
...
| 前置 / 后置 | 前置：UI-05A common primitive ownership window 结束、UI-03A URL helper、UI-06C0/C1；后置：由 UI-03C 接入，未来 UI-04A 可消费但不并行改此文件。 |
...
| 文件所有权 | 新 `pages/ResearchPage.tsx`、`components/research/*`、Research locales/tests；消费 UI-03A/B/C0/C1，只注册 `routeId`、H1 ref、`ready`，不再修改 `App.tsx` 或维护 focus store。 |
...
| rollout/兼容 | 机制 2：additive deep-link route；完整 smoke 通过前不进默认导航，通过后只把 exposure 交给 UI-06B；无运行时 flag。 |
| 验收测试 | 每个现有 view 的 direct、refresh、invalid、auth、Back/Forward；UI-06C0 验证同窗口 title/H1/focus 时序、同 URL PUSH 的新 `location.key`、direct/new-tab 正常初始焦点与 POP restore/fallback；AlphaSift 三态；desktop、mobile（390/320）、1440/1024、light/dark、zh/en、keyboard。全部通过才算可交 UI-06B。 |
...
| 前置 / 后置 | 前置：UI-03A/B、UI-06C0/C1；后置：UI-03D/E。只有本节完整 smoke 通过后，UI-06B 才可加入 `/research` 默认 nav entry。 |
...
| 前置 / 后置 | 前置：UI-03A/B/D/E 与相应 C0/C1/C2 route gate；只有 H2 批准 candidate Research 时才要求 UI-03C。H2 备选仍须覆盖 current canonical journeys。后置：UI-07 汇总 visual/real-interaction evidence。 |
...
父任务保持 tracker 的完整 E18 含义。UI-05A/B 只是把公共 primitive、低风险 pilot、页面接入和
aggregate gate 拆成可独立托管的候选子任务；UI-05A pilot 完成不得把 UI-05 标记为 Done。

#### UI-05A State vocabulary, primitives and pilot

| 字段 | 定义 |
| --- | --- |
| 用户行为 | 用户在 Usage pilot 中能区分 loading、empty、degraded、stale、error 和 retrying，并在可重试错误下保留稳定布局。 |
| 范围 | 冻结六个状态词汇；additive 扩展现有 `EmptyState/Loading/InlineAlert/ApiErrorAlert/DashboardStateBlock` 等公共 primitive、tests/exports；以 `TokenUsagePage` 作唯一低风险 pilot。 |
| 非目标 | 不新建第二套状态组件，不跨页面批量修改，不改变领域 API fallback，不修改 landmark/PageHeader；缺少 source/as-of/quality 时只登记 dependency，不在前端伪造。 |
| 文件所有权 | 公共状态 primitive、对应 tests/exports，以及 `TokenUsagePage` 和其 tests 的独占 pilot 窗口；完成后释放公共文件，不继续编辑其它领域页面。 |
...
| 前置 / 后置 | 前置：HITL-H7 + tracker 原子登记；后置：释放 primitive 给 UI-06C1/D/E 和页面 owner，UI-05B 开始维护 adoption gate。 |
...
| 页面所有权 | Home=`UI-02A`；Research=`UI-03C`；Chat/Screening=`UI-03D`；Signals/Alerts=`UI-03E`；Portfolio/Backtest=`UI-04A`；Settings=`UI-06F`；Usage pilot=`UI-05A`。 |
...
| rollout/兼容 | 机制 3：页面 owner 按 §20.1 顺序接入 UI-05A；UI-05B 只汇总完成度，无运行时 flag。 |
| 验收测试 | 每个 owner 提供 loading/empty/degraded/stale/error/retrying 的适用矩阵与反例；总 gate 校验 current route、zh/en、light/dark、320/390/desktop，明确不适用项及依据。 |
...
| 前置 / 后置 | 前置：UI-05A primitive/pilot 完成；后置：随各领域 owner 顺序关闭 aggregate gate，最后才允许父任务 UI-05 完成。 |

### UI-06 移动导航和无障碍

该任务拆成以下独立 slice。`App.tsx` 或 RouteOutletBoundary 的共享文件按
`UI-06A -> UI-03A -> UI-06C0 -> UI-06C1 -> page adoption/C2` 顺序移交；UI-06B 独占
...
| 前置 / 后置 | 前置：不需要额外产品或 API 决策，但仍受全局 HITL-H1～H8 批准/记录与 tracker 原子登记门禁约束；后置：UI-03A 才能编辑 `App.tsx`，随后把该共享边界交给 UI-06C0；UI-06G 补全四视口证据。 |
...
| 范围 | Shell breakpoint、Sidebar/Drawer route descriptors、brand/active/unavailable state、mobile opener；取消/Escape 回 opener，成功 route navigation 的目标焦点交给 UI-06C0，C2 只负责验证。 |
...
| API/数据依赖 | HITL-H1/H2/H3/H5/H8；AlphaSift status 只影响 capability state，不删除 route descriptor。若 `/research` 获批，必须等待 UI-03C 完整 smoke 通过。 |
| URL/状态 | navigation target 由 descriptor 提供；collapsed/open 只属 UI state；当前 business query 不因跨 breakpoint 清除。 |
| rollout/兼容 | 机制 5：获批 destination 通过对应 direct/refresh smoke 后再提交 navigation descriptor/labels；Research 还必须通过 UI-03C 的 invalid/auth/mobile/desktop/title/H1/focus smoke。随后独立提交 responsive/brand correctness；不以 UI-06C2 全路由完成为前置。若 H2 不批 `/research`，采用 H2 备选 tree 且不显示空槽；若要求分受众 rollout，先写 ADR，本 slice 保持 blocked。 |
| 验收测试 | 1440/1024/768/390/320、1440@200%、zh/en、light/dark、reduced motion；Drawer 取消/Escape/inert 回 opener，选中 route 后由 C0 执行、C2 验证 ready H1 focus；Research exposure gate 与无空槽备选树；Home gauge 无 occlusion。 |
...
| 前置 / 后置 | 前置：HITL-H1～H8、tracker 登记；如采用 `/research`，还需 UI-03A/C 且 UI-03C 完整 smoke 已通过。后置：UI-06G compatibility evidence。 |
...
| 唯一职责 | 结合 React Router navigation type 与 `location.key` 统一分类 PUSH、REPLACE、POP 与 initial/no-transition；direct、hard refresh、new-tab 均属 no-transition，但保留独立 journey。同 URL 的重复 PUSH 仍按新 key 作为独立 transition。等待 ready、title、H1 后执行 focus，并按 history entry 管理稳定 `focusKey`。 |
| 页面职责 | 页面只通过 hook 注册 `routeId`、H1 ref 和 `ready`；可选 `focusKey` 的捕获、校验、history-entry 关联和恢复归 coordinator。页面不得判断 PUSH/POP、保存第二套 focus store 或调用跨路由 `focus()`。 |
| 排除行为 | modifier click、中键、context-menu new-tab 不改变当前文档焦点；direct load、hard refresh、new tab 不执行 transition focus；找不到触发元素时才 fallback 到 ready H1；不使用 `localStorage` 持久化瞬态焦点。 |
| 文件所有权 | 新 `components/routing/RouteFocusCoordinator.tsx`、`hooks/useRouteFocusTarget.ts`、tests/exports，以及在 `App.tsx` 或既有 RouteOutletBoundary 挂载 coordinator 的独占窗口；`WorkspaceNavigation` 不直接调用 `focus()`。 |
| API/数据依赖 | 无新 API；只消费 React Router location/navigation contract 和 C1 将提供的稳定 H1 ref。 |
| URL/状态 | 不改变业务 URL。`location.key` 与内存中的 history-entry focus metadata 是 transition 真源；页面业务 state、URL parser 和 storage 不复制该状态。 |
| rollout/兼容 | 机制 3：先建立 coordinator/hook contract，再由 C1 提供稳定 H1/ref，最后由页面 owner 顺序注册；未注册 route 保持现状，无运行时 flag。 |
| 验收测试 | PUSH、REPLACE、同 URL 新 key PUSH、POP Back/Forward、触发元素移除后的 H1 fallback、direct、hard refresh、new-tab、modifier click；ready/title/H1 时序；H1 不进普通 Tab；不写 `localStorage`。 |
...
| 前置 / 后置 | 前置：UI-06A -> UI-03A 已按序完成并释放 `App.tsx`/RouteOutletBoundary；后置：UI-06C1，再由各页面 owner 接入并由 UI-06C2 验收。共享边界禁止并行编辑。 |
...
| 前置 / 后置 | 前置：UI-05A common primitive ownership 完成、UI-06C0、tracker 登记；后置：UI-06C2 与各 page owner。 |
...
| 范围 | 按 owner 独占窗口顺序迁移 Home、Chat、Screening、Portfolio、Signals、Backtest、Alerts、Usage、Settings、Login、404；现有 `AppPage` consumer 在 Shell 下显式采用 C1 的非-main variant；页面只注册 `routeId`、H1 ref、`ready`，C2 维护 title/H1/registration/focus 结果总 gate，C0 执行分类与恢复。若 HITL-H2 批准 `/research`，UI-03C 完整 smoke 通过后再加入 Research candidate route。 |
| 非目标 | 不在 C2 重做页面 composition/API/业务状态，不让 C2 Agent 与页面 owner 并发编辑同一文件，不以 candidate `/research` 替代遗漏的 `/chat` 或 `/screening`。 |
| 文件所有权 | Login=`UI-06A`；Research=`UI-03C`；Chat/Screening=`UI-03D`；Home=`UI-02A`；Signals/Alerts=`UI-03E`；Portfolio/Backtest=`UI-04A`；Settings=`UI-06F`；Usage=`UI-05A`；`NotFoundPage` 由 C2 取得隔离窗口。各 page owner 更新 title/H1，并只注册 `routeId`、H1 ref、`ready`；C2 维护总 gate/fixtures，不抢占 owner 文件。 |
| API/数据依赖 | 无新 API；依赖 C0/C1 contract 与页面 owner 的稳定 composition。 |
| URL/状态 | 不改变业务 route/query、filters 或 task。C0 结合 navigation type/`location.key` 处理 PUSH/REPLACE、POP、direct/refresh/new-tab；页面和 C2 不复制分类或 focus store。 |
| rollout/兼容 | 机制 3：逐 route consumer 接入，无运行时 flag；任何 route 未完成时 C2 状态保持未完成。 |
| 验收测试 | 每个 route 恰有一个 `main`/H1，控件有 label；C0 的同窗口 ready/title/H1 时序、同 URL 新 key PUSH、H1 不进普通 Tab、modifier/new-tab 排除、direct/refresh、Back/Forward restore/fallback 全矩阵；zh/en、320/390/desktop、原生 200% zoom；ARIA snapshot/axe + 人工键盘；Research 仅在获批且 UI-03C smoke 通过时加入。 |
...
| 前置 / 后置 | 前置：UI-06C0/C1；与 UI-06A、UI-03C/D/E、UI-02A、UI-04A、UI-05A、UI-06F 的文件窗口串行。全部 route 通过后才释放 UI-06G/UI-07 最终证据。 |
...
| 前置 / 后置 | 前置：UI-05A common ownership 完成；后置：UI-03E/UI-04A consumer adoption，UI-06G visual evidence。 |
...
| 前置 / 后置 | 前置：UI-05A common ownership；后置：Provider/Settings owner 释放文件窗口后单独迁 LLM menu，UI-02A 完成后迁 Home strategy menu。 |
...
| 范围 | Playwright/ARIA/axe/visual evidence、四视口、200% zoom、reduced motion、人工读屏 checklist、C0 route-boundary focus 事件矩阵与 artifact hygiene。 |
...
| 验收测试 | `/login` 补 1440/1024/390/320、light/dark、zh/en、first-run/returning-admin、keyboard/reduced-motion/200%；Settings 15 views；执行 PUSH/REPLACE、同 URL 新 key PUSH、POP、direct/refresh/new-tab/modifier focus 事件矩阵；人工读屏结果单列。证据须脱敏并通过 PR 附件、Actions artifact 或其它可访问 URL 发布。 |
...
| 字段 | 定义 |
| --- | --- |
| 用户行为 | 无新增产品行为；证明已获批的 UI-02A、UI-03A～F、UI-04A、UI-05A/B 与 UI-06 slices 在真实后端/fixture 下可完成；不冒充完整 UI-02/03/04/05 已交付。 |
| 范围 | Playwright journeys、visual snapshots、contrast/occlusion、artifact hygiene；不在测试中重做业务实现。 |
| 依赖 | 稳定脱敏 fixture、四视口、fake provider、最终 routes、UI-06C0 event classification 与 §17.2 rollout contract。 |
| 主要文件 | `e2e/*`, `playwright.config.ts`, test fixtures, artifact scanner；一次性图片不入库。 |
| URL/状态 | 覆盖 canonical + legacy + invalid + Back/Forward，以及 C0 对 PUSH/REPLACE/POP/direct/refresh/new-tab 的 focus event 分类。 |
| 测试/截图 | 12 条关键流程、全状态、light/dark zh/en、长内容、读屏人工记录；包含 ready H1 focus、同 URL 新 key PUSH、modifier/new-tab 排除、direct/refresh 正常初始焦点和 Back/Forward restore/fallback；每个实施 PR 发布可访问的脱敏前后对比。 |
...
文件所有权总门禁：UI-05A 先独占 common state primitives 与 `TokenUsagePage` pilot，UI-05B 只维护
matrix/fixtures/aggregate gate；`App.tsx` 或 RouteOutletBoundary 严格按
`UI-06A -> UI-03A -> UI-06C0` 移交，UI-06C1 随后独占 `AppPage/PageHeader`，页面 owner 再注册
C0/C1，UI-06C2 只维护总 gate；UI-06B 独占 Shell/Sidebar。UI-02A 独占 Home；UI-03C/D/E 分别
独占 Research composition、Chat/Screening context、Signals/Alerts loop；UI-04A 独占
...
| --- | --- | --- | --- | --- | --- |
| `HITL-H1` | Shell 一级 IA 与 zh/en canonical terms | 批准六个目标槽位：`今日/Today · 研究/Research · 组合/Portfolio · 信号/Signals · [More 或 Usage，由 H8 决定] · 设置/Settings`；主推荐与 H8 推荐组合时第五项为 More | 保持当前 8～9 个功能入口平铺，先只修 bug | H1 批准目标 IA 不等于立即暴露 Research；该槽位仍受 H2 与 UI-03A -> UI-03C smoke -> UI-06B 生命周期约束。H1 与 H8 必须作为一组批准，禁止同时显示一级 More 和一级 Usage | UI-02A label/entry、UI-03C、UI-04A label、UI-06B；UI-05A/B 可设计但仍不得开工 |
| `HITL-H2` | 问股、选股与 Research hub | 问股作为 Research route navigation + report/asset contextual command，选股归 Research；UI-03A 只建 additive `/research`，UI-03C 完整 smoke 后 UI-06B 才暴露；当前 `/chat`、`/screening`、`/?recordId=` 保持 canonical | 问股继续一级；只做 Sidebar 分组，不新增 `/research`，UI-06B 不显示空 Research 槽位 | 推荐方案减少顶层项但需要 context/compat contract；备选迁移小但研究信息仍分散。两者都不允许未验收或空 Research 入口 | UI-03A～D、UI-06B、UI-06C0/C2 |
...
| `HITL-H6` | 新旧 Web path 策略与退出条件 | UI-02～UI-07 期间 `/chat`、`/screening`、`/?recordId=` 等 current paths 继续 canonical；`/research` 只 additive，UI-06B 暴露主导航也不构成 cutover；退出条件基于实际使用、兼容测试和另一次 HITL | 立即把 `/research` 等设为 canonical，并 replace redirect legacy URL | H6 是 canonical 切换和旧路径退出的唯一后续 gate；推荐风险最小、可回滚但双入口期更长，备选可能破坏书签/Desktop/通知链接 | UI-03A/F、UI-04A compatibility、UI-06B/G |
| `HITL-H7` | 执行顺序、rollout 与文件所有权 | 批准 §17 的父子身份、无虚构 flag、tracker 原子登记和独占顺序：UI-05A -> UI-05B gate setup + UI-06D/E -> UI-06A -> UI-03A -> UI-06C0 -> UI-06C1 -> UI-03B -> [H2 批准时 UI-03C smoke] -> UI-06B -> domain owner adoption -> UI-05B/C2 gate closure -> evidence | 仍按 UI-02～UI-07 六个大任务逐项托管，或先建设未定义的通用 flag 系统 | 推荐把 `App.tsx`/RouteOutletBoundary、public primitives 和领域页串行移交，避免同文件并发、页面私有 focus store 或 UI-05B 接管领域代码 | UI-02A、UI-03A～F、UI-04A、UI-05A/B、UI-06A/B/C0/C1/C2/D/E/F/G、UI-07 全部 |
| `HITL-H8` | Usage / More 归属 | `/usage` 继续是独立、可直达、可收藏 route；Desktop Sidebar 与 mobile Drawer 将它放在 More disclosure；不创建空 `/more`，不并入 Settings，不把 token usage 与 credential/runtime diagnostics 混合；API、权限和 Desktop deep link 不变 | Usage 直接占据 H1 的第五个一级槽位并替换 More；Desktop/mobile 均继续显示一级 Usage，禁止保留空 More | 推荐方案让用户多一次 disclosure 操作但不丢直达；URL 始终 `/usage`。Mobile 取消/Escape More 时回 opener，选择 Usage 后由 UI-05A 页面 registration + UI-06C0 在 ready 后聚焦 H1；Back/Forward 由 C0 恢复、C2 验收。两者按 UI-06B descriptor/tests 同单元回退，route/API/auth/page identity 不变 | UI-05A、UI-06B/C0/C2 |
...
UI-05A/B、UI-06A/B/C0/C1/C2/D/E/F/G 均不得分配，UI-02～UI-07 父任务也不得据此标记 In progress 或 Done。
...
1. `UI-05A`：冻结 additive state vocabulary/public primitives，在 `TokenUsagePage` 独占窗口完成低风险 pilot；缺 capability 字段只记录依赖。Pilot 完成不代表 UI-05 Done。
2. `UI-05B`：建立 matrix/fixtures/aggregate gate 后只跟踪页面 owner；`UI-06D/E` 可在 UI-05A 释放 common files 后分别完成 Pagination、Menu/Popover primitive。
...
4. `UI-03A`：先建立共享 route/parser/compat helper；只有 H2 批准时才增加 additive `/research`，且不暴露默认 Research nav。H2 采用备选时本 slice 缩为 helper，不创建该 route；完成后释放 shared routing boundary。
5. `UI-06C0`：在 `App.tsx` 或 RouteOutletBoundary 独占窗口建立 focus coordinator/hook，冻结 navigation type + `location.key` 分类。
6. `UI-06C1`：在 C0 后完成 landmark/PageHeader primitive；`UI-03B` 随后建立 `<nav>` + Router Link primitive。页面 adoption/C2 只能从此处开始。
7. H2 批准时执行 `UI-03C`：建立 current-capability Research destination 并注册 C0/C1，先通过 direct/refresh/invalid/auth/mobile/desktop/title/H1/focus smoke。H2 采用备选时跳过 candidate Research composition；UI-05A 仍在 C0/C1 后完成 Usage registration。
8. `UI-06B`：H2 批准时只有 UI-03C 完整 smoke 通过后才暴露 `/research`；H2 采用备选时直接使用无空 Research 槽位的备选树。两种情况都须先完成 H8 Usage 决定，再以 nav-only commit 接获批 Shell tree。
9. `UI-03D`：先稳定 ContextLink，再顺序接 Report/Chat/Screening、C0/C1 与 C2 gate；这必须早于 Home/Portfolio 消费 helper。
10. `UI-02A`：只做 current-data Home hierarchy、1024 修复与 C0/C1/C2 adoption；完整 E04 仍属父任务 UI-02。
11. `UI-03E`：关闭 signal/report/alert loop，并完成 C0/C1/C2 Signals/Alerts adoption；不实现 E06 timeline/Evidence。
12. `UI-04A`：只稳定当前股票 Portfolio/Backtest、URL、task tracking 与 C0/C1/C2 adoption；完整配置/再平衡仍属 UI-04。
13. `UI-06F`：取得 Provider/Settings 文件独占窗口，修复 15 个 Settings view 并完成 C0/C1/C2 Settings adoption。
14. `UI-06C2` 在隔离窗口完成 404 并关闭 route 总 gate；`UI-05B` 同时关闭 state aggregate gate。任一 current route 未完成时对应父任务不得 Done。
15. `UI-03F`、`UI-06G` 和 `UI-07`：汇总 current/additive/legacy、visual 与真实 a11y/interaction 证据，并发布可访问的脱敏 artifact。
...
- UI-02A、UI-03A～F、UI-04A、UI-05A/B 与 UI-06A/B/C0/C1/C2/D/E/F/G 仍未登记到外部 tracker；HITL 后必须原子登记，登记前不能分配。
