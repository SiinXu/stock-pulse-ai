...
| pr-review | 通过 `workflow_dispatch` 且必须填写 `pr_number` 的按需辅助审查。`security-check` 仅经 GitHub Pulls API 读取变更文件清单，不会 checkout 或对 PR head 做 `git diff`。后续任务仍可运行变更文件语法/flake8、约 1000 行规模提示（排除 lock/生成文件），以及同仓 AI 审查（需仓库 Secrets `GEMINI_API_KEY` 和/或 `OPENAI_API_KEY`）。Fork PR 只读（无 secrets、无写操作）。AI 审查不阻断合入。请在 Actions 页选择该 PR 分支作为 workflow ref（以便后续静态检查仍看到该工作树），并传入 PR 编号；缺少有效 `pr_number` 时失败关闭。 | ❌（辅助项） |
