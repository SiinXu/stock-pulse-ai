...
| web-gate | 始终给出结论；Web 变更时执行 `npm run lint` + `npm run test:i18n` + `npm run test:coverage` + `npm run build`，并在构建后阻断执行 bundle size budget；无 Web 变更时记录摘要；变更检测不可用时 fail closed | ✅ |
