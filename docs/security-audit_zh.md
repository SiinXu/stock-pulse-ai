...
| 敏感配置创建/更新 | `system_config.write` | **已落地** | `SystemConfigService.update`（`src/services/system_config_service_parts/write_audit.py` 绑定到门面）。HTTP PUT `/config` 注入 recorder 且不再双写。嵌套 `.env` import 走 `_update_validated`，只保留 `system_config.import` |
...
| 配置预设应用/保存 | `src/services/config_profile_service.py` → `SystemConfigService.update` | **已落地** | 同一 `system_config.write` 事件；`source=config_profile_preset` / `config_profile_import`。空 no-op 不写事件 | DAG-5 |
| Onboarding apply | `src/services/onboarding_plan_service.py` → `SystemConfigService.update` | **已落地** | 同一 write 事件；`source=onboarding_apply`。`confirm=false` 与空 items 不调用 `update()` | DAG-5 |
| 写入配置的本地模型注册/分配/删除 | `src/services/local_model_service.py` → `SystemConfigService.update` | **已落地** | 同一 write 事件；`source=local_model`。HTTP 将审计不可用映射为 503 | DAG-5 |
| 模型包导入 / 桌面激活 | `src/api/v1/endpoints/model_packs.py` | **具名延期** | 可信制品安装（上传/暂存/解包）是另一类特权操作。桌面激活过程中经 `update()` 的配置写入已作为 `system_config.write` 落地 | 具名延期 |
...
DAG-4 在 HTTP `GET /api/v1/history/{record_id}/export` 写入 `report.export`，
并在 HTTP `DELETE /api/v1/history/by-code/{stock_code}` 与
`DELETE /api/v1/history` 写入 `history.delete`。attempt 在加载 markdown /
`HistoryService.delete_history_*` 前提交。导出 metadata 仅含 format、
lookup_key、resolved_record_id、lookup_mode 与成功时的 `byte_length`，不含
报告正文、文件名、字体路径、cookie 或密钥。删除 metadata 仅含 scope、
deleted_count、有界 stock_code 或有界 id_sample / id_count /
ids_truncated，不含公司名或无界 id 列表。HTTP `503`
`security_audit_unavailable` 带 `operation_completed`：attempt 无法写入时为
`false`（无字节、无删除）；特权操作已发生但 completion 无法写入时为
`true`。这些路由在管理员认证关闭时仍可运行（`local_operator`），不复制
research-pack 的 `admin_session` `403`。能力探测、markdown GET 与内部
HistoryService 删除不写这些事件。

DAG-5 在 `SystemConfigService.update` 上只写一次 `system_config.write`。HTTP PUT
`/config` 不再在适配器层写入。嵌套 `.env` import/export/rollback 保留原有事件
类型。attempt 在 `apply_updates` 前提交。HTTP `503` `security_audit_unavailable`
带 `operation_completed`：attempt 无法写入时为 `false`（未持久化）；持久化已
发生但 completion 无法写入时为 `true`（含 `config_version` / `applied_count` /
`reload_triggered`）。领域 400/409 保持原状态码；拒绝/失败 completion 为尽力
写入。配置预设 preview、onboarding `confirm=false` 与 `apply_simple_updates`
不是本事件。
...
不要把剩余具名后续行或延期行折进更早切片。不要纳入自选、组合 CRUD 或告警。
不要把市场复盘、候选发现或 AlphaSift 折进 DAG-1。
...
DAG-0  本覆盖图（仅文档；无运行时行为）
  ...
  └── DAG-5  SystemConfigService.update 写入（已落地）
               HTTP PUT / 配置预设 apply/import / onboarding apply /
               本地模型配置写入 / 遗留通道迁移；
               嵌套 import 跳过 write；HTTP 路径不得双写

剩余具名后续 / 延期行（不是本 PR）：
  HTTP 市场复盘 / 候选发现 / AlphaSift
  投资框架变更（延期，除非重新归类）
  模型包可信制品安装（具名延期；经 update() 的配置写入已落地）
```
...
- 线上 #1062 工作流 A–D 与验收复选框仍全部未勾。原始 A 清单的
  HTTP/MCP/工具/HITL/插件/本地进程路径以及证据包导出已经是 **已落地**。
  后续复选框应列具名后续行与延期行，而不是 DAG-5。
...
不调用工具 handler、不入队分析、MCP 拒绝、管理员插件变更停止、本地
OCR/CLI 进程不启动、定时任务创建/启用/禁用不落库、HTTP 分析 cancel 不调用
