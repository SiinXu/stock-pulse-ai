...
| `DINGTALK_WEBHOOK_URL` | 钉钉群机器人 Webhook URL；可在 Web 设置中配置并测试 | 可选 |
| `DINGTALK_SECRET` | 钉钉群机器人加签密钥；仅当该机器人启用加签校验时必填 | 可选 |
...
| `DSA_RUNTIME_SCHEDULER_TIMEOUT_SECONDS` | 已挂载 Web/API/Desktop runtime scheduler 单次分析硬超时（秒，最小 60；非法值回退 2700）。仅环境变量，不出现在 Web 设置页 | `2700` |
| `LOG_DIR` | 日志目录 | `./logs` |
...
| `docker-compose -f ./docker/docker-compose.yml up -d` | 同时启动两种模式（`analyzer` 独占遗留日批与版本化任务执行，`server` 只提供 API） | 8000 |
...
services:
  # 定时任务模式（遗留日批与版本化任务的唯一执行者）
  analyzer:
    <<: *common
    container_name: stock-analyzer
    command: ["python", "main.py", "--schedule"]

  # FastAPI 模式（API/CRUD；不挂载遗留日批）
  server:
    <<: *common
...
**智能导入依赖**：`pypinyin`（名称→代码拼音匹配）和 `openpyxl`（Excel .xlsx 解析）已包含在 `requirements.txt` 中，执行上述受约束的依赖安装命令时会自动安装。若使用智能导入（图片/CSV/Excel/剪贴板）功能，请确保依赖已正确安装；缺失时可能报 `ModuleNotFoundError`。
...
默认连接 `127.0.0.1:11111`，可通过 `FUTU_OPEND_HOST` / `FUTU_OPEND_PORT` 调整；`FUTU_ACC_ID` 可限定一个符合条件的真实账户，留空则合并所有符合条件的账户；`FUTU_SECURITY_FIRM=NONE` 使用 SDK 自动识别。Docker 内的 `127.0.0.1` 指容器本身，使用容器运行时需显式配置一个受信任且从容器网络可达的 OpenD 地址。该集成只调用账户列表、持仓列表和证券基础信息查询，不解锁交易，也不下单、改单或撤单。

实盘持仓还可通过 `POST /api/v1/portfolio/imports/futu`（预览：`/imports/futu/preview`）写入组合管理：将符合条件的多头正股映射为共享交易导入路径上的合成买入。详见 [Futu OpenD 组合持仓导入](futu-opend-portfolio-import.md)。
...
>
> 从 `python main.py --schedule` 或等价纯 CLI 调度模式启动后，WebUI 保存新的 `SCHEDULE_TIME` / `SCHEDULE_TIMES` 会在下一轮调度检查内自动重绑 daily jobs，无需重启进程；旧的执行时间不会继续保留。`python main.py --serve --schedule` 会由 Web/API runtime scheduler 接管定时任务，已挂载的 WebUI/API/Desktop 长运行进程保存 `SCHEDULE_ENABLED`、`SCHEDULE_TIME` 或 `SCHEDULE_TIMES` 后会按当前配置启停或重建 runtime scheduler。独立进程或 Desktop 重启 `python main.py --serve-only` 时，已启用的 daily jobs 会自动恢复，但服务启动本身不会立即执行分析。默认 Compose 同时启动 `analyzer` + `server` 时，由 `analyzer` 独占遗留日批，`server` 不挂载。
>
> Web/API runtime scheduler 的立即执行入口只会在没有分析任务运行时接受请求；如果已有分析在执行，会返回忙碌状态而不是假装排队成功。已挂载进程的遗留日批与 run-now 会在名为 `runtime-scheduled-analysis` 的 spawn 子进程中执行，并由 `DSA_RUNTIME_SCHEDULER_TIMEOUT_SECONDS`（默认 2700 秒，最小 60 秒，非法值回退默认值；每次分析重新读取）看门狗隔离；超时或 stop 会终止进程树。该超时仅环境变量生效，Web 设置页不提供控件。
...
| `DSA_RUNTIME_SCHEDULER_TIMEOUT_SECONDS` | 已挂载 Web/API/Desktop runtime scheduler 单次分析硬超时；仅环境变量，不出现在 Web 设置 | `2700` | `3600` |
...
> - 可核验证据摘要：官方 provider / Base URL / 模型命名来源沿用 [LLM 配置指南](LLM_CONFIG_GUIDE.md#常用官方文档来源用于核对预设-provider--base-url--模型命名)，当前运行时依赖窗口沿用 `requirements.txt` 中的 `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0`；本轮不新增配置迁移脚本或清理分支，保存/导入仍只写本次提交键。`tests/services/system_config/test_system_config_service.py::SystemConfigServiceTestCase::test_update_market_review_region_does_not_trigger_runtime_model_cleanup` 覆盖只保存 `MARKET_REVIEW_REGION` 时不清空或改写 `LITELLM_CONFIG`、`LLM_CHANNELS`、`LLM_OPENAI_*`、`LITELLM_MODEL`、`AGENT_LITELLM_MODEL`、`LITELLM_FALLBACK_MODELS`、`VISION_MODEL`、`OPENAI_*` 等旧配置。
...
`AgentResult.runtime_facts` 是内部可选字段，用于保存本次 Orchestrator 运行中已收集的基础 Agent 意见、degradation event、Pipeline termination 和实际 risk application。degradation event 使用 `DURING_STAGE` 区分 stage 自身失败，使用 `BEFORE_STAGE` 表示该 stage 因 Pipeline deadline 或 budget guard 未启动。stage 已完成后触发 deadline check 时不把该 stage 记录为 timeout degradation；`pipeline_termination.last_completed_stage` 从 `AgentRunStats.stage_results` 中最后一个真实 `COMPLETED` 结果取得，也可能为空。取消仍高于 timeout/degradation：取消结果可以携带取消前已收集的低敏 facts，但不会创建 degradation 或 termination 事实。

结构化 Orchestrator dashboard 按 input preparation、单次 risk application 和 post-risk finalization 的顺序处理。post-risk finalization 更新 top-level decision/operation advice、core signal/position advice、battle-plan position strategy、risk warning，以及 DecisionAgent signal/canonical payload；普通未覆盖的 hold 展示保持既有语义。模型生成 payload 中保留给确定性后处理的 `agent_disagreement_explanation` 会在共享解析边界移除，`runtime_facts` 也不会写入 dashboard、报告 schema 或公开 API。本阶段不重写 dashboard 其他自由文本中的方向性措辞，不生成公开 explanation 字段。
...
6. App Bot 发送路径复用 `requirements.txt` 中已有的 `lark-oapi>=1.0.0`；标准源码安装、Docker、GitHub Actions daily workflow 和桌面构建链路都通过受 `constraints.txt` 约束的 requirements 输入安装，不需要单独安装新库。参考：[Feishu message create OpenAPI](https://open.feishu.cn/document/server-docs/im-v1/message/create)、[lark-oapi PyPI](https://pypi.org/project/lark-oapi/)、[SDK repo](https://github.com/larksuite/oapi-sdk-python)。
...
核验路径：相关逻辑在上述运行时路径与对应测试（`tests/core/test_backtest_engine.py`、`tests/analyzer/test_analyzer_news_prompt.py`、`tests/analyzer/test_decision_stability.py`、`tests/test_agent_pipeline.py` 等）中生效；未在 `src/config.py`、`src/report.py`、存储/持久化链路新增配置字段或清理逻辑。
...
所有 `DecisionSignalItem` 响应还包含只读 canonical `presentation`：`action/label/confidence/summary/risk/timestamp` 分别提供动作镜像、本地化标签、置信度、理由摘要、风险和创建时间。顶层 `action` 是方向唯一权威，`presentation.action` 只是由它派生的同值镜像；兼容 `action_label` 或冲突的嵌套 action 都不能覆盖它。告警通知读取本次 worker 的 `REPORT_LANGUAGE` 并完整支持 `zh/en/ko`，Web 则按当前 UI 语言从同一顶层 action 生成标签。创建时显式写入的 `report_language` 是 presentation 的正式 provenance；metadata 中同名值只有在它是受支持的字符串时才参与展示语言选择，非字符串或不支持的 caller metadata 会原样保留但被 presentation 忽略。状态 PATCH 的 object/null metadata 替换会保留正式语言键，没有正式值时也不会从替换 object 提升该键，因此状态更新不能改变同一信号的 canonical label。卡片、详情、组合摘要、时间线和 API JSON 序列化消费者以顶层 `action` 决定方向，并优先从 `presentation` 读取其余五个展示字段；旧平铺字段只保留兼容与滚动升级 fallback。当前没有独立导出端点或导出 UI，API JSON 是现有唯一导出面。该字段不加入主报告、历史、StockBar 或回测 schema；这些 schema 已有的 `action/action_label` 同样以 `action` 为方向权威，Web 报告动作卡和历史 badge 复用同一标签工具，只有缺少结构化 action 的 legacy 报告才原样回退 `operation_advice`。
...
- `PATCH /api/v1/decision-signals/{signal_id}/status`：更新合法状态和可选 `metadata`；省略 metadata 时保留原值，显式 `null` 时清空调用方 metadata，object 时整包替换调用方 metadata。正式 `decision_profile` 非 `NULL` 时会覆盖 metadata 中的冲突 profile；正式字段为 legacy `NULL` 时会移除请求 object 中的 profile key，且不会提升正式字段。若记录含创建时持久化的 `report_language`，object/null 替换都会保留这一 presentation provenance（因此 null 后 metadata 仍包含该正式键，而不是 SQL `NULL`）；若记录没有正式语言，替换 object 中的同名键会被移除而不会提升，保证状态更新不改变 canonical label 语言。`expired/invalidated/closed/archived` 等 terminal 状态不能直接 PATCH 回 `active`，expired 续期仍只能重新 `POST` active + 未来 `expires_at`。
...
#1390 P4 在 Web 端接入已有 `DecisionSignal` API。当前唯一信号中心入口为 `/signals`，顶层包含信号流、规则、推送历史、再评估与统计四个共享 tab，并用非 tab 的 pressed-button scope 组表达信号列表与规则的 `scope=all|holdings|watchlist` 上下文；旧 `/decision-signals` 与 `/alerts` 保留参数映射 redirect。当前股票/时间线使用自身股票上下文，`stock` 参数在直接打开与 Back/Forward 中保持权威；推送历史与 outcome 统计保持现有 API 的全局口径，因此这些视图不显示范围控件，但 URL 中的合法 scope 会保留供返回列表或规则时继续使用。信号流默认展示 `status=active` 的信号，并支持按市场、股票代码、动作、市场阶段、来源、来源报告 ID 和状态筛选；时间线区域新增 profile filter，复用 list API 的 server-side `decision_profile` 查询，`unknown` 仅用于筛选和展示 legacy `NULL` 行，普通高级列表不新增 profile filter。页面还提供按股票代码查询最新 active 信号的入口。卡片、详情和时间线展示优先读取正式 `decision_profile` 字段，只有字段缺失时才 fallback legacy metadata；显式 `null`、历史缺失或非法 profile 显示为 unknown。信号详情展示动作、风格、置信度/评分、horizon、plan_quality、market_phase、价格计划、风险、观察条件、来源报告和数据质量；Web 只允许把信号标记为 `closed`、`invalidated` 或 `archived`，不提供 terminal 状态恢复为 active。

#1390 P5 新增信号级反馈、后验评估和统计 sidecar，不扩展 `decision_signals` 主表，也不复用绑定 `analysis_history_id` 的 `BacktestResult`。`decision_signal_feedback` 按 `signal_id` 保存最新 `useful|not_useful` 反馈、可选原因/备注和来源；`decision_signal_outcomes` 按 `(signal_id, horizon, engine_version)` 幂等保存后验结果，当前 `engine_version=decision-signal-v1`。#1124 DAG-1 锁定事实/意见写入键：反馈不得改写 outcome actuals，混入 PredictionOutcome 字段的反馈载荷会被拒绝。#1124 DAG-2 在该写路径拒绝 Soul 边界标记和超限自由文本。#1124 DAG-3 由服务端盖章 `provenance_source` / 可选 `actor_id`，传输通道 `source`（`web`/`api`）不是 provenance。Outcome 在评估时冻结 `action/market/market_phase/source_type/source_agent/plan_quality/data_quality_level/holding_state` 等统计维度，历史统计不依赖后续 live join 改写。删除历史报告时，会先找出 `source_type=analysis` 且绑定被删历史 ID 的信号，再清理对应 feedback/outcome 子表。
...
P5 的 outcome engine 整体统计卡片现在位于 `/signals?tab=review`；详情抽屉按需读取该信号 outcomes，并可提交 useful/not useful 反馈。该页面不进入 BacktestPage，也不新增后台定时任务；后验计算由 `POST /api/v1/decision-signals/outcomes/run` 显式触发。批量运行默认优先推进缺失 outcome 的信号，再重试可恢复 unable，不会让已完成或终态 unable 的最新信号长期占满 `limit`。`DECISION_PROFILE_CALIBRATION_ENABLED=true` 时，`GET /api/v1/decision-signals/outcomes/stats` 额外返回 `profile_calibration`（六个独立分组、每组 `completed >= 30` 门槛、最大不利波动只读已保存价格）；默认关闭时响应不含该字段，旧统计卡片保持兼容。细节见 [decision-signals.md](decision-signals.md)。
...
#1390 P6 将 `DecisionSignal` 复用到告警、通知和组合风险，不新增表、迁移或配置。真实股票级告警触发会优先关联同标的 latest active 信号，并把低敏 `decision_signal_summary` 写入 `alert_triggers.diagnostics`；没有 active 信号时，worker 只创建最小 `source_type=alert`、`action=alert` 信号，`trace_id=alert-rule-<hash>` 仅用于同源重试的 best-effort 幂等去重，不覆盖 active 信号本体，且不写 `market_phase` 避免跨阶段重复。分析流程只在运行时挂载 canonical `presentation`，聚合分析报告不重复渲染 DecisionSignal excerpt；告警通知引用 `action/label/confidence/summary/risk/timestamp` 以及 `horizon/watch_conditions/source_report_id` 等公开字段，并按报告语言从 canonical action 生成标签。通知失败不影响 trigger 或信号写入。`GET /api/v1/portfolio/risk` 追加 `decision_signal_risk` 聚合块，只统计当前持仓中的 active `sell/reduce/alert` 信号，明确排除 `avoid/buy/add/hold/watch`；信号查询失败时风险接口 fail-open，Web 风险区显示降级状态。
...
普通个股历史报告详情不再内嵌展示该报告提取出的 `source_type=analysis` 信号，也不会因打开报告详情而发起 `source_report_id=<recordId>` 的信号查询；需要查看结构化 AI 建议时统一进入 `/signals` 页面筛选来源报告 ID、打开 `/signals?sourceReportId=<recordId>` deep link，或按股票查询。填写来源报告 ID 或使用该 URL 参数时，Web 会发起 `source_type=analysis + source_report_id=<recordId>` 的精确查询，不叠加默认 `status=active` 等其他列表筛选，以保留旧报告 best-effort 懒回填语义。
...
| `python main.py --serve-only` | 仅启动 API 服务；可手动触发分析。独立进程会恢复已启用的定时任务但启动时不立即分析；默认 Compose `server` 不挂载遗留日批 |

### 功能特性

- 📝 **配置管理** - 查看/修改自选股列表
- 🏠 **首页注意力中枢** - 首页默认只展示「今日焦点 / 待办 / 信号摘要」三块；晨报/大盘复盘与今日版本化定时任务位于默认收起的可配置区，展开偏好保存在浏览器 `localStorage`。另有独立可定制仪表盘（关注列表、组合健康、告警、最近报告）支持显示/隐藏/重排，偏好按浏览器配置文件 revision 持久化；详见 [home-dashboard-layout.md](./home-dashboard-layout.md)。
- 🔬 **研究概览** - `/research` 是大盘复盘、发现、分析工作台与回测的独立入口；大盘复盘仍位于 `/research/market`。桌面展开侧栏与移动抽屉中，「研究」父级行进入 `/research`，独立的行尾控件展开或收起四个研究工具，不再重复显示「研究概览」子项；紧凑侧栏中「研究」仍是菜单触发器，弹出菜单的第一项为「研究概览」。
- 🔗 **旧首页分析链接兼容** - `/?recordId=...`、`/?runFlow=...`、`stock` 与旧 `workspace` 分析参数会使用 replace 导航到 `/research/analysis`，规范化为对应的 `segment=launch|history|tasks`（`launch` 为默认段，因此 URL 可省略），并保留安全的无关 query 与 hash。无分析参数的 `/` 始终停留在首页，不恢复旧报告上下文。
- 🧪 **分析工作台** - 研究导航下的 `/research/analysis` 在同一页面提供「发起与批量 / 运行中任务 / 历史与对比」三分段；`segment`、`recordId` 与 Run Flow 参数可通过 URL 恢复。单股、自选股和图表/文档导入任务提交后会直接切到任务段，完成通知可进入对应历史报告；历史段提供趋势对比、完整 Markdown 报告、使用当前策略重新分析、携带未消费报告上下文追问 AI，以及居中全屏的 Run Flow Modal，多选删除需经确认后执行。普通股票历史会跳过仅含大盘复盘记录的 API 分页，直到找到股票报告或确认历史已耗尽。
- 🔔 **全局通知与快捷导航** - Shell 在桌面侧边栏或移动顶栏只挂载一个通知 Bell，聚合最新信号与告警，并用信号 ID 或告警 trigger ID 深链到 `/signals` 中的对应条目；信号与告警分别维护当前浏览器的未读边界，单通道失败会显示可重试的部分不可用提示且不会把恢复后的项目提前标为已读，清理站点数据后边界会重置。侧边栏搜索与 `Cmd/Ctrl+K` 打开同一个命令面板，可前往页面、执行信号中心操作、触发大盘复盘、搜索股票，并将分析入口统一指向 `/research/analysis`。
...
- 🧩 **会话级 Skill 选择** - 问股会话会持久化当前 Skill 选择；刷新与会话切换按会话独立恢复，新会话继续使用服务器默认
...
| `/api/v1/usage/dashboard?period=today|month|all&limit=50` | GET | 返回 Token 用量看板数据：总量、Prompt/Completion 拆分、模型用量、调用类型分布和最近调用明细；Web 侧入口为“设置 > 用量与成本”，旧 `/usage` 链接会兼容重定向 |
...
> 审计依据：优先级与回退语义以 `src/config.py` 的 `Config._load_from_env()` 为准（`LITELLM_CONFIG` > `LLM_CHANNELS` > legacy）。配套回归见 `tests/config/test_llm_channel_config.py`（配置源解析）与 `tests/market/test_market_review_runtime.py`（共享装配路径）。该接口当前仅提供单进程/单机级防重复能力，若为多实例部署需通过外部任务队列或分布式锁补齐全局幂等。
...
> 说明：`POST /api/v1/backtest/run` 是**同步**请求。响应没有 job / task / run / 幂等标识，也没有 status 或 cancel 路由。Web 客户端等待超时不能证明服务端处理已结束。Web 回测页会进入未知结果状态，并在用户确认前拒绝第二次 POST。
> 说明：`POST /api/v1/backtest/run` 支持 `analysis_date_from` / `analysis_date_to`（`YYYY-MM-DD`）、`eval_window_days`、`min_age_days`、`limit` 作为运行配置；若 `analysis_date_from > analysis_date_to`，接口返回 400，错误码 `invalid_analysis_date_range`。其他受控校验失败使用稳定错误码，例如 `invalid_stock_code`、`invalid_eval_window_days`、`invalid_min_age_days`、`invalid_run_limit`。
> 说明：执行成功时 `BacktestRunResponse.applied_config` 回显实际生效配置（并保留 `applied_eval_window_days` 兼容字段）。无新入库结果时，`message` 返回可读诊断，`diagnostics` 返回排查上下文（示例：`empty_reason`、`engine_version`、`skipped_count`、日期范围、窗口、最小天龄、limit）。
> 说明：结果行可包含 `resolution_notes` 标记，例如 `legacy_analysis_date`、`prior_session_start`、`missing_daily_bars`、`insufficient_forward_bars`。数据不足/跳过行仍可查询，以保证汇总完整性（已完成 vs 数据不足）诚实可见。

> Skill 会话状态：上述两个 Chat 请求中的顶层 `skills` 为三态字段，也是请求 Skill 选择的唯一权威来源。省略或传 `null` 时沿用该 `session_id` 已保存的选择；会话尚无状态时使用服务器运行时默认 Skill。传 `[]` 表示清空显式选择并使用现有通用/服务器默认执行语义；传非空列表时会按现有 Skill catalog 规则清理、去重并保存。非空列表中有效项与无效项混合时保留有效项；若全部条目均无效，则不会把归一化后的空结果视为显式 `[]`，而是沿用会话状态或运行时默认且不写入空状态。用于复用分析数据的 `context` 中即使残留 `skills` 或 `strategies`，服务端也会移除这些 legacy 字段，不能覆盖顶层三态或会话状态。用户消息与本轮显式 Skill 更新在同一事务中写入。`GET /api/v1/agent/chat/sessions/{session_id}` 会在消息之外返回 `session_state.selected_skill_ids`：没有持久化状态时为 `null`，显式清空时为 `[]`，否则为保存的 Skill 列表。Web 端使用该字段恢复持久化选择；对于 `null`，页面可以显示服务器默认 Skill，但未操作直接追问时仍省略 `skills`，不会把历史会话静默转换为显式 Skill 会话。删除会话会同时删除对应状态。
...
>   - `tests/market/test_market_review_runtime.py`（`build_market_review_runtime` 复用装配路径）
...
失败合同：Intel 失败且 isolate、且为非关键阶段时记录 degraded，保留 Technical，继续 Decision。Technical 失败会取消 Intel，不提交仅 Intel 的 context，pipeline fail-fast。Intel 超时按 isolate 处理，不丢 Technical。硬预算（`budget_tools` / `budget_turns` / `budget_cost` / `budget_tokens`）fail-fast，不把 Intel 当 isolate。用户取消在 wave join 之后、commit/checkpoint 之前检查：若 `cancelled_check` 为真且 Technical 未失败，则不提交 context、不合并 Intel、不写入 `technical_intel` 或 `technical` checkpoint，仍按声明顺序发出 `stage_done`，返回 cancelled，不进入 Decision。Checkpoint 仅在两阶段都 `COMPLETED` 且未被取消时写入 `technical_intel`；Intel isolate 后只保存 `technical`。resume 时已完成的 `technical_intel` 展开为 technical 与 intel 并跳过两者；仅有 `technical` 时跳过 Technical，Intel 走串行。SSE：两个 `stage_start` 先按声明顺序发出；`stage_done` 也按声明顺序发出；波次内 `thinking` / `tool_*` 允许交错但必须带 `stage`。未知 type 仍可忽略；不改 `done` / `error`。
...
每种运行模式（`quick` / `standard` / `full` / `specialist` / chat）对 LLM 轮次、工具调用与估算 USD 成本设有硬上限（可选 token 上限见 `AGENT_MODE_BUDGET_MAX_TOKENS`）。消耗记录在共享的 `mode_budget` 账户中，并可在诊断中查看（`ctx.meta["mode_budget"]` / `result.budget_snapshot`）。Multi 的 `AgentOrchestrator.run()` / `chat()` 会把 `budget_snapshot` 与 `failure_reason` 复制到公开 `AgentResult`（无预算元数据的结果仍保持这些字段缺省；成功结果的排序不变）。可选的运行结束反思与进程内后验 LLM 调用计入同一轮次上限；装不下时记 `budget_skipped`，返回的 `budget_snapshot` 包含这些计次。超限时以 `success=false` 明确终止并给出原因码（`budget_turns` / `budget_tools` / `budget_cost` / `budget_tokens`）。既有的剩余墙钟预算跳过仍使用 `budget_skip` / `timeout`，并写入同一快照——预算概念统一，不另造并行体系。已注册键为 `AGENT_MODE_BUDGET_ENABLED` 与全局收紧项 `AGENT_MODE_BUDGET_MAX_*`。可选的按模式上限是 Config 属性（`agent_mode_budget_<mode>_max_llm_turns` / `max_tool_calls` / `max_cost_usd`，`mode` 为 `quick|standard|full|specialist|chat`）；加载器会把未注册的可选环境变量 `AGENT_MODE_BUDGET_<MODE>_MAX_*` 映射到这些属性。它们不是 Web Settings 字段。除非需要按模式封顶，否则优先使用全局收紧项；`0` 表示保留该模式内置默认值。详见 `.env.example`。仓库没有 `AGENT_MAX_RUN_LLM_CALLS` 键；运行级 LLM 轮次上限是 `AGENT_MODE_BUDGET_MAX_LLM_TURNS`（以及可选的按模式 Config 覆盖）。

生产路径 Deep Research（`POST /api/v1/agent/research`、bot `/research`、Native `ExecutionMode.RESEARCH`）把同一次研究的分解、每道子问题循环与综合写入同一个 `mode_budget` 账户，并使用 specialist 档位。`AGENT_DEEP_RESEARCH_BUDGET` 是该账户上的研究 token 上限（specialist 内置 `max_tokens=0` 不会关闭这一上限）；若全局 `AGENT_MODE_BUDGET_MAX_TOKENS` 为正，则取 `min()`。模式预算开启时，specialist 的轮次/工具/成本默认值生效（12 轮 / 64 次工具 / $1.50，再叠加全局收紧项与可选 specialist 覆盖）。每道子问题循环按该共享账户的剩余 LLM 轮次封顶，因此 specialist 轮次上限不会多计一次调用。硬上限超限以 typed `budget_*` 且 `success=false` 终止，残留 markdown 不算完成报告。`AGENT_MODE_BUDGET_ENABLED=false` 会恢复以前的“仅 token 上限”研究停机，但超过 `AGENT_DEEP_RESEARCH_BUDGET` 仍必须以 `budget_tokens` fail-closed，而不能警告后综合出 `success=true`。墙钟 `AGENT_DEEP_RESEARCH_TIMEOUT` 以及剩余时间的 `budget_skip` / `timeout` 映射保持不变。取消优先于预算。当 `AGENT_PLANNING_ENABLED` 为 true 时，可选的 RUN 与单 Agent Chat planning gather 计入同一 chat 档 `mode_budget` 账户；`budget_*` 超限在综合前 fail-closed。


## Agent 运行时护栏

Agent 的超时分为三层：`AGENT_TOOL_TIMEOUT_S` 默认以 120 秒限制单次工具调用；可选的 `AGENT_DATA_TOOL_TIMEOUT_S`、`AGENT_SEARCH_TOOL_TIMEOUT_S`、`AGENT_ANALYSIS_TOOL_TIMEOUT_S` 和 `AGENT_ACTION_TOOL_TIMEOUT_S` 默认为 0，表示不设类别上限并继续使用全局单次调用预算，正值只约束对应 `ToolDefinition.category`（行情类网络数据工具使用 data 类别），可接受范围为 0-3600 秒，超过 3600 会告警并钳制到 3600；`AGENT_TECHNICAL_AGENT_TIMEOUT_S`、`AGENT_INTEL_AGENT_TIMEOUT_S`、`AGENT_RISK_AGENT_TIMEOUT_S`、`AGENT_DECISION_AGENT_TIMEOUT_S`、`AGENT_PORTFOLIO_AGENT_TIMEOUT_S` 和 `AGENT_SKILL_AGENT_TIMEOUT_S` 可为对应 Stage 设置独立上限；`AGENT_ORCHESTRATOR_TIMEOUT_S` 默认以 600 秒限制 single-agent 整体循环或 multi-agent Pipeline。多个预算同时生效时使用剩余时间最短的一项（正值类别上限、全局单次调用上限、剩余编排预算），超时后的工具结果会被运行时 fence 丢弃，不会回写为成功结果。非法、非有限或负的类别超时会告警并降级为无类别上限，不会导致启动失败。Multi-agent Stage 会在隔离的上下文副本中执行，只有按时返回 `COMPLETED` 才提交状态；超时后的 late state 和 progress 不会进入后续 Stage。Python 无法强制终止已经运行的原生线程，因此自定义 Stage 或工具 handler 仍可能在后台结束并产生自身的外部副作用，但不能回写已接受的 Agent 上下文或 tool-session 缓存。Progress callback 是同步 hook，必须及时返回；仓库支持的 SSE 与 runtime adapter 只负责入队或发布，不会同步等待下游。Stage fence 会保证已接受的 callback 排在关闭之前，但不会尝试抢占已经进入任意调用方 callback 的代码。
...
worker 会把 `triggered`、`skipped`、`degraded`、`failed` 写入 `alert_triggers` 作为评估历史；正常未触发不写历史。DB 持久化规则的 `triggered` 历史按 `rule_id + target + data_source + data_timestamp` 对同一数据点做 best-effort 去重，重复命中会复用最早一条触发记录，`data_timestamp` 缺失时不去重。真实触发后会把每个通知渠道的 attempt 写入 `alert_notifications`，并为 Alert API 创建的持久化规则写入 `alert_cooldowns` 业务冷却状态；若读取持久化冷却失败，worker 会临时使用进程内 fingerprint 防止 DB 异常期间重复推送。legacy `AGENT_EVENT_ALERT_RULES_JSON` 规则继续使用进程内 fingerprint 抑制，不写持久化冷却；通知基础设施的 `src/notification_parts/noise.py` 降噪仍独立生效。Web 规则列表使用后端返回的 `cooldown_active` 判断冷却状态，避免浏览器本地时区解析影响展示。

技术指标规则只使用日线 close 的边缘触发，partial bar 处理是服务器本地时区 + 16:00 的启发式，不做市场日历精确判定。`watchlist` 每轮刷新 `STOCK_LIST` 后展开，`portfolio_holdings` 从持仓快照的非零持仓按 symbol 去重展开，`portfolio_account` 复用持仓风险服务做账户级聚合评估。`market` 规则的 target 仅支持 `cn|hk|us|jp|kr`，使用结构化 `MarketLightSnapshot`；`trade_date` 来自当次 market overview，`data_quality=unavailable` 会跳过触发，非交易日会被交易日 gate 跳过，`market_light_score_drop` 只比较跨交易日 score。WebUI 信号中心的“规则”和“推送历史”tab 可以管理持久化规则、执行一次性 dry-run 测试，并查看触发历史、通知尝试结果和只读冷却状态；All / Holdings / Watchlist scope 作用于信号列表和规则列表，推送历史与再评估统计保持全局口径且不显示 scope 控件。批量规则的列表冷却状态是父规则摘要，子目标冷却以触发历史为准。详细边界见 [实时告警中心](alerts.md)。
