...
> - A 股板块 Top 表后会追加板块指数分析：以本次可用主要指数平均涨跌幅为比较基准，对行业/概念领涨与领跌项给出当日会话趋势、相对强弱和风险等级，并在结构化 `market_review_payload.sector_analysis` 中保留同一结果。当前公共 provider 合同只提供当日排行，不提供板块指数命名空间/代码/点位、无冲突规范 ID、ETF 映射、历史序列或板块资金流；这些维度会明确标记为不可用，不会由 LLM 推断。
> - 字段契约：
