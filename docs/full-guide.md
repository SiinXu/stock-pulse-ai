...
所有 `DecisionSignalItem` 响应还包含只读 canonical `presentation`：`action/label/confidence/summary/risk/timestamp` 分别统一方向、本地化标签、置信度、理由摘要、风险和创建时间。`action` 是方向唯一权威，兼容 `action_label` 即使冲突也不会覆盖它；告警通知读取本次 worker 的 `REPORT_LANGUAGE` 并完整支持 `zh/en/ko`，Web 则按当前 UI 语言从同一 action 生成标签。卡片、详情、组合摘要、时间线和 API JSON 序列化消费者优先读取 `presentation`，旧平铺字段只保留兼容与滚动升级 fallback；当前没有独立导出端点或导出 UI，API JSON 是现有唯一导出面。该字段不加入主报告、历史、StockBar 或回测 schema。
