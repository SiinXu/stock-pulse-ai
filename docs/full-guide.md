...
- 🔔 **全局通知与快捷导航** - Shell 在桌面侧边栏或移动顶栏只挂载一个通知 Bell，聚合最新信号与告警并深链到 `/signals`；未读边界保存在当前浏览器，清理站点数据后会重置。侧边栏搜索与 `Cmd/Ctrl+K` 打开同一个命令面板，可前往页面、执行信号中心操作、搜索股票，并将分析入口统一指向 `/research/analysis`。
