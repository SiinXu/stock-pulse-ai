...
- 🧪 **分析工作台** - 研究导航下的 `/research/analysis` 在同一页面提供「发起与批量 / 运行中任务 / 历史与对比」三分段；`segment`、`recordId` 与 Run Flow 参数可通过 URL 恢复。单股、自选股和图表/文档导入任务提交后会直接切到任务段，完成通知可进入对应历史报告；历史段提供趋势对比、完整 Markdown 报告和 Run Flow 详情 Drawer，多选删除需经确认后执行。
