...
所有 `DecisionSignalItem` 响应还包含只读 canonical `presentation`：`action/label/confidence/summary/risk/timestamp` 分别统一方向、本地化标签、置信度、理由摘要、风险和创建时间。`action` 是方向唯一权威，兼容 `action_label` 即使冲突也不会覆盖它；告警通知读取本次 worker 的 `REPORT_LANGUAGE` 并完整支持 `zh/en/ko`，Web 则按当前 UI 语言从同一 action 生成标签。创建时显式写入的 `report_language` 是 presentation 的正式 provenance；metadata 中同名值只有在它是受支持的字符串时才参与展示语言选择，非字符串或不支持的 caller metadata 会原样保留但被 presentation 忽略。状态 PATCH 的 object/null metadata 替换会保留正式语言键，没有正式值时也不会从替换 object 提升该键，因此状态更新不能改变同一信号的 canonical label。卡片、详情、组合摘要、时间线和 API JSON 序列化消费者优先读取 `presentation`，旧平铺字段只保留兼容与滚动升级 fallback；当前没有独立导出端点或导出 UI，API JSON 是现有唯一导出面。该字段不加入主报告、历史、StockBar 或回测 schema；这些 schema 已有的 `action/action_label` 同样以 `action` 为方向权威，Web 报告动作卡和历史 badge 复用同一标签工具，只有缺少结构化 action 的 legacy 报告才原样回退 `operation_advice`。
