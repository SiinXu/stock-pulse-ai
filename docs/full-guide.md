...
#1390 P4 在 Web 端接入已有 `DecisionSignal` API。当前唯一信号中心入口为 `/signals`，顶层包含信号流、规则、推送历史、再评估与统计四个 tab，并用 `scope=all|holdings|watchlist` 表达信号列表与规则的上下文范围；旧 `/decision-signals` 与 `/alerts` 保留参数映射 redirect。当前股票/时间线使用自身股票上下文，推送历史与 outcome 统计保持现有 API 的全局口径，因此这些视图不显示范围控件，但 URL 中的合法 scope 会保留供返回列表或规则时继续使用。信号流默认展示 `status=active` 的信号，并支持按市场、股票代码、动作、市场阶段、来源、来源报告 ID 和状态筛选；时间线区域新增 profile filter，复用 list API 的 server-side `decision_profile` 查询，`unknown` 仅用于筛选和展示 legacy `NULL` 行，普通高级列表不新增 profile filter。页面还提供按股票代码查询最新 active 信号的入口。卡片、详情和时间线展示优先读取正式 `decision_profile` 字段，只有字段缺失时才 fallback legacy metadata；显式 `null`、历史缺失或非法 profile 显示为 unknown。信号详情展示动作、风格、置信度/评分、horizon、plan_quality、market_phase、价格计划、风险、观察条件、来源报告和数据质量；Web 只允许把信号标记为 `closed`、`invalidated` 或 `archived`，不提供 terminal 状态恢复为 active。
