...
| Remaining privileged-path coverage: deferred investment-framework mutations unless reclassified as analysis policy (owner: product / [#465](https://github.com/SiinXu/stock-pulse-ai/issues/465)). Analysis admission (DAG-1), scheduled-task create/enable/disable (DAG-2), HTTP analysis cancel (DAG-3), report-export/history-delete (DAG-4), `SystemConfigService.update` writes including watchlist `STOCK_LIST` (DAG-5), HTTP market-review / candidate discovery / AlphaSift `background.submit` (DAG-6), and HTTP Model Pack trusted-artifact import `model_pack.import` (DAG-7) are landed. | `AUDIT-02` | [#1062](https://github.com/SiinXu/stock-pulse-ai/issues/1062) (coverage map in [security-audit.md](security-audit.md); remaining named later-owner / deferred rows, not a mega-PR). Parent requirement [#535](https://github.com/SiinXu/stock-pulse-ai/issues/535) |
...
| Agent memory writes lock fact versus opinion fields, reject mixed PredictionOutcome/feedback payloads, reject Soul-boundary markers plus oversized user-writable notes, and stamp server-owned `provenance_source` / optional `actor_id`. Optional default-off `AGENT_MEMORY_ENABLED` BaseAgent history inject wraps data lines as untrusted prompt data and canonicalizes `signal` to `buy` / `hold` / `sell`. Transport channel (`web` / `api`) is not provenance. Optional `actor_id` is not multi-tenant authorization. Threat notes are in agent-memory docs. | `AUTH-05`, `COMP-04`, `INPUT-01`, `INPUT-02` | [#1124](https://github.com/SiinXu/stock-pulse-ai/issues/1124) DAG-0..3 landed; DAG-4 inject isolation in `src/agent/memory.py` / `src/agent/agents/base_agent.py`; remaining product store/UX is [#1118](https://github.com/SiinXu/stock-pulse-ai/issues/1118); notes in [`docs/agent-memory.md`](agent-memory.md#threat-notes); lock: `src/schemas/memory_fact_opinion.py`; write guard: `src/schemas/memory_write_guard.py`; stamp: `src/schemas/memory_provenance.py` |
