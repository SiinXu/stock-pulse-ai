...
| Remaining privileged-path coverage: HTTP analysis cancel (route already on `main` via [#1466](https://github.com/SiinXu/stock-pulse-ai/pull/1466); audit still missing), optional report-export/history-delete, and `SystemConfigService.update` bypasses. Analysis admission (DAG-1) and scheduled-task create/enable/disable (DAG-2) are landed. | `AUDIT-02` | [#1062](https://github.com/SiinXu/stock-pulse-ai/issues/1062) (coverage map in [security-audit.md](security-audit.md); remaining DAG-3..5, not a mega-PR). Parent requirement [#535](https://github.com/SiinXu/stock-pulse-ai/issues/535) |
