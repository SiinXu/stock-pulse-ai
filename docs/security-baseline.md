...
| Agent memory writes now lock fact versus opinion fields and reject mixed PredictionOutcome/feedback payloads. Remaining: server-stamped provenance and Soul/oversize reject. Transport channel (`web` / `api`) is not provenance. Threat notes are in agent-memory docs. | `AUTH-05`, `COMP-04`, `INPUT-01`, `INPUT-02` | [#1124](https://github.com/SiinXu/stock-pulse-ai/issues/1124) DAG-2/DAG-3; notes in [`docs/agent-memory.md`](agent-memory.md#threat-notes); lock: `src/schemas/memory_fact_opinion.py` |
