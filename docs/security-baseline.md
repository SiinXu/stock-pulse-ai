...
| Remaining privileged-path coverage: scheduled-task mutations, HTTP analysis cancel (route already on `main` via [#1466](https://github.com/SiinXu/stock-pulse-ai/pull/1466); audit still missing), optional report-export/history-delete, and `SystemConfigService.update` bypasses. Analysis admission (bot, scheduled dispatch, portfolio analyze, HTTP sync `/analyze`) is landed on DAG-1. | `AUDIT-02` | [#1062](https://github.com/SiinXu/stock-pulse-ai/issues/1062) (coverage map in [security-audit.md](security-audit.md); remaining DAG-2..5, not a mega-PR). Parent requirement [#535](https://github.com/SiinXu/stock-pulse-ai/issues/535) |
...
| Agent memory writes lock fact versus opinion fields, reject mixed PredictionOutcome/feedback payloads, and reject Soul-boundary markers plus oversized user-writable notes. Remaining: server-stamped provenance. Transport channel (`web` / `api`) is not provenance. Threat notes are in agent-memory docs. | `AUTH-05`, `COMP-04`, `INPUT-01`, `INPUT-02` | [#1124](https://github.com/SiinXu/stock-pulse-ai/issues/1124) DAG-3; notes in [`docs/agent-memory.md`](agent-memory.md#threat-notes); lock: `src/schemas/memory_fact_opinion.py`; write guard: `src/schemas/memory_write_guard.py` |
