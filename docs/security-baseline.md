...
| Remaining privileged-path coverage: scheduled-task mutations, incoming HTTP analysis cancel after [#1466](https://github.com/SiinXu/stock-pulse-ai/pull/1466), optional report-export/history-delete, and `SystemConfigService.update` bypasses. Analysis admission (bot, scheduled dispatch, portfolio analyze, HTTP sync `/analyze`) is landed on DAG-1. | `AUDIT-02` | [#1062](https://github.com/SiinXu/stock-pulse-ai/issues/1062) (coverage map in [security-audit.md](security-audit.md); remaining DAG-2..5, not a mega-PR). Parent requirement [#535](https://github.com/SiinXu/stock-pulse-ai/issues/535) |
