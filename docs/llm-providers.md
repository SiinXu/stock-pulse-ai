...
4. 新建连接不会预填示例模型（模型名会过期，不作为默认写入配置）。只有 Provider Catalog 标记支持发现时才显示「获取模型」；OpenAI-compatible 使用模型列表接口，Ollama 使用 `/api/tags`。用户可以从发现结果中多选，但系统不会自动全选；不支持发现或发现失败时仍可逐个手动添加模型，没有模型时连接保持“未完成”状态。
...
连接与任务路由修改会在停止输入 800ms 后按模型图分组自动保存，普通设置页没有全局 Save。状态区会显示等待、保存中、已保存、失败或冲突；失败保留草稿并可重试/恢复服务器值，409 必须完成三方冲突处理。连接测试不会由自动保存触发，也不作为保存硬门禁。
