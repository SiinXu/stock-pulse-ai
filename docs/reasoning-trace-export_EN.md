...
1. **Positive form** — the value matches one of the identifier forms the runtime actually generates: an integer history primary key, `history:<pk>`, a UUID hex, a dashed UUID, or a lowercase `<prefix>_<hex>` generated id such as `market_review_<uuid hex>` or `daily_brief_<date>_<hex>`. Restoration is *not* decided by which characters are absent; a permissive charset admits `sk-…`, `sk-ant-…`, `ghp_…`, `github_pat_…`, `xox…`, `AIza…`, `SG.…`, and `AKIA…`/`ASIA…` shapes that the shared redactor does recognise.
