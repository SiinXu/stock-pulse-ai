...
Structural correlation identities (`record_id`, `query_id`, `trace_id`, `run_id`, `lookup_key`) are preserved through redaction so exports stay correlatable with runtime logs and audit rows. Restoration exists only because the generic opaque-token rule cannot tell a 32-character UUID correlation id from a bearer token, and it is bounded by two independent gates that must both hold:

1. **Positive form** — the value matches one of the identifier forms the runtime actually generates: an integer history primary key, `history:<pk>`, a lowercase UUID hex, a lowercase dashed UUID, or one of the exact prefixed forms `market_review_<32hex>`, `daily_brief_<YYYY-MM-DD>_<12hex>`, and `market_context_<32hex>_<region>`. No generic prefix form is accepted. Restoration is *not* decided by which characters are absent; a permissive charset admits `sk-…`, `sk-ant-…`, `ghp_…`, `github_pat_…`, `xox…`, `AIza…`, `SG.…`, and `AKIA…`/`ASIA…` shapes that the shared redactor does recognise.
2. **Negative form** — `redact_sensitive_data`, run with only the generic opaque-token rule disabled, leaves the value untouched, and the export-local JWT and local-path scrubber leaves it untouched. Any value the shared redactor recognises as a credential, API key, bearer token, credential-bearing URL, JWT, or filesystem path therefore stays redacted regardless of its shape, and a probe failure fails closed.

Evidence payloads are always redacted and are never restored.

### Loss accounting

Coverage is reconciled against the payload that is actually returned on every exit path, including each size-budget step, and count/presence invariants are asserted before the response is emitted. A source that was dropped to satisfy the budget reports `present=false`, `returned_count=0`, and the full `dropped_count`; it can never keep a stale `present=true` or an original `returned_count`. `present` is derived from real projected content, so an empty or null-only source is reported absent.

No value may be clipped outside loss accounting. The bounded string and string-list projections require a loss ledger and a registered ledger path, so no call site — `run` identities and metadata, agent roles, event summaries, tool calls, input summaries, opinions, provider/LLM/pipeline-stage projections, data-quality, or synthesis — can shorten a string or drop an unsupported string-list item without an entry landing in `truncation.dropped`. Ledger paths come from a closed vocabulary, so the ledger cannot overflow the response schema's bound. Whenever any returned value carries the `…[truncated]` sentinel, `truncated` is `true`, a `truncation` block is present, and the `X-Reasoning-Trace-Truncated` response header (derived from the same flag) reports `1`; this is asserted before the response is emitted, so the header can never report `0` for a clipped body.
