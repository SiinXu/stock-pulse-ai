...
- 下载：只允许下载权威 catalog 中标记为 pullable 的推荐模型；loopback HTTP `POST /api/pull` 流式展示进度。任意自定义名称和 guided-only 金融模型不会被下载。
...
- 装完即用：下载完成后由后端 `SystemConfigService` 追加 `LLM_OLLAMA_MODELS`、确保 Ollama channel 存在并热加载；只有没有主模型时才自动选为主模型。主模型切换与 Agent 模型指定均为面板内独立显式操作，配置写入不在 Electron 中重复实现。
- 删除：共享面板先调用后端取消注册，由后端阻止删除被主模型、Agent、Vision、fallback 或有效隐式主 channel 引用的模型；校验成功后再由 IPC 调用 Ollama `DELETE /api/delete`。主进程会对 plain route 和 connection-aware ModelRef 做同等复核以关闭取消注册后的竞态。只有实际移除注册时，后端才签发绑定模型与取消注册后配置版本的短时一次性恢复 token。若 IPC 删除失败且权重仍在或运行时无法确认，Web 端用该 token 调用仅恢复 catalog 注册的后端通道，不探测 Ollama；版本冲突时拒绝恢复，不覆盖并发配置修改。
