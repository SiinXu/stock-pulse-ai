...
- 装完即用：下载完成后由后端 `SystemConfigService` 追加 `LLM_OLLAMA_MODELS`、确保 Ollama channel 存在并热加载；只有没有主模型时才自动选为主模型。Desktop 在下载前记录后端配置版本，主进程返回其实际使用的 Base URL；激活时两者仅作为相等性断言，后端仍只使用服务端配置的地址，任一值变化都拒绝跨运行时激活。主模型切换与 Agent 模型指定均为面板内独立显式操作。配置写入不在 Electron 中重复实现。
- 删除：共享面板先取得同一配置版本和运行时快照，再调用后端取消注册；后端阻止删除被主模型、Agent、Vision、fallback 或有效隐式主 channel 引用的模型，主进程也会在请求前确认自身当前 Base URL 未变化，并对 plain route 和 connection-aware ModelRef 做同等复核。只有实际移除注册时，后端才签发绑定模型与取消注册后配置版本的短时一次性恢复 token。若 IPC 删除失败且权重仍在或运行时无法确认，Web 端用该 token 调用仅恢复 catalog 注册的后端通道，不探测 Ollama；版本冲突时拒绝恢复，不覆盖并发配置修改。若权重删除成功，Web 端立即调用后端撤销该 token，已删除的权重不能再通过恢复通道重新注册。
