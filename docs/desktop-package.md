...
tray 的 `Local Models…` 会打开一个独立、隔离的桌面本地模型中心，用于在桌面壳层内管理本地 [Ollama](https://ollama.com/download) 运行时的生命周期。它只做“桌面能准确掌控”的事：发现、下载、启停与状态展示，不改动 Web 前端，也不替代设置页的模型渠道配置。

- 发现与状态：先探测 `LLM_OLLAMA_BASE_URL`（默认 `http://127.0.0.1:11434`）的 `/api/tags`；只要该端点已有健康 Ollama 服务就直接复用，不会再启动第二个实例。端点不可用时，先以固定参数 `ollama --version` 解析系统安装，再验证安装包内 `resources/ollama/runtime-manifest.json`、完整运行时文件集合和内嵌二进制；只有两者都不可用时才显示原有的 Ollama 安装引导。状态分为 `running` / `stopped` / `not-installed` / `starting` / `error`。
- 下载：只允许下载内置的推荐模型（附硬件建议），下载通过 loopback HTTP `POST /api/pull` 流式进行并展示进度；任意自定义名称不会被下载。
- 启停：`Start service` 在检测到运行时但服务未运行时，以解析后的系统或内嵌二进制加固定参数数组 `['serve']` 拉起受管进程并轮询健康；`Stop` 只结束桌面自己拉起的受管进程，不会影响用户自行运行的 Ollama。内嵌服务只接受 loopback HTTP 地址，并把 `OLLAMA_HOST` 对齐到 `LLM_OLLAMA_BASE_URL`。
- 选用于分析：`Make available` 会把该模型以增量方式写入桌面 `.env`（向 `LLM_CHANNELS` 追加 `ollama`，并写入 `LLM_OLLAMA_PROVIDER` / `LLM_OLLAMA_BASE_URL` / `LLM_OLLAMA_MODELS`），保留既有键与密钥，幂等重复注册不产生重复行。注册在下次桌面启动后端时生效；若用户已通过 Web 设置页保存过渠道配置，设置页仍是最终权威（env 仅作为启动注入回退）。

内嵌服务的模型数据与系统安装完全隔离：Windows 安装版和免安装版使用 `<StockPulse.exe 所在目录>\data\ollama\models`，macOS 使用 Electron `userData/data/ollama/models`（默认通常为 `~/Library/Application Support/StockPulse/data/ollama/models`）。选择系统 Ollama 时不设置或改写 `OLLAMA_MODELS`，继续使用用户原来的系统模型库。Windows 自动更新会先发出进程树终止请求，并在有界超时内确认桌面托管的 Ollama 已停止，才把内嵌模型目录与其他运行时数据一起备份和恢复；终止失败会中止安装准备，不会在服务仍写入时复制模型库。更新时因此需要预留接近当前模型库大小的额外临时磁盘空间。

卸载与清理语义：Windows NSIS 安装版卸载时会删除安装目录，因此其中的内嵌模型通常一并删除；需要保留时应在卸载前复制上述目录。免安装版由用户删除应用目录，若只删除可执行文件，需手动删除同级 `data\ollama\models`。macOS 把 `.app` 移到废纸篓不会删除 Electron `userData`，需要释放模型空间时手动删除 `~/Library/Application Support/StockPulse/data/ollama/models`。这些操作都不删除系统 Ollama 自己的模型库。

安全基线：任何本地进程仅以系统白名单命令 `ollama` 或经内嵌清单验证的资源内绝对路径加固定参数数组启动，从不拼接命令行、从不使用 `shell`；模型名称先经严格校验（`model[:tag]` 字符集、长度上限）再进入任何 HTTP 请求体或 `.env`。解析日志只记录 `system` / `embedded` / `external-service` / `unavailable` 来源和内嵌版本，不记录二进制绝对路径。本地模型中心使用独立 `model-preload.js`，窗口保持 `contextIsolation: true`、`nodeIntegration: false`、`sandbox: true`，renderer CSP 为 `connect-src 'none'`（所有网络访问都在主进程完成），主进程拒绝新窗口与导航，IPC handler 校验调用方必须是当前本地模型窗口。模型管理日志只记录模型 ID 与渠道名，不写入任何密钥。
...
准备阶段会递归枚举归档展开后的每个普通文件与文件型符号链接路径，把完整集合及其逐文件 SHA-256 写入 schema v2 `runtime-manifest.json`；桌面解析器与 release workflow 都会重新枚举并校验相同集合。任一推理库、可选加速库、helper 或主程序缺失、增加或字节损坏时，内嵌 runtime 都不会被判定为可用。缓存归档也会在每次使用前重新校验精确字节数与 SHA-256；任何下载策略、大小、超时、SHA-256、解压、文件集合或可执行权限错误都会立即使构建失败，不会生成缺少运行时的安装包。
