...
- 发现与状态：先探测 `LLM_OLLAMA_BASE_URL`（默认 `http://127.0.0.1:11434`）的 `/api/tags`；只要该端点已有健康 Ollama 服务就直接复用，不会再启动第二个实例。端点不可用时，先以固定参数 `ollama --version` 解析系统安装，再验证安装包内 `resources/ollama/runtime-manifest.json`、必要文件和内嵌二进制；只有两者都不可用时才显示原有的 Ollama 安装引导。状态分为 `running` / `stopped` / `not-installed` / `starting` / `error`。
...
`apps/dsa-desktop/ollama-runtime.json` 固定官方 Ollama release 版本、平台归档、支持架构和 SHA-256；当前目标为 Windows x64 与 macOS arm64/x64。`npm run build` 的 `prebuild` 会调用 `scripts/prepare-embedded-ollama.js`，从该 release 的 HTTPS 地址下载归档、流式计算 SHA-256、校验必要文件并原子生成 gitignored 的 `apps/dsa-desktop/vendor/ollama/`。准备阶段还会把主程序和推理 helper 的逐文件 SHA-256 写入 `runtime-manifest.json`；桌面解析器与 release workflow 都会重新校验这些哈希，helper 被修改时不会把内嵌 runtime 判定为可用。缓存归档也会在每次使用前重新校验；任何下载、SHA-256、解压、必要文件或可执行权限错误都会立即使构建失败，不会生成缺少运行时的安装包。

electron-builder 通过 `extraResources` 把完整运行时目录与根目录 `THIRD_PARTY_NOTICES` 放进发行物；notices 包含所捆绑 Ollama 版本及其原生 runtime 组件的许可证和版权声明。版本升级时必须同时更新清单版本、官方归档 SHA-256、第三方组件版本与 notices、对应测试和实际安装包体积报告。
