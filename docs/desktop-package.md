...
内嵌服务的模型数据与系统安装完全隔离：Windows 安装版和免安装版使用 `<StockPulse.exe 所在目录>\data\ollama\models`，macOS 使用 Electron `userData/data/ollama/models`（默认通常为 `~/Library/Application Support/StockPulse/data/ollama/models`）。选择系统 Ollama 时不设置或改写 `OLLAMA_MODELS`，继续使用用户原来的系统模型库。Windows 自动更新会先停止桌面托管的 Ollama，再把内嵌模型目录与其他运行时数据一起备份和恢复；更新时因此需要预留接近当前模型库大小的额外临时磁盘空间。
...
`apps/dsa-desktop/ollama-runtime.json` 固定官方 Ollama release 版本、平台归档、支持架构和 SHA-256；当前目标为 Windows x64 与 macOS arm64/x64。`npm run build` 的 `prebuild` 会调用 `scripts/prepare-embedded-ollama.js`，从该 release 的 HTTPS 地址下载归档、流式计算 SHA-256、校验必要文件并原子生成 gitignored 的 `apps/dsa-desktop/vendor/ollama/`。缓存归档也会在每次使用前重新校验；任何下载、SHA-256、解压、必要文件或可执行权限错误都会立即使构建失败，不会生成缺少运行时的安装包。

electron-builder 通过 `extraResources` 把完整运行时目录与根目录 `THIRD_PARTY_NOTICES` 放进发行物；notices 包含所捆绑 Ollama 版本的 MIT 许可文本。版本升级时必须同时更新清单版本、官方归档 SHA-256、notices 版本、对应测试和实际安装包体积报告。
