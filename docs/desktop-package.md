...
- 删除：共享面板先取得同一配置版本和运行时身份摘要，再调用后端取消注册或预留未注册模型；后端阻止删除被主模型、Agent、Vision、fallback 或有效隐式主 channel 引用的模型，主进程也会在请求前重算摘要确认自身运行时未变化，并对 plain route 和 connection-aware ModelRef 做同等复核。后端对已注册模型和仅存在于 Ollama 的未注册模型都签发绑定模型、原运行时身份与预留后配置版本的短时一次性恢复 token，并在 token 待处理期间阻止同一模型经本地模型服务并发下载或重新注册。若 IPC 删除失败，恢复通道按精确快照乐观恢复原 catalog 注册；未注册模型只释放预留，不创建注册。即使 Ollama 已停止或暂不可达，也不会额外探测，版本或运行时身份冲突时则拒绝恢复。若权重删除成功，Web 端会重试一次幂等 token 撤销；撤销确认失败不会把已完成的删除误报为失败，未撤销的 token 会按短 TTL 过期。
