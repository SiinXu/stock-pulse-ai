...
## [Unreleased]
- [Fixed] Preserved structured diagnostic fields in prose log lines by narrowing the composite-key redaction walker to reject bare-whitespace prose joiners for the fail-closed and single-word paths, so debug messages such as retry summaries and webhook payload sizes no longer collapse `error_code=...`, `exception_type=...`, or `body_bytes=...` to `[REDACTED]` while canonical multi-word phrases (`api key`, `session token`) and structural long compounds still redact.
