...
- [改进] Web 应用外壳用 typed descriptor 固化九项一级导航并保留 owner 选定的 framed/floating 全局 `main` 表面；1023px 及以下提供唯一共享导航 `IconButton`、完整品牌与可直接访问的 Profile dialog，主题/语言收进 Profile，导航走 Drawer；业务 `ResponsiveRail` 在 768–1023px 使用 Navigation Drawer，在更窄移动端与 1024–1279px 使用页内 disclosure，1280px 起持久显示。1024–1279px 无显式偏好时默认 80px compact rail，保存的 80/240px 展开折叠选择在所有桌面断点保持有效。Drawer 仅在 primary same-window 路由激活时关闭，modifier/new-tab 保留原生行为；移动 route marker 转交持久 opener 供 Back 恢复。icon-only 导航统一 labelled Tooltip，44px Profile/偏好/路由目标与可滚动短视口导航、可达横向内容、跨断点焦点、light/dark、reduced-motion 由组件和浏览器回归覆盖。
