...
- [改进] Agent Runtime 按 ADR-002 恢复仅供测试/证据 harness 显式构造的 PydanticAI Single RUN POC：`start()` 现返回可观察、取消、订阅与等待的 live handle；完整可选依赖闭包精确锁定并由 `pydanticai-installed` CI 执行 `pip check`；cross-runtime conformance 使用显式 fixture ID 允许清单且未知 `single_run` fixture fail closed；Native 是唯一生产装配，无 factory/config/env/API selector、runtime fallback、用户设置或公开 API。
