...
- [Added] Added schema-versioned deterministic daily stock-analysis tasks with strict trading-day classification, durable occurrence audit records, bounded retries, exact side-effect coalescing, deployment-specific ownership, and execution through the existing analysis task queue.
- [Added] Added schema-versioned deterministic daily stock-analysis tasks with opaque future-schema reads, DST-safe wall times, exchange-timezone trading-day classification, durable occurrence audit records, serialized same-stock conflicts, bounded execution retries, full execution-contract coalescing, deployment-specific ownership, and execution through the existing analysis task queue.
- [Added] Added schema-versioned deterministic daily stock-analysis tasks with opaque future-schema reads and mutation CAS, DST-safe wall times, exchange-timezone trading-day classification, durable occurrence audit records, serialized same-stock conflicts, disable-safe bounded retries, full execution-contract coalescing, deployment-specific ownership, and execution through the existing analysis task queue.
