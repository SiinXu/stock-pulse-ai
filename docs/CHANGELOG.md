...
- [修复] Web 设置多选下拉改用视口定位并在空间不足时向上展开，避免被设置分组或弹窗的 `overflow` 边界裁切；弹层在窄屏中保留 8px 安全边距，并与触发控件保持一致宽度。
- [改进] Web 公共 API 错误提示改用紧凑的图标关闭、轻量详情折叠和无边框重试操作，长原始错误限制高度并保持 44px 可访问命中区域。
- [修复] Web 主题菜单选中态改用中性表面色和前景色，避免深色模式把品牌绿色叠加成突兀的橄榄色块。
