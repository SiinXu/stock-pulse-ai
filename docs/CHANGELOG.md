...
- [Fixed] Complete missing Simplified Chinese settings-help source copy for RSS feeds and multi-strategy deliberation, regenerate locale bundles, and align identical-to-English baseline plus playground story string allowances so web i18n gates pass.
- [Fixed] Backtest page clears production design-guard violations (token font sizes, native title, button overrides) and shares strategy run options (`minAge`/`limit`) plus core filters through `urlState` with stable research-route wire keys (#879 A1).
