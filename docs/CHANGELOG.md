...
- [改进] Web 应用外壳移除主内容最外层卡片边框、圆角与阴影，并用 typed descriptor 固化九项一级导航；1023px 及以下只保留一个共享 `IconButton` 全局菜单入口与完整品牌，主题/语言收进 Profile dialog，全局导航走 Drawer；业务 `ResponsiveRail` 在 768–1023px 使用 Navigation Drawer，在更窄移动端与 1024–1279px 使用页内 disclosure，1280px 起持久显示。1024–1279px 强制 76px 紧凑 Rail 且不覆盖宽屏折叠偏好或在 hover 时隐藏无替代动作的品牌；移动 Drawer 路由选择将返回焦点标记转交给持久 opener，Back 可恢复菜单按钮。跨断点焦点交接、Profile entry/restore、九个固定视口、light/dark、reduced-motion 与 Router H1 焦点由确定性浏览器 fixture 覆盖。
