...
## [Unreleased]
- [Fixed] Preserved structured diagnostic fields in prose log lines by preventing whitespace prose from triggering the composite-key walker's bounded fail-closed path and excluding punctuation-delimited prose prefixes from its loose single-word path, so retry summaries and webhook payload sizes no longer collapse `error_code=...`, `exception_type=...`, or `body_bytes=...` to `[REDACTED]` while sensitive whitespace labels (`password value`, `token value`), canonical multi-word phrases (`api key`, `session token`), and structural long compounds still redact.
