...
## [Unreleased]
- [修复] DecisionSignal 新增 canonical `presentation` 视图模型，以 `action` 统一动作方向，并集中提供本地化标签、置信度、摘要、风险和时间戳；API、通知、Web 卡片/详情/时间线与组合风险不再分别信任可能冲突的 `action_label` 或平铺展示字段，旧字段继续保留用于兼容。
