...
- [Added] Agent/strategy simulation sandbox V1: isolated config, fake clock, read-only or snapshot data, SIMULATION labels, production-isomorphic traces for agent variants, review-only promotion receipts, and hard fences that block production DecisionSignal / analysis-history / notification writes during sandbox runs (Refs #247, #202, #442).
