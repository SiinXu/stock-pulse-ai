...
## [Unreleased]
- [Changed] Unify Decision Signals selection on `selectedSignalId` with a fixed Context Chip for symbol, source, and status; closing the detail drawer keeps the chip reachable for the same id, and list/latest/timeline refresh by id without overwriting a newer entry point (Refs #1332).
- [Chore] Move optional `requirements-*.txt` extras into `requirements/` and third-party notices into `docs/legal/`, and slim `.env.example` comments without changing configuration keys or defaults.
