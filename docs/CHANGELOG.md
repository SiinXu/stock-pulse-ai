...
- [改进] Web 应用外壳移除主内容最外层卡片边框、圆角与阴影，并用 typed descriptor 固化九项一级导航；1023px 及以下只保留一个共享 `IconButton` 菜单入口与完整品牌，主题/语言收进 Profile dialog，导航走 Drawer；1024–1279px 强制 76px 紧凑 Rail 且不覆盖 1280px 以上的持久折叠偏好，业务 `ResponsiveRail` 在该区间并入正文并保持可折叠。跨断点关闭已打开 Drawer 时焦点交接到当前桌面路由或侧栏，Drawer 原本关闭时 resize 不移焦；九个固定视口、light/dark、reduced-motion 与 Router H1 焦点由确定性浏览器 fixture 覆盖。
