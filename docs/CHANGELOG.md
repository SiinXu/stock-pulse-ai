...
- [Changed] Mount interactive DCF sensitivity panel on Stock Details so valuation #238 UI is reachable outside Playground (Refs #238, #1008).
...
- [Fixed] Register 31 historical long-tail Settings config keys (batch 1: daily brief notify/persist, outbound allowlist, fundamental/portfolio risk, news intel, LLM timeout tokens, failure notify, paper cash) with correct categories, controls, defaults, validation, and bilingual titles/help.
...
- [Added] Reusable Web financial chart components: KlineChart (volume + MA overlays + zoom, dirty OHLC guards) and RiskHeatmap (score + level labels; color not sole carrier), with playground previews and unit tests; no page wiring (#142).
- [Added] Event calendar for watchlist/holdings with date certainty labels, opt-in fetch (EVENT_CALENDAR_ENABLED), impact preview via build_impact_context, API and Web /events view (#153).
...
- [Changed] Modularized the web API error catalog into parse/categorize/catalog/format modules with import-path parity and actionable remediation for high-impact codes.
