...
- [修复] Web Router 焦点协调器不再把同路径 query/hash 的 PUSH、REPLACE 或无稳定 opener 的 POP 误判为跨页导航，筛选、分页和 URL 视图状态更新后焦点继续保留在当前控件；跨路径导航仍在目标页就绪后聚焦 H1，同路径 Back 仅恢复唯一可见的稳定触发器；Toolbar 类型同步强制提供可访问名称。
