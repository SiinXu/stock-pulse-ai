...
The current production registry target is `202607190002_decision_signal_profile_schema`. `202607160001_migration_runner_registry` establishes additive metadata required by the ordered registry; `202607190001_llm_usage_telemetry_columns` and `202607190002_decision_signal_profile_schema` convert the startup `_ensure_*` steps that previously backfilled the `llm_usage` telemetry columns and the `decision_signals` `decision_profile` column, profile-aware indexes, and legacy backfill from `metadata_json` into formal migrations that repair legacy databases idempotently and are no-ops on fresh databases.
...
The upgrade does not remove existing business tables, fields, or data. The `llm_usage` telemetry and `decision_signals` `decision_profile` column/index/backfill startup steps are now the formal migrations `202607190001_llm_usage_telemetry_columns` and `202607190002_decision_signal_profile_schema`; `create_all` and the remaining `_ensure_*` steps (Portfolio idempotency scope, Intelligence scope value/unique index) stay as compatibility debt and continue to be converted into formal migrations one slice at a time.
