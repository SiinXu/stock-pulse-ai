...
The current production registry target is `202608260001_layered_memory_observation_schema`. That slice adds `layered_memory_observations` / `layered_memory_consent` / `layered_memory_access_audit` (#1118 storage contract; the observation table allows DELETE for clear/expiry; the audit table rejects UPDATE/DELETE via triggers; `downgrade` drops only those three tables and their indexes/triggers and does not touch `agent_episodes` / `agent_evolution_events`). Later additive migrations after the approval gate include skill-opinion outcomes, task-queue inflight checkpoints, watchlist groups, portfolio health snapshots, notification inbox read state, chat turn identity, analysis-history FTS, `llm_usage` cost attribution columns, `agent_predictions` (forecast verification rows with due-scan indexes and a resolved-row immutability trigger), nullable memory-write provenance columns on `decision_signal_feedback` / `decision_signal_outcomes` / `agent_predictions` / `agent_episodes`, optional `agent_run_feedback` / `agent_prediction_feedback` sidecars (#1105; they do not rewrite prediction actuals or append-only episodes), the optional `agent_episode_forward_returns` sidecar (#1096 CLI research-only forward-return buckets; episodes stay append-only), the optional `agent_episode_curator_grades` sidecar (#1096 CLI eval-fixture curator grades; episodes stay append-only), and the append-only `agent_evolution_events` table (#1113; UPDATE/DELETE are trigger-aborted; this is not security audit, episode, or curator-grade storage). `202607160001_migration_runner_registry` establishes additive metadata required by the ordered registry; `202607190001`–`202607190005` convert the startup `_ensure_*` steps that previously backfilled the `llm_usage` telemetry columns, the `decision_signals` `decision_profile` column/indexes/backfill, the `portfolio_idempotency_records` scope columns/unique-index/normalization/guard-trigger, the `intelligence_items` legacy scope-value normalization, and the `intelligence_items` rebuild from legacy url uniqueness to the scoped composite unique key into formal migrations. `202607240001_security_audit_events` adds the append-oriented `security_audit_events` table and bounded-query indexes; `202607240002_scheduled_task_schema` adds versioned scheduled-task definitions, definition/generation/dispatch fences, admission-failure counts, notification outcomes, and run records; `202607240003_investment_framework_schema` adds the personal investment framework aggregate and immutable version-history tables; `202607250001_approval_gate_schema` adds owner-scoped approval rules and one-shot approval proposals. Legacy databases are repaired idempotently; fresh databases create the tables from metadata and migrations verify their shape. Startup no longer runs ad hoc business schema DDL compatibility steps. See the [Human-in-the-Loop Approval Safety Gate](human-approvals_EN.md#migration-and-rollback) for the approval business and rollback contract.
...
A historical database validates any existing registry under the initialization lock, runs `metadata.create_all`, applies the ordered runner within the same transaction, and then proves the baseline. The explicitly supported release boundaries are:
...
The upgrade does not remove existing business tables, fields, or data. The `llm_usage`, `decision_signals`, `portfolio_idempotency_records`, and `intelligence_items` startup compatibility steps are now the formal migrations `202607190001`–`202607190005`; `202607240003` owns the personal investment framework tables, and `202607250001` owns the human-approval rule and proposal tables. Startup no longer runs any business schema DDL compatibility step, keeping only `create_all` (fresh baseline table creation) and the baseline record stamp, and every other schema change must ship as a new dedicated migration.
...
| `InvestmentFrameworkAnalysisContext` | `INVESTMENT_FRAMEWORK_CONTEXT_SCHEMA_VERSION` | `investment-framework-context-v1` | `schema_version` | read-only analysis-context adapter built on demand |
...
- **Consumers degrade gracefully on an unrecognized version.** Skip the block, return empty, or fall back to defaults; never hard-fail a historical read because its version does not match. Existing behavior: `src/market/structure_prompt.py` and `src/utils/data_processing.py` skip on a `schema_version` mismatch, and `AnalysisContextPack.pack_version` rejects unknown values via `Literal`.
- Scheduled-task APIs project an unrecognized definition as opaque metadata without parsing its payload. Mutations return a stable conflict, and a due slot is fenced once as `interrupted` without changing any definition field, so an older application neither executes nor rewrites a newer contract.
- Personal investment framework reads reject an unknown or corrupt persisted-content version with a stable data error. They neither misreport corruption as absence nor let an Agent silently consume drifted decision criteria. Future analysis assembly must degrade this error explicitly to no injection with a visible diagnostic.
