...
The current production registry target is `202607190001_llm_usage_telemetry_columns`. `202607160001_migration_runner_registry` establishes additive metadata required by the ordered registry; `202607190001_llm_usage_telemetry_columns` converts the `llm_usage` telemetry columns that a startup `_ensure_*` step previously backfilled into a formal migration that adds the missing columns idempotently on legacy databases and is a no-op on fresh databases that already have them.
...
The upgrade does not remove existing business tables, fields, or data. The `llm_usage` telemetry startup step is now the formal migration `202607190001_llm_usage_telemetry_columns`; `create_all` and the remaining `_ensure_*` steps (DecisionSignal profile, Portfolio idempotency scope, Intelligence scope value/unique index) stay as compatibility debt and continue to be converted into formal migrations one slice at a time.
