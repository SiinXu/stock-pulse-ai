...
The current production registry target is `202607240002_investment_framework_schema`. `202607160001_migration_runner_registry` establishes additive metadata required by the ordered registry; `202607190001`–`202607190005` convert the startup `_ensure_*` steps that previously backfilled the `llm_usage` telemetry columns, the `decision_signals` `decision_profile` column/indexes/backfill, the `portfolio_idempotency_records` scope columns/unique-index/normalization/guard-trigger, the `intelligence_items` legacy scope-value normalization, and the `intelligence_items` rebuild from legacy url uniqueness to the scoped composite unique key into formal migrations. `202607240002` adds the personal investment framework aggregate and immutable version-history tables; fresh and supported legacy startup paths receive an equivalent schema in one transaction, while direct migration execution creates and verifies both tables idempotently. See the [Personal Investment Framework Backend Contract](personal-investment-framework_EN.md) for the business and rollback contract.
...
The upgrade does not remove existing business tables, fields, or data. The `llm_usage`, `decision_signals`, `portfolio_idempotency_records`, and `intelligence_items` startup compatibility steps are now the formal migrations `202607190001`–`202607190005`, and `202607240002` owns the personal investment framework tables. Startup no longer runs any business schema DDL compatibility step, keeping only `create_all` (fresh baseline table creation) and the baseline record stamp, and every other schema change must ship as a new dedicated migration.
