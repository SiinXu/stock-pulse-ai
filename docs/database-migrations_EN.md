...
The current production registry target is `202607240002_scheduled_task_schema`. `202607160001_migration_runner_registry` establishes additive metadata required by the ordered registry; `202607190001`–`202607190005` convert the startup `_ensure_*` steps that previously backfilled the `llm_usage` telemetry columns, the `decision_signals` `decision_profile` column/indexes/backfill, the `portfolio_idempotency_records` scope columns/unique-index/normalization/guard-trigger, the `intelligence_items` legacy scope-value normalization, and the `intelligence_items` rebuild from legacy url uniqueness to the scoped composite unique key into formal migrations. `202607240001` adds the append-oriented `security_audit_events` table and bounded-query indexes; `202607240002` adds versioned scheduled-task definitions, definition/generation/dispatch fences, admission-failure counts, notification outcomes, and run records. Legacy databases are repaired idempotently; fresh databases create the tables from metadata and the migrations are no-ops. Startup no longer runs ad hoc business schema DDL compatibility steps.
