...
3. 同一优先级内，参与互换的 provider 必须处于 `closed`，且都至少达到 `PROVIDER_ADAPTIVE_PRIORITY_MIN_SAMPLES`，然后才按近期 `health_score`、成功率和平均延迟排序。样本不足或正在 `open` / `half_open` 的 provider 固定在原位置并作为不可跨越边界；其两侧 provider 仅在各自连续的 eligible 段内排序，既避免冷启动采样饥饿，也确保冷却结束后的静态位置恢复探测不会被健康源长期遮挡。
...
`DataProvider` 与 `DataProviderRegistration` 现在是稳定的插件契约。内建数据源也通过
统一 X2 registry 注册，但仍保留原有实例名、凭据开关、构造顺序和 priority；因此未加载
插件时，上述市场过滤、固定路由、熔断、缓存、自适应排序和诊断行为不变。

插件必须声明稳定 `provider_id`、工厂、支持市场和能力，并通过
`DataFetcherManager.plugin_registry` 所暴露的同一个 registry 注册。工厂在插件加载事务中
执行；失败、非法实现、ID 冲突或运行时名称冲突只会使该插件加载失败。卸载会精确移除该
插件拥有的 provider。数值 priority 只影响原本按 priority 排序的链路；美股指数、美股
个股、Longbridge 优先链和实时行情固定 built-in 链仍先执行，插件只作为其尾部 fallback。
manager 会固定工厂校验时的运行时名称，用于路由、健康状态、缓存归因和诊断；插件之后
...
`DataFetcherManager.get_daily_data()` 和 `_call_fetcher_method()` 不会统一包裹所有 provider
调用并施加通用 deadline。执行网络或 SDK I/O 的插件 provider 必须在自己的 transport/client
层为每次调用设置有限的 connect/read 或 SDK transport timeout；timeout 必须作为当前单次
provider attempt 的异常抛回 manager，由 manager 记录失败并继续符合条件的 fallback 链。
插件仍不得自建跨 provider fallback loop、缓存、路由表或动态 priority override。

`ApplicationServices.start_plugins()` 已接入显式 opt-in 的 external 目录扫描和生命周期管理：
仅当 `PLUGINS_DIR` 非空时，才按顺序扫描其直接子目录、注册合法插件并执行 load，关闭时再
反序 disable。该接线只提供 discovery 和 lifecycle；Data Provider 实现要真正激活，组合方
仍必须把 `PluginManager` 绑定到目标 `DataFetcherManager.plugin_registry` 的同一个 registry。
默认进程 root 不会凭空创建 process-wide `DataFetcherManager`。完整字段、能力标识、可加载
示例和验证命令见[插件扩展契约](plugin-extension-contract.md#data-providers)与
[Data Provider 插件作者指南（英文）](data-provider-plugin-authoring.md)。
