...
2. **Docker 部署 / WebUI 系统设置**：
   ...
   - `SCHEDULE_ENABLED`、`SCHEDULE_TIME`、`SCHEDULE_TIMES` 保存后会让已挂载 runtime scheduler 的 WebUI/API/Desktop 长运行进程按新配置启停或重建；独立进程或 Desktop 重启 `--serve-only` 时会恢复已启用的 daily jobs，但不会在启动时立即执行分析。默认 Compose 的 `server` 设置 `DSA_RUNTIME_SCHEDULER_SUPPRESS_START=true`，由 `analyzer` 独占遗留日批，避免与 `server` 重复执行
...
1. **推荐（Web 设置）**：打开 **设置 → 系统与安全 → 服务与日志**，启用「启用本地代理」，填写代理主机与端口后保存。字段带有重启提示：完整生效（尤其是关闭代理或刷新已缓存连接）需要重启进程。
2. **或编辑 `.env`**：
```bash
USE_PROXY=true
...
PROXY_PORT=10809
```
也可使用标准 `HTTP_PROXY=http://host:port`（需要完整 URL 或凭据时优先）。

> ⚠️ 注意：代理配置仅对本地运行生效，GitHub Actions 环境无需配置代理。`PROXY_HOST` 可能嵌入凭据，请勿写入截图、公开日志或共享配置导出。
...
**解决方案**：
1. 确保 `.env` 中设置了 `ADMIN_AUTH_ENABLED=true`；认证关闭时容器会拒绝非本机监听
2. 确保启动命令包含 `--host 0.0.0.0`（不能是 127.0.0.1）
3. 检查端口映射是否正确：
   ```yaml
   ports:
     - "8000:8000"
...
1. 只从项目的 [GitHub Releases](https://github.com/SiinXu/stock-pulse-ai/releases) 下载附件，并确认安装包架构与 Mac 一致：Apple 芯片使用 `stockpulse-macos-arm64-<tag>.dmg`，Intel 芯片使用 `stockpulse-macos-x64-<tag>.dmg`。不要对第三方转载或来源不明的应用绕过 Gatekeeper。
2. 将 `StockPulse` 拖入“应用程序”，先在“系统设置 → 隐私与安全性”中尝试“仍要打开”。
...
> 📌 相关上游 Issue: [ZhuLinsen/daily_stock_analysis#2113](https://github.com/ZhuLinsen/daily_stock_analysis/issues/2113)
