...
| [Agent Router Rules Library](agent-router_EN.md) | Rules-first AgentRouter plus structured fact projection; dashboard `run()` applies the router and records run-local metadata; Chat skips `_execute_pipeline` on `incremental_tool` (#1120 slices 1–5; factory remains not wired; [Chinese](agent-router.md)) |
