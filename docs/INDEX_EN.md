...
| [Agent Router Rules Library](agent-router_EN.md) | Rules-first AgentRouter plus structured fact projection; dashboard `AgentOrchestrator.run()` applies the router once per run, honors constructor/Settings depth unless an explicit override is provided, restores constructor mode, and records the secret-free decision on run-local metadata (#1120 slices 1–4; Chat/factory remain not wired; [Chinese](agent-router.md)) |
...
| [Prediction verification safe rollout](prediction-verification-rollout_EN.md) | Issue #1115 operator rollout order, issue-example-to-landed-key mapping, current deferrals ([Chinese](prediction-verification-rollout.md)) |
...
| [Agent prediction persistence](agent-prediction-persistence.md) | A3 `agent_predictions` schema and CAS store (Issue #1112); optional #1105 run/prediction feedback sidecar APIs; Web professional reports expose the run-feedback control |
| [EvolutionEvent audit log](evolution-events.md) | Append-only EvolutionEvent store and inclusive UTC time-range / exact-type query (#1113 first slice; producers not wired; [中文](evolution-events_CN.md)) |
...
| [Sentiment Analysis Pipeline](sentiment-analysis-pipeline_EN.md) | News/event sentiment evidence pipeline with source traceability, freshness, confidence, and AnalysisContextPack evidence role (#179) |
...
| [Shared runtime session contract owners](runtime-session-contract-owners.md) ([中文](runtime-session-contract-owners_CN.md)) | BoundToolSession / TaskRunContext / HTTP cancel / ToolAccessContext / generation-backend probe / config-registry owners, paired tests, fail-closed fence reads, and deprecation path (#1055 T1–T3) |
...
| [Service-split template](service-split-template.md) | Behavior-preserving facade / re-export checklist, ratchet verification, shim-removal criteria, a historical audit of merged PR #1352, and independently accepted end-to-end evidence from PR #1547 (#1088) |
...
| [Web UI Foundation Contract](web-ui-foundation.md) | Semantic controls, visible sizes, coarse-pointer targets, surface-role/density D5 ratchet (#877), native-button adoption ratchet (#878), Theme token freeze Phase 0 (#1300), guardrails, and migration ownership |
...
| [Web Runtime Performance Budgets](web-runtime-performance.md) | Issue #883 ten surfaces plus T16 DataTable virtualization, per-scenario blocking/observe/skip gates, and aggregate gzip families; [中文](web-runtime-performance_CN.md) |
