...
4. **Production Chat opt-in** — the same flag gathers evidence through `try_gather_with_planning` on `AgentExecutor.chat` and single-symbol `AgentOrchestrator.chat`, then synthesizes a free-form answer (`parse_dashboard=False`). `chat_path=incremental_tool` skips both the full pipeline and the planning gather. Compare multi-symbol Chat gathers **per symbol** inside `_execute_multi_symbol_chat` (`product_path=agent_orchestrator_chat`), then still runs that leg's `_execute_pipeline` and `_synthesize_multi_symbol_chat`. Gather failure is fail-closed for that leg (no extra classic tool gather); remaining legs still synthesize.
...
Default remains **off**: classic ReAct RUN/Chat/Research is byte-stable when the switch is false. Durable product UI is still remaining.
...
- Trace channel: the loop emits structured plan/action/observation/replan/terminate events (`agent.plan` … `agent.terminate`) through the shared agent observability channel (issue #1078; taxonomy aligned with the unified run-trace draft in #1125). Events carry `run_id`, optional `step`, `reason` / `error_type` when applicable, and a budget snapshot (tool calls, observation replans, tokens). Terminate always includes a non-empty reason. The same ordered list is on `PlanExecutionResult.trace_events` so one run can be reconstructed without log diving. Phase/tool/decision events continue as before. Recording is gated by `AGENT_OBSERVABILITY_ENABLED` (default on; set false to disable). Full structured metadata remains available via `PlanExecutionResult.to_metadata()`. Local and metadata event lists share `MAX_PLANNING_TRACE_EVENTS` (200); step observations remain capped by `MAX_TRACE_STEPS` (64).
...
| --- | --- | --- |
| `AGENT_PLANNING_ENABLED` | `false` | Master switch for `AgentExecutor.run`, Chat gather (including compare-chat legs), and Deep Research sub-question gather |
