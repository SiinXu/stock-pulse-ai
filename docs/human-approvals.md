...
Web 页面位于 `/approvals`，从 Home 的“待办”卡片进入，不新增一级导航域。页面显示待审批/终态、倒计时、原始与保守信号、批准/拒绝控件，以及最小规则设置。批准与拒绝会打开二次确认，并用本地化文案标明动作与目标；Escape、点击遮罩或取消会保持提案为待审批。重复提交会被本地禁用。当决策接口返回 `409` 时，确认层会保持忙碌并继续持有文档隔离锁（应用根节点的 `inert` / `aria-hidden`，以及 `body` 的 `overflow: hidden`），直到服务器刷新完成，然后才关闭确认层并恢复页面交互。未知状态/信号/风险代码会以净化后的诊断值显示，而不是未标注的机器码。
