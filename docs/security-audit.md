...
| Sensitive config create/update | `system_config.write` | **Landed** | `SystemConfigService.update` is the only writer. HTTP PUT `/config` injects the recorder with `source=http_put` and no longer double-emits. Nested `.env` import stays `system_config.import` only. |
...
DAG-5 records `system_config.write` once at `SystemConfigService.update`. HTTP PUT no longer double-emits. Nested import stays `system_config.import` only. Attempt-store failure does not call `apply_updates`. After persist, completion-store failure is HTTP `503` `operation_completed=true` plus `config_version` / `applied_count` / `reload_triggered` on every write-capable adapter (PUT, profiles, onboarding, local-model, watchlist, legacy migration). Rolled-back `runtime_activation_failed` keeps domain 400.
