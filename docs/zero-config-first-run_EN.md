...
- Completing the local-model path starts a real brief analysis (`notify: false`) only after the selected local model is the configured primary route. If download or import preserves an existing cloud/CLI primary, the wizard stays on that step until the user clicks **Set as primary**. Market data may still be fetched. This is not the offline `LOCAL_ONLY_MODE` path.
