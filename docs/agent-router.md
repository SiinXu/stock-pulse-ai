...
## Chat incremental 跳过（第五切片 / AC3）

`AgentOrchestrator.chat()` 从结构化事实（`entry_kind=chat`、StockScope、可选的 context `tool_suitable` / news / risk / override）投影并路由一次，**不**解析用户消息。`tool_suitable` 缺省为 false，因此未带该标志的同标的 follow-up 仍是 `full_repipeline`。

当 `chat_path=incremental_tool` 时，跳过 `_execute_pipeline` 与 `_execute_multi_symbol_chat`，用 Chat 工具注册表跑 `run_agent_loop`，再走既有 risk-gate 与历史写入。Chat 路由拒绝返回 `AGENT_CHAT_FAILURE_MESSAGE` 加上 secret-free 的 `planning_metadata["router_decision"]`。
