...
| `src/data_provider/efinance_parts/` | efinance implementation ownership by capability domain: `etf` (ETF history fetch and ETF realtime quote), `realtime` (stock realtime quote), plus `facade_bind` helpers | Stock-path history/rankings/info bodies, market-stats/index methods, module-level code and timeout helpers, and cross-provider manager policy (ADR-005) |
