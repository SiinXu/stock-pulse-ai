...
| Public path | Role |
| --- | --- |
| `src.data_provider.base` | Canonical facade and current home of manager/fetcher workflows still mixed in, re-exports of extracted pure helpers/errors/chip helpers, and rebound capability-catalog / health / daily-cache / daily-execution / realtime field-trust / realtime quote orchestration / chip-distribution orchestration / stock-name lookup / money-flow cache / money-flow orchestration / fundamental cache / fundamental loaders / fundamental-config accessor / CN fundamental sub-blocks / fundamental payload helpers / rankings / market-overview / belong-board / prefetch / destructor descriptors. |
...
## Ownership Map (after destructor extraction)
...
| `src/data_provider/base_parts/` | `BaseFetcher` implementation ownership by capability domain: `daily_pipeline` (daily template method, clean, indicators), `market_stubs` (default market-overview/rankings `return None` stubs), plus `facade_bind` re-export | Abstract hooks `_fetch_raw_data` / `_normalize_data`, `random_sleep`, manager destructor (`manager_parts/del_methods.py`) / timeout-slot construction, manager-layer policy (`manager_parts`), and provider-specific bodies |
...
| `src/data_provider/manager_parts/tickflow_lifecycle_methods.py` | Manager-owned TickFlow lifecycle rebound onto `DataFetcherManager`: `_get_tickflow_fetcher` (create/replace/registry reuse; converts the former `get_config()` site to `self._get_fundamental_config()`) and `close` (best-effort TickFlow release). Rebound `__del__` lives in `del_methods` and still calls rebound `self.close()` | Prefetch (`prefetch_methods`), `_init_default_fetchers`, timeout slot construction, remaining `get_config()` sites, market-overview routing (`get_main_indices` / `get_market_stats` still call rebound `_get_tickflow_fetcher`), destructor (`del_methods`), capability inventory |
| `src/data_provider/manager_parts/del_methods.py` | Manager-owned destructor rebound onto `DataFetcherManager`: `__del__` (best-effort `self.close()`, swallows `Exception`). Generic dunder binding skips `__del__`, so the bind helper attaches it explicitly. Facade helper `__module__` / `__qualname__` stay `src.data_provider.base` / `DataFetcherManager.__del__` | `close` (`tickflow_lifecycle_methods`), `__init__`, timeout slot construction, prefetch, default-fetcher init, remaining `get_config()` sites |
| `src/data_provider/manager_parts/prefetch_methods.py` | Manager-owned prefetch rebound onto `DataFetcherManager`: `prefetch_realtime_quotes` (Local Only skip, config disable, early-source routing, TickFlow batch, first-code cache warm) and `prefetch_daily_klines` (TickFlow daily K-line warm). Converts the former `prefetch_realtime_quotes` `get_config()` site to `self._get_fundamental_config()` | Default-fetcher init (`init_default_fetchers_methods`), facade `__init__`, destructor (`del_methods`), TickFlow lifecycle (`_get_tickflow_fetcher` / `close`), realtime routing (`get_realtime_quote`), capability lookup, Local Only policy body (`is_market_data_local_only`), BaseFetcher methods |
| `src/data_provider/manager_parts/init_default_fetchers_methods.py` | Manager-owned default-fetcher initialization rebound onto `DataFetcherManager`: `_init_default_fetchers(self, config)` (built-in fetchers, optional Tushare/TickFlow/Longbridge/Finnhub/AlphaVantage skip, crypto `is True` default-off, `_register_builtin_data_provider` + `_sync_registered_data_providers`). Does not import or call `get_config` and does not use `_get_fundamental_config()`. Facade `__init__` dynamically imports `src.config.get_config` in the no-explicit-fetchers branch and calls rebound `self._init_default_fetchers(get_config())`. Private signature now requires `config`; there is no `src.data_provider.base.get_config` seam | Facade `__init__` (still constructs defaults when no explicit fetchers are passed), destructor (`del_methods`), timeout-slot construction, prefetch, TickFlow lifecycle, provider priority/fallback/circuit/cache policy |
...
| `src/data_provider/efinance_fetcher.py` | Compatibility facade for the efinance provider: public class, constants (`_EF_CALL_TIMEOUT`, `USER_AGENTS`, caches, `_ETF_SH_PREFIXES` / `_ETF_SZ_PREFIXES`), and ADR-006 method rebinding / timeout clone / error-classifier clone / symbols clone seams | New capability-domain bodies (add under `efinance_parts/`) |
| `src/data_provider/efinance_parts/` | efinance implementation ownership by capability domain: `etf` (ETF history fetch and ETF realtime quote), `realtime` (stock realtime quote), `market_boards` (main indices, market stats, sector rankings), `history` (`_fetch_raw_data` / `_fetch_stock_data` / `_normalize_data`), `info` (`get_base_info` / `get_belong_board` / `get_enhanced_data`), `rate_limit` (UA rotation, request pacing, history-failure formatting), `timeout_client` (bounded-wait SDK wrapper `_ef_call_with_timeout`), `eastmoney_errors` (Eastmoney request-failure classifier `_classify_eastmoney_error`), `symbols` (ETF/US classifiers and Eastmoney ETF secid mapping), plus `facade_bind` helpers | `USER_AGENTS` / caches, and cross-provider manager policy (ADR-005) |
...
| `src/data_provider/tickflow_parts/` | TickFlow implementation ownership by capability domain: `market_boards` (main indices, market stats, sector rankings), `history` (`_fetch_raw_data` / `_normalize_data`), `realtime` (`get_realtime_quote` / `_quote_to_unified_quote` / `_format_provider_timestamp`), `stock_identity` (`get_stock_name` / `_extract_instrument_name` / `get_stock_list`), `prefetch` (`prefetch_daily_klines` / `_iter_batch_frames` / `prefetch_realtime_quotes`), plus `facade_bind` helpers | Quote-cache helpers, `_get_realtime_cache_ttl`, `_dedupe_symbols`, universe parse, capability probing, client access, and cross-provider manager policy (ADR-005) |
...
| `src/data_provider/pytdx_fetcher.py` | Compatibility facade for the Pytdx provider: public class, connection/session helpers, market-code/list cache, stock-name/realtime, and ADR-006 method rebinding | New capability-domain bodies (add under `pytdx_parts/`) |
| `src/data_provider/pytdx_parts/` | Pytdx implementation ownership by capability domain: `history` (`_fetch_raw_data` / `_normalize_data`), plus `facade_bind` re-export | Connection/session (`_get_pytdx` / `_pytdx_session` / cooldown), market-code/list cache, stock-name/realtime, and cross-provider manager policy (ADR-005) |
| `src/data_provider/baostock_fetcher.py` | Compatibility facade for the Baostock provider: public class, session/code-conversion, stock-name/list, and ADR-006 method rebinding | New capability-domain bodies (add under `baostock_parts/`) |
| `src/data_provider/baostock_parts/` | Baostock implementation ownership by capability domain: `history` (`_fetch_raw_data` / `_normalize_data`), plus `facade_bind` re-export | Session (`_get_baostock` / `_baostock_session`), code conversion, stock-name/list, and cross-provider manager policy (ADR-005) |
...
| `src/data_provider/longbridge_fetcher.py` | Compatibility facade for the Longbridge provider: public class, credentials/OAuth helpers, SDK context and cooldown policy, and ADR-006 method rebinding / symbols clone seams | New capability-domain bodies (add under `longbridge_parts/`) |
| `src/data_provider/longbridge_parts/` | Longbridge implementation ownership by capability domain: `realtime` (quote routing, static-info cache, volume ratio), `history` (`_fetch_raw_data` / `_normalize_data`), `symbols` (US/HK classifiers and Longbridge symbol conversion), plus `facade_bind` re-export | SDK context construction and availability/cooldown policy (they read application config directly), and cross-provider manager policy (ADR-005) |
...
Slice 26 rebinds `__del__` from `manager_parts/del_methods.py` while
preserving its `src.data_provider.base` module, qualname
`DataFetcherManager.__del__`, descriptor kind, globals, and patch
behavior. The six-line body still best-effort calls rebound
`self.close()` and swallows `Exception`. Generic dunder binding skips
`__del__`, so the bind helper attaches it explicitly. Facade class
body keeps `close = None` then `__del__ = None` with no live
FunctionDef for this name. The owner does not import `close`,
`__init__`, timeout slots, or `get_config()`. Import the facade
(`src.data_provider.base` / `src.data_provider`), not
`manager_parts.del_methods`.

Slice 26 leftover on the facade: timeout slot construction in
`__init__`.
