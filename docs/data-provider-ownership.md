...
| `src/data_provider/longbridge_parts/` | Longbridge implementation ownership by capability domain: `realtime` (quote routing, static-info cache, volume ratio), `history` (`_fetch_raw_data` / `_normalize_data`), plus `facade_bind` re-export | SDK context construction and availability/cooldown policy (they read application config directly), and cross-provider manager policy (ADR-005) |
