...
| `src/data_provider/tickflow_parts/` | TickFlow implementation ownership by capability domain: `market_boards` (main indices, market stats, sector rankings), `history` (`_fetch_raw_data` / `_normalize_data`), `realtime` (`get_realtime_quote` / `_quote_to_unified_quote` / `_format_provider_timestamp`), `stock_identity` (`get_stock_name` / `_extract_instrument_name` / `get_stock_list`), `prefetch` (`prefetch_daily_klines` / `_iter_batch_frames`), plus `facade_bind` helpers | Quote-cache helpers, `_get_realtime_cache_ttl`, `prefetch_realtime_quotes`, `_dedupe_symbols`, universe parse, capability probing, client access, and cross-provider manager policy (ADR-005) |
