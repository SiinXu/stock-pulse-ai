...
| `src/data_provider/tickflow_parts/` | TickFlow implementation ownership by capability domain: `market_boards` (main indices, market stats, sector rankings), `history` (`_fetch_raw_data` / `_normalize_data`), `realtime` (`get_realtime_quote` / `_quote_to_unified_quote` / `_format_provider_timestamp`), plus `facade_bind` helpers | Quote-cache helpers, `_get_realtime_cache_ttl`, `prefetch_realtime_quotes`, daily prefetch, capability probing, client access, stock identity, and cross-provider manager policy (ADR-005) |
