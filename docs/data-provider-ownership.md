...
| `src/data_provider/tickflow_parts/` | TickFlow implementation ownership by capability domain: `market_boards` (main indices, market stats, sector rankings), `history` (`_fetch_raw_data` / `_normalize_data`), plus `facade_bind` helpers | Realtime fetch bodies, prefetch paths, capability probing, client access, and cross-provider manager policy (ADR-005) |
