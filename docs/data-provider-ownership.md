...
| `src/data_provider/manager_parts/init_default_fetchers_methods.py` | Manager-owned default-fetcher initialization rebound onto `DataFetcherManager`: `_init_default_fetchers` (built-in fetchers, optional Tushare/TickFlow/Longbridge/Finnhub/AlphaVantage skip, crypto `is True` default-off, `_register_builtin_data_provider` + `_sync_registered_data_providers`). Keeps `from src.config import get_config` | Facade `__init__` (still calls rebound `_init_default_fetchers()`), `__del__`, timeout-slot construction, prefetch, TickFlow lifecycle, provider priority/fallback/circuit/cache policy |
...
descriptor kind, globals, and patch behavior. The body still calls
`from src.config import get_config` (not `_get_fundamental_config()`),
skips unconfigured optional providers, and activates CoinGecko only
when `crypto_provider_enabled is True`. Facade `__init__` still calls
rebound `self._init_default_fetchers()`. Import the facade
(`src.data_provider.base` / `src.data_provider`), not
`manager_parts.init_default_fetchers_methods`. After this slice,
`base.py` has zero remaining `get_config()` sites.
