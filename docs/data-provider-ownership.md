...
| `src/data_provider/manager_parts/init_default_fetchers_methods.py` | Manager-owned default-fetcher initialization rebound onto `DataFetcherManager`: `_init_default_fetchers_with_config` (built-in fetchers, optional Tushare/TickFlow/Longbridge/Finnhub/AlphaVantage skip, crypto `is True` default-off, `_register_builtin_data_provider` + `_sync_registered_data_providers`). Does not import or call `get_config` and does not use `_get_fundamental_config()`. Facade `_init_default_fetchers` stays a live FunctionDef that calls patchable `src.data_provider.base.get_config` and passes the resolved config into the helper | Facade `__init__` (still calls live `_init_default_fetchers()`), `__del__`, timeout-slot construction, prefetch, TickFlow lifecycle, provider priority/fallback/circuit/cache policy |
...
Slice 25 rebinds `_init_default_fetchers_with_config` from
`manager_parts/init_default_fetchers_methods.py` while preserving
its `src.data_provider.base` module, qualname, signature,
descriptor kind, globals, and patch behavior. Facade
`_init_default_fetchers` stays a live FunctionDef that calls
`get_config()` (the patchable `src.data_provider.base.get_config`
live-lookup symbol, which also preserves `src.config.get_config`
patches) and passes that resolved config into the rebound helper.
The helper does not import or call `get_config` and does not use
`_get_fundamental_config()`. The body still skips unconfigured
optional providers and activates CoinGecko only when
`crypto_provider_enabled is True`. Facade `__init__` still calls
live `self._init_default_fetchers()`. Import the facade
(`src.data_provider.base` / `src.data_provider`), not
`manager_parts.init_default_fetchers_methods`. After this slice,
`base.py` still owns the default-fetcher `get_config()` lookup on
the thin delegator so the new-module config-access ratchet stays
clean.
