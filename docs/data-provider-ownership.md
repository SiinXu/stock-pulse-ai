...
| Public path | Role |
| --- | --- |
| `src.data_provider.base` | Canonical facade and current home of manager/fetcher workflows still mixed in, re-exports of extracted pure helpers/errors/chip helpers, and rebound capability-catalog / health / daily-cache / daily-execution / realtime field-trust / realtime quote orchestration / money-flow cache / money-flow orchestration / fundamental cache / belong-board / rankings descriptors. |
...
## Ownership Map (after rankings orchestration extraction)
...
| `src/data_provider/manager_parts/realtime_quote_methods.py` | Manager-owned realtime quote orchestration rebound onto `DataFetcherManager`: timestamp parse/enrich, plugin realtime fallback, `get_realtime_quote` routing, quote supplement helpers, and Longbridge preference | Field-trust attempt bookkeeping (`realtime_field_trust_methods`), `prefetch_realtime_quotes`, Local Only / outbound HTTP policy, chip / money-flow / stock-name / fundamental workflows, and rankings (`rankings_methods`) |
...
| `src/data_provider/manager_parts/rankings_methods.py` | Manager-owned rankings orchestration rebound onto `DataFetcherManager`: `_get_sector_rankings_with_meta`, `get_sector_rankings`, `_copy_ranking_rows`, `clear_concept_rankings_cache_for_tests`, `get_concept_rankings`, `get_hot_stocks`, and `get_limit_up_pool` | `BaseFetcher` provider methods of the same names, `DataFetcherManager.get_main_indices` / `get_market_stats`, concept-rankings TTL/lock/dict class attributes, capability inventory, and `get_board_context` (which still *calls* rebound `_get_sector_rankings_with_meta`) |
...
| `src/data_provider/base.py` (remainder) | `BaseFetcher` / `DataFetcherManager`, manager-owned priority/plugin policy and state, fundamental/chip/stock-name workflows still co-located, money-flow TTL/size class attributes plus cache/circuit instance state, concept-rankings TTL/lock/dict class attributes, `_SUPPLEMENT_FIELDS`, facade bindings/re-exports | New pure symbol rules, typed errors, chip helpers, capability-catalog mechanics, or extracted health/daily-cache/daily-execution/field-trust/realtime-quote/money-flow-cache/money-flow-orchestration/fundamental-cache/belong-board/rankings descriptors |
...
fundamental stay on the facade. Rankings travel with Slice 12. `get_realtime_quote` is rebound
...
Slice 12 rebinds rankings orchestration descriptors from
...
stay on the facade. `get_board_context` stays on the facade and still
calls rebound `_get_sector_rankings_with_meta`. `_copy_ranking_rows`
remains a `staticmethod` and `clear_concept_rankings_cache_for_tests`
remains a `classmethod`. Import the facade
(`src.data_provider.base` / `src.data_provider`), not
`manager_parts.rankings_methods`.
