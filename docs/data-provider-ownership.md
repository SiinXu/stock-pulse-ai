...
| `src/data_provider/efinance_parts/` | efinance implementation ownership by capability domain: `etf` (ETF history fetch and ETF realtime quote), `realtime` (stock realtime quote), `market_boards` (main indices, market stats, sector rankings), `history` (`_fetch_raw_data` / `_fetch_stock_data` / `_normalize_data`), `info` (`get_base_info` / `get_belong_board` / `get_enhanced_data`), `rate_limit` (UA rotation, request pacing, history-failure formatting), plus `facade_bind` helpers | module-level timeout/code helpers and cross-provider manager policy (ADR-005) |
