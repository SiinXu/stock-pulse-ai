...
| `src/data_provider/efinance_fetcher.py` | Compatibility facade for the efinance provider: public class, constants (`_EF_CALL_TIMEOUT`, `USER_AGENTS`, caches), module-level code/error classifiers, and ADR-006 method rebinding / timeout clone seams | New capability-domain bodies (add under `efinance_parts/`) |
| `src/data_provider/efinance_parts/` | efinance implementation ownership by capability domain: `etf` (ETF history fetch and ETF realtime quote), `realtime` (stock realtime quote), `market_boards` (main indices, market stats, sector rankings), `history` (`_fetch_raw_data` / `_fetch_stock_data` / `_normalize_data`), `info` (`get_base_info` / `get_belong_board` / `get_enhanced_data`), `rate_limit` (UA rotation, request pacing, history-failure formatting), `timeout_client` (bounded-wait SDK wrapper `_ef_call_with_timeout`), plus `facade_bind` helpers | module-level error classifier / code classifiers / `USER_AGENTS` / caches, and cross-provider manager policy (ADR-005) |
