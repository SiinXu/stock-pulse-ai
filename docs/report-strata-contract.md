...
3. **模型推断**
4. **风险与反证**
5. **与个人框架对齐** — 复用 #465 本地框架槽位；未配置时 `not_configured`
...
成功 JSON 解析路径会调用 `attach_report_strata_to_dashboard`：若 LLM 已输出分层则规范化填充；否则写入空六层结构（框架未配置 + 免责声明）。这保证**新 run 的持久化/渲染 payload 始终带有六层槽位**，不等同于 LLM 已填满事实条目。
