...
- This module does **not** render markdown, templates, or Web UI itself.
- **Report top section (Issue #148):** `src/services/notification_delta_formatter.py` (`prepend_report_delta_section` / `build_delta_section_markdown`) prepends a compact delta section to generated stock reports (dashboard / brief / single-stock). Both scheduled local-save and manual aggregate paths share this generator. Delivery single-stock path passes the active report_type for same-shape history selection. Web/history markdown via `HistoryService._generate_single_stock_markdown` uses a record-aware previous-sibling loader through `compare_analyses`. First analysis (`missing_history`) is never labeled as "no change." Non-finite numeric deltas are omitted from presentation lines; comparison failures leave the original report body and log a structured warning.
- **Notifications:** optional `NOTIFICATION_DELTA_FIRST` reuses the same presentation helpers (idempotent when the report body already includes the section); it does not recompute diffs.
- **Web version compare (T18 / #188):** calls `compare_analyses` via the report-version adapter.
- Consumers must not invent a parallel comparison implementation in agent or renderer code.
