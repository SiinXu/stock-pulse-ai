...
"""Partition offline pytest modules into duration-balanced shards.

Adapts the *idea* of upstream daily_stock_analysis sharding (duration-greedy
whole-module partitions) without copying that repository's files.
...
def discover_test_files(repo_root: Path = REPO_ROOT) -> list[str]:
    """Return every ``tests/**/test_*.py`` module under the repo root."""
    return sorted(
...
def load_durations(path: Path = DEFAULT_DURATIONS_PATH) -> dict[str, float]:
    """Load optional duration weights; empty map when the file is absent."""
    if not path.is_file():
        return {}
...
def partition_test_files(
    ...
    """Greedily balance whole test modules; preserve order within each shard."""
    if splits < 1:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--list-only",
        action="store_true",
        help="Print selected module paths and exit without running pytest",
    )
