...
verify_expected_unsigned_app() {
  ...
  bash "${ROOT_DIR}/apps/dsa-desktop/scripts/macos-signature-audit.sh" check "${app_path}"
  ...
  if [[ "${signature_details}" != *"code object is not signed at all"* ]]; then
    echo "ERROR: application bundle has an unreadable or invalid signature: ${app_path}" >&2
    echo "${signature_details}" >&2
    exit 1
  fi
  ...
  if [[ "${assessment_output}" == *"code has no resources but signature indicates they must be present"* ]]; then
    echo "ERROR: Gatekeeper detected a broken-signature defect." >&2
    exit 1
  fi
...
verify_unsigned_dmg() {
  ...
  if [[ ! -d "${mounted_app}" ]]; then
    echo "ERROR: application bundle not found in mounted DMG: ${mounted_app}" >&2
    exit 1
  fi
...
shopt -s nullglob
app_candidates=(dist/mac*/"${PRODUCT_NAME}.app")
dmg_candidates=(dist/*.dmg)
shopt -u nullglob

if [[ "${#app_candidates[@]}" -ne 1 ]]; then
  echo "ERROR: expected one unpacked macOS app, found ${#app_candidates[@]}." >&2
  exit 1
fi
if [[ "${#dmg_candidates[@]}" -ne 1 ]]; then
  echo "ERROR: expected one macOS DMG, found ${#dmg_candidates[@]}." >&2
  exit 1
fi

verify_expected_unsigned_app "${app_candidates[0]}"
verify_unsigned_dmg "${dmg_candidates[0]}" "${PRODUCT_NAME}"
