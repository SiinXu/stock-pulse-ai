...
class ReportVersionCompareService:
    ...
    def list_runs(
        ...
        *,
        ...
        for record in records:
            if report_type is None and str(getattr(record, "report_type", "") or "") == "market_review":
                continue
            items.append(self._record_to_run_item(record))

        return {
            "stock_code": code,
            "total": total if report_type is not None else (
                len(items) if page_norm == 1 and len(items) < limit_norm else total
            ),
            "page": page_norm,
            "limit": limit_norm,
            "items": items,
    ...
    def _record_to_run_item(self, record: Any) -> Dict[str, Any]:
        ...
        fingerprint = self._fingerprint_components(config_components)
    ...
    def _build_config_diff(
        ...
        *,
        ...
        return {
            ...
            "identical": base_fingerprint == target_fingerprint and bool(base_fingerprint),
