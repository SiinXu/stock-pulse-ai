...
Phase 1 structured analysis-target parsing (semantic port of upstream
ZhuLinsen/daily_stock_analysis@a8f084bf / issue #2063).
...
Three core contracts are pinned by this module and the regression tests:

1. **Prefix-whitelisted index** — a prefixed token whose prefix is ``sh``/``sz``
   and whose bare code is registered in :class:`IndexRegistry` is parsed as an
   ``index``.
2. **Bare code defaults to stock** — a bare numeric code without any prefix
   always parses as a ``stock`` regardless of whether the same code happens to
   belong to a known index (e.g. ``"000300"`` is a stock, ``"sh000300"`` is an
   index).
3. **Prefixed unknown degrades to stock** — a prefixed token whose prefix is
   not a recognized ``sh``/``sz`` index prefix, or whose bare code is not in the
   index registry, degrades to a ``stock`` rather than raising.

``split_stock_list`` lives in :mod:`src.utils.stock_list` (stdlib leaf) and is
re-exported here for existing callers. Portfolio helpers
(:func:`normalize_stock_codes`, :func:`resolve_portfolio_stock_list`) remain
fork-specific entry points used by CLI / Futu scope resolution.
"""
...
# ---------------------------------------------------------------------------
# Local normalization adapters.
#
# Upstream parse_analysis_target depends on stock_code_utils helpers that this
# fork does not yet expose (``_normalize_code_and_exchange`` /
# ``_split_explicit_exchange``). Keep those adapters local so the Phase-1
# contract is self-contained within the writable parser surface while still
# reusing the fork's digit-length tables and exchange validation rules.
# ---------------------------------------------------------------------------
def _split_explicit_exchange(
    ...
    from src.services.stock_code_utils import _PREFIX_DIGIT_LENS, _SUFFIX_DIGIT_LENS
...
def _normalize_explicit_exchange_parts(
    ...
    from src.services.stock_code_utils import _valid_exchange_code
...
def _normalize_code_and_exchange(raw: str) -> Tuple[Optional[str], str]:
    ...
    Mirrors upstream ``stock_code_utils._normalize_code_and_exchange`` using
    this fork's digit tables, validation, and suffix-market helpers.
    """
    from src.services.market_symbol_utils import normalize_suffix_market_symbol
    from src.services.stock_code_utils import _PRESERVE_SUFFIXES
...
def parse_analysis_target(
    ...
    if norm_exchange and norm_exchange in _EXCHANGE_NORMALIZER:
        if norm_code is None:
            from src.services.stock_code_utils import _PREFIX_DIGIT_LENS
            raw_upper = raw.upper()
