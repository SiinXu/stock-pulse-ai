...
# Rebound from ``src.services.run_flow_parts.graph`` and
# ``src.services.run_flow_parts.agent_events`` at import. Placeholders
# keep flake8 F821 clean for facade call sites; bind overwrites these names.
_put_node = None  # type: ignore[assignment,misc]
_append_edge = None  # type: ignore[assignment,misc]
_refresh_incoming_edge_status = None  # type: ignore[assignment,misc]
_append_event = None  # type: ignore[assignment,misc]
_valid_status = None  # type: ignore[assignment,misc]
_safe_text = None  # type: ignore[assignment,misc]
_safe_key = None  # type: ignore[assignment,misc]
_safe_int = None  # type: ignore[assignment,misc]
_sanitize_metadata = None  # type: ignore[assignment,misc]
_as_mapping = None  # type: ignore[assignment,misc]
_as_list = None  # type: ignore[assignment,misc]
_datetime_to_iso = None  # type: ignore[assignment,misc]
_elapsed_ms = None  # type: ignore[assignment,misc]
_started_at_from_end_and_duration = None  # type: ignore[assignment,misc]
_local_timezone = None  # type: ignore[assignment,misc]
_datetime_for_elapsed = None  # type: ignore[assignment,misc]
_events_elapsed_ms = None  # type: ignore[assignment,misc]
_append_agent_events = None  # type: ignore[assignment,misc]
_agent_event_status = None  # type: ignore[assignment,misc]
...
def build_history_run_flow_snapshot(
    ...
    *,
    ...
    agent_anchor = _append_agent_events(
        nodes,
        edges,
        events,
    ...
    for raw_event in flow_events:
        ...
        if node_id and node_id in nodes and node_id not in known_node_ids:
            if provider_data_type and provider_run:
                ...
                anchor = (
                    last_llm_node
                    or ("analysis_pipeline" if "analysis_pipeline" in nodes else "task_queue")
                )
                _append_edge(
                    edges,
                    anchor,
...
_GRAPH_MODULE_NAME = "src.services.run_flow_parts.graph"
_AGENT_EVENTS_MODULE_NAME = "src.services.run_flow_parts.agent_events"
_run_flow_graph_module = importlib.import_module(_GRAPH_MODULE_NAME)
_run_flow_agent_events_module = importlib.import_module(_AGENT_EVENTS_MODULE_NAME)


def _assemble_run_flow_graph_helpers(graph_module=_run_flow_graph_module) -> None:
    """Rebind graph mutators and scalar helpers onto this facade."""
    bound_names = graph_module.bind_graph_helpers_facade(globals())
    if bound_names != graph_module.EXPECTED_GRAPH_HELPER_NAMES:
        raise ImportError(
            "Unexpected run-flow graph helpers: "
            f"{bound_names!r}"
        )


def _assemble_run_flow_agent_events(
    agent_events_module=_run_flow_agent_events_module,
) -> None:
    """Rebind agent-event projection onto this facade after graph helpers."""
    bound_names = agent_events_module.bind_agent_events_facade(globals())
    if bound_names != agent_events_module.EXPECTED_AGENT_EVENT_NAMES:
        raise ImportError(
            "Unexpected run-flow agent-event helpers: "
            f"{bound_names!r}"
        )


_assemble_run_flow_graph_helpers()
_run_flow_graph_module._install_facade_reload_hook(_assemble_run_flow_graph_helpers)
_assemble_run_flow_agent_events()
_run_flow_agent_events_module._install_facade_reload_hook(
    _assemble_run_flow_agent_events
)

del (
    _GRAPH_MODULE_NAME,
    _AGENT_EVENTS_MODULE_NAME,
    _assemble_run_flow_graph_helpers,
    _assemble_run_flow_agent_events,
    _run_flow_graph_module,
    _run_flow_agent_events_module,
)
