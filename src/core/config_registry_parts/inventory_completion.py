...
INVENTORY_COMPLETION_FIELD_DEFINITIONS: Dict[str, Dict[str, Any]] = {
    ...
    "AKSHARE_PRIORITY": _integer("AkShare Priority", "Daily-data provider priority; lower values run earlier.", "data_source", "1", 700),
    ...
    "BAOSTOCK_PRIORITY": _integer("Baostock Priority", "Daily-data provider priority; lower values run earlier.", "data_source", "3", 702),
    "EFINANCE_CALL_TIMEOUT": _integer("Efinance Call Timeout", "Maximum seconds allowed for one efinance call.", "data_source", "30", 703),
    "EFINANCE_PRIORITY": _integer("Efinance Priority", "Daily-data provider priority; lower values run earlier.", "data_source", "99", 704),
    ...
    "PROVIDER_CIRCUIT_COOLDOWN_SECONDS": _integer("Provider Circuit Cooldown", "Seconds before a half-open provider probe.", "data_source", "300", 713),
    ...
    "PROVIDER_DAILY_CACHE_MEMORY_TTL_SECONDS": _integer("Daily Cache Memory TTL", "Fresh lifetime in seconds for in-memory daily-data entries.", "data_source", "60", 717),
    "PROVIDER_DAILY_CACHE_PERSISTENT_TTL_SECONDS": _integer("Daily Cache Persistent TTL", "Fresh lifetime in seconds for persistent daily-data entries.", "data_source", "3600", 718),
    "PROVIDER_DAILY_CACHE_STALE_IF_ERROR_SECONDS": _integer("Daily Cache Stale If Error", "Additional seconds a stale entry may be used after provider failure.", "data_source", "86400", 719),
    ...
    "PYTDX_PRIORITY": _integer("Pytdx Priority", "Daily-data provider priority; lower values run earlier.", "data_source", "2", 721),
    "SNAPSHOT_SOURCE_PRIORITY": _text("Snapshot Source Priority", "Ordered AlphaSift full-market snapshot providers.", "data_source", "tushare,sina,efinance,akshare_em,em_datacenter", 722),
    ...
    "TUSHARE_PRIORITY": _integer("Tushare Priority", "Daily-data provider priority; configured Tushare may be promoted at runtime.", "data_source", "2", 725),
    "YFINANCE_PRIORITY": _integer("YFinance Priority", "Daily-data provider priority for eligible routes.", "data_source", "0", 726),
    ...
    "LONGBRIDGE_PRIORITY": _integer("Longbridge Priority", "Longbridge priority for eligible daily-data routes.", "data_source", "5", 746),
    ...
    "MARKDOWN_TO_IMAGE_CHANNELS": _text("Markdown To Image Channels", "Comma-separated channels that receive image-rendered Markdown.", "notification", "telegram,wechat,custom,email,slack", 703, control="textarea"),
