...
- The packaged directory must contain the required shipped markets. A
  market without a pack renders an explicit "no pack" section that forbids
...
- Loaded pack mappings are immutable.
...
LEGAL_AUTHORITY = "static_reference_not_live_legal_advice"
REQUIRED_SHIPPED_MARKETS = frozenset({"cn", "hk", "us", "crypto"})
...
def _freeze_text_map(values: Mapping[str, str]) -> Mapping[str, str]:
    return MappingProxyType({key: values[key] for key in values})
...
def _parse_localized_block(
    value: object, *, file_name: str, field: str
) -> Dict[str, str]:
    ...
    unknown = sorted(set(value) - set(_LANG_KEYS))
    if unknown:
        raise RegimePackError(
            f"Trading-regime pack {file_name}: field '{field}' has unknown "
            f"keys {unknown}; allowed keys are {list(_LANG_KEYS)}"
        )
    parsed: Dict[str, str] = {}
...
def _parse_sessions(
    value: object, *, file_name: str
) -> tuple[str, Dict[str, str]]:
    if not isinstance(value, Mapping):
        raise RegimePackError(
            f"Trading-regime pack {file_name}: field 'sessions' must be a "
            f"mapping with 'timezone', '{_LANG_KEYS[0]}' and "
            f"'{_LANG_KEYS[1]}' texts, got {type(value).__name__}"
        )
    allowed = set(_LANG_KEYS) | {"timezone"}
    unknown = sorted(set(value) - allowed)
    if unknown:
        raise RegimePackError(
            f"Trading-regime pack {file_name}: field 'sessions' has unknown "
            f"keys {unknown}; allowed keys are {sorted(allowed)}"
        )
    timezone = _require_non_empty_str(
        value.get("timezone"), file_name=file_name, field="sessions.timezone"
    )
    texts = _parse_localized_block(
        {lang: value.get(lang) for lang in _LANG_KEYS},
        file_name=file_name,
        field="sessions",
    )
    return timezone, texts
...
def _load_pack_file(path: Path) -> TradingRegimePack:
    ...
    timezone, sessions = _parse_sessions(
        data.get("sessions"), file_name=file_name
    )

    localized: Dict[str, Mapping[str, str]] = {}
    for field in _REQUIRED_LOCALIZED_FIELDS:
        localized[field] = _freeze_text_map(
            _parse_localized_block(
    ...
    return TradingRegimePack(
        ...
        timezone=timezone,
        sessions=_freeze_text_map(sessions),
...
def load_trading_regime_packs(
    ...
    *directory* is omitted, results for the packaged directory are cached
    and the required shipped markets must all be present.
    """
    global _pack_cache
    ...
    if cache_result:
        missing = REQUIRED_SHIPPED_MARKETS - set(packs)
        if missing:
            raise RegimePackError(
                f"Trading-regime pack directory {directory}: missing "
                f"required packs {sorted(missing)}"
            )
        _pack_cache = packs
    return packs
...
def build_trading_regime_context_metadata(market: str) -> Dict[str, object]:
    """Return queryable run/context metadata for a detected market."""
    market_key = (market or "").strip().lower() or "unknown"
    pack = get_trading_regime_pack(market_key)
    return {
        "market": pack.market if pack is not None else market_key,
        "pack_version": pack.pack_version if pack is not None else None,
        "schema_version": REGIME_PACK_SCHEMA_VERSION,
        "has_pack": pack is not None,
        "timezone": pack.timezone if pack is not None else None,
...
_SECTION_LABELS = {
    ...
    "en": {
        ...
        "disclaimer": (
            ...
            "exchange rules, and not live legal advice; defer to the "
        ...
        "missing": (
            ...
            "any regime constraint you state as unverified. This note is "
