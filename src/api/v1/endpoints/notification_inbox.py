...
from src.api.deps import get_notification_inbox_service
from src.api.v1.errors import api_error
from src.api.v1.schemas.common import ErrorResponse
from src.api.v1.schemas.notification_inbox import (
...
def _translate_error(
    exc: NotificationInboxValidationError | RepositoryError,
) -> None:
    ...
    if isinstance(exc, RepositoryError):
        ...
        status_code = 503 if error_code in {
            "notification_inbox_no_source_available",
            "notification_inbox_partial_source",
            "notification_inbox_source_unavailable",
        } else 500
        raise api_error(
            status_code,
            error_code,
            "Notification inbox storage is temporarily unavailable",
        ) from exc
...
def list_inbox_items(
    ...
    cursor: Optional[str] = Query(None, min_length=1, max_length=NOTIFICATION_INBOX_MAX_CURSOR_LENGTH),
