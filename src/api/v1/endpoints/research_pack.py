...
from src.api.v1.schemas.research_pack import ResearchPackJsonEnvelope
from src.application_services import get_application_services
from src.auth import COOKIE_NAME, is_auth_enabled, verify_session
from src.services.history_service import HistoryService
from src.services.research_pack_export_service import (
    ResearchPackExportDisabled,
    ResearchPackExportService,
...
def _record_export_audit(
    ...
    *,
    ...
    common = {
        "event_type": "research_pack.export",
        "actor_type": "administrator",
        "actor_id": "admin_session",
        "execution_id": correlation_id,
        "action": "research_pack.export",
...
def _progress_header(progress: list) -> str:
    parts = []
...
@router.get(
    ...
    response_model=ResearchPackJsonEnvelope,
    responses={
        200: {
            ...
            "content": {
                ...
                "application/json": {
                    "schema": {"$ref": "#/components/schemas/ResearchPackJsonEnvelope"}
                },
            },
        },
    ...
    description=(
        "One-click ZIP (or JSON metadata envelope) containing redacted report, "
        "decision card, evidence refs, signals, and reasoning trace. Default off. "
        "format=json does not assemble ZIP bytes; format=zip returns the package."
    ),
    operation_id="exportResearchPack",
)
def export_research_pack(
    request: Request,
    record_id: str,
    format: Literal["zip", "json"] = Query(
        "zip",
        description="zip returns the downloadable package; json returns metadata only (no ZIP assembly)",
    ),
    language: str = Query("en", description="Language for brief-card and README copy (en or zh)"),
    ...
    if not is_research_pack_export_enabled(config):
        raise api_error(
            404,
            "research_pack_export_disabled",
            "Research pack export is not enabled",
        )
    ...
    try:
        ...
        log_safe_exception(
            ...
            exc,
            error_code="research_pack_export_failed",
    ...
    byte_length = (
        len(result.zip_bytes) if result.zip_included else result.content_byte_length
    )
    headers = {
        ...
        "X-Research-Pack-Bytes": str(byte_length),
        "X-Research-Pack-Progress": _progress_header(result.progress),
        "X-Research-Pack-Zip-Included": "1" if result.zip_included else "0",
    }
    ...
    if format == "json":
        ...
        return Response(
            ...
            headers={
                **headers,
                "Content-Disposition": f'attachment; filename="research-pack-{safe_id}.json"',
            },
        )

    if not result.zip_included or not result.zip_bytes:
        _record_export_audit(
            security_audit,
            phase="completion",
            correlation_id=correlation_id,
    ...
    return Response(
        ...
        headers={
            **headers,
            "Content-Disposition": f'attachment; filename="research-pack-{safe_id}.zip"',
        },
    )
