...
from src.api.middlewares.model_pack_upload import ModelPackUploadLimitMiddleware
from src.api.middlewares.security_headers import add_security_headers_middleware
...
def _bundled_stock_index_path() -> Path:
    return Path(__file__).resolve().parents[2] / "apps" / "dsa-web" / "public" / _STOCK_INDEX_FILENAME
