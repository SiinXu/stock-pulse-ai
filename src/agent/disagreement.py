...
from src.agent.protocols import AgentContext, is_valid_strategy_signal, normalize_strategy_signal
...
def build_agent_disagreement_summary(
    ...
    *,
    ...
    ``ctx.opinions`` is already partitioned by the orchestrator's StrategyEngine,
    so invalid skill opinions never appear here. Invalid counts come from
    ``ctx.meta["invalid_opinions"]`` (Diagnostics), not from re-filtering.
    """
    buckets = {
        "bullish_agents": [],
        "bearish_agents": [],
