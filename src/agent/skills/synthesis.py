...
def strategy_opinion_from_agent_opinion(opinion: AgentOpinion) -> StrategyOpinion:
    ...
    # A truly missing signal (None) is invalid — never silently defaulted to hold.
...
class StrategySynthesizer:
    ...
    def synthesize(
        ...
        *,
        ...
        valid_opinions = [op for op in opinions if not op.invalid_signal]
        # When called directly (e.g. unit tests) infer invalid_count from the
    ...
    def _group_opinions(
        ...
        """Dynamic bipartite grouping: every valid opinion is supporting or opposing.

        A dedicated ``neutral`` bucket is deliberately omitted so a "high
        consensus" summary can never render alongside "supporting: none".
        """
    ...
    def _consensus_level(
        ...
        *,
        ...
        """Consensus level precedence:

        1. aggregator-signalled insufficient (zero valid weight) -> insufficient
        2. <= 1 valid opinion -> insufficient
        3. sum(confidence) == 0 -> insufficient
        4. conflict_severity == "high" -> low
        5. aligned_ratio >= 2/3 and no conflicts -> high
