...
class CommunityIntelCoverage(_StrictCommunityIntelModel):
    """One provider-neutral source coverage statement."""
...
class CommunityIntelCitation(_StrictCommunityIntelModel):
    """A capped source reference. Phase A fixtures may use ids without URLs."""
...
class CommunityIntelObservation(_StrictCommunityIntelModel):
    """Strict output a bounded provider must produce before prompt projection."""
    ...
    confidence_basis: str = Field(
        min_length=1,
        max_length=_MAX_CONFIDENCE_BASIS_CHARS,
    )
...
class CommunityIntelBriefResult(_StrictCommunityIntelModel):
    """Stable prompt-facing result shared by success and degradation paths."""
...
class CommunityIntelProvider(Protocol):
    """Provider port used by deterministic Phase A fixtures and later adapters."""
...
def _serialized_size(payload: dict[str, Any]) -> int:
    return len(
        json.dumps(payload, ensure_ascii=False, allow_nan=False, default=str).encode("utf-8")
    )
...
def _project_observation(
    ...
    *,
    ...
    if (
        ...
        and "citation_url_redacted" not in projected_gaps
        and len(projected_gaps) < 8
    ):
    ...
    return _result_dict(
        CommunityIntelBriefResult(
            ...
            confidence_basis=_safe_text(
                observation.confidence_basis,
                _MAX_CONFIDENCE_BASIS_CHARS,
            ),
            themes=themes,
