# -*- coding: utf-8 -*-
"""Deny-by-default Agent ToolSurface.

Every production tool call must go:

    ToolRegistry.resolve/bind → ToolSurface (authz / timeout / audit) → handler

``ToolRegistry.execute`` is permanently disabled. Unregistered and
unauthorized names are rejected before the implementation starts. New tools
must satisfy ``NEW_TOOL_CHECKLIST`` (permission, timeout, audit, HITL need).
"""
...
# Keep the historical logger name so existing WARNING probes still match.
logger = logging.getLogger("src.agent.tool_surface")

NEW_TOOL_CHECKLIST: tuple[str, ...] = (
    "permission",
    "timeout",
    "audit",
    "hitl_need",
)
_TOOL_SURFACE_DISPATCH: contextvars.ContextVar[bool] = contextvars.ContextVar(
...
@contextmanager
def _authorized_tool_surface_dispatch() -> Iterator[None]:
    """Mark the current call stack as ToolSurface-authorized handler dispatch."""
    token = _TOOL_SURFACE_DISPATCH.set(True)
    try:
        yield
    finally:
        _TOOL_SURFACE_DISPATCH.reset(token)


def tool_surface_dispatch_authorized() -> bool:
    """Return whether the current stack is inside ToolSurface handler dispatch."""
    return bool(_TOOL_SURFACE_DISPATCH.get())
    ...
    def execute_tool(
        ...
        *,
        ...
        def _invoke_handler() -> Any:
            ...
            with _authorized_tool_surface_dispatch():
