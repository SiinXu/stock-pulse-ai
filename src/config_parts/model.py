...
\x20\x20\x20\x20
    ...
    searxng_timeout_seconds: int = 10  # Self-hosted SearXNG per-search timeout in seconds
    ...
    agent_reflection_llm_budget: int = 1  # Max LLM calls per reflection loop (0-64)
...
_CONFIG_DOMAIN_BINDINGS = {
    "notification": NotificationConfig,
    "share_image": ShareImageConfig,
}

# Preserve historical flat attribute access (config.share_image_max_chars, etc.)
# while values live on the composed domain objects.
install_flat_domain_facade(Config, "notification", NotificationConfig)
install_flat_domain_facade(Config, "share_image", ShareImageConfig)
wrap_config_init_for_domains(Config, _CONFIG_DOMAIN_BINDINGS)


def _iter_init_closure_targets(init_function: FunctionType):
    """Yield ``__init__`` callables that may capture dataclass default factories."""
    seen: set[int] = set()
    current: Any = init_function
    while isinstance(current, FunctionType) and id(current) not in seen:
        seen.add(id(current))
        yield current
...
def _bind_config_facade(facade_globals: Dict[str, Any]) -> None:
    ...
    for config_field in Config.__dataclass_fields__.values():
        ...
        for init_target in _iter_init_closure_targets(init_function):
            replace_closure_reference(init_target, default_factory, cloned_factory)
