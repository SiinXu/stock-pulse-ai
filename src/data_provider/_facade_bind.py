...
"""ADR-006 helpers to rebind fetcher method bodies onto their facade.

Shared by every ``<provider>_parts`` package. The per-package copies that
preceded this module (``akshare_parts``, ``efinance_parts``, ``tushare_parts``)
were byte-identical across all four helpers, so they are now thin re-exports.

``manager_parts.daily_cache_methods`` keeps its own variant on purpose: it
resolves annotations through a domain-specific lookup table
(``_resolve_annotations``) that only makes sense for the DataFetcherManager
facade. It is not duplication of this module.
"""
...
def _clone_facade_function(
    ...
    *,
    ...
    """Clone one method so free-name lookups keep the facade module patch seams."""
