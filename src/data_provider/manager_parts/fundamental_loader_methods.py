...
"""Manager-owned fundamental CN/offshore loaders rebound onto DataFetcherManager.

Extracted from ``src.data_provider.base`` behind an ADR-006 compatibility
facade. Cache key/prune/in-flight get-or-load stay in
``fundamental_cache_methods``. Payload helpers, timeout/retry workers,
``_get_fundamental_config``, ``_should_cache_fundamental_context``,
failed/validation-rejected builders, and CN sub-block public APIs remain
on the facade. These descriptors own ``_build_offshore_fundamental_context``
and ``get_fundamental_context``. ``DataFetcherManager`` remains the public
...
class _FundamentalLoaderMethods:
...
EXPECTED_FUNDAMENTAL_LOADER_METHOD_NAMES = (
    "_build_offshore_fundamental_context",
    "get_fundamental_context",
)


def bind_fundamental_loader_methods_facade(
    target_class: Type[Any],
    global_namespace: Dict[str, Any],
) -> Tuple[str, ...]:
    """Bind fundamental CN/offshore loader descriptors without changing the manager API."""

    bound_names = []
    for name, descriptor in vars(_FundamentalLoaderMethods).items():
