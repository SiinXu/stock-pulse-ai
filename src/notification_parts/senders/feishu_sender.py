...
class FeishuSender:
    ...
    def _ensure_app_client(self) -> Any:
        ...
        with self._app_client_lock:
            ...
            except Exception as exc:  # broad-exception: fallback_recorded - Client initialization failure is safely logged and disables the client.
                log_safe_exception(
                    logger,
                    "Feishu App Bot client initialization failed",
                    exc,
                    error_code="feishu_app_bot_client_init_failed",
                    context={"domain": self._feishu_domain},
                )
                self._app_client = None
            return self._app_client
    ...
    def _app_send_chunked(self, client: "_lark.Client", content: str) -> bool:
        ...
        except Exception as exc:  # broad-exception: fallback_recorded - Chunking failure is safely logged before returning false.
            log_safe_exception(
                logger,
                "Feishu App Bot message chunking failed",
                exc,
                error_code="feishu_app_bot_chunking_failed",
            )
            return False
    ...
    def _app_send_raw(self, client: "_lark.Client", msg_type: str, content_json: str) -> bool:
        ...
        except Exception as exc:  # broad-exception: fallback_recorded - Request construction failure is safely logged before returning false.
            log_safe_exception(
                logger,
                "Feishu App Bot request construction failed",
                exc,
                error_code="feishu_app_bot_request_build_failed",
            )
            return False
        ...
        for attempt in range(_APP_SEND_RETRIES):
            ...
            except Exception as exc:  # broad-exception: fallback_recorded - Each failed SDK attempt is safely logged before retry.
                log_safe_exception(
                    logger,
                    "Feishu App Bot delivery attempt failed",
                    exc,
                    error_code="feishu_app_bot_delivery_attempt_failed",
                    level=logging.WARNING,
                    context={
                        "attempt": attempt + 1,
                        "max_attempts": _APP_SEND_RETRIES,
                    },
                )
            ...
            except (AttributeError, Exception):  # broad-exception: optional_metadata - Missing SDK log identifiers use a fixed label.
                log_id = "N/A"
    ...
    def _send_file_via_app_bot(self, path: Path) -> bool:
        ...
        except Exception as exc:  # broad-exception: fallback_recorded - File-upload failure is safely logged before returning false.
            log_safe_exception(
                logger,
                "Feishu App Bot file upload failed",
                exc,
                error_code="feishu_app_bot_file_upload_failed",
        ...
        if not resp.success():
            try:
                log_id = resp.get_log_id()
            except (AttributeError, Exception):  # broad-exception: optional_metadata - Missing upload log identifiers use a fixed label.
                log_id = "N/A"
            logger.error(
    ...
    def _send_file_via_webhook(self, path: Path) -> bool:
        ...
        try:
            ...
            log_safe_exception(
                logger,
                "Feishu webhook fallback file read failed",
                exc,
                error_code="feishu_webhook_fallback_file_read_failed",
                context={"file_suffix": path.suffix.lower() or "none"},
            )
            return False
    ...
    def _send_via_webhook(self, content: str, *, timeout_seconds: Optional[float] = None) -> bool:
        ...
        except Exception as exc:  # broad-exception: fallback_recorded - Webhook failure is safely logged before returning false.
            log_safe_exception(
                logger,
                "Feishu webhook message delivery failed",
                exc,
                error_code="feishu_webhook_delivery_failed",
            )
            return False
    ...
    def _send_feishu_chunked(self, content: str, max_bytes: int) -> bool:
        try:
            ...
            log_safe_exception(
                logger,
                "Feishu webhook message chunking failed",
                exc,
                error_code="feishu_webhook_chunking_failed",
            )
            return False
        ...
        for i, chunk in enumerate(chunks):
            ...
            except Exception as exc:  # broad-exception: fallback_recorded - Per-chunk failure is safely logged and isolated.
                log_safe_exception(
                    logger,
                    "Feishu webhook message chunk delivery failed",
                    exc,
                    error_code="feishu_webhook_chunk_delivery_failed",
                    context={"chunk_index": i + 1, "chunk_count": total_chunks},
                )
            if i < total_chunks - 1:
                time.sleep(1)
    ...
    def _send_feishu_message(self, content: str, *, timeout_seconds: Optional[float] = None) -> bool:
        ...
        def _post_payload(payload: Dict[str, Any]) -> bool:
            ...
            try:
                ...
                log_safe_exception(
                    logger,
                    "Feishu webhook network request failed",
                    exc,
                    error_code="feishu_webhook_network_request_failed",
                )
                return False
