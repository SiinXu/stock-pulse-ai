...
except Exception:  # broad-exception: optional_metadata - Unsupported zoneinfo falls back to validation-only stubs.  # pragma: no cover
...
def evaluate_notification_noise(
    ...
    *,
    ...
    except Exception as exc:  # broad-exception: fallback_recorded - Evaluation failures are safely logged before the existing allow decision.  # pragma: no cover
        log_safe_exception(
            logger,
...
def release_notification_noise(decision: NotificationNoiseDecision) -> None:
    ...
    except Exception as exc:  # broad-exception: fallback_recorded - Release failures are safely logged without blocking notification delivery.  # pragma: no cover
        log_safe_exception(
            logger,
...
def record_notification_noise(decision: NotificationNoiseDecision, now: Optional[datetime] = None) -> None:
    ...
    except Exception as exc:  # broad-exception: fallback_recorded - Persistence failures are safely logged without blocking notification delivery.  # pragma: no cover
        log_safe_exception(
            logger,
