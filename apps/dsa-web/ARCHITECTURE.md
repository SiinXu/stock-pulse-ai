...
| Portfolio projection session (snapshot + risk + ledger events) | `hooks/portfolio/usePortfolioProjectionSession.ts`, `hooks/portfolio/usePortfolioProjectionQueries.ts` | Mount + dependency (`accountId` / `costMethod` / language / event type / applied filters / page); no poll; no window-focus refetch; `retry: false`; `staleTime: 0`; transport stays in `api/portfolio.ts`; snapshot then risk (`includeRealtime: false`); snapshot failure clears snapshot+risk; risk failure after snapshot success keeps snapshot and surfaces `riskWarning`; silent `CancelledError` / fence cancellation always terminates `fetchStatus` via a same-key successor `fetchQuery` or exact-key `removeQueries` (bare observer-key cancel is not used; off-key paper-trade page-1 loads are exact-removed); unmount / account-scope change silent-cancels and `removeQueries` for this owner only; event key is account + eventType + applied filters + page; paper-trade refresh forces trade page 1; FX refresh is a silent follow-up (`fxRefreshing` owner) and keeps success feedback if only risk fails; account-change ledger page-1 is visible on the first new-account render and `totalEventPages` is derived from the settled event query so the page-owned URL clamp cannot `writeParams`/reorder `keep` on the Query success render. | #789 |
...
Suggested remaining migration order (issue #789): Notification Center page (cursor pagination; header bell preview is migrated) → Settings system-config loads → Screening → Chat/agent status → Backtest / calculators / report compare / event calendar / token usage. Defer surfaces owned by concurrent open PRs.
