...
Suggested remaining migration order (issue #789): remaining Settings section-local loaders. `StockScreeningPage` interval poll, Chat (#1616), Backtest (#1615), Research Analysis Workbench (#1617), `useWatchlist` GET scheduling, report version-compare, watchlist scores, watchlist groups, watchlist coverage, the Settings Signal Scorecard public preview GET, the Settings data-provider runtime status GET, and the Settings Kronos status GET already landed. Portfolio projection ledger events stay hand-rolled (HOLD). Remaining Settings section-local loaders (scheduler status, scheduled tasks, security audit, outbound activity, local models, plugins, config presets, generation backend, investment framework, capabilities) stay hand-rolled. Kronos status GET has landed. Defer surfaces owned by concurrent open PRs. Issue #789 stays open; graduating this GET does not mean every remaining surface has migrated.
