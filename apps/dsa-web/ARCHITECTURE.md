...
Suggested remaining migration order (issue #789): Portfolio projection ledger events (snapshot + risk Query is migrated; events stay hand-rolled) → Settings system-config loads → Screening → Chat/agent status → Backtest / calculators / report compare / event calendar / token usage. Defer surfaces owned by concurrent open PRs.
