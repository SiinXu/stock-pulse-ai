...
Suggested remaining migration order (issue #789): `StockScreeningPage` interval poll (2s `SCREEN_TASK_POLL_INTERVAL_MS` while a screen task is active; cadence change, own PR). Chat (#1616) and Backtest (#1615) already landed. Portfolio projection ledger events stay hand-rolled (HOLD). Settings section-local loaders still hand-rolled. Defer surfaces owned by concurrent open PRs.
