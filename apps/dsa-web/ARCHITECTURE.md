...
Suggested remaining migration order (issue #789): report compare. `StockScreeningPage` interval poll, Chat (#1616), Backtest (#1615), Research Analysis Workbench (#1617), and `useWatchlist` GET scheduling already landed. Portfolio projection ledger events stay hand-rolled (HOLD). Settings section-local loaders still hand-rolled. Watchlist groups / scores / coverage stay hand-rolled. Defer surfaces owned by concurrent open PRs. Issue #789 stays open; graduating this GET does not mean every remaining surface has migrated.
