...
Suggested remaining migration order (issue #789): Screening → Chat/agent status → Backtest / calculators / report compare / event calendar / token usage. Portfolio projection ledger events stay hand-rolled (HOLD). Settings section-local loaders still hand-rolled. Defer surfaces owned by concurrent open PRs.
