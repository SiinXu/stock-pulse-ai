...
Suggested remaining migration order (issue #789): calculators / report compare. `StockScreeningPage` interval poll, Chat (#1616), Backtest (#1615), and Research Analysis Workbench (#1617) already landed. Portfolio projection ledger events stay hand-rolled (HOLD). Settings section-local loaders still hand-rolled. Defer surfaces owned by concurrent open PRs. Issue #789 stays open; graduating this poll does not mean every remaining surface has migrated.
