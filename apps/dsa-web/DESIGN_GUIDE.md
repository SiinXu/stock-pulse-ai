...
### 2.22 Phase 2 无引用包装删除：Home hero-gradient-start（#1300）

- 无引用定义 `--home-hero-gradient-start` 已删除（`:root` 与 `.dark` 各一份）。未新增页面级或领域 token，也没有调用点需要迁移。
- `THEME_PAGE_SCOPED_TOKEN_CEILING` 由 35 降到 34；`themeContractGuard.test.ts` 的 `TOKEN_FORMAT_DEBT` 保持 12（该名字不在格式债清单中）。非空下界由严格 `> 154` 调整为 `> 153`（已定义清单约 154）。
- 该名字没有生产或测试消费者，因此亮色 / 暗色计算样式与 theme-pack 行为不变，**不是**视觉改版。Layer 0 涨跌色与 `--home-price-up/down` 别名不变。其余在用 `--home-*` 族（accent chip / panel / surface / price）、在用 class `.home-report-hero`，以及未使用的 `--home-hero-gradient-mid` / `--home-hero-gradient-end` / history / panel / rail leftovers 不在本切片。`.home-report-hero` 是在用 class，但 `--home-hero-*` 自定义属性本身仍是无引用 leftover，本切片不把该类当作在用 `--home-hero-*` token 族。

### 2.23 Phase 2 无引用包装删除：Home hero-gradient-mid（#1300）

- 无引用定义 `--home-hero-gradient-mid` 已删除（`:root` 与 `.dark` 各一份）。未新增页面级或领域 token，也没有调用点需要迁移。
- `THEME_PAGE_SCOPED_TOKEN_CEILING` 由 34 降到 33；`themeContractGuard.test.ts` 的 `TOKEN_FORMAT_DEBT` 保持 12（该名字不在格式债清单中）。非空下界由严格 `> 153` 调整为 `> 152`（已定义清单约 153）。
- 该名字没有生产或测试消费者，因此亮色 / 暗色计算样式与 theme-pack 行为不变，**不是**视觉改版。Layer 0 涨跌色与 `--home-price-up/down` 别名不变。其余在用 `--home-*` 族（accent chip / panel / surface / price）、在用 class `.home-report-hero`，以及未使用的 `--home-hero-gradient-end` / `--home-hero-shadow` / history / panel / rail leftovers 不在本切片。`.home-report-hero` 是在用 class，但 `--home-hero-*` 自定义属性本身仍是无引用 leftover，本切片不把该类当作在用 `--home-hero-*` token 族。

### 2.24 Phase 2 无引用包装删除：Home hero-gradient-end（#1300）

- 无引用定义 `--home-hero-gradient-end` 已删除（`:root` 与 `.dark` 各一份）。未新增页面级或领域 token，也没有调用点需要迁移。
- `THEME_PAGE_SCOPED_TOKEN_CEILING` 由 33 降到 32；`themeContractGuard.test.ts` 的 `TOKEN_FORMAT_DEBT` 保持 12（该名字不在格式债清单中）。非空下界由严格 `> 152` 调整为 `> 151`（已定义清单约 152）。
- 该名字没有生产或测试消费者，因此亮色 / 暗色计算样式与 theme-pack 行为不变，**不是**视觉改版。Layer 0 涨跌色与 `--home-price-up/down` 别名不变。其余在用 `--home-*` 族（accent chip / panel / surface / price）、在用 class `.home-report-hero`，以及未使用的 `--home-hero-shadow` / history / panel / rail leftovers 不在本切片。`.home-report-hero` 是在用 class，但 `--home-hero-*` 自定义属性本身仍是无引用 leftover，本切片不把该类当作在用 `--home-hero-*` token 族。

### 2.25 Phase 2 无引用包装删除：Home panel-border-selected（#1300）

- 无引用定义 `--home-panel-border-selected` 已删除（`:root` 与 `.dark` 各一份）。未新增页面级或领域 token，也没有调用点需要迁移。
- `THEME_PAGE_SCOPED_TOKEN_CEILING` 由 32 降到 31；`themeContractGuard.test.ts` 的 `TOKEN_FORMAT_DEBT` 保持 12（该名字不在格式债清单中）。非空下界由严格 `> 151` 调整为 `> 150`（已定义清单约 151）。
- 该名字没有生产或测试消费者，因此亮色 / 暗色计算样式与 theme-pack 行为不变，**不是**视觉改版。Layer 0 涨跌色与 `--home-price-up/down` 别名不变。其余在用 `--home-*` 族（accent chip / panel / surface / price）、在用 class `.home-report-hero`、未使用的 `--home-hero-shadow`，以及未使用的 panel-gradient / history / rail leftovers 不在本切片。`.home-report-hero` 是在用 class，但 `--home-hero-*` 自定义属性本身仍是无引用 leftover，本切片不把该类当作在用 `--home-hero-*` token 族。

### 2.26 Phase 2 无引用包装删除：Home panel-gradient-start（#1300）

- 无引用定义 `--home-panel-gradient-start` 已删除（`:root` 与 `.dark` 各一份）。未新增页面级或领域 token，也没有调用点需要迁移。
- `THEME_PAGE_SCOPED_TOKEN_CEILING` 由 31 降到 30；`themeContractGuard.test.ts` 的 `TOKEN_FORMAT_DEBT` 保持 12（该名字不在格式债清单中）。非空下界由严格 `> 150` 调整为 `> 149`（已定义清单约 150）。
- 该名字没有生产或测试消费者，因此亮色 / 暗色计算样式与 theme-pack 行为不变，**不是**视觉改版。Layer 0 涨跌色与 `--home-price-up/down` 别名不变。其余在用 `--home-*` 族（accent chip / panel / surface / price）、在用 class `.home-report-hero`、未使用的 `--home-hero-shadow`，以及未使用的 `--home-panel-gradient-mid` / `--home-panel-gradient-end` / history / rail leftovers 不在本切片。`.home-report-hero` 是在用 class，但 `--home-hero-*` 自定义属性本身仍是无引用 leftover，本切片不把该类当作在用 `--home-hero-*` token 族。

### 2.27 Phase 2 无引用包装删除：Home panel-gradient-mid（#1300）

- 无引用定义 `--home-panel-gradient-mid` 已删除（`:root` 与 `.dark` 各一份）。未新增页面级或领域 token，也没有调用点需要迁移。
- `THEME_PAGE_SCOPED_TOKEN_CEILING` 由 30 降到 29；`themeContractGuard.test.ts` 的 `TOKEN_FORMAT_DEBT` 保持 12（该名字不在格式债清单中）。非空下界由严格 `> 149` 调整为 `> 148`（已定义清单约 149）。
- 该名字没有生产或测试消费者，因此亮色 / 暗色计算样式与 theme-pack 行为不变，**不是**视觉改版。Layer 0 涨跌色与 `--home-price-up/down` 别名不变。其余在用 `--home-*` 族（accent chip / panel / surface / price）、在用 class `.home-report-hero`、未使用的 `--home-hero-shadow`，以及未使用的 `--home-panel-gradient-end` / history / rail leftovers 不在本切片。`.home-report-hero` 是在用 class，但 `--home-hero-*` 自定义属性本身仍是无引用 leftover，本切片不把该类当作在用 `--home-hero-*` token 族。

### 2.28 Phase 2 无引用包装删除：Home panel-gradient-end（#1300）
...
- 该名字没有生产或测试消费者，因此亮色 / 暗色计算样式与 theme-pack 行为不变，**不是**视觉改版。Layer 0 涨跌色与 `--home-price-up/down` 别名不变。其余在用 `--home-*` 族（accent chip / panel / surface / price）、在用 class `.home-report-hero`、未使用的 `--home-hero-shadow`，以及未使用的 `--home-history-item-hover-bg` / `--home-history-item-selected-bg` / rail leftovers 不在本切片。`.home-report-hero` 是在用 class，但 `--home-hero-*` 自定义属性本身仍是无引用 leftover，本切片不把该类当作在用 `--home-hero-*` token 族。后续落地切片删除了 `--home-rail-border`；未删除 `--home-rail-bg` / `--home-rail-shadow` / `--home-hero-shadow` / history hover-selected。

### 2.30 Phase 2 无引用包装删除：Home rail-border（#1300）

- 无引用定义 `--home-rail-border` 已删除（`:root` 与 `.dark` 各一份）。未新增页面级或领域 token，也没有调用点需要迁移。
- `THEME_PAGE_SCOPED_TOKEN_CEILING` 由 27 降到 26；`themeContractGuard.test.ts` 的 `TOKEN_FORMAT_DEBT` 保持 12（该名字不在格式债清单中）。非空下界由严格 `> 146` 调整为 `> 145`（已定义清单约 146）。
- 该名字没有生产或测试消费者，因此亮色 / 暗色计算样式与 theme-pack 行为不变，**不是**视觉改版。Layer 0 涨跌色与 `--home-price-up/down` 别名不变。其余在用 `--home-*` 族（accent chip / panel / surface / price）、在用 class `.home-report-hero`、在用 `--home-rail-bg`（`.home-rail-card`）、未使用的 `--home-hero-shadow` / `--home-rail-shadow`，以及未使用的 `--home-history-item-hover-bg` / `--home-history-item-selected-bg` 不在本切片。`.home-rail-card` 是在用 class，但只读取 `--home-rail-bg`，本切片不把该类当作在用 `--home-rail-border` token 族。
