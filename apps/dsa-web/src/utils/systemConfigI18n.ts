...
const fieldDescriptionMap: Record<string, string> = {
  ...
  AGENT_EPISODE_LOG_ENABLED: "默认关闭。开启后在 Agent 运行结束时写入紧凑 episode（轨迹摘要/教训），失败不中断分析。",
  AGENT_EPISODE_RETENTION_DAYS: "episode 行按时间清理的最大保留天数。",
  AGENT_EPISODE_MAX_ROWS: "episode 表容量上限；超出后按最旧优先删除。",
