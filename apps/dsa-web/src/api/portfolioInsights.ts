...
const basketRequestSchema = z.object({
  stockCodes: z.array(z.string().trim().min(1).max(16)).min(1).max(20)
    .refine(
      (items) => new Set(items.map((item) => item.toUpperCase())).size === items.length,
  ...
  for (const key of Object.keys(request.weights)) {
    if (!symbols.has(key.toUpperCase())) {
      context.addIssue({
        code: 'custom',
        path: ['weights', key],
        message: 'weight key is not in stockCodes',
      });
    }
  }
});
...
const basketResponseSchema = z.object({
  ...
  degradedSymbols: z.array(z.object({
    stockCode: z.string(),
    reason: z.string(),
    detail: z.string().nullable().optional(),
  }).passthrough()),
  annotations: z.array(z.string()),
  correlation: riskBlockSchema.extend({
    symbols: z.array(z.string()),
    matrix: z.array(z.array(finite.nullable())),
  ...
  correlationHighlights: z.array(z.object({
    ...
    right: z.string(),
    correlation: finite,
    absCorrelation: finite,
    direction: z.enum(['positive', 'negative']),
  }).passthrough()),
  concentration: riskBlockSchema.extend({
    hhi: optionalFinite,
  ...
  stanceDistribution: riskBlockSchema.extend({
    scoredCount: z.number().int(),
    unanalyzedCount: z.number().int(),
    averageScore: optionalFinite,
    byOperationAdvice: z.record(z.string(), z.number().int()),
    items: z.array(z.record(z.string(), z.unknown())),
    formulaVersion: z.string().nullable().optional(),
  }),
  health: riskBlockSchema.extend({
    score: optionalFinite,
    partialScore: optionalFinite,
    coverageRatio: optionalFinite,
    dataQuality: z.record(z.string(), z.unknown()).nullable().optional(),
    disclaimer: z.string().nullable().optional(),
  }),
  stress: riskBlockSchema.extend({
    scenario: z.record(z.string(), z.unknown()).optional(),
  }).nullable().optional(),
  riskMetricsStatus: z.string().nullable().optional(),
  riskHistory: z.record(z.string(), z.unknown()),
...
const scenarioSchema = z.object({
  ...
  shocks: z.array(shockSchema).min(1),
  requiresTargetSector: z.boolean(),
  availability: z.enum(['ready', 'requires_parameters']),
  source: z.enum(['built_in', 'yaml', 'custom_api']),
  version: z.number().int().positive(),
  scenarioHash: z.string().regex(/^[0-9a-f]{64}$/),
}).passthrough();
...
const impactSchema = z.object({
  ...
  shockPct: finite,
  pnl: finite,
  stressedMarketValue: finite,
  priceSource: z.string().nullable().optional(),
  priceProvider: z.string().nullable().optional(),
  priceDate: z.string().nullable().optional(),
  priceStale: z.boolean(),
  dataQuality: z.enum(['ok', 'partial']),
...
const stressResponseSchema = z.object({
  ...
  snapshotVersion: z.literal('portfolio_snapshot_v1'),
  accountId: z.number().int().nullable().optional(),
  costMethod: costMethodSchema,
  currency: z.string(),
  status: z.enum(['ok', 'empty_portfolio', 'partial', 'unavailable']),
  statusMessage: z.string().nullable().optional(),
  portfolioValue: finite,
  authoritativePortfolioValue: finite,
  reconciliationDelta: finite,
  positionsUsed: z.number().int(),
  excludedPositionCount: z.number().int(),
  excludedKnownMarketValue: finite,
  excludedUnknownValueCount: z.number().int(),
  excludedPositions: z.array(z.record(z.string(), z.unknown())),
  simulationMethod: z.literal('deterministic_factor_shock'),
  historicalReplayAvailable: z.literal(false),
  scenario: scenarioSchema.extend({ targetSector: z.string().nullable().optional() }),
  assumptions: z.object({
    simplifiedAssumptions: z.array(z.string()),
    dataSource: z.string(),
  }).passthrough(),
  snapshotFxStale: z.boolean(),
  snapshotDataQuality: z.enum(['ok', 'partial']),
  snapshotLimitations: z.array(z.string()),
  missingData: z.array(z.string()),
  portfolioPnl: optionalFinite,
  portfolioPnlPct: optionalFinite,
  stressedPortfolioValue: optionalFinite,
  positionImpacts: z.array(impactSchema),
  topLosers: z.array(impactSchema),
  topWinners: z.array(impactSchema),
  concentration: riskBlockSchema,
}).passthrough();
...
const presetQuerySchema = z.object({
  ...
  asOf: z.string().min(1).optional(),
  costMethod: costMethodSchema.optional(),
...
const rebalanceResponseSchema = z.object({
  ...
  costMethod: costMethodSchema,
  currency: z.string(),
  status: z.enum(['ok', 'empty_portfolio', 'insufficient_data', 'refused']),
  statusMessage: z.string().nullable().optional(),
  disclaimer: z.string(),
  riskTolerance: z.enum(['conservative', 'moderate', 'aggressive']),
  isSuggestionOnly: z.literal(true),
  autoExecute: z.literal(false),
  targetModel: z.object({
    name: z.string(),
    description: z.string(),
    maxSingleWeightPct: finite,
    bandMaxSingleWeightPct: finite,
    softMaxSingleNameWeight: finite,
    minEffectiveN: finite,
    maxHhi: finite,
    targetVarPctCeiling: finite,
  ...
  suggestions: z.array(z.object({
    action: z.enum(['trim', 'add', 'hold']),
    symbol: z.string(),
    fromWeightPct: finite,
    toWeightPct: finite,
    deltaWeightPct: finite,
    approxNotional: finite,
    rationale: z.string(),
    assumptions: z.array(z.string()),
    isSuggestionOnly: z.literal(true),
    autoExecute: z.literal(false),
  }).passthrough()),
  positionBands: z.array(z.object({
    symbol: z.string(),
    action: z.enum(['add', 'reduce', 'hold', 'exit']),
    currentWeightPct: finite,
    targetWeightPctLow: finite,
    targetWeightPctMid: finite,
    targetWeightPctHigh: finite,
    effectiveCapPct: finite,
    signal: z.string(),
    mode: z.string(),
    rationale: z.string(),
    assumptions: z.array(z.string()),
  ...
  assumptions: z.object({
    method: z.string(),
    riskMetricsSource: z.string(),
    taxAndTransactionCosts: z.string(),
    recommendationHonesty: z.string(),
  }).passthrough(),
  riskMetricsSummary: z.object({
    status: z.string(),
    varStatus: z.string().nullable().optional(),
    correlationStatus: z.string().nullable().optional(),
    concentrationStatus: z.string().nullable().optional(),
  }).passthrough(),
}).passthrough();
const rebalanceQuerySchema = z.object({
  ...
  driftThresholdPct: finite.min(0).max(100).optional(),
  confidence: finite.refine((value) => value > 0.5 && value < 1).optional(),
  horizonDays: z.number().int().positive().optional(),
  lookbackTradingDays: z.number().int().min(60).max(1000).optional(),
}).strict();
...
export const portfolioInsightsApi = {
  async analyzeBasket(request: PortfolioBasketRequest): Promise<PortfolioBasketResponse> {
    ...
    const response = await apiClient.post<unknown>('/api/v1/analysis/portfolio', {
      stock_codes: body.stockCodes.map((item) => item.toUpperCase()),
      weights: body.weights,
      as_of: body.asOf,
      lookback_trading_days: body.lookbackTradingDays ?? 252,
      confidence: body.confidence ?? 0.95,
      horizon_days: body.horizonDays ?? 1,
      include_stress: body.includeStress ?? true,
      scenario_id: body.scenarioId ?? 'market_down_10',
      sector_map: body.sectorMap,
      high_correlation_threshold: body.highCorrelationThreshold ?? 0.7,
      currency: (body.currency ?? 'CNY').toUpperCase(),
    });
    return parseCamelCasePayload<PortfolioBasketResponse>(
      response.data,
  ...
  async listStressScenarios(): Promise<StressScenarioListResponse> {
    ...
    return parseCamelCasePayload<StressScenarioListResponse>(
      response.data,
      scenarioListSchema,
      'StressScenarioListResponse',
      'portfolioInsights',
  ...
  async runStressPreset(query: PortfolioStressPresetQuery): Promise<PortfolioStressResponse> {
    ...
    const response = await apiClient.get<unknown>('/api/v1/portfolio/stress-test', {
      params: {
        scenario_id: value.scenarioId,
        account_id: value.accountId,
        as_of: value.asOf,
        cost_method: value.costMethod ?? 'fifo',
        rate_sensitivity_pct_per_100bp: value.rateSensitivityPctPer100bp,
      },
    });
    return parseCamelCasePayload<PortfolioStressResponse>(
      response.data,
  ...
  async runStressCustom(request: PortfolioStressCustomRequest): Promise<PortfolioStressResponse> {
    ...
    const response = await apiClient.post<unknown>('/api/v1/portfolio/stress-test', {
      ...
      cost_method: value.costMethod ?? 'fifo',
      scenario_id: value.scenarioId,
      target_sector: value.targetSector,
      sector_map: value.sectorMap,
  ...
  async getRebalancing(query: PortfolioRebalanceQuery = {}): Promise<PortfolioRebalancingResponse> {
    ...
    const response = await apiClient.get<unknown>('/api/v1/portfolio/rebalancing-recommendations', {
      params: {
        account_id: value.accountId,
        as_of: value.asOf,
        cost_method: value.costMethod ?? 'fifo',
        risk_tolerance: value.riskTolerance ?? 'moderate',
        drift_threshold_pct: value.driftThresholdPct ?? 5,
        confidence: value.confidence ?? 0.95,
        horizon_days: value.horizonDays ?? 1,
        lookback_trading_days: value.lookbackTradingDays ?? 252,
      },
    });
    return parseCamelCasePayload<PortfolioRebalancingResponse>(
      response.data,
