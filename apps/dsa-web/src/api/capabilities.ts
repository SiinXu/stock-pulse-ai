...
const capabilityItemSchema = z.discriminatedUnion('domain', [
  capabilityItemBaseSchema.extend({ domain: z.literal('data'), type: z.enum(['data_provider', 'data_method']) }),
  capabilityItemBaseSchema.extend({ domain: z.literal('tool'), type: z.literal('agent_tool') }),
  capabilityItemBaseSchema.extend({ domain: z.literal('extension'), type: z.enum(['plugin_lifecycle', 'extension_registration']) }),
