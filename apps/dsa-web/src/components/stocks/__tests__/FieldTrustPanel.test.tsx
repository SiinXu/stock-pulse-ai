...
const renderPanel = (ui: ReactElement, language: 'en' | 'zh' = 'en') =>
  render(<UiLanguageProvider initialLanguage={language}>{ui}</UiLanguageProvider>);
...
describe('FieldTrustPanel', () => {
  ...
  it('localizes known zh failure and degradation copy instead of backend English', () => {
    renderPanel(<FieldTrustPanel stockCode="600519" initialView={staleConflictView} />, 'zh');
    expect(screen.getByTestId('field-trust-status')).toHaveTextContent('已降级');
    expect(screen.getByTestId('field-trust-degraded')).toHaveTextContent('过期、冲突、未归因或数据源失败');
    expect(screen.getByTestId('field-trust-gaps')).toHaveTextContent('数据源对该字段意见不一致');
    expect(screen.getByTestId('field-trust-gaps')).toHaveTextContent('该字段的数据源时间已超过实时有效期');
    expect(screen.getByTestId('field-trust-gaps')).not.toHaveTextContent(/providers disagreed/i);
    expect(screen.getByTestId('field-trust-degraded')).not.toHaveTextContent(/Quote trust degraded/i);
  });

  it('localizes zh unavailable copy instead of the English API message', () => {
    renderPanel(<FieldTrustPanel stockCode="600519" initialView={unavailableView} />, 'zh');
    expect(screen.getByTestId('field-trust-status')).toHaveTextContent('不可用');
    expect(screen.getByTestId('field-trust-unavailable')).toHaveTextContent('所有数据源都未能返回行情');
    expect(screen.getByTestId('field-trust-gaps')).toHaveTextContent('所有数据源都未能返回实时行情');
    expect(screen.getByTestId('field-trust-unavailable')).not.toHaveTextContent(
      'No realtime quote available from any provider',
    );
  });
});
