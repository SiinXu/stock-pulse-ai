...
describe('RunFlowPanel', () => {
  ...
  it('renders a successful graph, event stream, and selectable node details', async () => {
    ...
    expect(screen.getByTestId('run-flow-events-column')).toHaveClass('h-[32rem]', 'max-h-[calc(100dvh-18rem)]');
    expect(screen.getByTestId('run-flow-events')).toHaveClass('h-full', 'overflow-hidden');
