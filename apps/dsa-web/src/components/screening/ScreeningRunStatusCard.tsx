...
export const ScreeningRunStatusCard: React.FC<ScreeningRunStatusCardProps> = ({
  ...
  const provider = screenMeta?.snapshotSource?.trim();
    ...
    <Surface as="section" level="interactive" padding="md">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <span
            className={`grid h-7 w-7 place-items-center rounded-full ${candidatesCount > 0 && !showingLastGood ? 'text-success' : isScreeningEnabled ? 'text-primary' : 'text-warning'}`}
          >
            {candidatesCount > 0 && !showingLastGood ? (
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-secondary-text">
              {loading
                ...
                : formatUiText(text.currentStrategy, {
        ...
        <div className="grid gap-1 text-xs text-secondary-text sm:text-right">
          ...
          {provider ? <span>{formatUiText(text.snapshotProvider, { source: provider })}</span> : null}
          <span>
            {text.llm}: {screenMeta?.llmRanked ? text.reranked : screenMeta ? text.notReranked : '-'}
            {screenMeta?.llmCoverage != null
              ? ` · ${formatUiText(text.coverage, { value: formatPercent(screenMeta.llmCoverage) })}`
              : ''}
          </span>
          <span>
            {formatUiText(text.dsaEnrichment, {
