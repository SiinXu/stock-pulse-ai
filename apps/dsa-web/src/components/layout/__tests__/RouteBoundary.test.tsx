...
describe('RouteOutletBoundary', () => {
  it('catches rejected lazy route imports inside the shell and resets on navigation', async () => {
    ...
    try {
      ...
      expect(screen.getByRole('button', { name: '重新加载页面' })).toHaveClass('ui-touch-target', 'h-6', 'rounded-md');
      expect(screen.getByRole('button', { name: '返回首页' })).toHaveClass('ui-touch-target', 'h-6', 'rounded-md');
