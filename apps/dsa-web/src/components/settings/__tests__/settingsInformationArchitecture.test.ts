...
describe('settingsInformationArchitecture', () => {
  ...
  it('splits content-heavy sections into per-view tabs with a default', () => {
    ...
    expect(getSectionViews('advanced').map((view) => view.id)).toEqual([
      'raw_config',
      'diagnostics',
      'capabilities',
      'backup',
    ...
    expect(viewLabel('advanced', 'capabilities', 'en')).toBe('Capabilities');
    expect(viewLabel('advanced', 'capabilities', 'zh')).toBe('运行能力');
  });
  ...
  it('exposes bilingual section and view labels', () => {
    ...
    expect(viewLabel('ai_models', 'connections', 'zh')).toBe('模型来源');
    expect(viewLabel('ai_models', 'connections', 'en')).toBe('Model Sources');
