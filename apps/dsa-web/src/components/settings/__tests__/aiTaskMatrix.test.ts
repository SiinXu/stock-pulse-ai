...
describe('resolveAiTaskMatrix', () => {
  ...
  describe('authoritative status', () => {
    ...
    it('treats report and Agent tasks as active for local CLI without a route set', () => {
      const rows = resolveAiTaskMatrix(accessor({ GENERATION_BACKEND: 'codex_cli' }), {
        availableRoutes: new Set<string>(),
      });
      // Report/market-review run on CLI; Agent uses the controlled CLI tool bridge.
      ...
      expect(rows.find((row) => row.id === 'agent')!.backendId).toBe('codex_cli');
      expect(rows.find((row) => row.id === 'vision')!.status).toBe('unconfigured');
    });

    it('allows Agent to override a report CLI backend with LiteLLM', () => {
      ...
      expect(rows.find((row) => row.id === 'agent')!.status).toBe('active');
      expect(rows.find((row) => row.id === 'agent')!.backendId).toBe('litellm');
    });
