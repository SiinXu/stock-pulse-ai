...
export const AgentBehaviorPanel: React.FC<AgentBehaviorPanelProps> = ({
  ...
  useEffect(() => {
    ...
    if (focusAskCtaRef.current) return;
  ...
  return (
    <div className="space-y-4" data-testid="agent-behavior-panel">
      ...
      {showAskPath ? (
        <section className="space-y-2 rounded-lg border border-success/30 bg-success/5 p-4" data-testid="agent-ask-path" aria-labelledby="agent-ask-path-title">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {modelNeedsConfig ? (
              <Link to={modelSourcesHref} className={cn('inline-flex items-center justify-center rounded-md border border-[var(--settings-border)] bg-background px-3 py-1.5 text-sm font-medium text-foreground', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40')} data-testid="agent-configure-model-cta">
                {copy.configureModelCta}
              </Link>
            ) : null}
            <Link ref={askCtaRef} to={APP_ROUTE_PATHS.agent} className={cn('inline-flex items-center justify-center rounded-md bg-primary px-3 py-1.5 text-sm font-medium text-primary-foreground', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40', modelNeedsConfig ? 'opacity-90' : '')} data-testid="agent-ask-cta">
              {copy.askCta}
            </Link>
          </div>
        </section>
      ...
      {behaviorItems.length ? (
        <details className="group/agent-behavior overflow-hidden rounded-lg border border-[var(--settings-border)] bg-[var(--settings-surface)]" data-testid="agent-behavior-fields">
          ...
          <div className="space-y-4 border-t border-[var(--settings-border-soft)] p-3">{renderGroupedFields(behaviorItems)}</div>
        </details>
      ) : null}

      {governanceItems.length ? (
        <details className="group/agent-governance overflow-hidden rounded-lg border border-[var(--settings-border)] bg-[var(--settings-surface)]" data-testid="agent-governance-fields">
          <summary className="flex cursor-pointer list-none items-start justify-between gap-3 px-4 py-4 [&::-webkit-details-marker]:hidden">
            <div className="space-y-1">
          ...
          <div className="space-y-4 border-t border-[var(--settings-border-soft)] p-3">{renderGroupedFields(governanceItems)}</div>
        </details>
      ) : null}
