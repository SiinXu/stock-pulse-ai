...
function isExportConfigurationError(code: string | undefined): boolean {
  return code === 'audit_export_disabled'
    || code === 'evidence_chain_disabled'
    || code === 'audit_export_auth_required'
    || code === 'evidence_chain_auth_required';
}
...
const EvidencePackExportControls: React.FC<EvidencePackExportControlsProps> = ({
  ...
  const handleExport = useCallback(async (kind: ExportKind) => {
    ...
    try {
      ...
      if (isExportConfigurationError(parsed.code)) {
        setFeedback({ kind: 'disabled', message: parsed.message || text.exportFailed });
      } else {
        setFeedback({ kind: 'error', message: parsed.message || text.exportFailed });
      }
    } finally { setExporting(null); }
  }, [recordId, text.exportFailed, text.exportTruncated, uiLanguage]);
      ...
      <div className="space-y-2 rounded-lg border border-border bg-card p-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button type="button" variant="secondary" size="compact" onClick={() => { void handleExport('chain'); }} disabled={controlsDisabled} aria-busy={exporting === 'chain'} data-testid="evidence-chain-export-json">
              <Braces className={`h-3.5 w-3.5 ${exporting === 'chain' ? 'animate-pulse' : ''}`} aria-hidden="true" />
              {text.exportEvidenceChainJson}
            </Button>
            <Button type="button" variant="secondary" size="compact" onClick={() => { void handleExport('zip'); }} disabled={controlsDisabled} aria-busy={exporting === 'zip'} data-testid="audit-package-export-zip">
              <Download className={`h-3.5 w-3.5 ${exporting === 'zip' ? 'animate-pulse' : ''}`} aria-hidden="true" />
              {text.exportAuditPackageZip}
            </Button>
          </div>
        </div>
      </div>
      {feedback?.kind === 'disabled' ? (
        <InlineAlert variant="warning" className="mt-3" data-testid="evidence-pack-export-disabled" message={(
          <span className="inline-flex flex-wrap items-center gap-x-2 gap-y-1">
            ...
            <a href={settingsHref} className="font-medium text-primary underline-offset-2 hover:underline" data-testid="evidence-pack-export-settings-link">{UI_TEXT[uiLanguage]['onboarding.openSettings']}</a>
          </span>
        )} />
      ) : null}
