...
export const RED_TEAM_FIELD_DESCRIPTION = {
  AGENT_RED_TEAM_ENABLED:
    '可选的 Decision 后对抗二审（默认关闭；仅 Native Multi 非 Chat 的 full/specialist）。不替换主决策对象。',
} as const;
...
const redTeamSettingsHelpEn: SettingsHelpDefinition = {
  ...
  valueNotes: [
    'Default is off. Chat never runs this stage. Available on Native Multi full and specialist analysis only; quick and standard skip it.',
...
const redTeamSettingsHelpZh: SettingsHelpDefinition = {
  ...
  valueNotes: [
    '默认关闭。Chat 永不运行。仅 Native Multi 的 full / specialist 可用；quick / standard 不运行。',
...
const EXTRA_RED_TEAM_HELP = {
  de: {
    ...
    valueNotes: [
      'Standardmäßig aus. Chat führt diese Stufe nie aus. Nur Native-Multi-full/specialist-Analysen führen sie aus; quick und standard überspringen sie.',
  ...
  es: {
    ...
    valueNotes: [
      'Desactivado por defecto. Chat nunca ejecuta esta etapa. Solo está disponible en análisis Native Multi full y specialist; quick y standard la omiten.',
  ...
  fr: {
    ...
    valueNotes: [
      'Désactivé par défaut. Chat n’exécute jamais cette étape. Disponible uniquement pour les analyses Native Multi full et specialist ; quick et standard la sautent.',
  ...
  id: {
    ...
    valueNotes: [
      'Default nonaktif. Chat tidak pernah menjalankan tahap ini. Hanya tersedia pada analisis Native Multi full dan specialist; quick dan standard melewatinya.',
  ...
  ja: {
    ...
    valueNotes: [
      '既定では無効です。Chat はこの段階を実行しません。Native Multi の full と specialist のみで利用でき、quick と standard はスキップします。',
  ...
  ko: {
    ...
    valueNotes: [
      '기본값은 비활성입니다. Chat은 이 단계를 실행하지 않습니다. Native Multi full 및 specialist에서만 사용할 수 있으며, quick과 standard는 건너뜁니다.',
  ...
  ms: {
    ...
    valueNotes: [
      'Lalai dimatikan. Chat tidak pernah menjalankan peringkat ini. Hanya tersedia pada analisis Native Multi full dan specialist; quick dan standard melaluinya.',
  ...
  "zh-TW": {
    ...
    valueNotes: [
      '預設關閉。Chat 永不執行。僅 Native Multi 的 full / specialist 可用；quick / standard 不執行。',
