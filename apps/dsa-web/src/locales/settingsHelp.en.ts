...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.ai_model.GENERATION_BACKEND': {
    ...
    valueNotes: [
      ...
      'Docker, cloud servers, and CI do not automatically have your local login state. StockPulse does not read Codex/Claude/OpenCode credential files, but the corresponding CLI itself may use its login state.',
    ],
    ...
    notes: [
      ...
      'Default model settings continue to use your existing API keys, model connections, and fallback model settings.',
    ],
    examples: [],
  },
  ...
  'settings.ai_model.OPENCODE_CLI_MODEL': {
    ...
    usage: 'Only applies when Analysis Generation Method is OpenCode CLI. Leave it empty and StockPulse will not pass --model, so OpenCode uses its local default model configuration.',
    valueNotes: [
  ...
  'settings.data_source.ALPHASIFT_ENABLED': {
    ...
    usage: 'Disabled by default. When true, the Web app checks alphasift.dsa_adapter installed with backend dependencies; if it is missing, run python -m pip install --upgrade --constraint constraints.txt pip, python -m pip install --build-constraint build-constraints.txt -r requirements.txt, and python -m pip check from the repository root, or rebuild the backend artifact.',
    valueNotes: ['AlphaSift is installed as a StockPulse backend dependency; /install is retained only as an explicit repair action.', 'Screening output is for research support only and is not investment advice.'],
    impact: ['Affects the Web screening entry, AlphaSift strategy loading, and screening API.'],
    notes: ['AlphaSift generates candidates, while StockPulse enriches them with quote, fundamental, and news context; disabling it does not affect existing analysis, reports, or notifications.'],
  },
  ...
  'settings.llm_channel.channel_name': {
    ...
    showFieldKey: false,
    summary: 'A unique identifier for this model connection, used wherever the connection is referenced.',
    usage: 'Use lowercase letters, numbers, and underscores only. Pick a name that identifies the service.',
    valueNotes: ['Examples: deepseek, my_proxy. Names must be unique across connections.'],
    impact: ['Primary and fallback model choices in usage scenarios reference models by connection name.'],
    notes: ['After renaming, model selections that referenced the old name need to be reconfirmed.'],
  },
  ...
  'settings.llm_channel.base_url': {
    title: 'Base URL',
    showFieldKey: false,
    summary: 'Endpoint root for this connection.',
  ...
  'settings.llm_channel.api_key': {
    title: 'API Key',
    showFieldKey: false,
    summary: 'Credential used by this connection.',
  ...
  'settings.llm_channel.temperature': {
    ...
    valueNotes: ['Leave empty to use the system default; applies to all analysis after saving.'],
  ...
  'settings.llm_channel.primary_model': {
    ...
    usage: 'Choose from enabled connection models. Auto uses the first available model.',
    valueNotes: ['Leave empty to pick the first available model automatically.'],
  ...
  'settings.llm_channel.agent_primary_model': {
    ...
    valueNotes: ['Leave empty to follow the regular analysis primary model.'],
  ...
  'settings.llm_channel.fallback_models': {
    ...
    valueNotes: ['Leave empty to disable fallbacks; primary-model failures then surface directly.'],
  ...
  'settings.llm_channel.vision_model': {
    ...
    valueNotes: ['Leave empty to follow the default vision-model selection.'],
  ...
  'settings.notification.channel_routing': {
    ...
    usage: 'Three routing fields control the target channels for report pushes, alert pushes, and system error pushes. Pick one or more channels from the list. Leave all unchecked to push to all configured channels.',
    valueNotes: [
  ...
  'settings.system.market_review': {
    ...
    valueNotes: [
      ...
      'MARKET_REVIEW_REGION is a multi-select; pick one or more regions such as cn, us, jp.',
  ...
  // ------------------------------------------------------------------
  // Plain-language education help (Issue #201)
  // summary = what · usage = why · impact = what it means for you
  // ------------------------------------------------------------------
