...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.agent.AGENT_PLANNING_ENABLED': {
    ...
    usage: 'Keep disabled for classic ReAct behavior; enable it only when the planning budgets below are configured for the deployment.',
    valueNotes: ['Disabled by default.', 'The switch applies after restart.'],
    impact: ['Changes AgentExecutor.run only; Chat, Research, and multi-agent orchestration keep their existing paths.'],
    notes: ['Planning and tool failures terminate explicitly instead of falling through as successful analysis.'],
  },
  'settings.agent.AGENT_PLANNING_STRATEGY': {
    ...
    usage: 'Use template for predictable cost; use llm when task-specific proposals justify an additional model call.',
    valueNotes: ['Allowed values are template and llm.'],
    impact: ['LLM proposals consume the configured token and timeout budgets.'],
    notes: ['A failed proposal is retried only within the configured proposal retry budget.'],
  },
  'settings.agent.AGENT_PLANNING_MAX_PLAN_STEPS': {
    ...
    usage: 'Choose an integer from 1 to 16.',
    valueNotes: ['Default: 8.'],
    impact: ['Lower values shorten plans; higher values permit more staged evidence collection.'],
    notes: ['The execution tool-call budget remains an independent hard bound.'],
  },
  'settings.agent.AGENT_PLANNING_MAX_REPLANS': {
    ...
    usage: 'Choose an integer from 0 to 3.',
    valueNotes: ['Default: 1.', 'Zero terminates after the first failed proposal.'],
    impact: ['Each LLM retry can add latency and token cost.'],
    notes: ['This is separate from observation-driven replans during tool execution.'],
  },
  'settings.agent.AGENT_PLANNING_MAX_TOKENS': {
    ...
    usage: 'Choose an integer from 1 to 8192.',
    valueNotes: ['Default: 1500.', 'Template planning does not make a planner LLM call.'],
    impact: ['Directly bounds planner model cost.'],
    notes: ['This does not replace the existing synthesis-step limits.'],
  },
  'settings.agent.AGENT_PLANNING_PROPOSAL_TIMEOUT_SECONDS': {
    ...
    usage: 'Choose a finite value from 0.1 to 60 seconds.',
    valueNotes: ['Default: 30 seconds.', 'NaN and positive or negative infinity are rejected.'],
    impact: ['Short values fail slow planners sooner.'],
    notes: ['Timeout is passed into the planner transport and checked at the deadline.'],
  },
  'settings.agent.AGENT_PLANNING_MAX_TOTAL_TOOL_CALLS': {
    ...
    usage: 'Choose an integer from 1 to 32.',
    valueNotes: ['Default: 16.'],
    impact: ['Bounds external calls and execution cost.'],
    notes: ['Exhaustion terminates with max_tool_calls_exceeded.'],
  },
  'settings.agent.AGENT_PLANNING_MAX_OBSERVATION_REPLANS': {
    ...
    usage: 'Choose an integer from 0 to 3.',
    valueNotes: ['Default: 1.', 'Zero makes a failed step terminate.'],
    impact: ['Higher values may recover from failures but add tool and planner work.'],
    notes: ['Every replan shares the total tool-call and execution-time bounds.'],
  },
  'settings.agent.AGENT_PLANNING_EXEC_TIMEOUT_SECONDS': {
    ...
    usage: 'Choose a finite value from 0.1 to 120 seconds.',
    valueNotes: ['Default: 60 seconds.', 'NaN and positive or negative infinity are rejected.'],
    impact: ['Bounds how long planned tool execution can delay synthesis.'],
    notes: ['Timeout terminates explicitly and does not continue as success.'],
  },
  'settings.agent.AGENT_PLANNING_ON_STEP_FAILURE': {
    ...
    usage: 'Use replan for bounded recovery or terminate for strict first-failure behavior.',
    valueNotes: ['Allowed values are replan and terminate.', 'Default: replan.'],
    impact: ['Controls failure handling without changing tool authority.'],
    notes: ['Neither policy treats a failed step as a successful analysis.'],
  },
