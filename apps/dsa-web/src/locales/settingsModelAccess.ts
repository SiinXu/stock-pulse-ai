...
export const MODEL_ACCESS_ISSUES: Record<UiLanguage, Record<string, string>> = {
  zh: { name_required: '连接名称必填', name_invalid: '连接名称仅限小写字母、数字或下划线', missing_api_key: '缺少 API 密钥', missing_base_url: '缺少服务地址', missing_models: '至少配置一个模型' },
  en: { name_required: 'Connection name is required', name_invalid: 'Use lowercase letters, numbers, or underscores', missing_api_key: 'API key is required', missing_base_url: 'Base URL is required', missing_models: 'Add at least one model' },
};
...
export function localizeModelAccessIssue(issue: string, language: UiLanguage): string {
  const codeByZh: Record<string, keyof typeof MODEL_ACCESS_ISSUES.zh> = {
    '连接名称必填': 'name_required', '连接名称仅限小写字母、数字或下划线': 'name_invalid', '缺少 API 密钥': 'missing_api_key', '缺少服务地址': 'missing_base_url', '至少配置一个模型': 'missing_models',
  };
