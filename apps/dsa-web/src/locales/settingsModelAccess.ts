...
const editorZh = {
  ...
  invalidDescription: '以下模型服务需补全后才能保存（点击顶部“保存并应用”统一提交）：',
  ...
  removeDraftConnection: '将从当前草稿中移除模型服务「{name}」，保存后才生效。',
...
const editorEn: Record<keyof typeof editorZh, string> = {
  ...
  invalidDescription: 'Complete these model services before using Save & Apply:',
  ...
  removeDraftConnection: 'Remove model service "{name}" from the draft? This takes effect after saving.',
...
export const MODEL_ACCESS_ISSUES: Record<UiLanguage, Record<string, string>> = {
  zh: { duplicate_name: '连接名称已存在，请更换', name_required: '连接名称必填', name_invalid: '连接名称仅限小写字母、数字或下划线', missing_api_key: '缺少 API 密钥', missing_base_url: '缺少服务地址', missing_models: '至少配置一个模型' },
  en: { duplicate_name: 'Connection name already exists', name_required: 'Connection name is required', name_invalid: 'Use lowercase letters, numbers, or underscores', missing_api_key: 'API key is required', missing_base_url: 'Base URL is required', missing_models: 'Add at least one model' },
};

export function localizeModelAccessIssue(issue: string, language: UiLanguage): string {
  return MODEL_ACCESS_ISSUES[language][issue] ?? issue;
}
