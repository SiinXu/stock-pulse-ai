...
const editorZh = {
  ...
  retry: '重试',
  hubSearchLabel: '筛选模型来源',
  hubSearchPlaceholder: '按名称、服务商或模型筛选…',
  hubCloudGroup: '云端 / API 连接',
  hubLocalGroup: '本地模型服务',
  hubCliGroup: '本机 CLI',
  hubFallbackGroup: '主用与回退',
  hubCloudEmpty: '尚未接入云端或兼容 API 渠道。',
  hubLocalConfigured: '已配置本地模型',
  hubLocalPrimary: '主用：{model}',
  hubLocalAgent: 'Agent：{model}',
  hubLocalRuntime: '运行时：{status}',
  hubLocalNone: '尚未注册本地模型。可在「本地模型」页下载或导入。',
  hubLocalLoading: '正在检测本地模型状态…',
  hubLocalFailed: '无法读取本地模型状态。',
  hubCliLoading: '正在检测本机 CLI 状态…',
  hubCliFailed: '无法读取本机 CLI 状态。',
  hubCliPrimary: '主后端：{backend}',
  hubCliFallback: '备用后端：{backend}',
  hubCliNone: '当前使用 LiteLLM / 云路径，或尚未配置本机 CLI。',
  hubPrimaryModel: '主用模型：{model}',
  hubFallbackOrder: '回退顺序：{models}',
  hubFallbackNone: '尚未配置回退模型。',
  hubFallbackEditHint: '完整回退链编辑仍在「可靠性」视图；此处展示主用与回退关系。',
  hubNoSearchMatches: '没有匹配的模型来源。',
...
const editorEn: Record<keyof typeof editorZh, string> = {
  ...
  retry: 'Retry',
  hubSearchLabel: 'Filter model sources',
  hubSearchPlaceholder: 'Filter by name, provider, or model…',
  hubCloudGroup: 'Cloud / API connections',
  hubLocalGroup: 'Local model server',
  hubCliGroup: 'Local CLI',
  hubFallbackGroup: 'Primary and fallback',
  hubCloudEmpty: 'No cloud or compatible API channels yet.',
  hubLocalConfigured: 'Local models configured',
  hubLocalPrimary: 'Primary: {model}',
  hubLocalAgent: 'Agent: {model}',
  hubLocalRuntime: 'Runtime: {status}',
  hubLocalNone: 'No local models registered yet. Download or import them on the Local Models view.',
  hubLocalLoading: 'Checking local model status…',
  hubLocalFailed: 'Could not load local model status.',
  hubCliLoading: 'Checking local CLI status…',
  hubCliFailed: 'Could not load local CLI status.',
  hubCliPrimary: 'Primary backend: {backend}',
  hubCliFallback: 'Fallback backend: {backend}',
  hubCliNone: 'Using LiteLLM / cloud path, or no local CLI is configured yet.',
  hubPrimaryModel: 'Primary model: {model}',
  hubFallbackOrder: 'Fallback order: {models}',
  hubFallbackNone: 'No fallback models configured.',
  hubFallbackEditHint: 'Full fallback editing remains on the Reliability view; this hub shows primary/fallback relationships.',
  hubNoSearchMatches: 'No model sources match this filter.',
