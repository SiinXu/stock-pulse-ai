...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.agent.AGENT_PLANNING_ENABLED': {
    ...
    usage: '经典 ReAct 行为请保持关闭；仅在确认下列成本与步数预算后开启。',
    valueNotes: ['默认关闭。', '保存后需重启生效。'],
    impact: ['仅改变 AgentExecutor.run；Chat、Research 与多 Agent 编排保持原路径。'],
    notes: ['规划或工具失败会显式终止，不会按成功结果继续。'],
  },
  'settings.agent.AGENT_PLANNING_STRATEGY': {
    title: 'Agent 规划策略',
    summary: '选择确定性模板规划或有界的 LLM 提案规划。',
    usage: '优先使用 template 控制成本；仅在需要任务定制提案时使用 llm。',
    valueNotes: ['可选 template、llm。'],
    impact: ['LLM 提案会消耗已配置的 Token 与超时预算。'],
    notes: ['提案失败只会在已配置的重试预算内重试。'],
  },
  'settings.agent.AGENT_PLANNING_MAX_PLAN_STEPS': {
    ...
    usage: '填写 1 到 16 的整数。',
    valueNotes: ['默认值：8。'],
    impact: ['值越小规划越短；值越大可分阶段收集更多证据。'],
    notes: ['工具调用总预算仍是独立硬上限。'],
  },
  'settings.agent.AGENT_PLANNING_MAX_REPLANS': {
    title: '规划提案重试次数',
    summary: '限制规划生成或校验失败后的重试次数。',
    usage: '填写 0 到 3 的整数。',
    valueNotes: ['默认值：1。', '设为 0 时首次提案失败即终止。'],
    impact: ['每次 LLM 重试都可能增加延迟与 Token 成本。'],
    notes: ['与工具执行中的观察驱动重规划预算相互独立。'],
  },
  'settings.agent.AGENT_PLANNING_MAX_TOKENS': {
    title: '规划提案 Token 预算',
    summary: '限制 LLM 规划提案及其重试的 Token 总量。',
    usage: '填写 1 到 8192 的整数。',
    valueNotes: ['默认值：1500。', 'template 规划不会调用规划 LLM。'],
    impact: ['直接限制规划模型成本。'],
    notes: ['不会替代既有综合阶段的限制。'],
  },
  'settings.agent.AGENT_PLANNING_PROPOSAL_TIMEOUT_SECONDS': {
    ...
    usage: '填写 0.1 到 60 秒之间的有限数值。',
    valueNotes: ['默认值：30 秒。', '拒绝 NaN、正无穷和负无穷。'],
    impact: ['较短的值会更早终止缓慢的规划器。'],
    notes: ['超时会传入规划传输层，并在截止时间再次校验。'],
  },
  'settings.agent.AGENT_PLANNING_MAX_TOTAL_TOOL_CALLS': {
    ...
    usage: '填写 1 到 32 的整数。',
    valueNotes: ['默认值：16。'],
    impact: ['限制外部调用次数与执行成本。'],
    notes: ['预算耗尽会以 max_tool_calls_exceeded 显式终止。'],
  },
  'settings.agent.AGENT_PLANNING_MAX_OBSERVATION_REPLANS': {
    title: '观察驱动重规划次数',
    summary: '限制工具步骤失败后由观察结果触发的重规划次数。',
    usage: '填写 0 到 3 的整数。',
    valueNotes: ['默认值：1。', '设为 0 时步骤失败即终止。'],
    impact: ['较高的值可能恢复失败，但会增加规划与工具工作量。'],
    notes: ['所有重规划共享工具调用与执行超时硬上限。'],
  },
  'settings.agent.AGENT_PLANNING_EXEC_TIMEOUT_SECONDS': {
    title: '规划执行超时',
    summary: '限制完整规划执行循环的墙钟时间。',
    usage: '填写 0.1 到 120 秒之间的有限数值。',
    valueNotes: ['默认值：60 秒。', '拒绝 NaN、正无穷和负无穷。'],
    impact: ['限制规划工具执行对综合阶段造成的延迟。'],
    notes: ['超时会显式终止，不会继续并伪装成成功。'],
  },
  'settings.agent.AGENT_PLANNING_ON_STEP_FAILURE': {
    title: '规划步骤失败策略',
    summary: '选择步骤失败后在预算内重规划，或立即终止。',
    usage: '需要有界恢复时选 replan；需要首次失败即停止时选 terminate。',
    valueNotes: ['可选 replan、terminate。', '默认值：replan。'],
    impact: ['只改变失败处理，不扩大工具权限。'],
    notes: ['两种策略都不会把失败步骤视为分析成功。'],
  },
