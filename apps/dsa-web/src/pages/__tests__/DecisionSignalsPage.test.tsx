...
function openSelectListbox(trigger: HTMLElement) {
...
describe('DecisionSignalsPage', { timeout: 15_000 }, () => {
  ...
  it('uses accessible icon refresh actions with loading state in both history views', async () => {
    window.history.pushState({}, '', buildSignalCenterHref({
      tab: SIGNAL_CENTER_TAB_VALUES.history,
    }));
    renderPage();

    await waitFor(() => expect(alertsApi.listTriggers).toHaveBeenCalled());
    const triggerRefreshRequest = createDeferred<{
      items: [];
      total: number;
      page: number;
      pageSize: number;
    }>();
    vi.mocked(alertsApi.listTriggers).mockReturnValueOnce(triggerRefreshRequest.promise);
    const triggerRefresh = screen.getByRole('button', { name: '刷新' });
    expect(triggerRefresh).toHaveAttribute('data-control', 'icon-button');
    fireEvent.click(triggerRefresh);
    await waitFor(() => expect(triggerRefresh).toHaveAttribute('aria-busy', 'true'));
    expect(triggerRefresh).toBeDisabled();

    await act(async () => {
      triggerRefreshRequest.resolve({ items: [], total: 0, page: 1, pageSize: 20 });
    });
    await waitFor(() => expect(triggerRefresh).not.toHaveAttribute('aria-busy'));

    fireEvent.click(screen.getByRole('tab', { name: '通知尝试记录' }));
    await waitFor(() => expect(alertsApi.listNotifications).toHaveBeenCalled());
    const notificationRefreshRequest = createDeferred<{
      items: [];
      total: number;
      page: number;
      pageSize: number;
    }>();
    vi.mocked(alertsApi.listNotifications).mockReturnValueOnce(notificationRefreshRequest.promise);
    const notificationRefresh = screen.getByRole('button', { name: '刷新' });
    expect(notificationRefresh).toHaveAttribute('data-control', 'icon-button');
    fireEvent.click(notificationRefresh);
    await waitFor(() => expect(notificationRefresh).toHaveAttribute('aria-busy', 'true'));

    await act(async () => {
      notificationRefreshRequest.resolve({ items: [], total: 0, page: 1, pageSize: 20 });
    });
    await waitFor(() => expect(notificationRefresh).not.toHaveAttribute('aria-busy'));
  });
  ...
  it('renders decision signal enum filter labels in Chinese', async () => {
    ...
    expect(within(marketListbox).getByRole('option', { name: '日股' })).toHaveAttribute('data-value', 'jp');
    expect(within(marketListbox).getByRole('option', { name: '韩股' })).toHaveAttribute('data-value', 'kr');
    closeSelectListbox(marketSelect);
    const phaseSelect = screen.getByLabelText('阶段');
    const phaseListbox = openSelectListbox(phaseSelect);
    expect(within(phaseListbox).getByRole('option', { name: '午间休市' })).toHaveAttribute('data-value', 'lunch_break');
    expect(within(phaseListbox).getByRole('option', { name: '集合竞价' })).toHaveAttribute('data-value', 'closing_auction');
    closeSelectListbox(phaseSelect);
    const sourceSelect = screen.getByLabelText('来源');
    const sourceListbox = openSelectListbox(sourceSelect);
    expect(within(sourceListbox).getByRole('option', { name: '大盘复盘' })).toHaveAttribute('data-value', 'market_review');
    closeSelectListbox(sourceSelect);
  ...
  it('renders decision signal filters and card value labels in English', async () => {
    ...
    expect(within(marketListbox).getByRole('option', { name: 'Korea' })).toHaveAttribute('data-value', 'kr');
    expect(screen.getAllByText('Japan').length).toBeGreaterThan(1);
    closeSelectListbox(marketSelect);
    const phaseSelect = screen.getByLabelText('Phase');
    const phaseListbox = openSelectListbox(phaseSelect);
    expect(within(phaseListbox).getByRole('option', { name: 'Closing auction' })).toHaveAttribute('data-value', 'closing_auction');
    closeSelectListbox(phaseSelect);
    const sourceSelect = screen.getByLabelText('Source');
    const sourceListbox = openSelectListbox(sourceSelect);
    expect(within(sourceListbox).getByRole('option', { name: 'Market review' })).toHaveAttribute('data-value', 'market_review');
    closeSelectListbox(sourceSelect);
  ...
  it('passes filter parameters when applying filters', async () => {
    ...
    chooseOption(screen.getByLabelText('市场'), 'cn');
    fireEvent.change(screen.getByLabelText('股票代码'), { target: { value: '600519' } });
    chooseOption(screen.getByLabelText('动作'), 'hold');
  ...
  it('uses an exact analysis source report lookup when a report id filter is applied', async () => {
    ...
    fireEvent.change(screen.getByLabelText('股票代码'), { target: { value: '600519' } });
    chooseOption(screen.getByLabelText('动作'), 'hold');
    chooseOption(screen.getByLabelText('来源'), 'alert');
    chooseOption(screen.getByLabelText('状态'), 'closed');
  ...
  it('does not request the timeline before a current stock is selected', async () => {
    ...
    expect(decisionSignalsApi.list).toHaveBeenCalledTimes(1);
    const timelineStatusSelect = screen.getByLabelText('时间线状态');
    const statusListbox = openSelectListbox(timelineStatusSelect);
    expect(within(statusListbox).queryByRole('option', { name: '已关闭' })).not.toBeInTheDocument();
    closeSelectListbox(timelineStatusSelect);
    const timelineProfileSelect = screen.getByLabelText('时间线风格');
    const profileListbox = openSelectListbox(timelineProfileSelect);
    expect(within(profileListbox).getByRole('option', { name: '未知' })).toHaveAttribute('data-value', 'unknown');
    closeSelectListbox(timelineProfileSelect);
  });
  ...
  it('queries timeline with independent filters and no default status', async () => {
    ...
    chooseOption(screen.getByLabelText('时间线风格'), 'unknown');
  ...
  it('initializes timeline market from a new stock context once and preserves later user overrides', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('时间线市场')).toHaveAttribute('data-value', 'cn');
      expect(decisionSignalsApi.list).toHaveBeenLastCalledWith(expect.objectContaining({
        stockCode: '600519',
        market: 'cn',
      }));
    ...
    chooseOption(screen.getByLabelText('时间线市场'), 'hk');
    openStockContextModal();
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('时间线市场')).toHaveAttribute('data-value', 'hk');
      expect(decisionSignalsApi.list).toHaveBeenLastCalledWith(expect.objectContaining({
        stockCode: '600519',
        market: 'hk',
      }));
  ...
  it('clears timeline market from a previous stock context before a later manual stock submit without metadata', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('时间线市场')).toHaveAttribute('data-value', '');
    });
  ...
  it('applies timeline draft filters only after the query button is clicked', async () => {
    ...
    chooseOption(screen.getByLabelText('时间线风格'), 'conservative');
  ...
  it('restores the next selectedSignalId on browser forward after back', async () => {
    const sibling = makeSignal({
      id: 8,
      stockCode: 'AAPL',
      stockName: 'Apple',
  ...
  it('keeps a latest-owned selectedSignalId while latest refresh is in flight and after it returns the same id', async () => {
    const latestSignal = makeSignal({
      id: 8,
      stockCode: 'AAPL',
      stockName: 'Apple',
      market: 'us',
      riskSummary: 'Latest owned risk',
    });
    const refreshedLatest = { ...latestSignal, riskSummary: 'Latest refreshed risk' };
    const pendingLatest = createDeferred<DecisionSignalListResponse>();
    vi.mocked(decisionSignalsApi.getLatest)
      .mockResolvedValueOnce(listResponse([latestSignal]))
    ...
    submitCurrentStock('AAPL');
    fireEvent.click(await screen.findByRole('button', { name: '查看 Apple AI 建议详情' }));
    expect(within(await screen.findByRole('dialog')).getByText('Latest owned risk')).toBeInTheDocument();
    closeSignalDetailsDrawer();
    await waitFor(() => expect(screen.queryByRole('dialog', { name: '信号详情' })).not.toBeInTheDocument());
    expect(screen.getByTestId('decision-signal-context-chip')).toHaveAttribute('data-selected-signal-id', '8');
  ...
  it('does not let a latest refresh overwrite a newer list-owned selection', async () => {
    const latestSignal = makeSignal({
      id: 8,
      stockCode: 'AAPL',
      stockName: 'Apple',
    ...
    await act(async () => {
      pendingLatest.resolve(listResponse([latestSignal]));
    });
    expect(screen.getByTestId('decision-signal-context-chip')).toHaveAttribute('data-selected-signal-id', '7');
    expect(new URLSearchParams(window.location.search).get('signal')).toBe('7');
    expect(within(await screen.findByRole('dialog', { name: '信号详情' })).getByText('趋势保持')).toBeInTheDocument();
  ...
  it('keeps a timeline-owned selectedSignalId across timeline refresh', async () => {
    const timelineSignal = makeSignal({
      id: 8,
      stockCode: 'AAPL',
      stockName: 'Apple',
      market: 'us',
      riskSummary: 'Timeline owned risk',
    });
    const pendingTimeline = createDeferred<DecisionSignalListResponse>();
    vi.mocked(decisionSignalsApi.list)
      .mockResolvedValueOnce(listResponse())
      .mockResolvedValueOnce(listResponse([timelineSignal], 1))
      .mockReturnValueOnce(pendingTimeline.promise);
    renderPage();
    await screen.findByText('贵州茅台');
    submitCurrentStock('AAPL');
    openSignalsView('股票信号时间线');
    fireEvent.click(await screen.findByTestId('timeline-click-8'));
    expect(within(await screen.findByRole('dialog')).getByText('Timeline owned risk')).toBeInTheDocument();
    closeSignalDetailsDrawer();
    await waitFor(() => expect(screen.queryByRole('dialog', { name: '信号详情' })).not.toBeInTheDocument());
    ...
    await act(async () => {
      pendingTimeline.resolve(listResponse([timelineSignal], 1));
    });
    expect(screen.getByTestId('decision-signal-context-chip')).toHaveAttribute('data-selected-signal-id', '8');
    expect(new URLSearchParams(window.location.search).get('signal')).toBe('8');
  });

  it('updates ?signal= to the persisted id without restoring the previous pending id', async () => {
    let persisted = false;
    vi.mocked(decisionSignalsApi.reassess).mockImplementation(async (request) => {
      if (!request.persist) return persistableReassessResponse;
      persisted = true;
    ...
    renderPage();
    await persistReassessFromFirstSignal();
    await waitFor(() => expect(new URLSearchParams(window.location.search).get('signal')).toBe('88'));
    expect(screen.getByTestId('decision-signal-context-chip')).toHaveAttribute('data-selected-signal-id', '88');
    expect(within(await screen.findByRole('dialog', { name: '信号详情' })).getByText('已保存为新的 DecisionSignal #88。')).toBeInTheDocument();
  });
