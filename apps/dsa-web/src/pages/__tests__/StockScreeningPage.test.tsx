...
describe('StockScreeningPage', () => {
  ...
  it('loads AlphaSift hotspot themes on demand', async () => {
    ...
    const coverage = screen.getByText(/覆盖 8 股/);
    expect(coverage).toBeInTheDocument();
    expect(coverage.parentElement).toHaveClass('flex-1', 'pr-24');
    ...
    expect(analyzeButton).toHaveClass('ui-touch-target', 'min-w-6');
  ...
  it('surfaces AlphaSift LLM fallback instead of showing empty LLM fields as normal', async () => {
    ...
    expect(screen.getByRole('button', { name: /^(展开|收起)$/ })).toHaveClass('ui-touch-target', 'h-9', 'min-w-9');
  });
