...
import { ApiErrorAlert, Badge, Button, Checkbox, ConfirmDialog, Drawer, IconButton, InlineAlert, ScrollArea, SearchInput, Spinner, StatePanel, Switch, Textarea, Tooltip, useClipboard } from '../components/common';
...
type ChatText = (key: UiTextKey, params?: UiTextParams) => string;

const CHAT_STAGE_LABEL_KEYS: Readonly<Record<string, UiTextKey>> = {
  agent_loop: 'chat.stage.agentLoop',
  decision: 'chat.stage.decision',
  generation: 'chat.stage.generation',
  intel: 'chat.stage.intel',
  portfolio: 'chat.stage.portfolio',
  research: 'chat.stage.research',
  risk: 'chat.stage.risk',
  skill_consensus: 'chat.stage.skillConsensus',
  strategy_consensus: 'chat.stage.skillConsensus',
  technical: 'chat.stage.technical',
};

const getStageLabel = (stage: string | undefined, t: ChatText): string => {
  const normalized = stage?.trim().toLowerCase();
  if (!normalized) return t('chat.stageFallback');
  if (normalized.startsWith('skill_') || normalized.startsWith('strategy_')) {
    return t('chat.stage.skill');
  }
  const key = CHAT_STAGE_LABEL_KEYS[normalized];
  return key ? t(key) : t('chat.stageFallback');
};

const getStageDoneLabel = (step: ProgressStep, t: ChatText): string => {
  const stage = getStageLabel(step.stage, t);
  if (isStageDoneSuccessful(step.status)) return t('chat.stageCompleted', { name: stage });
  if (step.status?.trim().toLowerCase() === 'skipped') {
    return t('chat.stageSkipped', { name: stage });
  }
  return t('chat.stageStatus', {
    name: stage,
    status: step.status ? t('common.failure') : t('chat.stageFinishedStatus'),
  });
};

const getPipelineBudgetSkippedLabel = (step: ProgressStep, t: ChatText): string => {
  return t('chat.pipelineBudgetSkipped', {
    name: getStageLabel(step.stage, t),
  });
};
...
const ChatPage: React.FC = () => {
  ...
  const getCurrentStage = (steps: ProgressStep[]): string => {
    ...
    if (last.type === 'stage_start')
      return t('chat.stageStarting', { name: getStageLabel(last.stage, t) });
    if (last.type === 'stage_done')
      return getStageDoneLabel(last, t);
    if (last.type === 'pipeline_timeout')
      return t('chat.stageTimedOut', { name: getStageLabel(last.stage, t) });
    ...
    <div className="mb-3 pl-5 border-l border-border/40 space-y-1.5 animate-fade-in">
      {steps.map((step, idx) => {
        ...
        if (step.type === 'thinking') {
          ...
          text = t('chat.stageStarting', { name: getStageLabel(step.stage, t) });
          statusClass = 'chat-progress-item-thinking';
          iconClass = 'chat-progress-dot-thinking';
          ...
          text = t('chat.stageTimedOut', { name: getStageLabel(step.stage, t) });
  ...
  const sidebarContent = (
    <>
      <div className="border-b border-border bg-surface-1/70 p-3">
        ...
        <SearchInput
          ...
          wrapperClassName="mt-2 w-full shadow-none"
        />
      </div>
      <ScrollArea testId="chat-session-list-scroll" viewportClassName="p-3">
        {sessionsLoading ? (
          ...
          />
        ) : sessions.filter((session) => session.title.toLocaleLowerCase().includes(sessionSearch.trim().toLocaleLowerCase())).length === 0 ? (
          <StatePanel status="empty"
            compact
          ...
          <div className="space-y-2">
            {sessions
              .filter((session) => session.title.toLocaleLowerCase().includes(sessionSearch.trim().toLocaleLowerCase()))
              .map((s) => (
                ...
                <Button
                  ...
                  className={`session-item h-auto justify-start ${s.session_id === sessionId ? 'active' : ''}`}
  ...
  return (
    ...
    >
      ...
      <div
        className="hidden h-full w-64 flex-shrink-0 flex-col overflow-hidden rounded-3xl border border-border bg-card/82 shadow-soft-card xl:flex"
      ...
      <Drawer
        ...
        panelClassName="glass-card !rounded-none overflow-hidden border-border bg-card/90 shadow-2xl"
      ...
      <div className="flex h-full min-w-0 flex-1 flex-col overflow-hidden">
        <header className="mb-4 flex-shrink-0 space-y-3">
          <div className="flex items-start justify-between gap-4">
            <h1 className="flex items-center gap-2 text-[1.75rem] font-semibold leading-tight tracking-normal text-foreground">
              <IconButton
        ...
        <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-hidden border border-border bg-card/78 glass-card">
          {/* Messages */}
          <ScrollArea
          ...
          >
            {messages.length === 0 && !loading ? (
              ...
              messages.map((msg) => {
                ...
                >
                  ...
                  <div
                    className={cn(
                      'group/message min-w-0 w-fit max-w-[min(100%,48rem)] px-5 py-3.5 transition-colors',
                      msg.role === 'user' ? 'overflow-hidden chat-bubble-user' : 'relative mb-12 overflow-visible chat-bubble-ai'
                    )}
                  >
            ...
            {loading && (
              <div className="flex gap-4">
                ...
                <div className="min-w-50 max-w-[min(100%,48rem)] overflow-hidden rounded-2xl rounded-tl-sm border border-border bg-card/72 px-5 py-4">
          ...
          {showJumpToBottom && (
            <div className="pointer-events-none absolute bottom-[5.75rem] right-4 z-20 md:bottom-24 md:right-6">
              <Button
                ...
                variant="secondary"
                size="sm"
                className="pointer-events-auto shadow-soft-card"
                onClick={() => {
                  requestScrollToBottom('smooth');
          ...
          {/* Input area */}
          <div className="relative z-20 border-t border-border bg-card/88 p-4 md:p-6">
              ...
              {skills.length > 0 && (
                <div className="relative space-y-2" ref={skillPickerRef}>
                  <Button
                    ...
                    variant="secondary"
                    size="md"
                    className="w-full justify-between gap-2 px-2 text-left text-xs shadow-none"
