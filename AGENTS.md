...
- `docs/CHANGELOG.md` 的 `[Unreleased]` 段使用**扁平格式**：每条独立一行，格式为 `- [Type] Description`，类型取值：`Added`/`Changed`/`Fixed`/`Docs`/`Tests`/`Chore`，描述使用英文；**禁止在 `[Unreleased]` 内新增 `### 类目标题`**，以减少并发 PR 的 merge 冲突。发版时由 maintainer 汇总整理成带标题的正式格式。
