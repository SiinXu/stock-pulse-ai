...
| `backend-gate` | `.github/workflows/ci.yml` | PR: path-selective offline pytest; FULL fallback uses the same 4 shards as push-to-main; push-to-main: full suite + coverage floor when backend paths match; `merge_group`: full suite + coverage floor even when path filters would skip backend | Yes |
| `python-minimum` | `.github/workflows/ci.yml` | PR: 3.10 import/schema smoke; push-to-main: sharded full offline suite on Python 3.10 when backend paths match; `merge_group`: the same full 3.10 shards even when path filters would skip backend | Yes |
...
**Event model:** pull requests use path-selective pytest; when mapping is `FULL` they reuse the four 30-minute `backend-tests` shards (required check name stays `backend-gate`) instead of an unsharded suite inside the 45-minute selective job. Pushes to `main` run the complete offline suite in four module-level shards on Python 3.11 (coverage combine + floor once) and the same sharded offline suite on Python 3.10 via `python-minimum` (45-minute shard bound; hosted 3.10 shard 2 is ~28-30 minutes). `merge_group` (`checks_requested`) fail-closes `changes.backend` to true and always uses that same full 3.11 shard + coverage aggregator path and the same full 3.10 shards + aggregator, including frontend-only groups such as `apps/dsa-web/src/api/auth.ts` that would skip backend on a PR or push. The PR selective planner stays pull-request-only, and push-only observation jobs (`web-e2e`, `api-real-client`) stay push-only. This is workflow readiness only and does not enable a merge queue.
