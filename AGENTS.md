...
| `backend-gate` | `.github/workflows/ci.yml` | PR: path-selective offline pytest; FULL fallback uses the same 4 shards as push-to-main; push-to-main: full suite + coverage floor when backend paths match, and when a push to main detects zero changed files; `merge_group`: full suite + coverage floor even when path filters would skip backend | Yes |
| `python-minimum` | `.github/workflows/ci.yml` | PR: 3.10 import/schema smoke; push-to-main: sharded full offline suite on Python 3.10 when backend paths match, and when a push to main detects zero changed files; `merge_group`: the same full 3.10 shards even when path filters would skip backend | Yes |
...
| `docker-build` | `.github/workflows/ci.yml` | Builds the Docker image and smoke-tests imports of key modules when Docker paths match, and when a push to main detects zero changed files | Yes |
...
| `web-e2e` | `.github/workflows/ci.yml` | Real backend + Vite + Playwright smoke | No on PR or `merge_group`; observation on push-to-main when path filter matches |
| `api-real-client` | `.github/workflows/ci.yml` | `tests/api` with real Starlette TestClient | No on PR or `merge_group`; observation on push-to-main |
| `network-smoke` | `.github/workflows/network-smoke.yml` | `pytest -m network` + `scripts/test.sh quick` | No, observation item |
| `pr-review` | `.github/workflows/pr-review.yml` | Opt-in PR static check + AI review; `security-check` is checkout-free and SHA-pins file inventory | No, `workflow_dispatch` with required `pr_number` |

**Event model:** pull requests use path-selective pytest; when mapping is `FULL` they reuse the four 30-minute `backend-tests` shards (required check name stays `backend-gate`) instead of an unsharded suite inside the 45-minute selective job. Pushes to `main` run the complete offline suite in four module-level shards on Python 3.11 (coverage combine + floor once) and the same sharded offline suite on Python 3.10 via `python-minimum` (45-minute shard bound; hosted 3.10 shard 2 is ~28-30 minutes) when backend paths match. A push to `main` with zero detected files is treated as unknown rather than "no backend change" and fail-closes `changes.backend` and `changes.docker` so the required 3.11 shards plus aggregator, 3.10 shards plus aggregator, and `docker-build` still run; nonempty pushes keep path-selective skips, including changelog-only and frontend-only diffs. `merge_group` (`checks_requested`) fail-closes `changes.backend` to true and always uses that same full 3.11 shard + coverage aggregator path and the same full 3.10 shards + aggregator, including frontend-only groups such as `apps/dsa-web/src/api/auth.ts` that would skip backend on a PR or a nonempty push. The PR selective planner stays pull-request-only, and push-only observation jobs (`web-e2e`, `api-real-client`) stay push-only. This is workflow readiness only and does not enable a merge queue.
