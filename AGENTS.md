...
**Event model:** pull requests use path-selective pytest with a fail-closed full-suite fallback. Pushes to `main` run the complete offline suite in four module-level shards on Python 3.11 (coverage combine + floor once) and the same sharded offline suite on Python 3.10 via `python-minimum`.
