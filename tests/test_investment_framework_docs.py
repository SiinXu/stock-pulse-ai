...
def test_framework_topic_documents_api_scope_context_and_rollback() -> None:
    ...
    for document in (chinese, english):
        ...
        assert "202607240003_investment_framework_schema" in document
        ...
        assert "unpaginated" in document.lower() or "不提供分页" in document
        assert "orphan" in document.lower() or "孤儿" in document
        assert "current" in document.lower() or "当前" in document
        assert "AUTOINCREMENT" in document
