...
# --- real TWSE T86 row for 2330 TSMC @ 20260626 (Gregorian date, comma-grouped values) ---
T86_FIXTURE = {
    "stat": "OK",
    "date": "20260626",
    "fields": [
...
# --- real TPEx OpenAPI row for 3105 Win Semiconductors @ Minguo 1150626 (plain ints, messy keys) ---
TPEX_FIXTURE = [
    {
        "Date": "1150626",
...
class TestPureHelpers(unittest.TestCase):
    def test_minguo_to_ad(self):
        self.assertEqual(minguo_to_ad("1150626"), "20260626")  # Minguo115 -> Gregorian 2026
        self.assertEqual(minguo_to_ad("1010101"), "20120101")  # Minguo101 -> Gregorian 2012
        self.assertEqual(minguo_to_ad("0010101"), "19120101")  # Minguo1 -> Gregorian 1912
        self.assertEqual(minguo_to_ad("0990101"), "20100101")  # Minguo99 -> Year 2010
...
class TestT86Parsing(unittest.TestCase):
    def test_twse_2330_net_breakdown(self):
        ...
        # foreign = Overseas Investment (Excludes Foreign Trading Entities) = -14,281,155
...
class TestTpexParsing(unittest.TestCase):
    def test_tpex_3105_net_breakdown_and_minguo_date(self):
        ...
        self.assertEqual(rec["date"], "20260626")        # Minguo 1150626 -> Gregorian
...
class TestMissingFieldAndEmptyCacheFailOpen(unittest.TestCase):
    ...
    def test_tpex_genuine_zero_is_kept(self):
        import copy
        row = copy.deepcopy(TPEX_FIXTURE[0])
        row["SecuritiesInvestmentTrustCompanies-Difference"] = "0"  # Investment Trust truly net-zero today
        with patch("data_provider.tw_institutional_fetcher.requests.get", return_value=_resp([row])):
            rec = _fetcher().get_institutional_net("3105.TWO")
        self.assertIsNotNone(rec)
    ...
    def test_twse_missing_core_field_drops_row(self):
        ...
        fix["data"][0][trust_idx] = ""   # Investment Trust buy/sell cell blank -> missing -> drop 2330, not 0
...
class TestStructureRobustness(unittest.TestCase):
    ...
    def test_twse_renamed_core_field_fails_open(self):
        import copy
        fix = copy.deepcopy(T86_FIXTURE)
        fix["fields"][10] = "投信買賣超股數_v2"          # Investment column renamed
        with patch("data_provider.tw_institutional_fetcher.requests.get", return_value=_resp(fix)):
            self.assertIsNone(_fetcher().get_institutional_net("2330.TW", "20260626"))
    ...
    def test_tpex_unconvertible_date_drops_row(self):
        import copy
        row = copy.deepcopy(TPEX_FIXTURE[0])
        row["Date"] = "bad-date"                        # not a 7-digit Minguo -> drop
        with patch("data_provider.tw_institutional_fetcher.requests.get", return_value=_resp([row])):
            self.assertIsNone(_fetcher().get_institutional_net("3105.TWO"))
