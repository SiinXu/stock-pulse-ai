...
def _env_flag_enabled(name: str, default: str = "1") -> bool:
    """Return True when the named env var is truthy (default matches historical CI)."""
    raw = os.environ.get(name, default)
    return str(raw).strip().lower() not in {"0", "false", "no", "off", ""}


# Default "1" preserves today's threadless TestClient + asyncio/anyio sandbox patches.
# Set STOCKPULSE_TEST_THREADLESS=0 to exercise the real Starlette TestClient (see api-real-client CI job).
_USE_THREADLESS_TEST_CLIENT = _env_flag_enabled("STOCKPULSE_TEST_THREADLESS", default="1")
