...
def test_schema_examples_remain_in_openapi_schema() -> None:
    ...
    assert root_schema["properties"]["message"]["example"] == "StockPulse API is running"
    assert root_schema["properties"]["message"]["description"] == "API runtime status message"
    assert root_schema["properties"]["version"]["description"] == "API version"
