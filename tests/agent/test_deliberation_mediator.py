...
from src.agent.deliberation_mediator import (
...
class TestStrategyDeliberationV0(unittest.TestCase):
    def test_omits_deliberation_without_conflicts(self):
        ...
        synthesis = StrategySynthesizer(deliberation_enabled=True).synthesize(
            opinions,
            weighted_score=4.0,
            final_signal="buy",
...
class TestStrategyDeliberationV2(unittest.TestCase):
    ...
    def test_self_review_projection_uses_accepted_reviews(self):
        ...
        synthesis = StrategySynthesizer(deliberation_mediator=mediator, deliberation_enabled=True).synthesize(
            opinions,
            weighted_score=3.0,
            final_signal="hold",
            weighted_confidence=0.8,
...
def _synthesize_high_conflict(mediator):
    ...
    conflicts = ConflictDetector().detect(opinions, final_signal="hold")
    return StrategySynthesizer(deliberation_mediator=mediator, deliberation_enabled=True).synthesize(
        opinions,
        weighted_score=3.0,
        final_signal="hold",
        weighted_confidence=0.8,
