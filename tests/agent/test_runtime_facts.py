...
def test_legacy_executor_uses_sanitization_metadata_from_selected_json_block():
    reserved_payload = _dashboard("buy")
    reserved_payload["agent_disagreement_explanation"] = {"token": "top-secret"}
    malformed_reserved_block = json.dumps(reserved_payload, ensure_ascii=False)[:-1]
    clean_later_block = json.dumps(_dashboard("hold"), ensure_ascii=False)
    raw_content = (
        f"```json\n{malformed_reserved_block}\n```\n"
        f"```json\n{clean_later_block}\n```"
    )
    loop_result = RunLoopResult(success=True, content=raw_content)
    executor = AgentExecutor(MagicMock(), MagicMock())
    ...
    assert json.loads(result.content) == result.dashboard
    assert "agent_disagreement_explanation" not in result.content
    assert "top-secret" not in result.content


def test_legacy_executor_preserves_raw_content_without_reserved_field():
    payload = _dashboard()
    raw_content = "Dashboard follows:\n```json\n" + json.dumps(
        payload,
        ensure_ascii=False,
    ) + "\n```"
    loop_result = RunLoopResult(success=True, content=raw_content)
    ...
    assert result.success is True
    assert result.dashboard == payload
...
def test_cancelled_pipeline_keeps_collected_facts_without_degradation():
    orchestrator = _orchestrator()
    ctx = AgentContext(query="test", stock_code="600519")
    stages = [_Stage("technical", "buy"), _Stage("decision", "buy")]
    ...
    assert result.runtime_facts.degraded_events == ()
    assert result.runtime_facts.pipeline_termination is None
    assert result.runtime_facts.risk_override_application is None
...
def test_pipeline_budget_guard_records_skipped_real_stage():
    ...
    with patch.object(orchestrator, "_build_agent_chain", return_value=stages):
        with patch(
            ...
            SimpleNamespace(time=_clock([0.0, 0.1, 0.2, 14.6], 14.7)),
        ):
            result = orchestrator._execute_pipeline(ctx, parse_dashboard=True)
...
def test_pipeline_timeout_before_stage_records_that_unstarted_stage():
    ...
    with patch.object(orchestrator, "_build_agent_chain", return_value=stages):
        with patch(
            "src.agent.orchestrator.time",
            SimpleNamespace(time=_clock([0.0, 0.1, 0.2, 1.2], 1.3)),
        ):
            result = orchestrator._execute_pipeline(ctx, parse_dashboard=True)
...
def test_pipeline_timeout_after_completed_stage_does_not_degrade_that_stage():
    ...
    with patch.object(orchestrator, "_build_agent_chain", return_value=stages):
        with patch(
            "src.agent.orchestrator.time",
            SimpleNamespace(time=_clock([0.0, 0.1, 0.2, 1.0, 20.2], 20.3)),
        ):
            result = orchestrator._execute_pipeline(ctx, parse_dashboard=True)
...
def test_post_risk_dashboard_finalization_is_idempotent():
    orchestrator = _orchestrator()
    ctx = AgentContext(query="test", stock_code="600519")
    ctx.add_opinion(AgentOpinion(agent_name="decision", signal="buy", confidence=0.8))
    ctx.add_opinion(AgentOpinion(
    ...
    payload = _dashboard("buy")
    payload["key_points"] = ["base evidence"]
    first = orchestrator._resolve_dashboard_payload(ctx, payload, None)

    second = orchestrator._resolve_dashboard_payload(ctx, first, None)

    assert second == first
    assert second["risk_warning"].count("风控接管") == 1
    assert second["analysis_summary"].count("[风控下调: buy -> hold]") == 1
...
def test_base_agent_timeout_keeps_public_error_and_sanitized_logging():
    agent = _TestAgent(MagicMock(), MagicMock())
    raw_error = "provider timeout token=super-secret at https://private.example/v1"

    with patch(
        "src.agent.agents.base_agent.run_agent_loop",
        side_effect=TimeoutError(raw_error),
    ):
        with patch("src.agent.agents.base_agent.logger") as logger:
            stage_result = agent.run(AgentContext())

    assert stage_result.status == StageStatus.FAILED
    assert stage_result.error == AGENT_EXECUTION_FAILURE_MESSAGE
    assert stage_result.failure_reason == StageFailureReason.TIMEOUT
    rendered_log = " ".join(str(value) for value in logger.method_calls)
    assert "super-secret" not in rendered_log
    assert "private.example" not in rendered_log
