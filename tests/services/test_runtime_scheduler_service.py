...
class RuntimeSchedulerServiceTestCase(unittest.TestCase):
    ...
    def test_runtime_scheduler_timeout_is_hidden_from_web_settings(self) -> None:
        from src.core.config_registry import WEB_SETTINGS_HIDDEN_FROM_UI, build_schema_response

        self.assertIn(RUNTIME_SCHEDULER_TIMEOUT_ENV, WEB_SETTINGS_HIDDEN_FROM_UI)
        schema_keys = {
            field["key"]
            for category in build_schema_response()["categories"]
            for field in category["fields"]
        }
        self.assertNotIn(RUNTIME_SCHEDULER_TIMEOUT_ENV, schema_keys)
        ...
        self.assertNotIn(RUNTIME_SCHEDULER_TIMEOUT_ENV, groups_src)

        i18n_src = (
            _REPO_ROOT / "apps" / "dsa-web" / "src" / "utils" / "systemConfigI18n.ts"
        ).read_text(encoding="utf-8")
        self.assertNotIn(RUNTIME_SCHEDULER_TIMEOUT_ENV, i18n_src)
    ...
    def test_run_now_runs_analysis_with_default_stock_scope(self) -> None:
        ...
        self.assertTrue(status["last_run_at"].endswith("+00:00"))
        self.assertIsNotNone(status["last_success_at"])
        self.assertTrue(status["last_success_at"].endswith("+00:00"))
    ...
    def test_blocked_scheduled_analysis_times_out_and_allows_next_run(self) -> None:
        ...
        try:
            ...
            service._analysis_process_target = _successful_spawn_runner
            accepted = service.run_now()
            self.assertTrue(accepted["accepted"])
            self.assertIsNotNone(accepted["run_id"])
            self.assertTrue(accepted["started_at"].endswith("+00:00"))
            ...
            self.assertIsNotNone(status["last_success_at"])
            self.assertIsNone(status["last_error"])
            self.assertEqual(status["last_run_id"], accepted["run_id"])
            self.assertEqual(status["last_run_outcome"], "succeeded")
        finally:
    ...
    def test_watchdog_reads_large_worker_result_before_joining(self) -> None:
        ...
        self.assertFalse(service.status()["running"])
        self.assertEqual(service.status()["last_run_outcome"], "failed")
