...
@patch("src.config.get_config")
def test_service_degrades_when_us_extras_are_unavailable(mock_get_config, monkeypatch):
    """Unavailable Longbridge/Finnhub/AlphaVantage must not stay status=ok."""
    from src.application_services import reset_application_services
    from src.config import Config
    ...
    view = service.get_field_trust("AAPL")

    assert view["status"] == "degraded"
    statuses = {(row["provider"], row["status"]) for row in view["provider_health"]}
