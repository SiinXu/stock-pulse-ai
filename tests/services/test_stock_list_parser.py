...
Semantic port of upstream ZhuLinsen/daily_stock_analysis@a8f084bf (issue #2063
Phase 1 + explicit-suffix reject contract). Covers the three core contracts:

* Prefix-whitelisted index: prefixed ``sh``/``sz`` + registry-known code → ``index``
* Bare code defaults to stock: bare numeric code → ``stock`` even if it's an index code
* Prefixed unknown degrades to stock: prefixed unknown code → ``stock`` (never ``unsupported``)

Also keeps the legacy ``split_stock_list`` / ``serialize_stock_list`` tests and
fork-specific portfolio helpers so this suite is a strict superset of pre-port
coverage.
"""
...
# ---------------------------------------------------------------------------
# Fork-specific portfolio / analysis-identity helpers (pre-existing).
# ---------------------------------------------------------------------------
