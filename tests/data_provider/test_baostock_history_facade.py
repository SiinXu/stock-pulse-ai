# -*- coding: utf-8 -*-
"""Facade identity, patch-seam, reload, retry, and ABC tests for Baostock history.

Issue #1068: ``_fetch_raw_data`` and ``_normalize_data`` moved into
``src/data_provider/baostock_parts/history.py`` and are rebound onto the
public ``BaostockFetcher`` class.
"""
...
import src.data_provider.baostock_fetcher as baostock_mod
import src.data_provider.baostock_parts.history as history_mod
...
FACADE_PATH = REPO_ROOT / "src" / "data_provider" / "baostock_fetcher.py"
OWNER_PATH = REPO_ROOT / "src" / "data_provider" / "baostock_parts" / "history.py"
...
def _facade_body(name: str):
    method = BaostockFetcher.__dict__[name]
    return getattr(method, "__wrapped__", method)


def _make_fetcher() -> BaostockFetcher:
...
def _facade_class_methods() -> set:
    ...
    cls = next(
        ...
        if isinstance(node, ast.ClassDef) and node.name == "BaostockFetcher"
    )
    return {
        node.name
...
def test_moved_methods_remain_on_the_public_fetcher(name) -> None:
    assert callable(getattr(BaostockFetcher, name))


@pytest.mark.parametrize("name", MOVED)
def test_module_and_qualname_still_point_at_the_facade(name) -> None:
    method = BaostockFetcher.__dict__[name]
    assert method.__module__ == "src.data_provider.baostock_fetcher", name
    assert method.__qualname__ == f"BaostockFetcher.{name}", name


@pytest.mark.parametrize("name", MOVED)
def test_free_names_resolve_through_the_facade_globals(name) -> None:
    assert _facade_body(name).__globals__ is vars(baostock_mod), name


@pytest.mark.parametrize("name", MOVED)
def test_signatures_are_unchanged(name) -> None:
    signature = inspect.signature(getattr(BaostockFetcher, name))
...
def test_connection_and_sibling_methods_stay_on_the_facade(name) -> None:
    ...
    assert callable(getattr(BaostockFetcher, name)), name


def test_fetch_raw_data_keeps_provider_retry_wrapper() -> None:
    fetch = BaostockFetcher._fetch_raw_data
    assert callable(fetch)
    assert getattr(fetch, "__wrapped__", None) is not None


def test_fetcher_instantiates_after_abstract_clearing() -> None:
    assert not BaostockFetcher.__abstractmethods__
    fetcher = BaostockFetcher()
    assert isinstance(fetcher, BaostockFetcher)
...
def test_owner_module_does_not_import_the_facade() -> None:
    ...
    assert not any("baostock_fetcher" in module for module in imported)
...
def test_facade_bind_is_a_re_export_not_a_copy() -> None:
    ...
    from src.data_provider.baostock_parts.facade_bind import bind_methods_from_class
...
def test_owner_reload_rebinds_onto_the_facade() -> None:
    ...
    for name in MOVED:
        ...
        assert body.__globals__ is vars(baostock_mod), name
        assert method.__qualname__ == f"BaostockFetcher.{name}", name
    for name in CONNECTION_STAYS:
        assert callable(getattr(BaostockFetcher, name)), name
        assert name in _facade_class_methods(), name
    assert getattr(BaostockFetcher._fetch_raw_data, "__wrapped__", None) is not None
...
def test_expected_names_mismatch_is_an_import_error() -> None:
    ...
    with pytest.raises(ImportError):
        bind_methods_from_class(
            _Partial, _Target, vars(baostock_mod), expected_names=MOVED,
        )


def test_us_code_raises_without_opening_a_session() -> None:
    fetcher = _make_fetcher()
    with patch.object(fetcher, "_baostock_session") as session:
...
def test_moved_fetch_sees_patched_call_with_timeout() -> None:
    fetcher = BaostockFetcher()
    sentinel = pd.DataFrame({"date": ["2024-01-02"], "open": [1.0]})
    with patch("src.data_provider.baostock_fetcher.call_with_timeout", return_value=sentinel) as mocked:
        result = fetcher._fetch_raw_data("600519", "2024-01-01", "2024-01-10")
    mocked.assert_called_once()
    assert mocked.call_args.kwargs["call_name"] == "baostock.query_history_k_data_plus"
    assert result is sentinel
    assert (
        _facade_body("_fetch_raw_data").__globals__["call_with_timeout"]
        is baostock_mod.call_with_timeout
    )


def test_normalize_maps_pctchg_and_coerces_numerics() -> None:
    fetcher = _make_fetcher()
    raw = pd.DataFrame(
        {
    ...
    assert "pctChg" not in out.columns
    assert list(out["code"]) == ["600519", "600519"]
    assert out.loc[0, "pct_chg"] == pytest.approx(0.0)
    assert out.loc[1, "pct_chg"] == pytest.approx(10.0)
    assert out.loc[0, "open"] == pytest.approx(10.0)
    assert out.loc[1, "volume"] == pytest.approx(120.0)
