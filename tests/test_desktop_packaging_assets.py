...
class DesktopPackagingAssetsTestCase(unittest.TestCase):
    ...
    def test_migration_package_is_bundled_and_probed_on_both_platforms(self) -> None:
        ...
        self.assertIn(
            "for module in alphasift.dsa_adapter orjson src.migrations.registry src.api.app src.data_provider.base",
            macos_script,
        )
        self.assertIn(
            "@('alphasift.dsa_adapter', 'orjson', 'src.migrations.registry', 'src.api.app', 'src.data_provider.base')",
            windows_script,
        )
        self.assertNotIn(
