...
class StockFieldTrustApiTests(unittest.TestCase):
    ...
    def test_unavailable_us_extras_return_degraded_not_500(self) -> None:
        """Yfinance primary + unavailable Longbridge/Finnhub/AV must be HTTP 200 degraded."""
        fresh_ts = datetime.now(timezone.utc).isoformat()
        success = _make_quote(
            code="AAPL",
        ...
        resp = self._get_trust(manager, code="AAPL")
        self.assertEqual(resp.status_code, 200, resp.text)
        payload = resp.json()
        self.assertEqual(payload["status"], "degraded")
        self.assertEqual(payload["quote_source"], "yfinance")
        statuses = {
            (row["provider"], row["status"]) for row in payload["provider_health"]
        }
        self.assertIn(("yfinance", "ok"), statuses)
        self.assertIn(("longbridge", "unavailable"), statuses)
        self.assertIn(("finnhub", "unavailable"), statuses)
        self.assertIn(("alphavantage", "unavailable"), statuses)
        self.assertNotEqual(payload["analysis_input"]["confidence"], "high")
        self.assertTrue(
            any(
                gap["code"] == "provider_unavailable"
                for gap in payload["analysis_input"]["gaps"]
            )
        )
