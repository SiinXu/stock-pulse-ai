...
class LLMChannelConfigTestCase(unittest.TestCase):
    ...
    def test_anspire_channel_reuses_shared_key_and_defaults(self, _mock_parse_yaml, _mock_setup_env) -> None:
        ...
        self.assertEqual(config.llm_channels[0]["api_surface"], "chat_completions")
        params = config.llm_model_list[0]["litellm_params"]
        self.assertEqual(params["api_base"], ANSPIRE_LLM_BASE_URL_DEFAULT)
    ...
    @patch("src.config.setup_env")
    @patch.object(Config, "_parse_litellm_yaml", return_value=[])
    def test_default_channel_api_surface_is_chat_completions_wire_parity(
        self,
        _mock_parse_yaml,
        _mock_setup_env,
    ) -> None:
        """Unset LLM_*_API_SURFACE must keep historical wire models unchanged."""
        env = {
        ...
        self.assertNotIn("responses/", config.llm_model_list[0]["litellm_params"]["model"])
    ...
    def test_anspire_responses_channel_supports_multiple_models_without_changing_aliases(
        ...
        deployments = {
            ...
            if not str(entry.get("model_name") or "").startswith("modelref:")
        }
    ...
    def test_duplicate_route_alias_allows_same_surface_deployments(
        ...
        self.assertTrue(
            any(str(e.get("model_name") or "").startswith("modelref:") for e in config.llm_model_list)
        )
        self.assertFalse(
            any(
                issue["code"] == "mixed_api_surfaces_for_route"
