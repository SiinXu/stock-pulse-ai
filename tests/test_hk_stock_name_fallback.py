...
class TestHKRealtimeFallback(unittest.TestCase):
    """stock_hk_spot_em Failure when should fallback To stock_hk_spot."""
    ...
    def test_em_success_returns_quote_with_name(self, mock_cb):
        """When stock_hk_spot_em succeeds, directly return the quote with its name."""
    ...
    def test_em_failure_falls_back_to_spot(self, mock_cb):
        """When stock_hk_spot_em throws an exception, fallback to stock_hk_spot and return the name."""
        mock_cb.return_value = _DummyCircuitBreaker()
        ak_mock = MagicMock()
    ...
    def test_both_fail_returns_none(self, mock_cb):
        """When both stock_hk_spot_em and stock_hk_spot fail, return None, do not raise exceptions."""
        mock_cb.return_value = _DummyCircuitBreaker()
        ak_mock = MagicMock()
    ...
    def test_em_returns_empty_df_falls_back_to_spot(self, mock_cb):
        """When stock_hk_spot_em returns an empty DataFrame, fallback to stock_hk_spot."""
        mock_cb.return_value = _DummyCircuitBreaker()
        ak_mock = MagicMock()
    ...
    def test_circuit_breaker_open_returns_none(self, mock_cb):
        """Return None directly in the circuit breaker state."""
